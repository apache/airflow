...
class BaseK8STest:
        ...
        for i in range(max_tries):
            try:
                response = self.session.get(
                    f"http://{KUBERNETES_HOST_PORT}/v2/monitor/health",
                    timeout=1,
                )
    ...
    def monitor_task(self, host, dag_run_id, dag_id, task_id, expected_final_state, timeout):
        ...
        while tries < max_tries:
            ...
            # Check task state
            try:
                get_string = f"http://{host}/v2/dags/{dag_id}/dagRuns/{dag_run_id}/taskInstances/{task_id}"
    ...
    def ensure_dag_expected_state(self, host, logical_date, dag_id, expected_final_state, timeout):
        ...
        while tries < max_tries:
            time.sleep(5)
            get_string = f"http://{host}/v2/dags/{dag_id}/dagRuns"
    ...
    def start_dag(self, dag_id, host):
        patch_string = f"http://{host}/v2/dags/{dag_id}"
        ...
        post_string = f"http://{host}/v2/dags/{dag_id}/dagRuns"
