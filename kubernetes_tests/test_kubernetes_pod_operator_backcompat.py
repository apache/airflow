...
class TestKubernetesPodOperatorSystem(unittest.TestCase):
    ...
    def test_pod_template_file(self, mock_client, monitor_mock, start_mock):
        ...
        k = KubernetesPodOperator(task_id='task', pod_template_file=fixture, do_xcom_push=True)
        monitor_mock.return_value = (State.SUCCESS, None)
        context = create_context(k)
                ...
                'containers': [
                    ...
                    },
                    {
                        'command': [
                            'sh',
                            '-c',
                            'trap "exit 0" INT; while true; do sleep '  # pylint: disable=implicit-str-concat
                            '30; done;',  # pylint: disable=implicit-str-concat
                        ],
                        'image': 'alpine',
    ...
    def test_on_kill(self, monitor_mock):  # pylint: disable=unused-argument
        ...
        k = KubernetesPodOperator(
            ...
            image="ubuntu:16.04",
            cmds=["bash", "-cx"],
            arguments=["sleep 1000"],  # pylint: disable
            labels={"foo": "bar"},
            name="test",
            task_id=name,
            in_cluster=False,
