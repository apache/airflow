...
def _default_config_file_path(file_name: str) -> str:
    ...
    # Option 1: For installed packages (airflow-core)
    config_path = (
    ...
    # Option 2: Try relative to current file (if config_templates exists nearby)
    config_path = pathlib.Path(__file__).parents[2] / "config_templates" / file_name
    if config_path.exists():
        return str(config_path)
...
def create_default_config_parser(configuration_description: dict[str, dict[str, Any]]) -> ConfigParser:
    ...
    for section, section_desc in configuration_description.items():
        ...
        options = section_desc.get("options", {})
        for key in options:
            default_value = options[key].get("default")
...
def initialize_config() -> AirflowConfigParser:
    ...
    Reads config.yml and creates a default configuration parser.
    This allows shared libraries (like timezone) to access configuration
    without importing from airflow.settings.
    """
