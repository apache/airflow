...
class TestStats:
    ...
    def test_enabled_by_config(self):
        ...
        with conf_vars({("metrics", "statsd_on"): "True"}):
            importlib.reload(airflow_shared.observability.stats)
            assert isinstance(airflow_shared.observability.stats.Stats.statsd, statsd.StatsClient)
            assert not hasattr(airflow_shared.observability.stats.Stats, "dogstatsd")
        # Avoid side-effects
        importlib.reload(airflow_shared.observability.stats)
    ...
    def test_load_custom_statsd_client(self):
        with conf_vars(
            ...
            importlib.reload(airflow_shared.observability.stats)
            assert isinstance(airflow_shared.observability.stats.Stats.statsd, CustomStatsd)
        # Avoid side-effects
        importlib.reload(airflow_shared.observability.stats)
    ...
    def test_load_invalid_custom_stats_client(self):
        with conf_vars(
            ...
            with pytest.raises(AirflowConfigException, match=error_message):
                airflow_shared.observability.stats.Stats.incr("empty_key")
        importlib.reload(airflow_shared.observability.stats)
    ...
    def test_load_allow_list_validator(self):
        with conf_vars(
            ...
            importlib.reload(airflow_shared.observability.stats)
            assert (
                type(airflow_shared.observability.stats.Stats.metrics_validator) is PatternAllowListValidator
            )
            assert airflow_shared.observability.stats.Stats.metrics_validator.validate_list == (
                "name1",
                "name2",
            )
        # Avoid side-effects
    ...
    def test_load_block_list_validator(self):
        with conf_vars(
            ...
            importlib.reload(airflow_shared.observability.stats)
            assert (
                type(airflow_shared.observability.stats.Stats.metrics_validator) is PatternBlockListValidator
            )
            assert airflow_shared.observability.stats.Stats.metrics_validator.validate_list == (
                "name1",
                "name2",
...
class TestDogStats:
    ...
    def test_enabled_by_config(self):
        ...
        with conf_vars({("metrics", "statsd_datadog_enabled"): "True"}):
            importlib.reload(airflow_shared.observability.stats)
            assert isinstance(airflow_shared.observability.stats.Stats.dogstatsd, DogStatsd)
            assert not hasattr(airflow_shared.observability.stats.Stats, "statsd")
        # Avoid side-effects
        importlib.reload(airflow_shared.observability.stats)
...
class TestPatternValidatorConfigOption:
    ...
    def test_pattern_picker(self, config, expected):
        with conf_vars(config):
            ...
            assert isinstance(airflow_shared.observability.stats.Stats.statsd, statsd.StatsClient)
            assert type(airflow_shared.observability.stats.Stats.instance.metrics_validator) is expected
    ...
    def test_setting_allow_and_block_logs_warning(self, caplog):
        with caplog.at_level(logging.WARNING):
            ...
            assert isinstance(airflow_shared.observability.stats.Stats.statsd, statsd.StatsClient)
            assert (
                type(airflow_shared.observability.stats.Stats.instance.metrics_validator)
