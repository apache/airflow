...
1. **Build**: `registry-build.yml` extracts metadata, builds the 11ty site, and syncs to S3.
   ...
   - **Incremental build** (`provider=amazon`): extracts one provider (~30s), merges
     ...
     The S3 sync step excludes per-provider `connections.json` and `parameters.json`
     for non-target providers to avoid overwriting real data with Eleventy's empty
     fallback stubs (Eleventy 3.x `permalink: false` does not work with pagination).
