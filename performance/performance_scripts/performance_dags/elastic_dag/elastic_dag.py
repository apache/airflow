...
def get_task_list(dag_object, operator_type_str, task_count, trigger_rule, sleep_time, operator_extra_kwargs):
    ...
    if operator_type_str == "bash":
    ...
    elif operator_type_str == "big_query_insert_job":
        ...
        task_list = [
            BigQueryInsertJobOperator(
                ...
                configuration={
                    "query": {
                        "query": "SELECT * FROM composer_performance_bigquery_operators_test.table1",
                        "useLegacySql": False,
                        "destinationTable": {
                            "project_id": os.environ.get("GCP_PROJECT"),
                            "dataset_id": "composer_performance_bigquery_operators_test",
                            "table_id": '{{ task.dag_id }}_{{ task.task_id }}_{{ ts_nodash_with_tz | replace(".", "_") | replace("+", "_") }}',
                        },
                    }
                },
                result_timeout=300,  # To prevent task indefinitely stuck.
                execution_timeout=timedelta(minutes=5),  # Has to be enough to execute BigQuery job.
                **operator_extra_kwargs,
            )
            for i in range(1, task_count + 1)
        ]
