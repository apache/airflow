...
class TestPerformanceTest(TestCase):
    ...
    def test_init_composer_environment(
        ...
        self.assertEqual(
            performance_test.environment, mock_composer_environment.return_value
        )
        self.assertEqual(
            performance_test.storage_client, mock_storage_client.return_value
        )
        self.assertEqual(
            performance_test.big_query_client, mock_big_query_client.return_value
        )

        mock_get_environment_type.assert_called_once_with(
    ...
    def test_get_environment_type(self):
        file_path = os.path.join(
            ENVIRONMENT_SPECIFICATIONS_DIR, "get_environment_type.json"
        )
    ...
    def test_get_environment_type_no_key(self):
        file_path = os.path.join(
            ENVIRONMENT_SPECIFICATIONS_DIR, "no_environment_type_key.json"
        )
    ...
    def test_get_results_object_name(self):
        result = PerformanceTest.get_results_object_name(
            ["environment_user", "x.x.x", "aaa-bbb-ccc"]
        )
...
class TestPerformanceTestWithComposerEnvironment(TestCase):
    def setUp(self):
        ...
        with mock.patch(
            ...
            MODULE_NAME + ".ComposerEnvironment"
        ) as mock_composer_environment, mock.patch(
    ...
    def test_check_state(self):
        ...
        type(
            self.performance_test.environment
        ).is_terminal_state = is_terminal_state_mock
        ...
        self.performance_test.environment.get_state_method = mock.Mock(
            return_value=state_method_mock
        )
        ...
        self.assertEqual(
            self.performance_test.environment.state, State.WAIT_UNTIL_READY
        )
        self.assertEqual(
            wait_time,
    ...
    def test_check_outputs_all_outputs(self, mock_check_output_path):
        ...
        self.performance_test.storage_client.get_bucket.assert_called_once_with(
            RESULTS_BUCKET
        )
    ...
    def test_save_results_only_results_bucket(
        self, mock_save_output_file, mock_get_results_object_name
    ):
        ...
        mock_get_results_object_name.assert_called_once_with(
            self.performance_test.environment.results[1]
        )
    ...
    @mock.patch(MODULE_NAME + ".PerformanceTest.get_results_object_name")
    @mock.patch(MODULE_NAME + ".save_output_file")
    def test_save_results_only_results_dataset(
        self, mock_save_output_file, mock_get_results_object_name
    ):
    ...
    @mock.patch(MODULE_NAME + ".PerformanceTest.get_results_object_name")
    @mock.patch(MODULE_NAME + ".save_output_file")
    def test_save_results_only_output_file(
        self, mock_save_output_file, mock_get_results_object_name
    ):
    ...
    @mock.patch(MODULE_NAME + ".PerformanceTest.get_results_object_name")
    @mock.patch(MODULE_NAME + ".save_output_file")
    def test_save_results_all_outputs(
        self, mock_save_output_file, mock_get_results_object_name
    ):
