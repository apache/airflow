...
class TestMetricsDataframe(TestCase):
    def setUp(self) -> None:
        ...
        self.resource_data = {
            ResourceType.CLUSTER: {
                "df": pd.read_csv(
                    os.path.join(DATAFRAMES_DIR, f"{ResourceType.CLUSTER.value}.csv")
                ),
            ...
            ResourceType.NODE: {
                "df": pd.read_csv(
                    os.path.join(DATAFRAMES_DIR, f"{ResourceType.NODE.value}.csv")
                ),
                "resource_labels_set": {
                    "project_id",
                    "location",
            ...
            ResourceType.POD: {
                "df": pd.read_csv(
                    os.path.join(DATAFRAMES_DIR, f"{ResourceType.POD.value}.csv")
                ),
                "resource_labels_set": {
                    "project_id",
                    "location",
            ...
            ResourceType.CONTAINER: {
                "df": pd.read_csv(
                    os.path.join(DATAFRAMES_DIR, f"{ResourceType.CONTAINER.value}.csv")
                ),
                "resource_labels_set": {
                    "project_id",
                    "location",
    ...
    def test_join_metric_columns(self):
        ...
        self.assertTrue(
            df_1["A"]
            .combine(df_2["A"], func=join_metric_columns)
            .equals(pd.Series([1.0, 2.0, np.nan, 4.0]))
        )

    def test_join_metric_columns_raises_exception_for_different_values_in_same_row(
        self
    ):
    ...
    def test_join_resource_dataframes_raises_exception_on_metric_column_duplicates(
        self
    ):
