...
2. **`bundle.Register(items ...airflow.Registration)`** is the single registration verb, taking
   native Dags ([ADR 7](0007-native-dag-interface.md)) and task handlers in any mix.
3. **A Go bundle registers task handlers, not Dags**: `airflow.TaskHandler(dagId, taskId, fn)`,
...
- **One verb, over a sealed interface.** `Bundle.Register(items ...airflow.Registration)` accepts
  both kinds because `*airflow.DagRef` and `airflow.TaskHandlerRef` satisfy `Registration`, an
