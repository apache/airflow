...
This document is the structured source for a small set of contributor workflows
that AI tools can execute reliably while respecting host vs Breeze boundaries.

The machine-readable definition lives in the `agent-skill` block below and is
exported to `skills.json` by `scripts/ci/prek/extract_breeze_contribution_skills.py`.
