...
def get_dag_source(
    *,
    ...
    dag_version_obj_select = select(DagVersion).where(DagVersion.dag_id == dag_id)
    if version_name:
        dag_version_obj_select = dag_version_obj_select.where(DagVersion.version_name == version_name)
    if version_number:
        dag_version_obj_select = dag_version_obj_select.where(DagVersion.version_number == version_number)
    dag_version = session.scalar(dag_version_obj_select.order_by(DagVersion.version_number.desc()).limit(1))
    if not dag_version:
        raise NotFound(
