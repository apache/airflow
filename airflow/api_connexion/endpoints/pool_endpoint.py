...
def patch_pool(
    *,
    ...
    try:
        patch_body = pool_schema.load(get_mapping_from_request())
    ...
    if update_mask:
        ...
        fields_diff = required_fields - set(get_mapping_from_request().keys())
        if fields_diff:
...
def post_pool(*, session: Session = NEW_SESSION) -> APIResponse:
    ...
    try:
        post_body = pool_schema.load(get_mapping_from_request(), session=session)
    except ValidationError as err:
        raise BadRequest(detail=str(err.messages))
