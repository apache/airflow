...
def create_section(section_num):
    ...
    """
    dummies = [DummyOperator(task_id=f'task-{section_num}.{i + 1}') for i in range(5)]

    with TaskGroup(f"section_{section_num}.1") as inside_section_1:
        _ = [DummyOperator(task_id=f'task-{section_num}.1.{i + 1}',) for i in range(3)]

    with TaskGroup(f"section_{section_num}.2") as inside_section_2:
        _ = [DummyOperator(task_id=f'task-{section_num}.2.{i + 1}',) for i in range(3)]
