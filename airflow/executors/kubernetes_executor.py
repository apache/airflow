...
class _KubernetesJobType(NamedTuple):
    key: TaskInstanceKey
    command: CommandType
    config: Any
...
def _retry_backoff_seconds(count):
    return min(2 ** count, 120)
...
class AirflowKubernetesScheduler(LoggingMixin):
    ...
    def __init__(
        self,
        kube_config: Any,
        task_queue: 'Queue[_KubernetesJobType]',
    ...
    def run_next(self, next_job: _KubernetesJobType) -> None:
        """
        ...
        dag_id, task_id, run_id, try_number, _ = next_job.key

        if next_job.command[0:3] != ["airflow", "tasks", "run"]:
            raise ValueError('The command must start with ["airflow", "tasks", "run"].')

        base_worker_pod = get_base_pod_from_template(next_job.pod_template_file, self.kube_config)
        ...
        pod = PodGenerator.construct_pod(
            ...
            date=None,
            run_id=run_id,
            args=next_job.command,
            pod_override_object=next_job.config,
            base_worker_pod=base_worker_pod,
        )
        # Reconcile the pod generated by the Operator and the Pod
        # generated by the .cfg file
        self.log.debug("Kubernetes running for command %s", next_job.command)
...
class KubernetesExecutor(BaseExecutor):
    ...
    def __init__(self):
        ...
        self._manager = multiprocessing.Manager()
        self.task_queue: 'Queue[_KubernetesJobType]' = self._manager.Queue()
        ...
        for _ in range(self.kube_config.worker_pods_creation_batch_size):
            try:
                ...
                try:
                    ...
                    if not is_retryable_http_code(e.status):
                        self.log.error("Pod creation failed with reason %r. Failing task", e.reason)
                        self.change_state(task.key, State.FAILED, e)
                    else:
                        if task.retry_count >= _MAX_RETRIES - 1:
                            self.log.error(
                                "Pod creation failed after several retries with reason %r. Failing task",
                                e.reason,
                            )
                            self.change_state(task.key, State.FAILED, e)
                        else:
                            retry_time = timedelta(seconds=_retry_backoff_seconds(task.retry_count))
                            self.log.warning(
                                'ApiException when starting task, retry (%d/%d) in %ds. Reason: %r. Msg: %s',
                                task.retry_count + 1,
                                _MAX_RETRIES,
                                int(retry_time.total_seconds()),
                                e.reason,
                                json.loads(e.body)['message'],
                            )
                            next_retry = datetime.utcnow() + retry_time
                            requeued_task = _KubernetesJobType(
                                task.key,
                                task.command,
                                task.config,
                                task.pod_template_file,
                                next_retry,
                                task.retry_count + 1,
                            )
                            self.task_queue.put(requeued_task)
                finally:
                    self.task_queue.task_done()
            except Empty:
                break
