    ...
    def process_status(
        ...
        if status == "Pending":
            if event["type"] == "DELETED":
                self.log.info("Event: Failed to start pod %s, annotations: %s", pod_id, annotations_string)
                self.watcher_queue.put((pod_id, namespace, State.FAILED, annotations, resource_version))
            else:
                self.log.debug("Event: %s Pending, annotations: %s", pod_id, annotations_string)
        elif status == "Failed":
            self.log.error("Event: %s Failed, annotations: %s", pod_id, annotations_string)
            self.watcher_queue.put((pod_id, namespace, State.FAILED, annotations, resource_version))
        elif status == "Succeeded":
            self.log.info("Event: %s Succeeded, annotations: %s", pod_id, annotations_string)
            ...
            if event["type"] == "DELETED":
                ...
                self.log.info("Event: %s is Running, annotations: %s", pod_id, annotations_string)
        else:
            self.log.warning(
