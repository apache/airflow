...
class CeleryExecutor(BaseExecutor):
    ...
    def heartbeat(self):
        ...
        if task_tuples_to_send:
            ...
            if tasks[0].app is not None:
                self.logger.debug(
                    'before enqueue task app count: ' + str(
                        len(set([task.app for task in tasks])))
                )

            self.logger.debug(
                'before enqueue task backend count: ' + str(len(set([task.backend
                                                                     for task in
                                                                     tasks]))))
