...
<<<<<<< HEAD
        _log.info('Executing SQL check: ' + self.sql)
        records = self.get_db_hook().get_first(self.sql)
        _log.info("Record: " + str(records))
        if not records:
...
<<<<<<< HEAD
        _log.info('Executing SQL check: ' + self.sql2)
        row2 = hook.get_first(self.sql2)
        _log.info('Executing SQL check: ' + self.sql1)
=======
...
<<<<<<< HEAD
            ...
            _log.info(rlog.format(m, ratio, self.metrics_thresholds[m]))
            ratios[m] = ratio
        ...
        if not all(test_results.values()):
            ...
            _log.warning(countstr.format(**locals()))
            for k in failed_tests:
                _log.warning(fstr.format(k=k, r=ratios[k],
                             tr=self.metrics_thresholds[k]))
            raise AirflowException(estr.format(", ".join(failed_tests)))
        _log.info("All tests have passed")
=======
