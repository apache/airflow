...
class DockerOperator(BaseOperator):
    ...
    :type host_config: json object
        **Use of host_config**
        ...
        .. code-block:: python
            ...
            host_config = { 'binds': ['/home/some_path:/container/some_path'],
                            'network_mode': 'bridge',
                            'shm_size': 1000,
                            'cpu_shares': 2048,
                            'mem_limit': None,
    ...
    """
    template_fields = ('container_config', 'host_config',)
    ...
    def __sanitize_host_config(self):
        """
        This private method validate the instance attribute ``self.host_config``.
        It also sets the required default values of ``host_config`` dictionary
        ...
        if 'volumes' in self.host_config:
            ...
            del self.host_config['volumes']

    def __sanitize_container_config(self):
        """
        This private method validate the instance attribute ``self.container_config``.
        It also sets the required default values of ``container_config`` dictionary
        ...
        if 'command' in self.container_config and \
            ...
            self.container_config['command'] = ast.literal_eval(self.container_config['command'])
        elif 'command' not in self.container_config:
            self.container_config['command'] = None
    ...
    def execute(self, context):
        ...
        self.__sanitize_host_config()
        self.__sanitize_container_config()

        self.log.info('Starting docker container from image %s', self.container_config['image'])
        ...
        if self.force_pull or len(self.cli.images(name=self.container_config['image'])) == 0:
            self.log.info('Pulling docker image %s', self.container_config['image'])
            for l in self.cli.pull(self.container_config['image'], stream=True):
                output = json.loads(l.decode('utf-8').strip())
                if 'status' in output:
                    self.log.info("%s", output['status'])

        with TemporaryDirectory(prefix='airflowtmp') as host_tmp_dir:
            self.container_config['environment']['AIRFLOW_TMP_DIR'] = self.tmp_dir
            self.host_config['binds'].append('{0}:{1}'.format(host_tmp_dir, self.tmp_dir))
            self.container = self.cli.create_container(
                host_config=self.cli.create_host_config(**self.host_config), **self.container_config)
