...
@lru_cache
def health_check_threshold(job_type: str, heartrate: int, grace_multiplier: float) -> int | float:
...
class Job(Base, LoggingMixin):
    ...
    def __init__(self, executor=None, heartrate=None, grace_multiplier=None, **kwargs):
        # Save init parameters as DB fields
        self.heartbeat_failed = False
        self.hostname = get_hostname()
        if executor:
            self.executor = executor
        if grace_multiplier is None:
