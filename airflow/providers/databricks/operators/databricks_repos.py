...
class DatabricksReposDeleteOperator(BaseOperator):
    """
    Deletes specified repository using `api/2.0/repos/
...
class DatabricksReposCreateOperator(BaseOperator):
    """
    Creates a Databricks Repo using `api/2.0/repos
    <https://docs.databricks.com/dev-tools/api/latest/repos.html#operation/create-repo>`_ API endpoint and
    optionally checking it out to a specific branch or tag.
        ...
        **kwargs,
        ...
        if git_provider is None:
            ...
            if self.git_provider is None:
                raise AirflowException("git_provider isn't specified and couldn't be guessed"
                                       f" for URL {git_url}")
        else:
            self.git_provider = git_provider
        if repo_path is not None and not __repos_path_regexp__.match(repo_path):
            raise AirflowException("repo_path should have form of /Repos/{folder}/{repo-name}, "
                                   f"got '{repo_path}'")
        self.repo_path = repo_path
        if branch is not None and tag is not None:
            raise AirflowException("Only one of branch or tag should be provided, but not both")
    ...
    def __detect_repo_provider__(url):
        ...
        try:
            ...
            if idx != -1:
                netloc = netloc[(idx+1):]
            netloc = netloc.lower()
            provider = __git_providers__.get(netloc)
            if provider is None and __aws_code_commit_regexp__.match(netloc):
