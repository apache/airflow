...
class SnowflakeSqlApiAsyncHook(SnowflakeHook):
    """
    ...
    def execute_query(
        ...
        :param query_tag: (Optional) Query tag that you want to associate with the SQL statement.
            For details, see
            https://docs.snowflake.com/en/sql-reference/parameters.html#label-query-tag parameter.
    ...
    def get_headers(self) -> dict[str, Any]:
        """
        Based on the private key, and with connection details
        JWT Token is generated and header is formed
        """
    ...
    def check_query_output(self, query_ids: list[str]) -> None:
        """
        Based on the query ids passed as the parameter make HTTP request to snowflake SQL API and
        logs the response
