...
from airflow.providers.snowflake.hooks.snowflake_sql_api import SnowflakeSqlApiAsyncHook
...
class SnowflakeSqlApiOperatorAsync(SnowflakeOperator):
    ...
    Where can this operator fit in?
         ...
         - To execute the SQL statement asynchronously and to execute standard queries and most
    ...
    The following commands are not supported:
        ...
        - The CALL command with stored procedures that return a table(stored procedures with the
    ...
    def execute_complete(self, context: Context, event: dict[str, str | list[str]] | None = None) -> None:
        ...
        if event:
            if "status" in event and event["status"] == "error":
                ...
                hook = SnowflakeSqlApiAsyncHook(snowflake_conn_id=self.snowflake_conn_id)
