...
from airflow.providers.apache.hive.hooks.hive import HiveCliHook  # type: ignore
...
class CdwHiveMetastoreHook(CdwHook):
    ...
    def check_for_partition(self, schema, table, partition):
        ...
        """
        hql = (
            "select dbs.name as db_name, tbls.tbl_name as tbl_name, partitions.part_name as part_name "
            "from partitions "
            "left outer join tbls on tbls.tbl_id = partitions.tbl_id "
            "left outer join dbs on dbs.db_id = tbls.db_id "
            f"where dbs.name = '{schema}' "
            f"and tbls.tbl_name = '{table}' "
            f"and partitions.part_name = '{partition}';"
        )
    ...
    def get_table(self, table_name, db="default"):
        ...
        hql = (
            "select dbs.name as db_name, tbls.tbl_name as tbl_name"
            "from tbls "
            "left outer join dbs on dbs.db_id = tbls.db_id "
            f"where dbs.name = '{db}' "
            f"and tbls.tbl_name = '{table_name}' "
        )
    ...
    def get_tables(self, db, pattern="*"):
        ...
        hql = (
            ...
            "left outer join dbs on dbs.db_id = tbls.db_id "
            f"where dbs.name = '{db}' "
            f"and tbls.tbl_name like '{pattern.replace('*', '%')}' "
        )
        response = self.run_cli(hql, self.conn.schema, verbose=True, hive_conf=None)
        result_lines = CdwHiveMetastoreHook.parse_csv_lines(response)
