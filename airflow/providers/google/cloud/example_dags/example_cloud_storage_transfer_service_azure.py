...
* GCP_TRANSFER_SOURCE_AZURE_CONTAINER - Azure Storage container from which files are copied.
  .. warning::
    You need to provide a large enough set of data so that operations do not execute too quickly.
    Otherwise, DAG will fail.
* GCP_TRANSFER_AZURE_STORAGE_ACCOUNT - Azure Storage account
...
GCP_TRANSFER_SOURCE_AZURE_CONTAINER = os.environ.get('GCP_TRANSFER_SOURCE_AZURE_CONTAINER')
GCP_TRANSFER_AZURE_STORAGE_ACCOUNT = os.environ.get('GCP_TRANSFER_AZURE_STORAGE_ACCOUNT')
...
azure_to_gcs_transfer_body = {
    ...
    TRANSFER_SPEC: {
        AZURE_BS_DATA_SOURCE: {
            AZURE_BS_ACCOUNT: GCP_TRANSFER_AZURE_STORAGE_ACCOUNT,
            AZURE_CONTAINER: GCP_TRANSFER_SOURCE_AZURE_CONTAINER,
        },
...
with models.DAG(
    ...
    create_transfer_job_from_azure = CloudDataTransferServiceCreateJobOperator(
        task_id="create_transfer_job_from_aws", body=azure_to_gcs_transfer_body
    )
