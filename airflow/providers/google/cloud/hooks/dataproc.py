...
class DataprocHook(GoogleBaseHook):
    ...
    def wait_for_batch(
        ...
        state = None
        result: Batch = None
        while state not in [Batch.State.CANCELLED, Batch.State.FAILED, Batch.State.SUCCEEDED]:
            try:
                ...
                self.log.info("Retrying. Dataproc API returned server error when waiting for batch id %s: %s",
                              batch_id, err)
