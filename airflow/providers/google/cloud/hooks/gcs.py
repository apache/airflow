...
class GCSHook(GoogleBaseHook):
    ...
    def copy(
        ...
        get_hook_lineage_collector().add_input_dataset(
            context=self,
            scheme="gs",
            dataset_kwargs={"bucket": source_bucket.name, "key": source_object.name},  # type: ignore[attr-defined]
        )
        get_hook_lineage_collector().add_output_dataset(
            context=self,
            scheme="gs",
            dataset_kwargs={"bucket": destination_bucket.name, "key": destination_object.name},  # type: ignore[union-attr]
        )
    ...
    def rewrite(
        ...
        get_hook_lineage_collector().add_output_dataset(
            context=self,
            scheme="gs",
            dataset_kwargs={"bucket": destination_bucket.name, "key": destination_object},  # type: ignore[attr-defined]
        )
        self.log.info(
