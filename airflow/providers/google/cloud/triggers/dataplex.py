...
class DataplexJobTrigger(BaseTrigger):
    """
    DataplexJobTrigger runs on the trigger worker and waits for the job to be in the `SUCCEEDED` state.
    ...
    def serialize(self):
        return (
            "airflow.providers.google.cloud.triggers.dataplex.DataplexJobTrigger",
            {
                "job_id": self.job_id,
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        while True:
            ...
            if state in (DataScanJob.State.FAILED, DataScanJob.State.SUCCEEDED, DataScanJob.State.CANCELLED):
                break
            self.log.info(
                "Current state is: %s, sleeping for %s seconds.", state, self.polling_interval_seconds
            )
