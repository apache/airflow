...
def create_session_for_connection(connection_name: str):
    ...
    session.root_url = conn.host
    session.get = lambda *args, **kwargs: requests.Session.get(
        session, urllib.parse.urljoin(session.root_url, args[0]), *args[1:], **kwargs)
    session.post = lambda *args, **kwargs: requests.Session.post(
        session, urllib.parse.urljoin(session.root_url, args[0]), *args[1:], **kwargs)
    session.put = lambda *args, **kwargs: requests.Session.put(
        session, urllib.parse.urljoin(session.root_url, args[0]), *args[1:], **kwargs)
    return session


def _create_session_for_user(root_url: str, userid: str, password: str, client_id: str = 'sas.cli',
                             client_secret: str = ''):
