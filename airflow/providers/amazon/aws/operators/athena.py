...
class AWSAthenaOperator(BaseOperator):
    ...
    def execute(self, context: dict) -> Optional[str]:
        ...
        if query_status in AWSAthenaHook.FAILURE_STATES:
            ...
            raise Exception(
                f'Final state of Athena job is {query_status}, query_execution_id is {self.query_execution_id}. Error: {error_message}'
            )
        elif not query_status or query_status in AWSAthenaHook.INTERMEDIATE_STATES:
            raise Exception(
                f'Final state of Athena job is {query_status}. Max tries of poll status exceeded, query_execution_id is {self.query_execution_id}.'
            )
