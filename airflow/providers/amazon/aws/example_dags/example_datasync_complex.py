...
CREATE_SOURCE_LOCATION_KWARGS = json.loads(getenv(
    'CREATE_SOURCE_LOCATION_KWARGS', default_create_source_location_kwargs))
...
CREATE_DESTINATION_LOCATION_KWARGS = json.loads(getenv(
    'CREATE_DESTINATION_LOCATION_KWARGS', default_destination_location_kwargs))
...
with models.DAG(
    ...
    run_task = AWSDataSyncTaskOperator(
        task_id='run_task',
        task_arn="{{ ti.xcom_pull(task_ids='join_task') }}"
    )
