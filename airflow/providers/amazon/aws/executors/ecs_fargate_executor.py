...
class AwsEcsFargateExecutor(BaseExecutor):
    ...
    def _run_task_kwargs(self, task_id: TaskInstanceKey, cmd: CommandType,
    ...
    def _load_run_kwargs(self) -> dict:
        run_kwargs = import_string(
            conf.get(
                'ecs_fargate',
                'run_task_kwargs',
                fallback='airflow.providers.amazon.aws.executors.ecs_fargate_conf.ECS_FARGATE_RUN_TASK_KWARGS'
            )
        )
        ...
        if 'overrides' not in run_kwargs or 'containerOverrides' not in run_kwargs['overrides'] \
            ...
            raise KeyError('Rendered JSON template does not contain key '
...
class EcsFargateTaskCollection:
    ...
    def add_task(self, task: EcsFargateTask, airflow_task_key: TaskInstanceKey, queue: str,
