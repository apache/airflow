...
class AwsEcsExecutor(BaseExecutor):
        ...
        for _ in range(queue_len):
            ...
            if failure_reasons:
                ...
                if int(attempt_number) < int(self.__class__.MAX_RUN_TASK_ATTEMPTS):
                    ...
                    self.log_task_event(
                        ti_key=task_key,
                        description=f"Task could not be queued after {attempt_number} attempts. "
                ...
                self.log_task_event(ti_key=task_key, description=f"ECS RunTask Response: {run_task_response}")
    ...
    def log_task_event(self, *, ti_key: TaskInstanceKey, description: str):
        ...
        with suppress(AttributeError):
            super().log_task_event(ti_key=ti_key, description=description)
