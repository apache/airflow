...
class LivyHook(HttpHook, LoggingMixin):
    ...
    def post_batch(self, *args: Any, **kwargs: Any) -> int:
        ...
        self.log.info(f"Batch submitted with session id: {batch_id}")
    ...
    def get_batch(self, session_id: int | str) -> dict:
        ...
        self.log.debug(f"Fetching info for batch session {session_id}")
        response = self.run_method(endpoint=f"/batches/{session_id}", headers=self.extra_headers)

        try:
            response.raise_for_status()
        except requests.exceptions.HTTPError as err:
            self.log.warning(f"Got status code {err.response.status_code} for session {session_id}")
            raise AirflowException(
    ...
    def delete_batch(self, session_id: int | str) -> dict:
        ...
        self.log.info(f"Deleting batch session {session_id}")
        response = self.run_method(
    ...
    def dump_batch_logs(self, session_id: int | str) -> None:
        ...
        :return: response body
        """
        self.log.info(f"Fetching the logs for batch session with id: {session_id}")
        log_start_line = 0
        log_total_lines = 0
        log_batch_size = 100
...
class LivyAsyncHook(HttpAsyncHook, LoggingMixin):
    ...
    async def get_batch_state(self, session_id: int | str) -> Any:
        ...
        """
        self._validate_session_id(session_id)
        self.log.info(f"Fetching info for batch session {session_id}")
