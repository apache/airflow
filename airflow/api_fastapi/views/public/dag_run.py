...
from airflow.api_fastapi.serializers.dag_run import DAGRunNotePatchBody, DAGRunResponse
...
@dag_run_router.patch("/{dag_run_id}/setNote", responses=create_openapi_http_exception_doc([401, 403, 404]))
async def set_dag_run_note(
    dag_id: str, dag_run_id: str, note: DAGRunNotePatchBody, session: Annotated[Session, Depends(get_session)]
):
    dag_run = session.scalar(select(DagRun).filter_by(dag_id=dag_id, run_id=dag_run_id))
    if dag_run is None:
        raise HTTPException(
