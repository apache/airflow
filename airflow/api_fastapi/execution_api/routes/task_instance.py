...
async def ti_heartbeat(
    ...
    old = select(TI.state, TI.hostname, TI.unixname).where(TI.id == ti_id_str).with_for_update()

    try:
        (previous_state, hostname, unixname) = session.execute(old).one()
    except NoResultFound:
        log.error("Task Instance %s not found", ti_id_str)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if hostname != ti_payload.hostname or unixname != ti_payload.unixname:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail={
