...
def setup_task(func: Callable) -> Callable:
    ...
    func.operator_class._is_setup = True  # type: ignore[attr-defined]
    return func
...
def teardown_task(_func=None, *, on_failure_fail_dagrun: bool | None = None) -> Callable:
    def teardown(func: Callable) -> Callable:
        ...
        if isinstance(func, _TaskGroupFactory):
            raise AirflowException("Task groups cannot be marked as setup or teardown.")
        func.operator_class._is_teardown = True  # type: ignore[attr-defined]
        func.operator_class._on_failure_fail_dagrun = on_failure_fail_dagrun  # type: ignore[attr-defined]
        return func
