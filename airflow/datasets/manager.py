...
class DatasetManager(LoggingMixin):
    ...
    @classmethod
    def _queue_dagruns(
        cls, dataset_id: int, dags_to_queue: set[DagModel], timestamp: datetime, session: Session
    ) -> None:
        ...
        if session.bind.dialect.name == "postgresql":
            return cls._postgres_queue_dagruns(dataset_id, dags_to_queue, timestamp, session)
        return cls._slow_path_queue_dagruns(dataset_id, dags_to_queue, timestamp, session)
    ...
    def _slow_path_queue_dagruns(
        ...
        def _queue_dagrun_if_needed(dag: DagModel) -> str | None:
            item = DatasetDagRunQueue(target_dag_id=dag.dag_id, dataset_id=dataset_id, created_at=timestamp)
    ...
    def _postgres_queue_dagruns(
        ...
        values = [{"target_dag_id": dag.dag_id, "created_at": timestamp} for dag in dags_to_queue]
