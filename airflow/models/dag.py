...
from airflow.models.dataset import DatasetConsumerDag, DatasetDagRunQueue as DDRQ
...
def get_dataset_triggered_next_run_info(dag_ids: List[str], *, session: Session) -> Dict[str, str]:
    ...
    return {
        ...
        .join(
            DDRQ,
            and_(
                DDRQ.dataset_id == DatasetConsumerDag.dataset_id,
                DDRQ.target_dag_id == DatasetConsumerDag.dag_id,
            ),
            isouter=True,
        )
        .group_by(DatasetConsumerDag.dag_id)
        .filter(DatasetConsumerDag.dag_id.in_(dag_ids))
        .all()
    }
...
class DAG(LoggingMixin):
    ...
    def bulk_write_to_db(cls, dags: Collection["DAG"], session=NEW_SESSION):
        ...
        from airflow.datasets import Dataset
        from airflow.models.dataset import DatasetConsumerDag, DatasetModel, DatasetTaskRef
...
class DagModel(Base):
    ...
    def dags_needing_dagruns(cls, session: Session) -> Tuple[Query, Dict[str, Tuple[datetime, datetime]]]:
        ...
        dataset_triggered_dag_info_list = {
            ...
            .join(DatasetConsumerDag.queue_records, isouter=True)
            .group_by(DatasetConsumerDag.dag_id)
