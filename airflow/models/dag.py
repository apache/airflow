...
from airflow.models.dataset import DagDatasetConsumer, DatasetDagRunQueue as DDRQ
...
def get_dataset_triggered_next_run_info(dag_ids: List[str], *, session: Session) -> Dict[str, str]:
    ...
    return {
        ...
        .join(
            DDRQ,
            and_(
                DDRQ.dataset_id == DagDatasetConsumer.dataset_id,
                DDRQ.target_dag_id == DagDatasetConsumer.dag_id,
            ),
            isouter=True,
        )
        .group_by(DagDatasetConsumer.dag_id)
        .filter(DagDatasetConsumer.dag_id.in_(dag_ids))
        .all()
    }
...
class DAG(LoggingMixin):
    ...
    def bulk_write_to_db(cls, dags: Collection["DAG"], session=NEW_SESSION):
        ...
        from airflow.datasets import Dataset
        from airflow.models.dataset import DagDatasetConsumer, DatasetModel, DatasetTaskRef
...
class DagModel(Base):
    ...
    def dags_needing_dagruns(cls, session: Session) -> Tuple[Query, Dict[str, Tuple[datetime, datetime]]]:
        ...
        dataset_triggered_dag_info_list = {
            ...
            .join(DagDatasetConsumer.queue_records, isouter=True)
            .group_by(DagDatasetConsumer.dag_id)
