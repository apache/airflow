...
class AzureKubernetesOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        service_profile = ContainerServiceServicePrincipalProfile(
            client_id=ci_hook.client_id, secret=ci_hook.client_secret, key_vault_secret_ref=None)
        ...
        try:
            self.log.info("Checking if the AKS instance %s is present", self.name)
            ...
            poller = containerservice.managed_clusters.create_or_update(
                resource_group_name=self.resource_group, resource_name=self.name, parameters=managed_cluster)
