...
class JiraTicketSensor(JiraSensor):
    ...
    def poke(self, context):
        logging.info('Jira Sensor checking for change in ticket : %s', self.ticket_id)
            ...
            if issue is not None \
                ...
                if field_value is not None:
                    if isinstance(field_value, list):
                        ...
                        logging.warning("not implemented checker for issue field %s "
                                        "which is neither string nor list nor "
                                        "jira Resource", self.field)

        except JIRAError as jira_error:
            logging.error("jira error while checking with expected value: %s", jira_error)
        except Exception as e:
            logging.error("error while checking with expected value %s, error: %s",
                          self.expected_value, e)
        if result is True:
            logging.info("issue field %s has expected value %s, returning success",
                         self.field, self.expected_value)
        else:
            logging.info("issue field %s dont have expected value %s yet. ",
                         self.field, self.expected_value)
        return result
