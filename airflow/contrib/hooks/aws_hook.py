        ...
        if self.aws_conn_id:  # pylint: disable=too-many-nested-blocks
            ...
            try:
                ...
                if connection_object.login:
                    ...
                    "aws_access_key_id" in extra_config
                    and "aws_secret_access_key" in extra_config
                ):
                ...
                if creds_from:
                    ...
                    self.log.info("No credentials retrieved from Connection %s", self.aws_conn_id)
                ...
                if "config_kwargs" in extra_config:
                    self.log.info(
                        "Retrieving config_kwargs from Connection.extra_config['config_kwargs']"
                    )
                ...
                if (
                    role_arn is None
                    and aws_account_id is not None
                    and aws_iam_role is not None
                ):
                    self.log.info(
                ...
                if role_arn is not None:
                    ...
                    self.log.info(
                        "Doing assume_role with role_arn=%s role_session_name=%s",
                        role_arn,
                        role_session_name,
                    )
