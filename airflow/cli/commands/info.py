...
def show_info(anonymize, file_io, output, verbose):
    """Show information related to Airflow, system and other."""
    # Enforce anonymization, when file_io upload is tuned on.
    anonymizer = PiiAnonymizer() if anonymize or file_io else NullAnonymizer()
    info = AirflowInfo(anonymizer)
    if file_io:
        content = info.render_text(output)
        _send_report_to_fileio(content)
    else:
        info.show(output)
