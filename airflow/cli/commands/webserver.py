...
def webserver(
    ...
    access_logfile = access_logfile or conf.get('webserver', 'access_logfile')
    error_logfile = error_logfile or conf.get('webserver', 'error_logfile')
    access_logformat = access_logformat or conf.get('webserver', 'access_logformat')
    num_workers = workers or conf.get('webserver', 'workers')
    worker_timeout = worker_timeout or conf.get('webserver', 'web_server_worker_timeout')
    ssl_cert = ssl_cert or conf.get('webserver', 'web_server_ssl_cert')
    ssl_key = ssl_key or conf.get('webserver', 'web_server_ssl_key')
    ...
    if debug:
        console.print(f"Starting the web server on port {port} and host {hostname}.")
        ...
        pid_file, stdout, stderr, log_file = setup_locations("webserver", pid, stdout, stderr, log_file)
        ...
        if access_logformat and access_logformat.strip():
            run_args += ['--access-logformat', str(access_logformat)]
