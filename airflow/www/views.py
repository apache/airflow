...
class DagFilter(BaseFilter):
    ...
    def apply(self, query, func):
        if (
            current_app.appbuilder.sm.has_access(permissions.ACTION_CAN_READ, permissions.RESOURCE_DAG) or
            current_app.appbuilder.sm.has_access(permissions.ACTION_CAN_EDIT, permissions.RESOURCE_DAG)
        ):
            return query
        filter_dag_ids = current_app.appbuilder.sm.get_accessible_dag_ids(g.user)
...
class DagEditFilter(BaseFilter):
    ...
    def apply(self, query, func):  # pylint: disable=redefined-outer-name,unused-argument
        filter_dag_ids = {dag.dag_id for dag in current_app.appbuilder.sm.get_editable_dags(g.user)}
        return query.filter(self.model.dag_id.in_(filter_dag_ids))
