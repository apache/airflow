...
function setUpZoomSupport() {
  ...
  const zoomScale = Math.min(Math.min(width / graphWidth, height / graphHeight), maxZoom) * 0.8;
...
function focusGroup(nodeId, followMouse = true) {
  if (nodeId != null && zoom != null) {
    ...
    const scale = Math.min(Math.min(width / nodeWidth, height / nodeHeight), maxZoom) * 0.8;
