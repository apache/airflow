...
class SchedulerJob(BaseJob):
    ...
    def _process_executor_events(self, simple_dag_bag, session=None):
        ...
        for key, state in list(self.executor.get_event_buffer(simple_dag_bag.dag_ids)
                                   .items()):
            dag_id, task_id, execution_date, retry_number = key
            self.log.info(
                "Executor reports %s.%s execution_date=%s as %s for retry_number %s",
                dag_id, task_id, execution_date, state, retry_number
            )
            if state == State.FAILED or state == State.SUCCESS:
                ...
                if ti.try_number == retry_number and ti.state == State.QUEUED:
