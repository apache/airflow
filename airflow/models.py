...
class Connection(Base):
    ...
    @classmethod
    def add_type(cls, connection_type):
        cls._types[connection_type.name] = connection_type

    @classmethod
    def get_types(cls):
        return [(connection_type.name, connection_type.description) for connection_type in cls._types.values()]

    @classmethod
    def get_type(cls, conn_type):
        from airflow.connection_types import BaseConnectionType
        return cls._types.get(conn_type, BaseConnectionType)
    ...
    def parse_from_uri(self, uri):
        ...
        if conn_type == 'postgresql':
            conn_type = 'postgres'
        connection_type = self.get_type(conn_type)
        self.conn_type = conn_type
        (self.host, self.schema, self.login, self.password, self.port,
            self.extra) = connection_type.parse_from_uri(uri)

    def to_uri(self):
        connection_type = self.get_type(self.conn_type)
        return connection_type.to_uri(self.host, self.schema, self.login,
            self.password, self.port, self.extra)
    ...
    def get_hook(self):
        connection_type = self.get_type(self.conn_type)
        return connection_type.get_hook(self.conn_id)
