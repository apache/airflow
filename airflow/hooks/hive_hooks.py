        ...
        with TemporaryDirectory(prefix='airflow_hiveop_') as tmp_dir:
            with NamedTemporaryFile(dir=tmp_dir) as f:
                ...
                while True:
                    ...
                    if verbose:
                        _log.info(line.decode('UTF-8').strip())
                sp.wait()
    ...
    def test_hql(self, hql):
        ...
        for query_set in [create, insert]:
            for query in query_set:
                ...
                _log.info("Testing HQL [{0} (...)]".format(query_preview))
                if query_set == insert:
...
class HiveServer2Hook(BaseHook):
    ...
    def get_conn(self, schema=None):
        ...
        if auth_mechanism == 'KERBEROS':
            _log.warning("Detected deprecated 'KERBEROS' for authMechanism for %s. Please use 'GSSAPI' instead",
        ...
        with self.get_conn(schema) as conn:
            ...
            for statement in hql:
                ...
                try:
                    ...
                    _log.debug("get_results returned no records")
                if records:
                    results = {
                        'data': records,
            ...
            with conn.cursor() as cur:
                ...
                with open(csv_filepath, 'wb') as f:
                    ...
                    while True:
                        ...
                        i += len(rows)
                        _log.info("Written {0} rows so far.".format(i))
                    _log.info("Done. Loaded a total of {0} rows.".format(i))
