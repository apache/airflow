...
"""Add processor_subdir column to DagModel and SerializedDagModel tables.
...
def upgrade():
    ...
    with op.batch_alter_table('dag') as batch_op:
        if conn.dialect.name == "mysql":
            batch_op.add_column(sa.Column('processor_subdir', sa.Text(length=1000), nullable=True))
        else:
            batch_op.add_column(sa.Column('processor_subdir', sa.String(length=1000), nullable=True))
