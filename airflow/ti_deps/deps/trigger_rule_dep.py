        ...
        if trigger_rule == TR.ONE_SUCCESS:
            if successes <= 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires one upstream task success, but none were found. upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        elif trigger_rule == TR.ONE_FAILED:
            if not failed and not upstream_failed:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires one upstream task failure, but none were found. upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        elif trigger_rule == TR.ALL_SUCCESS:
            num_failures = upstream - successes
            if num_failures > 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream tasks to have succeeded, but found {num_failures} non-success(es). upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        elif trigger_rule == TR.ALL_FAILED:
            num_successes = upstream - failed - upstream_failed
            if num_successes > 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream tasks to have failed, but found {num_successes} non-failure(s). upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        elif trigger_rule == TR.ALL_DONE:
            if not upstream_done:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream tasks to have completed, but found {upstream_done} task(s) that were not done. upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        elif trigger_rule == TR.NONE_FAILED:
            num_failures = upstream - successes - skipped
            if num_failures > 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream tasks to have succeeded or been skipped, but found {num_failures} non-success(es). upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        elif trigger_rule == TR.NONE_FAILED_MIN_ONE_SUCCESS:
            num_failures = upstream - successes - skipped
            if num_failures > 0:
            ...
            if not upstream_done or (skipped > 0):
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream tasks to not have been skipped, but found {skipped} task(s) skipped. upstream_tasks_state={upstream_tasks_state}, upstream_task_ids={task.upstream_task_ids}"
                )
        else:
