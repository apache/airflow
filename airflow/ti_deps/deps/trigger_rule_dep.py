        ...
        if trigger_rule == TR.ONE_SUCCESS:
            if successes <= 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires one upstream "
            ...
            if num_failures > 0:
                yield self._failing_status(
                    ...
                    f"tasks to have succeeded, but found {num_failures} non-success(es). "
                    f"upstream_tasks_state={upstream_tasks_state}, "
                    f"upstream_task_ids={task.upstream_task_ids}")
        elif trigger_rule == TR.ALL_FAILED:
            ...
            if num_successes > 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream "
                    f"tasks to have failed, but found {num_successes} non-failure(s). "
                    f"upstream_tasks_state={upstream_tasks_state}, "
                    f"upstream_task_ids={task.upstream_task_ids}")
            ...
            if not upstream_done:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream "
                    f"tasks to have completed, but found {upstream_done} task(s) that "
                    f"weren't done. upstream_tasks_state={upstream_tasks_state}, "
                    f"upstream_task_ids={task.upstream_task_ids}")
        elif trigger_rule == TR.NONE_FAILED:
            ...
            if num_failures > 0:
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream "
                    f"tasks to have succeeded or been skipped, but found {num_failures} "
                    f"non-success(es). upstream_tasks_state={upstream_tasks_state}, "
                    f"upstream_task_ids={task.upstream_task_ids}")
        elif trigger_rule == TR.NONE_SKIPPED:
            if not upstream_done or (skipped > 0):
                yield self._failing_status(
                    reason=f"Task's trigger rule '{trigger_rule}' requires all upstream "
                    f"tasks to not have been skipped, but found {skipped} task(s) skipped. "
                    f"upstream_tasks_state={upstream_tasks_state}, "
                    f"upstream_task_ids={task.upstream_task_ids}")
        else:
            yield self._failing_status(
