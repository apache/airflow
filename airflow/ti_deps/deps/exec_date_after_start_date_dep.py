    ...
    def _get_dep_statuses(self, ti, session, dep_context):
        if ti.task.start_date and ti.execution_date < ti.task.start_date:
            yield self._failing_status(
                reason=f"The execution date is {ti.execution_date.isoformat()} but "
                f"this is before the task's start date {ti.task.start_date.isoformat()}.")

        if (ti.task.dag and ti.task.dag.start_date and
                ti.execution_date < ti.task.dag.start_date):
            yield self._failing_status(
                reason=f"The execution date is {ti.execution_date.isoformat()} but "
                f"this is before the task's DAG's start date {ti.task.dag.start_date.isoformat()}.")
