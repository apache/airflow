...
def execute_in_subprocess(cmd: list[str], cwd: str | None = None, env_vars: dict | None = None) -> None:
    """
    Execute a process and stream output to logger.

    :param cmd: command and arguments to run
    :param cwd: Current working directory passed to the Popen constructor
    :param env_vars: Additional environment variables to set for the subprocess. If None,
        the subprocess will inherit the current environment variables of the parent process
        (``os.environ``)
    """
    execute_in_subprocess_with_kwargs(cmd, cwd=cwd, env=dict(os.environ, **env_vars) if env_vars else None)
