...
class BaseSetupTeardownContext:
    ...
    def push_context_managed_setup_task(
        ...
        if setup_task and setup_task != task:
            cls._previous_context_managed_setup_task.append(
    ...
    def push_context_managed_teardown_task(
        ...
        if teardown_task and teardown_task != task:
            cls._previous_context_managed_teardown_task.append(
    ...
    def pop_teardown_downstream_of_setup(
        ...
        if cls._previous_teardown_downstream_of_setup:
            cls._teardown_downstream_of_setup = (
                cls._previous_teardown_downstream_of_setup.pop()
            )
    ...
    def pop_setup_upstream_of_teardown(cls) -> AbstractOperator | list[AbstractOperator]:
        ...
        if cls._previous_setup_upstream_of_teardown:
            cls._setup_upstream_of_teardown = (
                cls._previous_setup_upstream_of_teardown.pop()
            )
    ...
    @classmethod
    def _push_tasks(
        cls, operator: AbstractOperator | list[AbstractOperator], setup: bool = False
    ):
        if isinstance(operator, list):
    ...
    @classmethod
    def _update_teardown_downstream(
        cls, operator: AbstractOperator | list[AbstractOperator]
    ):
        """
        ...
        if teardown_task and teardown_task != teardowns:
            cls._previous_teardown_downstream_of_setup.append(
                cls._teardown_downstream_of_setup
            )
    ...
    def _update_setup_upstream(cls, operator: AbstractOperator | list[AbstractOperator]):
        ...
        if setup_task and setup_task != setups:
            cls._previous_setup_upstream_of_teardown.append(
                cls._setup_upstream_of_teardown
            )
    ...
    def set_setup_task_as_roots(cls, roots):
        ...
        if cls._context_managed_setup_task:
            cls.set_dependency(
                cls._context_managed_setup_task, setup_task, upstream=False
            )
        else:
    ...
    def set_work_task_roots_and_leaves(cls):
        ...
        if setup_task := cls._context_managed_setup_task:
            ...
            tasks_in_context = [
                x
                for x in cls.context_map.get(setup_task, [])
                if not x.is_teardown and not x.is_setup
            ]
            if tasks_in_context:
        ...
        if teardown_task := cls._context_managed_teardown_task:
            ...
            tasks_in_context = [
                x
                for x in cls.context_map.get(teardown_task, [])
                if not x.is_teardown and not x.is_setup
            ]
            if tasks_in_context:
                leaves = [task for task in tasks_in_context if not task.downstream_list]
    ...
    def add_task(task: AbstractOperator | PlainXComArg):
        ...
        if not SetupTeardownContext.active:
            raise AirflowException(
                "Cannot add task to context outside the context manager."
            )
