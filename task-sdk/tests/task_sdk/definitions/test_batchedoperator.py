...
class TestBatchedOperator:
    ...
    def test_batched_iterate_task_with_dict_return_annotation_pushes_whole_results(
        self, run_ti: RunTI, mock_supervisor_comms
    ):
        """Batched counterpart of the test in test_iterate.py: each of the batch's task instances pushes
        ...
        with DAG(dag_id="iterate_dict_return") as dag:
            ...
            enrich.batch(size=batch_size).iterate(item=list_items())
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (0, 5),  # Non-batched: a plain .iterate()
        ],
    )
    def test_batch_size_preserved_through_lifecycle(self, batch_size, expand_size):
        ...
        with DAG(dag_id=f"test_batch_{batch_size}") as dag:
            ...
            op = partial if batch_size == 0 else partial.batch(size=batch_size)
    ...
    def test_batched_iterate_marks_partial_as_expanded(self, recwarn):
        """Batched counterpart of the test in test_iterate.py: .batch().iterate() also flags the
        ...
        with DAG(dag_id="test_batched_iterate_expand_called"):
            partial = EmptyOperator.partial(task_id="test_task")
            expand_input = DictOfListsExpandInput({"retry_delay": [1, 2]})
            partial.batch(size=3)._iterate(expand_input, strict=False)
