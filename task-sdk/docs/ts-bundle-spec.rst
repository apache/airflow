...
Both halves of the suffix are load-bearing. ``.min`` records that the code region is minified, which is what makes a
digest over it worth taking: the artifact is not meant to be read or edited in place. ``.mjs`` is required because
the bundle is an ES module with top-level ``await`` and cannot fall back to CommonJS, while a ``.js`` file is treated
as an ES module only by Node's syntax detection. That detection is unavailable before Node 22.7, and skipped outright
when an enclosing ``package.json`` declares ``"type": "commonjs"``.
