...
# ``.mjs`` is required, not cosmetic: a ``.js`` bundle is not always an ES module.
# See ``task-sdk/docs/ts-bundle-spec.rst``.
BUNDLE_SUFFIX = ".min.mjs"


def _is_bundle(path: pathlib.Path) -> bool:
    # ``Path.suffix`` of ``bundle.min.mjs`` is ``.mjs``, so match the whole name.
...
class NodeCoordinator(SubprocessCoordinator):
    ...
    :param bundles_root: Directories searched recursively, in order, for the first verified
        ``*.min.mjs`` bundle declaring the task instance's Dag. Keep them to deployed bundles:
        every ``*.min.mjs`` under them is opened, and an unusable one is named in the error
        raised when no bundle declares the Dag.
