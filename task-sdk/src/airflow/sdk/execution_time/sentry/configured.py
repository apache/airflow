...
if TYPE_CHECKING:
    ...
    from airflow.sdk.types import DagRunProtocol, TaskInstanceState

    RunRetturn = tuple[TaskInstanceState, ToSupervisor | None, BaseException | None]
    Run = Callable[[RuntimeTaskInstance, Context, Logger], RunRetturn]
...
class ConfiguredSentry(NoopSentry):
    ...
    def add_tagging(self, dag_run: DagRunProtocol, task_instance: RuntimeTaskInstance) -> None:
    ...
    def add_breadcrumbs(self, task_instance: RuntimeTaskInstance) -> None:
        """Add breadcrumbs inside of a task_instance."""
        breadcrumbs = task_instance.get_task_breadcrumbs(
    ...
    def enrich_errors(self, run: Run) -> Run:
        ...
        @functools.wraps(run)
        def wrapped_run(ti: RuntimeTaskInstance, context: Context, log: Logger) -> RunRetturn:
