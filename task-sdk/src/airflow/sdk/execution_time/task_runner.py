...
def _serialize_template_field(template_field: Any, name: str) -> str | dict | list | int | float | bool:
    """
    ...
    1. **Make the template_field JSON-encodable** — every container is rebuilt
       with primitive leaves (str/int/float/bool/None), tuples are flattened to
       lists, and unsupported objects fall through to ``str()`` so ``json.dumps``
    ...
    2. **Keep the output deterministic across parses** — callables are replaced
       ...
       repr) and dicts are key-sorted so the same input always produces the
       same string.
    ...
    def serialize_object(obj):
        ...
        if isinstance(obj, dict):
            ...
            # then sort by the serialized key.
