...
class XComIterable(Sequence):
    ...
    def __iter__(self) -> Iterator:
        """Fetch all XCom values in a single round-trip, deserializing lazily."""
        if not self.length:
            return iter(())

        from airflow.sdk.execution_time.comms import GetXComByKeys, XComSequenceSliceResult
        from airflow.sdk.execution_time.task_runner import SUPERVISOR_COMMS

        keys = [f"{BaseXCom.XCOM_RETURN_KEY}_{i}" for i in range(self.length)]
        msg = SUPERVISOR_COMMS.send(
            GetXComByKeys(
                keys=keys,
                dag_id=self.dag_id,
        ...
        if not isinstance(msg, XComSequenceSliceResult):
            raise TypeError(f"Got unexpected response to GetXComByKeys: {msg!r}")
        return iter(XCom.deserialize_value(_XComWrapper(v)) for v in msg.root)
    ...
    def __getitem__(self, key: int | slice) -> Any | Sequence[Any]:
        ...
        if isinstance(key, slice):
            ...
            if not indices:
                return []
            keys = [f"{BaseXCom.XCOM_RETURN_KEY}_{i}" for i in indices]
            msg = SUPERVISOR_COMMS.send(
                GetXComByKeys(
                    keys=keys,
                    dag_id=self.dag_id,
            ...
            if not isinstance(msg, XComSequenceSliceResult):
                raise TypeError(f"Got unexpected response to GetXComByKeys: {msg!r}")
            return [XCom.deserialize_value(_XComWrapper(v)) for v in msg.root]
