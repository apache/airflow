...
class DagTaskGroupsExistenceResult(BaseModel):
    existing: list[str]
    missing: list[str]
    type: Literal["DagTaskGroupsExistenceResult"] = "DagTaskGroupsExistenceResult"

    @classmethod
    def from_api_response(cls, response: DagTaskGroupsExistenceResponse) -> DagTaskGroupsExistenceResult:
        """
        Create result class from API Response.

        API Response is autogenerated from the API schema, so we convert it to a
        Result for communication between the Supervisor and the task
        process since it needs a discriminator field.
        """
        return cls(existing=response.existing, missing=response.missing, type="DagTaskGroupsExistenceResult")


class DagTasksExistenceResult(BaseModel):
    existing: list[str]
    missing: list[str]
    type: Literal["DagTasksExistenceResult"] = "DagTasksExistenceResult"

    @classmethod
    def from_api_response(cls, response: DagTasksExistenceResponse) -> DagTasksExistenceResult:
        """
        Create result class from API Response.
        ...
        process since it needs a discriminator field.
        """
        return cls(existing=response.existing, missing=response.missing, type="DagTasksExistenceResult")
...
class GetDagTaskGroupsExistence(BaseModel):
    dag_id: str
    task_group_ids: list[str]
    type: Literal["GetDagTaskGroupsExistence"] = "GetDagTaskGroupsExistence"


class GetDagTasksExistence(BaseModel):
    dag_id: str
    task_ids: list[str]
    type: Literal["GetDagTasksExistence"] = "GetDagTasksExistence"
