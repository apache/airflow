...
from typing import TYPE_CHECKING, Annotated, Any, ClassVar, Generic, Literal, TypeVar, cast, overload
...
class _APIResponseResultMixin:
    @classmethod
    def from_api_response(cls, api_response: BaseModel) -> Self:
        result_cls = cast("type[BaseModel]", cls)
        return cast("Self", result_cls(**api_response.model_dump(exclude_defaults=True)))


class _TypedAPIResponseResultMixin(_APIResponseResultMixin):
    @classmethod
    def from_api_response(cls, api_response: BaseModel) -> Self:
        response_data = api_response.model_dump(exclude_defaults=True)
        response_data["type"] = getattr(cls, "model_fields")["type"].default
        result_cls = cast("type[BaseModel]", cls)
        return cast("Self", result_cls(**response_data))
...
class AssetResult(_TypedAPIResponseResultMixin, AssetResponse):
...
class AssetEventResult(_APIResponseResultMixin, AssetEventResponse):
...
class AssetEventsResult(_TypedAPIResponseResultMixin, AssetEventsResponse):
    ...
    def iter_asset_event_results(self) -> Iterator[AssetEventResult]:
        return (AssetEventResult.from_api_response(event) for event in self.asset_events)


class AssetEventDagRunReferenceResult(_APIResponseResultMixin, AssetEventDagRunReference):
    @cached_property
    def source_dag_run(self) -> DagRun | None:
        if not self.source_dag_id or not self.source_run_id:
            return None
...
class InactiveAssetsResult(_TypedAPIResponseResultMixin, InactiveAssetsResponse):
    """Response of InactiveAssets requests."""

    type: Literal["InactiveAssetsResult"] = "InactiveAssetsResult"


class XComResult(_TypedAPIResponseResultMixin, XComResponse):
    """Response to ReadXCom request."""
...
class VariableResult(_TypedAPIResponseResultMixin, VariableResponse):
    type: Literal["VariableResult"] = "VariableResult"


class TaskStateStoreResult(_TypedAPIResponseResultMixin, TaskStateStoreResponse):
...
class AssetStateStoreResult(_TypedAPIResponseResultMixin, AssetStateStoreResponse):
...
class AssetsByAliasResult(BaseModel):
    ...
    def from_asset_responses(cls, asset_responses: list[AssetResponse]) -> AssetsByAliasResult:
        return cls(
            assets=[AssetResult.from_api_response(a) for a in asset_responses],
...
class DagRunResult(_TypedAPIResponseResultMixin, DagRun):
    type: Literal["DagRunResult"] = "DagRunResult"


class DagRunStateResult(_TypedAPIResponseResultMixin, DagRunStateResponse):
...
class PrevSuccessfulDagRunResult(_TypedAPIResponseResultMixin, PrevSuccessfulDagRunResponse):
...
class TaskStatesResult(_TypedAPIResponseResultMixin, TaskStatesResponse):
    type: Literal["TaskStatesResult"] = "TaskStatesResult"


class TaskBreadcrumbsResult(_TypedAPIResponseResultMixin, TaskBreadcrumbsResponse):
...
class HITLDetailRequestResult(_TypedAPIResponseResultMixin, HITLDetailRequest):
    """Response to CreateHITLDetailPayload request."""

    type: Literal["HITLDetailRequestResult"] = "HITLDetailRequestResult"


class DagResult(_TypedAPIResponseResultMixin, DagResponse):
