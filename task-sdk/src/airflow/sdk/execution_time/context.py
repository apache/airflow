...
class AssetStoreAccessors:
    """
    Mapping of asset store accessors for all concrete inlets of a task.
    ...
    def _single_accessor(self) -> AssetStoreAccessor:
        if self._total != 1:
            raise ValueError(
                f"Task has {self._total} concrete inlets — use context['asset_store'][MY_ASSET] to specify which"
            )
        if self._by_name:
