...
in the ``airflow/provider_dependencies.json``. Pre-commits are also used to generate dependencies.
...
pre-commit phase and pre-commit will fail - and add entry in provider_dependencies.json so that the
package extra dependencies are properly handled when package is installed.
