...
from airflow.sdk.execution_time.comms import GetXCom, SetXCom, SucceedTask, TaskState, XComResult
...
def test_all_xcomargs_from_mapped_tasks_are_consumable(create_runtime_ti, mock_supervisor_comms):
    with DAG("test_all_xcomargs_from_mapped_tasks_are_consumable") as dag:
        op1 = MockOperator.partial(task_id="op1").expand(arg1=[1, 2, 3])
        downstream = MockOperator(task_id="op2", arg1=op1.output)

    mock_supervisor_comms.get_message.return_value = XComResult(
        key="return_value", value=json.dumps([1, 2, 3])
    )

    ti = create_runtime_ti(task=downstream, upstream_map_indexes={"op1": None})
    ti.task.render_template_fields(context=ti.get_template_context())
...
@pytest.fixture
def run_ti(create_runtime_ti, mock_supervisor_comms) -> RunTI:
