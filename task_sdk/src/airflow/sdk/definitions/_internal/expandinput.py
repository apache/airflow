...
class DictOfListsExpandInput(ResolveMixin):
    ...
    def _get_map_lengths(self, resolved_val: Sized, upstream_map_indexes: dict[str, int]) -> dict[str, int]:
        """
        ...
        def _get_length(v: OperatorExpandArgument) -> int | None:
            ...
            if isinstance(v, XComArg):
                return get_task_map_length(v, resolved_val, upstream_map_indexes)
    ...
    def _expand_mapped_field(
        self, key: str, value: Any, context: Mapping[str, Any], *, include_xcom: bool
    ) -> Any:
        if _needs_run_time_resolution(value):
            value = value.resolve(context, include_xcom=include_xcom) if include_xcom else str(value)
        map_index = context["ti"].map_index
        if map_index < 0 or map_index is None:
            raise RuntimeError("can't resolve task-mapping argument without expanding")
        all_lengths = self._get_map_lengths(value, context.get("_upstream_map_indexes", {}))
    ...
    def resolve(
        self, context: Mapping[str, Any], *, include_xcom: bool = True
    ) -> tuple[Mapping[str, Any], set[int]]:
        data = {
            k: self._expand_mapped_field(k, v, context, include_xcom=include_xcom)
    ...
    def resolve(
        ...
        if isinstance(self.value, Sized):
            ...
            if not isinstance(mapping, Mapping):
                mapping = mapping.resolve(context, include_xcom=include_xcom)
        elif include_xcom:
            mappings = self.value.resolve(context, include_xcom=include_xcom)
