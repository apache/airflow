...
class DagRunResult(DagRun):
    ...
    @classmethod
    def from_dagrun(cls, dag_run: DagRun) -> DagRunResult:
        """
        Get DagRunResult from DagRun object.

        DagRun is autogenerated from the API schema, so we need to convert it to DagRunResult
        for communication between the Supervisor and the task process.
        """
        return cls(**dag_run.model_dump(exclude_defaults=True), type="DagRunResult")
...
ToTask = Annotated[
    Union[
        StartupDetails, XComResult, ConnectionResult, VariableResult, ErrorResponse, AssetResult, DagRunResult
    ],
