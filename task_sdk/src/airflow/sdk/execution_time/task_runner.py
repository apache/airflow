...
def startup() -> tuple[RuntimeTaskInstance, Logger, Context]:
    msg = SUPERVISOR_COMMS.get_message()
    ...
    context = _prepare(ti, log)
    if not context:
        raise SystemExit("Pre-execute checks failed")
    return ti, log, context
...
def _prepare(ti: RuntimeTaskInstance, log: Logger) -> Context | None:
    ...
    # We need to return the context, as it might have been updated to have the unmapped task in it
    return context


def run(ti: RuntimeTaskInstance, log: Logger, context: Context):
