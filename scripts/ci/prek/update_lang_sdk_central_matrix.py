...
def _with_since(mark: str, entry: matrix.DimensionEntry, sdk_display_name: str) -> str:
    """Append the ``since``/``sdk_version`` pair to a supported mark, e.g. ``✓ 3.3 (Go SDK 1.0.0-beta3)``."""
    if mark != matrix.SUPPORTED_MARK:
        return mark
    since = matrix._since(entry, sdk_display_name)
    return mark if since == matrix.NO_VERSION_MARK else f"{mark} {since}"


def _get_state_cell(sdk: SdkCapabilities, name: str) -> str:
    if sdk.doc is None:
        return ABSENT_MARK
    entry = sdk.doc["states"][name]
    return _with_since(matrix._state_mark(entry), entry, sdk.display_name)


def _get_capability_cell(sdk: SdkCapabilities, capability: matrix.Capability) -> str:
    if sdk.doc is None:
        return ABSENT_MARK
    entry = sdk.doc["capabilities"][capability.name]
    return _with_since(matrix._capability_mark(sdk.doc, capability), entry, sdk.display_name)
...
def render_central_table(sdk_capabilities: Sequence[SdkCapabilities]) -> list[str]:
    ...
    for state, tier in matrix.STATE_DIMENSIONS:
        lines += _render_row(
            f"``{state}`` ({tier})", [_get_state_cell(sdk, state) for sdk in sdk_capabilities]
        )
    current_group = ""
    for capability in matrix.CAPABILITY_DIMENSIONS:
        ...
        lines += _render_row(
            ...
            [_get_capability_cell(sdk, capability) for sdk in sdk_capabilities],
        )
    lines += [
        "\n",
        f"*Marks:* ``{matrix.SUPPORTED_MARK}`` supported (its Airflow and SDK release, when known) · "
