...
DOCS_PY_PATH = AIRFLOW_ROOT_PATH / "airflow-core" / "src" / "airflow" / "utils" / "docs.py"
...
TS_CONSTANT_PATH = (
    ...
    / "src"
    / "constants"
    / "latestPublishedAirflowVersion.ts"
)
...
def get_latest_published_airflow_version() -> str:
    return SUPPORTED_VERSIONS[0][1]


def render_python_latest_published_airflow_version(version: str) -> str:
    return f'LATEST_PUBLISHED_AIRFLOW_VERSION = "{version}"\n'


def render_typescript_latest_published_airflow_version(version: str) -> str:
    return (
        TS_LICENSE_HEADER
        + "\n// AUTO-GENERATED by scripts/ci/prek/supported_versions.py - do not edit by hand.\n"
        + "\n"
        + f'export const LATEST_PUBLISHED_AIRFLOW_VERSION = "{version}";\n'
    )


def write_generated_latest_published_constants() -> None:
    version = get_latest_published_airflow_version()
    replace_text_between(
        file=DOCS_PY_PATH,
        start=PYTHON_CONSTANT_START,
        end=PYTHON_CONSTANT_END,
        replacement_text=render_python_latest_published_airflow_version(version),
    )
    TS_CONSTANT_PATH.write_text(render_typescript_latest_published_airflow_version(version))
...
if __name__ == "__main__":
    ...
    write_generated_latest_published_constants()
