...
Renders the Markdown matrix from ``go-sdk/capabilities.yaml`` between the AUTO-GENERATED markers.
Exits non-zero when the file changed so the contributor re-stages it. The hook also verifies that
the manifest's supervisor schema version matches the Go runtime constant embedded in bundle metadata.
"""
...
def check_schema_version(doc: CapabilitiesDoc) -> bool:
    ...
    if go_version == declared:
        return True
    console.print(
        f"[red]go-sdk/capabilities.yaml declares supervisor_schema_version {declared!r} but "
