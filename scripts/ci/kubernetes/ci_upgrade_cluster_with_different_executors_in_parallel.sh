...
export SEMAPHORE_NAME="helm-chart-upgrade-test"
...
function run_helm_upgrade_test() {
    local kubernetes_version=$1
    local python_version=$2
    local job="Helm-Cluster-${kubernetes_version}-python-${python_version}"
...
function run_helm_tests_in_parallel() {
    parallel::cleanup_runner
    start_end::group_start "Monitoring helm upgrade tests"
    ...
    do
        ...
        run_helm_upgrade_test "${kubernetes_version}" "${python_version}" "${@}"
    done
    set +e
