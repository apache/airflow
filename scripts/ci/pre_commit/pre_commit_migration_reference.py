...
def update_revision_version_map(revisions):
    revision_version_map = {}

    for rev in revisions:
        revision_version_map[rev.revision] = rev.module.airflow_version

    replace_text_between(
        file=project_root / "airflow" / "utils" / "db.py",
        start="# BEGIN autogenerated revision-version map\n",
        end="# END autogenerated revision-version map\n",
        replacement_text=format_str(
            "REVISION_VERSION_MAP = " + repr(revision_version_map) + "\n\n",
            mode=Mode(line_length=110, string_normalization=False),
        ),
    )
...
if __name__ == '__main__':
    ...
    update_revision_version_map(revisions)
