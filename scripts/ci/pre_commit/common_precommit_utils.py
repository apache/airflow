...
def pre_process_files(files: list[str]) -> list[str]:
    ...
    * When running "providers" package, then we need to add --namespace-packages flag.
    * When running "airflow-core" package, then we need to exclude providers.
    ...
    if "providers" in files:
        if len(files) > 1:
            raise RuntimeError(
                "When running `providers` package, you cannot run any other packages because only "
                "providers package requires --namespace-packages flag to be set"
            )
        result.append("--no-namespace-packages")
    if "airflow-core" in files:
        if len(files) > 1:
            raise RuntimeError(
                "When running `aairflow-core/airflow` package, you cannot run any other packages because only "
                "providers package requires --exclude providers/.* flag to be set"
            )
