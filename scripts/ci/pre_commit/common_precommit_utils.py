...
RUFF_FORMAT_CMD = "ruff format --force-exclude {file_path} 2>&1 | grep -v '`ISC001`. To avoid unexpected behavior'"
...
def ruff_format_file_until_no_change(file_path: Path):
    ...
    console.print("Running ruff & ruff format in a while loop to ensure that the files are formatted correctly")
