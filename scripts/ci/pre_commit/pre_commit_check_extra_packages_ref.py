...
for dependency in optional_dependencies:
    ...
    if not find_matching:
        ...
        is_devel_dep = dependency.startswith(("devel", "editable")) or dependency in ["doc", "doc_gen"]
        is_provider_dep = dependency.startswith("editable_")
        short_dep = dependency.replace("editable_", "").replace("devel_", "")
        if not is_devel_dep:
            suggestions.append(
                (
                    dependency,
            ...
            if is_provider_dep:
                suggestions_providers.append(
                    (
                        dependency,
                        f"pip install -e '.[{dependency}]'",
                        f"Adds all libraries needed by the {short_dep} provider",
                    )
                )
            else:
