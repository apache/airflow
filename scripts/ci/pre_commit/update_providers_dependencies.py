    ...
    for root, _, filenames in os.walk(AIRFLOW_PROVIDERS_SRC_DIR):
        for filename in filenames:
            if filename == "provider.yaml":
                ...
                provider_name = str(
                    provider_file.parent.relative_to(AIRFLOW_PROVIDERS_SRC_DIR)
                ).replace(os.sep, ".")
                provider_info = yaml.safe_load(provider_file.read_text())
                if provider_info["state"] == "suspended":
                    suspended_paths.append(
                        provider_file.parent.relative_to(
                            AIRFLOW_PROVIDERS_SRC_DIR
                        ).as_posix()
                    )
...
def get_provider_id_from_relative_import_or_file(
    relative_path_or_file: str,
) -> str | None:
...
def get_provider_id_from_import(import_name: str, file_path: Path) -> str | None:
    ...
    if provider_id is None:
        ...
        if relative_path_from_import.startswith(tuple(suspended_paths)):
            return None
        warnings.append(
            f"We could not determine provider id from import {import_name} in {file_path}"
        )
    return provider_id
...
def get_provider_id_from_file_name(file_path: Path) -> str | None:
    ...
    if provider_id is None and file_path.name not in [
        "__init__.py",
        ...
        if relative_path.as_posix().startswith(tuple(suspended_paths)):
            ...
            warnings.append(
                f"We had a problem to classify the file {file_path} to a provider"
            )
    return provider_id
...
def check_if_different_provider_used(file_path: Path) -> None:
    ...
    for import_name in imports:
        ...
        if imported_provider is not None and imported_provider not in ALL_PROVIDERS:
            warnings.append(
                f"The provider {imported_provider} from {file_path} cannot be found."
            )
            continue
        ...
        if imported_provider == "standard":
            ...
            if (
                file_path.name == "celery_executor_utils.py"
                or "/example_dags/" in file_path.as_posix()
            ):
                continue
        if imported_provider and file_provider != imported_provider:
            ALL_DEPENDENCIES[file_provider]["cross-providers-deps"].append(
                imported_provider
            )
...
if __name__ == "__main__":
    ...
    for provider, provider_yaml_content in ALL_PROVIDERS.items():
        ...
        ALL_DEPENDENCIES[provider]["plugins"].extend(
            provider_yaml_content.get("plugins") or []
        )
    ...
    for key in sorted(ALL_DEPENDENCIES.keys()):
        ...
        unique_sorted_dependencies[key]["devel-deps"] = sorted(
            ALL_DEPENDENCIES[key]["devel-deps"]
        )
    ...
    if new_content != old_content:
        if os.environ.get("CI"):
            ...
            console.print()
            console.print(
                "[yellow]Make sure to rebase your changes on the latest main branch!"
            )
            console.print()
            sys.exit(1)
        else:
