...
EDGE_PROVIDER_ROOT_PATH = AIRFLOW_ROOT_PATH / "providers" / "edge3"
EDGE_PROVIDER_WWW_PATH = (
    EDGE_PROVIDER_ROOT_PATH / "src" / "airflow" / "providers" / "edge3" / "plugins" / "www"
)
EDGE_PROVIDER_WWW_HASH_FILE = EDGE_PROVIDER_ROOT_PATH / "www-hash.txt"
...
def compile_assets(www_directory: Path):
    ...
    EDGE_PROVIDER_WWW_HASH_FILE.parent.mkdir(exist_ok=True, parents=True)
    if dist_directory.exists():
        old_hash = (
            EDGE_PROVIDER_WWW_HASH_FILE.read_text().strip() if EDGE_PROVIDER_WWW_HASH_FILE.exists() else ""
        )
    ...
    new_hash = get_directory_hash(www_directory, skip_path_regexps=SKIP_PATH_REGEXPS)
    EDGE_PROVIDER_WWW_HASH_FILE.write_text(new_hash + "\n")
    print(f"Assets compiled successfully. New hash: {new_hash}")


if __name__ == "__main__":
    # Compile assets for edge provider
    compile_assets(EDGE_PROVIDER_WWW_PATH)

# TODO this copy&paste must be consolidated, see it as WIP
