...
export SEMAPHORE_NAME="prepare_ci_images"

echo
echo "${COLOR_BLUE}Prepare all CI images on CI${COLOR_RESET}"
echo
...
start_end::group_start "Building CI images in parallel"
...
do
    ...
    parallel --ungroup --bg --semaphore --semaphorename "${SEMAPHORE_NAME}" \
        --jobs "${MAX_PARALLEL_FROM_VARIABLE}" \
            "$(dirname "${BASH_SOURCE[0]}")/ci_prepare_ci_image_on_ci.sh" "${python_version}" >"${JOB_LOG}" 2>&1
done
