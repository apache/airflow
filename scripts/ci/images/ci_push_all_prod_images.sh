...
export SEMAPHORE_NAME="push_prod_images"

echo
echo "${COLOR_BLUE}Push all PROD images${COLOR_RESET}"
echo
...
start_end::group_start "Pushing PROD images in parallel"
...
do
    ...
    parallel --ungroup --bg --semaphore --semaphorename "${SEMAPHORE_NAME}" \
        --jobs "${MAX_PARALLEL_FROM_VARIABLE}" \
            "$(dirname "${BASH_SOURCE[0]}")/ci_push_prod_image.sh" "${python_version}" >"${JOB_LOG}" 2>&1
done
