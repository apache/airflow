...
function push_pull_remove_images::pull_image_github_dockerhub() {
    ...
    if push_pull_remove_images::pull_image_if_not_present_or_forced "${GITHUB_IMAGE}"; then
        ...
        verbose_docker tag "${GITHUB_IMAGE}" "${DOCKERHUB_IMAGE}"
    else
...
" > "${DETECTED_TERMINAL}"
    ...
    if [[ ${USE_GITHUB_REGISTRY} == "true" ]]; then
        ...
        verbose_docker pull "${AIRFLOW_PYTHON_BASE_IMAGE}"
    fi
}
...
function push_pull_remove_images::push_python_image_to_github() {
    ...
    verbose_docker tag "${AIRFLOW_PYTHON_BASE_IMAGE}" \
        "${GITHUB_REGISTRY_PYTHON_BASE_IMAGE}${PYTHON_TAG_SUFFIX}"
...
function push_pull_remove_images::push_ci_images_to_github() {
    ...
    verbose_docker tag "${AIRFLOW_CI_IMAGE}" "${AIRFLOW_CI_TAGGED_IMAGE}"
    ...
    if [[ -n ${GITHUB_SHA=} ]]; then
        ...
        verbose_docker tag "${AIRFLOW_CI_IMAGE}" "${AIRFLOW_CI_SHA_IMAGE}"
...
function push_pull_remove_images::push_prod_images_to_github () {
    ...
    verbose_docker tag "${AIRFLOW_PROD_IMAGE}" "${AIRFLOW_PROD_TAGGED_IMAGE}"
    ...
    if [[ -n ${COMMIT_SHA=} ]]; then
        ...
        verbose_docker tag "${AIRFLOW_PROD_IMAGE}" "${AIRFLOW_PROD_SHA_IMAGE}"
    ...
    verbose_docker tag "${AIRFLOW_PROD_BUILD_IMAGE}" "${AIRFLOW_PROD_BUILD_TAGGED_IMAGE}"
...
function push_pull_remove_images::check_for_image_in_github_container_registry() {
    ...
    verbose_docker manifest inspect "${image_to_wait_for}"
    local res=$?
    if [[ ${res} == "0" ]]; then
