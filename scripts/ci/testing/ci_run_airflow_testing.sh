    ...
    if (( MEMORY_AVAILABLE_FOR_DOCKER < MEMORY_REQUIRED_FOR_HEAVY_TEST_PARALLEL_RUN )) ; then
        ...
        if [[ ${BACKEND} == "mysql" ]]; then
            if [[ ${test_types_to_run} == *"Providers"* ]]; then
                echo "${COLOR_YELLOW}Remove Providers from tests_types_to_run and add them to sequential tests (mysql case).${COLOR_RESET}"
                test_types_to_run="${test_types_to_run//Providers/}"
                sequential_tests+=("Providers")
            fi
        fi
        if [[ ${BACKEND} == "mssql" ]]; then
            # For mssql - we skip the Provider tests altogether as they take too much memory even
            # if run sequentially. Those tests will run in `main` anyway.
            if [[ ${test_types_to_run} == *"Providers"* ]]; then
                echo "${COLOR_YELLOW}Remove Providers from tests_types_to_run and skip running them altogether (mssql case).${COLOR_RESET}"
                test_types_to_run="${test_types_to_run//Providers/}"
            fi
        fi
    fi
