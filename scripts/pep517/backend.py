...
def _preprocess_source_tree(config_settings):
    """Compile assets and tag build before building a dist."""
    if config_settings:
        tag_build = config_settings.get("airflow-tag-build")
    else:
        tag_build = None
    if not tag_build:
        return _run_setup("compile_assets")
    return _run_setup("compile_assets", "egg_info", "--tag-build", tag_build)


def build_sdist(sdist_directory, config_settings=None):
    _preprocess_source_tree(config_settings)
