...
class TestRenderCentralTable:
    ...
    def test_supported_mark_shows_sdk_version_when_present(self):
        doc = _doc()
        doc["capabilities"]["xcom-read-write"] = _entry(True, since="3.3", sdk_version="1.0.0-beta1")

        rendered = "".join(hook.render_central_table([hook.SdkCapabilities("Java", doc)]))

        assert (
            f"   * - ``xcom-read-write`` (MUST)\n"
            f"     - {matrix.SUPPORTED_MARK} 3.3 (Java SDK 1.0.0-beta1)\n" in rendered
        )
