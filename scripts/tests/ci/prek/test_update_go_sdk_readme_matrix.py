...
import pytest
import yaml
from ci.prek import lang_sdk_compat_matrix as matrix, update_go_sdk_readme_matrix as hook
...
class TestMain:
    ...
    def wired(self, tmp_path, monkeypatch):
        ...
        go_messages.write_text(f'package execution\n\nconst SupervisorSchemaVersion = "{SCHEMA_VERSION}"\n')
        monkeypatch.setattr(
            hook,
            "LANG_SDKS",
            [{"id": "go", "capabilities_yaml": capabilities_yaml, "readme": readme}],
        )
    ...
    def test_schema_version_disagreeing_with_go_source_fails_without_writing(self, wired):
        hook.GO_MESSAGES.write_text('package execution\n\nconst SupervisorSchemaVersion = "2020-01-01"\n')
        assert hook.main() == 1
