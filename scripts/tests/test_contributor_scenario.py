        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("os.path.exists", return_value=False):
                with patch("os.path.isdir", return_value=False):
                    ...
                    cmd1 = BreezeContext.get_command("stage-changes", path="airflow/models/dag.py")
                    assert cmd1 == "git add airflow/models/dag.py"

                    # Step 2: Run static checks
                    cmd2 = BreezeContext.get_command("run-static-checks", from_ref="main")
                    assert cmd2 == "prek run --from-ref main --stage pre-commit"

                    # Step 3: Run targeted unit tests
                    cmd3 = BreezeContext.get_command(
                        "run-unit-tests",
                        project="airflow-core",
                        test_path="airflow-core/tests/models/test_dag.py",
                    )
                    assert (
                        cmd3 == "uv run --project airflow-core pytest airflow-core/tests/models/test_dag.py"
                    )
    ...
    def test_host_only_guard_in_breeze(self):
        ...
        with patch.dict(os.environ, {"BREEZE": "true"}):
            ...
            with pytest.raises(ValueError, match="host-only"):
                BreezeContext.get_command("stage-changes", path="airflow/models/dag.py")
            ...
            assert cmd3 == "breeze run pytest airflow-core/tests/models/test_dag.py"
