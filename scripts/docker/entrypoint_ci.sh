...
function check_boto_upgrade() {
    ...
    ${PACKAGING_TOOL} uninstall ${EXTRA_UNINSTALL_FLAGS} aiobotocore s3fs || true
    # We need to include oss2 as dependency as otherwise jmespath will be bumped and it will not pass
    # the pip check test
    # shellcheck disable=SC2086
    ${PACKAGING_TOOL} install ${EXTRA_INSTALL_FLAGS} --upgrade boto3 botocore "oss2>=2.14.0"
    pip check
}
...
function check_pydantic() {
    if [[ ${PYDANTIC=} == "none" ]]; then
        ...
        echo
        # shellcheck disable=SC2086
        ${PACKAGING_TOOL} install ${EXTRA_INSTALL_FLAGS} -e .
        echo
        echo "${COLOR_YELLOW}Remove pydantic and 3rd party libraries that depend on it${COLOR_RESET}"
        echo
        # shellcheck disable=SC2086
        ${PACKAGING_TOOL} uninstall ${EXTRA_UNINSTALL_FLAGS} pydantic aws-sam-translator openai \
        ...
        echo
        # shellcheck disable=SC2086
        ${PACKAGING_TOOL} uninstall ${EXTRA_UNINSTALL_FLAGS} pyiceberg waeviate-client
        echo
        echo "${COLOR_YELLOW}Downgrading Pydantic to < 2${COLOR_RESET}"
        echo
        # shellcheck disable=SC2086
        ${PACKAGING_TOOL} install ${EXTRA_INSTALL_FLAGS} --upgrade "pydantic<2.0.0"
        pip check
    else
        echo
...
function check_download_sqlalchemy() {
    ...
    echo
    # shellcheck disable=SC2086
    ${PACKAGING_TOOL} install ${EXTRA_INSTALL_FLAGS} "sqlalchemy==${min_sqlalchemy_version}"
    pip check
}
...
function check_download_pendulum() {
    ...
    echo
    # shellcheck disable=SC2086
    ${PACKAGING_TOOL} install ${EXTRA_INSTALL_FLAGS} "pendulum==${min_pendulum_version}"
    pip check
}
