...
function generate_test_dependency_report() {
    if [[ ${RUN_TESTS=} != "true" || ${GITHUB_ACTIONS=} != "true" ]]; then
        return
    fi
    echo
    echo "${COLOR_BLUE}Generating dependency report for test job${COLOR_RESET}"
    echo
    if ! python "${IN_CONTAINER_DIR}/generate_test_dependency_report.py"; then
        echo
        echo "${COLOR_YELLOW}Dependency report generation failed, continuing without it${COLOR_RESET}"
        echo
    fi
}
...
generate_test_dependency_report
initialize_db
