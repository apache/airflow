...
function install_from_sources() {
    ...
    if [[ "${UPGRADE_RANDOM_INDICATOR_STRING=}" != "" ]]; then
        ...
        echo
        set -x
        uv sync --all-packages --resolution highest --group doc --group doc-gen
    else
        ...
        echo
        installation_command_flags=" --editable .[${AIRFLOW_EXTRAS}] --editable ./airflow-core --editable ./task-sdk \
              --editable ./devel-common[all] --editable ./airflow-core --editable ./airflow-ctl \
        ...
        if [[ ${fallback_no_constraints_installation} == "true" ]]; then
            ...
            echo
            set -x
            uv sync --all-packages --group-doc --group doc-gen
            set +x
        fi
    fi
}
