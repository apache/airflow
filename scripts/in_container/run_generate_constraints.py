...
class ConfigParams:
    ...
    def constraints_dir(self) -> Path:
        constraints_dir = Path("/files") / self.python
    ...
    def constraints_diff_file(self) -> Path:
        return self.constraints_dir / f"diff-{self.airflow_constraints_mode}-{self.python}.txt"
...
def freeze_packages_to_file(config_params: ConfigParams, file: TextIO) -> None:
    ...
    for line in sorted(result.stdout.split("\n")):
        if line.startswith(("apache_airflow", "apache-airflow==", "/opt/airflow")):
            continue
        if "@" in line:
            continue
        file.write(line)
...
def get_core_airflow_dependencies(config_params) -> list[str]:
    result = run_command(
        ["python", "-c", "import setup; print(','.join(setup.CORE_EXTRAS_DEPENDENCIES.keys()))"],
        github_actions=config_params.github_actions,
        cwd=AIRFLOW_SOURCES,
        text=True,
        check=True,
...
def get_all_provider_packages(config_params) -> list[str]:
    result = run_command(
        ["python", "-c", "import setup; print(setup.get_all_provider_packages())"],
        github_actions=config_params.github_actions,
        cwd=AIRFLOW_SOURCES,
        text=True,
        check=True,
...
def generate_constraints_pypi_providers(config_params: ConfigParams) -> None:
    ...
    """
    dist_dir = Path("/dist")
    core_dependencies = get_core_airflow_dependencies(config_params)
    all_provider_packages = get_all_provider_packages(config_params)
