...
script copies the whole ``system`` package into the e2e dags folder and strips that pytest-only
footer so the DAGs parse and run in a real deployment.
...
AIRFLOW_ROOT_PATH = HERE.parents[1]

# Example DAGs that need a minimum Airflow version. When running against an older version, the DAG
# file is removed entirely — a module-level import (e.g. the HITL operators) would otherwise raise at
# parse time and take the whole dag-processor down, not just skip that one DAG.
MIN_AIRFLOW_VERSION_FOR_DAG: dict[str, tuple[int, int]] = {
    # HITL operators import-raise on Airflow < 3.1 (taking the dag-processor down at parse time).
...
def _target_airflow_version() -> tuple[int, int] | None:
    """Parse the (major, minor) of the targeted Airflow version, or None for the default/prod run."""
...
def prepare_dags() -> Path:
    """Populate the e2e dags folder from the provider system tests. Returns the dags folder path."""
    ...
    shutil.copytree(SYSTEM_TESTS_SOURCE, DAGS_DEST / "system")

    openlineage_dir = DAGS_DEST / "system" / "openlineage"
    ...
    for extra_file in DAGS_EXTRA_SOURCE.glob("*.py"):
        shutil.copy2(extra_file, DAGS_DEST / extra_file.name)
    ...
    (DAGS_DEST / "dag_doc.md").write_text("# MD doc file")
...
if __name__ == "__main__":
    dest = prepare_dags()
    print(f"Prepared OpenLineage e2e dags in {dest}")
