...
from airflow.providers.microsoft.azure.operators.synapse import AzureSynapsePipelineRunLink, AzureSynapseRunPipelineOperator, AzureSynapseRunSparkBatchOperator
...
class TestAzureSynapseRunPipelineOperator:
    ...
    def setup_test_cases(self, create_mock_connection):
        ...
        create_mock_connection(
            Connection(
                conn_id=AZURE_SYNAPSE_CONN_ID,
                conn_type="azure_synapse",
                host=f"azure-synapse-workspace-url",
                login="client-id",
                password="client-secret",
    ...
    def test_execute_wait_for_termination(self, mock_run_pipeline, pipeline_run_status, expected_output):
        ...
        assert operator.azure_synapse_workspace_dev_endpoint == self.config["azure_synapse_workspace_dev_endpoint"]
        ...
        with patch.object(AzureSynapsePipelineHook, "get_pipeline_run") as mock_get_pipeline_run:
            ...
            # Check if mock_run_pipeline called with particular set of arguements.
    ...
    def test_execute_no_wait_for_termination(self, mock_run_pipeline):
        ...
        with patch.object(AzureSynapsePipelineHook, "get_pipeline_run", autospec=True) as mock_get_pipeline_run:
            operator.execute(context=self.mock_context)
    ...
    def test_run_pipeline_operator_link(self, create_task_instance_of_operator):
        ...
        # Extract Extracts the workspace_name, subscription_id and resource_group from the Synapse workspace url.
