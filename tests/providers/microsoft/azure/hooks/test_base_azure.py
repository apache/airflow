...
class TestBaseAzureHook(unittest.TestCase):
    ...
    @patch('airflow.providers.microsoft.azure.hooks.base_azure.AzureBaseHook.get_connection',
           return_value=Connection(
               conn_id='azure_default',
               extra='{ "key_path": "key_file.json" }'
           ))
    ...
    @patch('airflow.providers.microsoft.azure.hooks.base_azure.get_client_from_auth_file')
    @patch('airflow.providers.microsoft.azure.hooks.base_azure.getenv', return_value='key_file.json')
    @patch('airflow.providers.microsoft.azure.hooks.base_azure.AzureBaseHook.get_connection',
           return_value=Connection(
               conn_id='azure_default'
           ))
    def test_get_conn_with_key_path_env_var(
        self,
        mock_connection,  # pylint: disable=unused-argument
        mock_getenv,
        ...
        mock_getenv.assert_called_once_with('AZURE_AUTH_LOCATION')
        mock_get_client_from_auth_file.assert_called_once_with(
            client_class=mock_sdk_client,
            auth_path=mock_getenv.return_value
        )
        assert auth_sdk_client == mock_get_client_from_auth_file.return_value
