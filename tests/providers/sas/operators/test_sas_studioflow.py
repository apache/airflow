...
from airflow.providers.sas.operators.sas_studioflow import SASStudioFlowOperator, \
    _generate_flow_code, _run_job_and_wait, _dump_logs, _create_or_connect_to_session
...
class TestSasStudioFlowOperator:
    ...
    def test_execute_sas_studio_flow_operator_basic(self, session_mock, flow_mock, runjob_mock,
                                                    dumplogs_mock):
        flow_mock.return_value = {"code": "test code"}
        runjob_mock.return_value = {"id": "jobid1", "state": "completed",
        ...
        operator = SASStudioFlowOperator(task_id='demo_studio_flow_1.flw',
    ...
    def test_execute_sas_studio_flow_operator_gen_code(self):
        ...
        session.post.assert_called_with('/studioDevelopment/code',
                                        json={'reference': {'mediaType': 'application/vnd.sas.dataflow',
                                                            'type': 'content', 'path': '/path'},
                                              'initCode': True, 'wrapperCode': True})
    ...
    def test_execute_sas_studio_flow_operator_gen_code_compute(self, c_mock):
        ...
        session.post.assert_called_with('/studioDevelopment/code?sessionId=abc',
                                        json={'reference': {'mediaType': 'application/vnd.sas.dataflow',
                                                            'type': 'compute', 'path': '/path'},
                                              'initCode': True, 'wrapperCode': True})
        assert r == {'code': 'code val'}
