...
"""
Boto3 uses Botocore to dynamically generate low-level clients. We can also use botocore
to dynamically assert function calls based on the documentation. It's not 100% full-proof.
It's pretty much a deeply nested dictionary. On the highest level here's how this works:
...
def __assert_botocore_shape(model, input_shape_name, input_shape, input_value):
    ...
    if "type" in input_shape:
        if input_shape["type"] == "boolean":
            ...
            assert isinstance(
                input_value, (dt.datetime, dt.date)
