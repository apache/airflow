...
class TestBedrockInvokeModelOperator:
    @pytest.mark.parametrize(
        "prompt_values",
        [
            pytest.param({"prompt": PROMPT}, id="only_prompt_supplied"),
            pytest.param({"input_data": {"prompt": PROMPT}}, id="only_input_data_provided"),
            pytest.param(
                {"prompt": PROMPT, "input_data": {"contentType": "application/json"}},
                id="both_provided_no_conflicts",
            ),
        ],
    )
    @mock.patch.object(BedrockRuntimeHook, "conn")
    def test_invoke_model_prompt_good_combinations(self, mock_conn, prompt_values):
    ...
    @mock.patch.object(BedrockRuntimeHook, "conn")
    def test_invoke_model_with_both_prompt_and_input_data_conflict(self, mock_conn):
        mock_conn.invoke_model.return_value["body"].read.return_value = MOCK_RESPONSE
        ...
        with pytest.raises(ValueError):
            BedrockInvokeModelOperator(
                ...
                model_id=MODEL_ID,
                prompt=PROMPT,
                input_data={"prompt": "A second prompt!"},
            )
