...
class TestBatchSensor:
    ...
    def test_poke_on_failure_state(self, mock_get_job_description):
        ...
        error_message = r"Batch sensor failed\. AWS Batch job status: FAILED"
    ...
    def test_poke_on_invalid_state(self, mock_get_job_description):
        ...
        error_message = r"Batch sensor failed\. Unknown AWS Batch job status: INVALID"
        with pytest.raises(AirflowException, match=error_message):
            self.batch_sensor.poke({})
        mock_get_job_description.assert_called_once_with(JOB_ID)
    ...
    def test_poke_on_intermediate_state(self, job_status):
        with mock.patch.object(
            BatchClientHook, "get_job_description", return_value={"status": job_status}
        ) as mock_get_job_description:
            assert not self.batch_sensor.poke({})
            mock_get_job_description.assert_called_once_with(JOB_ID)
...
class TestBatchJobQueueSensor:
    ...
    def test_poke_invalid(self, mock_batch_client):
        ...
        with pytest.raises(AirflowException, match="AWS Batch job queue failed"):
            self.sensor.poke({})
        mock_batch_client.describe_job_queues.assert_called_once_with(
            jobQueues=[self.job_queue],
        )
