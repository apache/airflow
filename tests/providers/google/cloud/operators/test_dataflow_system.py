...
class CloudDataflowExampleDagsSystemTest(GoogleSystemTest):
    ...
    def test_run_example_dag_native_java(self):
        self.run_dag('example_gcp_dataflow_java', CLOUD_DAG_FOLDER)

    @provide_gcp_context(GCP_DATAFLOW_KEY)
    def test_run_example_dag_native_python(self):
        self.run_dag('example_gcp_dataflow_python', CLOUD_DAG_FOLDER)

    @provide_gcp_context(GCP_DATAFLOW_KEY)
    def test_run_example_dag_template(self):
