...
class TestDataplexJobTrigger:
    def test_async_dataplex_job_trigger_serialization_should_execute_successfully(self, trigger):
        classpath, kwargs = trigger.serialize()
        assert classpath == "airflow.providers.google.cloud.triggers.dataplex.DataplexJobTrigger"
        assert kwargs == {
            "job_id": TEST_JOB_ID,
            "data_scan_id": TEST_DATA_SCAN_ID,
            "project_id": TEST_PROJECT_ID,
    ...
    @pytest.mark.asyncio
    @mock.patch("airflow.providers.google.cloud.hooks.dataplex.DataplexAsyncHook.get_data_scan_job")
    async def test_async_dataplex_job_triggers_on_success_should_execute_successfully(
        self, mock_hook, trigger, async_get_data_scan_job
    ):
    ...
    async def test_async_dataplex_job_trigger_run_returns_error_event(
        ...
        expected_event = TriggerEvent(
            {"job_id": TEST_JOB_ID, "job_state": DataScanJob.State.FAILED, "job": actual_event.payload["job"]}
        )
        assert expected_event == actual_event
