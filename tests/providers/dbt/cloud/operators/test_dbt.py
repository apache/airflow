...
class TestDbtCloudRetryJobOperator:
    def setup_method(self):
        self.dag = DAG("test_dbt_cloud_retry_job_op", start_date=DEFAULT_DATE)
        self.mock_ti = MagicMock()
        self.mock_context = {"ti": self.mock_ti}
        self.config = {
            "job_id": JOB_ID,
    ...
    @patch("airflow.providers.dbt.cloud.operators.dbt.DbtCloudRetryJobOperator.defer")
    @patch(
        "airflow.providers.dbt.cloud.hooks.dbt.DbtCloudHook.get_job_runs",
        return_value=mock_response_json({"data": []}),
    )
    @patch("airflow.providers.dbt.cloud.hooks.dbt.DbtCloudHook.get_connection")
    @patch("airflow.providers.dbt.cloud.hooks.dbt.DbtCloudHook.retry_job_run")
    def test_execute_succeeded_before_getting_deferred(
        self, mock_retry_job_run, mock_dbt_hook, mock_get_job_runs, mock_defer, mock_job_run_status
    ):
        dbt_op = DbtCloudRetryJobOperator(
            dbt_cloud_conn_id=ACCOUNT_ID_CONN,
            task_id=TASK_ID,
            job_id=JOB_ID,
            check_interval=1,
    ...
    @patch("airflow.providers.dbt.cloud.hooks.dbt.DbtCloudHook.get_connection")
    @patch(
        "airflow.providers.dbt.cloud.hooks.dbt.DbtCloudHook.retry_job_run",
        return_value=mock_response_json(DEFAULT_ACCOUNT_JOB_RUN_RESPONSE),
    )
    def test_execute_failed_before_getting_deferred(
        self, mock_retry_job_run, mock_dbt_hook, mock_get_job_runs, mock_defer, mock_job_run_status
    ...
    def test_dbt_retry_job_op_async(
        self, mock_retry_job_run, mock_dbt_hook, mock_get_job_runs, mock_job_run_status, status
    ):
        """
        Asserts that a task is deferred and an DbtCloudRunJobTrigger will be fired
        when the DbtCloudRetryJobOperator has deferrable param set to True
        """
        mock_job_run_status.return_value = status
        dbt_op = DbtCloudRetryJobOperator(
            dbt_cloud_conn_id=ACCOUNT_ID_CONN,
    ...
    @patch.object(
        DbtCloudHook, "retry_job_run", return_value=mock_response_json(DEFAULT_ACCOUNT_JOB_RUN_RESPONSE)
    )
    @pytest.mark.parametrize(
        "job_run_status, expected_output",
        [
    ...
    def test_execute_wait_for_termination(
        self, mock_retry_job, mock_get_job_runs, conn_id, account_id, job_run_status, expected_output
    ):
        operator = DbtCloudRetryJobOperator(
            task_id=TASK_ID, dbt_cloud_conn_id=conn_id, account_id=account_id, dag=self.dag, **self.config
        )
        ...
        with patch.object(DbtCloudHook, "get_job_run") as mock_get_job_run:
            ...
            if expected_output == "success":
                ...
                assert mock_retry_job.return_value.data["id"] == RUN_ID
            elif expected_output == "exception":
                # The operator should fail if the job run fails or is cancelled.
                with pytest.raises(DbtCloudJobRunException) as err:
                    operator.execute(context=self.mock_context)
    ...
    @patch.object(DbtCloudHook, "retry_job_run")
    @pytest.mark.parametrize(
        "conn_id, account_id",
        [(ACCOUNT_ID_CONN, None), (NO_ACCOUNT_ID_CONN, ACCOUNT_ID)],
        ids=["default_account", "explicit_account"],
    )
    def test_execute_no_wait_for_termination(self, mock_retry_job, mock_get_job_runs, conn_id, account_id):
        operator = DbtCloudRetryJobOperator(
            task_id=TASK_ID,
            dbt_cloud_conn_id=conn_id,
            account_id=account_id,
    ...
    def test_execute_no_wait_for_termination_and_reuse_existing_run(
        self, mock_retry_job, mock_get_jobs_run, conn_id, account_id
    ):
        mock_get_jobs_run.return_value.json.return_value = {
            "data": [
                {
    ...
    def test_retry_job_operator_link(self, conn_id, account_id, create_task_instance_of_operator, request):
        ti = create_task_instance_of_operator(
            DbtCloudRetryJobOperator,
            dag_id="test_dbt_cloud_run_job_op_link",
            execution_date=DEFAULT_DATE,
