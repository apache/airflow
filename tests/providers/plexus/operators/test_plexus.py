...
class TestPlexusOperator:
    ...
    @pytest.mark.parametrize("invalid_job_params", 
        ['name', 'app', 'queue', 'num_nodes', 'num_cores'],
        indirect=True)
    ...
    def test_valid_api_lookups(self, mock_hook, mock_requests, valid_job_params):
        ...
        queue_mock = Mock(**{'json.return_value': 
                                {'results': [ {'id': 2, 'public_name': 'queue2'},
                                    {'id': 3, 'public_name': 'queue3'},
                                    {'id': 1, 'public_name': 'queue'}]
                                }
                            })
        billing_mock = Mock(**{'json.return_value':{'results': [{'id': 1}]}})
        app_mock =  Mock(**{'json.return_value': 
                                {'results': [{'name': 'pipeline2', 'id': 2, 'is_service': True},
    ...
    def test_create_job(self, mock_hook, mock_construct_params, mock_request, mock_time, valid_job_params):
        """test submit/get job"""
        mock_construct_params.return_value = {'name': 'test',
                                        'app': 1,
                                        'queue': 2,
                                        'num_nodes': 1,
                                        'num_cores': 1,
        ...
        mock_request.post.return_value = Mock(**{'ok': True, 'json.return_value':{'id': 1, 'last_state': 'Pending'}})
