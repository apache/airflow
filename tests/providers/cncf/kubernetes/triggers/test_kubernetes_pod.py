...
TRIGGER_PATH = "airflow.providers.cncf.kubernetes.triggers.kubernetes_pod.KubernetesCreatePodTrigger"
...
class TestKubernetesCreatePodTrigger:
    @staticmethod
    def _mock_get_pod_result(result_to_mock):
        f = Future()
    ...
    async def test_run_loop_return_success_event(self, mock_hook, mock_method, trigger):
        mock_hook.return_value.get_pod.return_value = self._mock_get_pod_result(mock.MagicMock())
    ...
    async def test_run_loop_return_failed_event(self, mock_hook, mock_method, trigger):
        mock_hook.return_value.get_pod.return_value = self._mock_get_pod_result(
            mock.MagicMock(
    ...
    def test_define_container_state_should_throw_exception(self, trigger):
        pod = k8s.V1Pod(
            ...
            status=k8s.V1PodStatus(
                container_statuses=[
                    k8s.V1ContainerStatus(
                        ...
                        ready=True,
                        restart_count=1,
                        state=k8s.V1ContainerState(running=None, terminated=None, waiting=None),
                    )
                ]
            ),
        )

        with pytest.raises(ContainerStateUndefined):
            trigger.define_container_state(pod)
