...
class TestImportPoolsJob:
    def test_should_run_by_default(self):
        docs = render_chart(show_only=["templates/jobs/import-pools-job.yaml"])
        assert "Job" == docs[0]["kind"]
        assert "import-pools" == jmespath.search("spec.template.spec.containers[0].name", docs[0])
        assert 50000 == jmespath.search("spec.template.spec.securityContext.runAsUser", docs[0])

    @pytest.mark.parametrize(
        ("import_pools_job_enabled", "created"),
        [
            (False, False),
            (True, True),
        ],
    )
    def test_enable_import_pools_job(self, import_pools_job_enabled, created):
        docs = render_chart(
            values={"importPoolsJob": {"enabled": import_pools_job_enabled}},
            show_only=["templates/jobs/import-pools-job.yaml"],
        )
    ...
    def test_should_support_annotations(self):
        docs = render_chart(
            values={
                "importPoolsJob": {"annotations": {"foo": "bar"}, "jobAnnotations": {"fiz": "fuz"}},
            },
            show_only=["templates/jobs/import-pools-job.yaml"],
        )
    ...
    def test_should_disable_default_helm_hooks(self):
        docs = render_chart(
            values={"importPoolsJob": {"useHelmHooks": False}},
            show_only=["templates/jobs/import-pools-job.yaml"],
        )
    ...
    def test_should_set_correct_helm_hooks_weight(self):
        docs = render_chart(
            show_only=[
                "templates/jobs/import-pools-job.yaml",
            ],
        )
        annotations = jmespath.search("metadata.annotations", docs[0])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("1.10.14", 'airflow pool --import \\\n"/opt/airflow/pools.json"'),
            ("2.5.1", 'airflow pools import \\\n"/opt/airflow/pools.json"'),
        ],
    )
    ...
    def test_command_and_args_overrides(self, command, args):
        docs = render_chart(
            values={"importPoolsJob": {"command": command, "args": args}},
            show_only=["templates/jobs/import-pools-job.yaml"],
        )
    ...
    def test_command_and_args_overrides_are_templated(self):
        docs = render_chart(
            values={
                "importPoolsJob": {"command": ["{{ .Release.Name }}"], "args": ["{{ .Release.Service }}"]},
            },
            show_only=["templates/jobs/import-pools-job.yaml"],
        )
