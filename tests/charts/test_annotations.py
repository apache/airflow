...
class TestPerComponentControllerAnnotations:
    @pytest.mark.parametrize(
        "values,show_only,expected_annotations",
        [
            (
    ...
    def test_annotations_are_added(self, values, show_only, expected_annotations):
        ...
        # This test relies on the convention that the helm chart puts a single
        # controller (Deployment/StatefulSet/Job/Cronjob) in its own .yaml file
        # so by specifying `show_only`, we should only get a single k8s_object
        # here - the target object that we hope to test on.
        assert len(k8s_objects) == 1
        obj = k8s_objects[0]
