...
class PodTemplateFileTest(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ...
            ({"enabled": True}, {"persistentVolumeClaim": {"claimName": "RELEASE-NAME-airflow-logs"}}),
            (
    ...
    def test_mount_airflow_cfg(self):
        ...
        assert re.search("Pod", docs[0]["kind"])
        assert {'configMap': {'name': 'RELEASE-NAME-airflow-airflow-config'}, 'name': 'config'} in jmespath.search(
            "spec.volumes", docs[0]
        )
        assert {
            'name': 'config',
