...
from airflow.cli.commands import webserver
from airflow.cli.commands.webserver import GunicornMonitor
...
class TestGunicornMonitor:
    ...
    def setup_class(cls):
        ...
        cls.monitor_patched_methods = [
            mock.patch.object(cls.monitor, '_generate_plugin_state', return_value={}),
            mock.patch.object(cls.monitor, '_get_num_ready_workers_running', return_value=4),
            mock.patch.object(cls.monitor, '_get_num_workers_running', return_value=4),
            mock.patch.object(cls.monitor, '_spawn_new_workers', return_value=None),
            mock.patch.object(cls.monitor, '_kill_old_workers', return_value=None),
            mock.patch.object(cls.monitor, '_reload_gunicorn', return_value=None),
        ]

    def setup_method(self):
        for mock_ in self.monitor_patched_methods:
            mock_.start()
    ...
    @mock.patch('airflow.cli.commands.webserver.sleep')
...
class TestGunicornMonitorGeneratePluginState:
    ...
    def test_should_detect_changes_in_directory(self):
        with tempfile.TemporaryDirectory() as tempdir, mock.patch(
            "airflow.cli.commands.webserver.settings.PLUGINS_FOLDER", tempdir
        ):
...
class TestCLIGetNumReadyWorkersRunning:
    ...
    def setup_class(cls):
        ...
        cls.children = mock.MagicMock()
        cls.child = mock.MagicMock()
        cls.process = mock.MagicMock()
        cls.monitor = GunicornMonitor(
            gunicorn_master_pid=1,
            num_workers_expected=4,
            master_timeout=60,
...
class TestCliWebServer:
    ...
    @mock.patch("airflow.cli.commands.webserver.GunicornMonitor._get_num_workers_running", return_value=0)
    def test_cli_webserver_shutdown_when_gunicorn_master_is_killed(self, _):
        # Shorten timeout so that this test doesn't take too long time
        with conf_vars({('webserver', 'web_server_master_timeout'): '10'}):
            response = self.runner.invoke(webserver.webserver)

            assert response.exit_code == 1

    def test_cli_webserver_debug(self, app):
        with mock.patch.object(www_app, 'create_app') as create_app, mock.patch.object(app, 'run') as app_run:
            create_app.return_value = app

            self.runner.invoke(webserver.webserver, ['--debug'])
    ...
    def test_cli_webserver_args(self):
        with mock.patch("subprocess.Popen") as Popen, mock.patch.object(webserver, 'GunicornMonitor'):
            response = self.runner.invoke(
                webserver.webserver, ['--access-logformat', 'custom_log_format', '--pid', '/tmp/x.pid']
            )
