...
class TestCliDags:
    ...
    def test_reserialize(self):
        ...
        dag_command.dag_reserialize(
            self.parser.parse_args(["dags", "reserialize", "--clear-only"])
        )
    ...
    def test_reserialize_should_support_subdir_argument(self):
        ...
        with mock.patch(
            ...
            dag_command.dag_reserialize(
                self.parser.parse_args(["dags", "reserialize", "--subdir", dag_path])
            )
    ...
    def test_show_dag_dependencies_print(self):
        with contextlib.redirect_stdout(StringIO()) as temp_stdout:
            dag_command.dag_dependencies_show(
                self.parser.parse_args(["dags", "show-dependencies"])
            )
        ...
        with contextlib.redirect_stdout(StringIO()) as temp_stdout:
            dag_command.dag_dependencies_show(
                self.parser.parse_args(
                    ["dags", "show-dependencies", "--save", "output.png"]
                )
            )
        out = temp_stdout.getvalue()
    ...
    def test_show_dag_print(self):
        with contextlib.redirect_stdout(StringIO()) as temp_stdout:
            dag_command.dag_show(
                self.parser.parse_args(["dags", "show", "example_bash_operator"])
            )
        out = temp_stdout.getvalue()
    ...
    def test_next_execution(self, tmp_path):
        ...
        expected_output = {
            "future_schedule_daily": (
                ...
                future_run.isoformat()
                + os.linesep
                + (future_run + timedelta(days=1)).isoformat(),
            ),
            "future_schedule_every_4_hours": (
                future_run.isoformat(),
                future_run.isoformat()
                + os.linesep
                + (future_run + timedelta(hours=4)).isoformat(),
            ),
            ...
            "past_schedule_daily": (
                ...
                past_run.isoformat()
                + os.linesep
                + (past_run + timedelta(days=1)).isoformat(),
            ),
            "past_schedule_daily_catchup_false": (
        ...
        for dag_id in expected_output:
            ...
            args = self.parser.parse_args(
                ["dags", "next-execution", dag_id, "-S", str(tmp_path)]
            )
            with contextlib.redirect_stdout(StringIO()) as temp_stdout:
    ...
    def test_cli_get_dag_details(self):
        args = self.parser.parse_args(
            ["dags", "details", "example_complex", "--output", "yaml"]
        )
        with contextlib.redirect_stdout(StringIO()) as temp_stdout:
    ...
    def test_cli_list_dags(self):
        ...
        assert any(
            "airflow/example_dags/example_complex.py" in d["fileloc"] for d in dag_list
        )
    ...
    def test_cli_list_dags_invalid_cols(self):
        args = self.parser.parse_args(
            ["dags", "list", "--output", "json", "--columns", "dag_id,invalid_col"]
        )
    ...
    def test_cli_list_dags_prints_import_errors(self):
        ...
        args = self.parser.parse_args(
            ["dags", "list", "--output", "yaml", "--subdir", dag_path]
        )
        with contextlib.redirect_stderr(StringIO()) as temp_stderr:
            dag_command.dag_list_dags(args)
            out = temp_stderr.getvalue()
    ...
    def test_pause_regex(self, mock_yesno):
        args = self.parser.parse_args(
            ["dags", "pause", "^example_.*$", "--treat-dag-id-as-regex"]
        )
        dag_command.dag_pause(args)
        ...
        args = self.parser.parse_args(
            ["dags", "unpause", "^example_.*$", "--treat-dag-id-as-regex"]
        )
        dag_command.dag_unpause(args)
    ...
    def test_pause_regex_operation_cancelled(self, ask_yesno, capsys):
        args = self.parser.parse_args(
            ["dags", "pause", "example_bash_operator", "--treat-dag-id-as-regex"]
        )
    ...
    def test_pause_regex_yes(self, mock_yesno):
        args = self.parser.parse_args(
            ["dags", "pause", ".*", "--treat-dag-id-as-regex", "--yes"]
        )
        dag_command.dag_pause(args)
    ...
    def test_unpause_already_unpaused_dag_do_not_error(self):
        args = self.parser.parse_args(
            ["dags", "unpause", "example_bash_operator", "--yes"]
        )
        with contextlib.redirect_stdout(StringIO()) as temp_stdout:
            dag_command.dag_unpause(args)
            out = temp_stdout.getvalue().strip().splitlines()[-1]
    ...
    def test_trigger_dag(self):
        ...
        with create_session() as session:
            dagrun = (
                session.query(DagRun).filter(DagRun.run_id == "test_trigger_dag").one()
            )
        ...
        # Coerced to UTC.
        assert (
            dagrun.execution_date.isoformat(timespec="seconds")
        ...
        assert (
            dagrun.data_interval_start.isoformat(timespec="seconds")
            == "2021-06-03T00:00:00+00:00"
        )
        assert (
            dagrun.data_interval_end.isoformat(timespec="seconds")
    ...
    def test_trigger_dag_with_microseconds(self):
        ...
        with create_session() as session:
            dagrun = (
                ...
                .filter(DagRun.run_id == "test_trigger_dag_with_micro")
                .one()
            )
        ...
        assert dagrun.external_trigger
        assert (
            dagrun.execution_date.isoformat(timespec="microseconds")
    ...
    def test_dag_test_fail_raise_error(self, mock_get_dag):
        ...
        cli_args = self.parser.parse_args(
            ["dags", "test", "example_bash_operator", execution_date_str]
        )
    ...
    @mock.patch(
        "airflow.cli.commands.dag_command.render_dag",
    ...
    def test_dag_test_with_custom_timetable(self, mock__get_or_create_dagrun):
        ...
        with mock.patch.object(
            AfterWorkdayTimetable, "get_next_workday", side_effect=[DEFAULT_DATE]
        ):
            dag_command.dag_test(cli_args)
    ...
    def test_dag_test_no_triggerer_running(self, dag_maker):
        with mock.patch(
            "airflow.models.dag._run_inline_trigger", wraps=_run_inline_trigger
        ) as mock_run:
