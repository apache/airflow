...
from airflow.cli.commands.local_commands import webserver_command
...
class TestGunicornMonitor:
    ...
    @mock.patch("airflow.cli.commands.local_commands.webserver_command.sleep")
...
class TestGunicornMonitorGeneratePluginState:
    def test_should_detect_changes_in_directory(self, tmp_path):
        with mock.patch(
            "airflow.cli.commands.local_commands.webserver_command.settings.PLUGINS_FOLDER",
            os.fspath(tmp_path),
        ):
...
class TestCLIGetNumReadyWorkersRunning:
    ...
    def test_ready_prefix_on_cmdline_task_instances_without_heartbeats(self):
...
class TestCliWebServer(_CommonCLIGunicornTestClass):
    ...
    @mock.patch(
        "airflow.cli.commands.local_commands.webserver_command.GunicornMonitor._get_num_workers_running",
        return_value=0,
    )
