...
class TestListFailedRuns(unittest.TestCase):
    ...
    def test_list_failed_runs_simplified(self, mock_dag_run_find, mock_utcnow):
        ...
        mock_dag_run1 = Mock(dag_id="dag1", logical_date=fixed_now - timedelta(hours=1), state="failed")
        mock_dag_run3 = Mock(dag_id="dag2", logical_date=fixed_now - timedelta(hours=2), state="failed")
        mock_dag_run5 = Mock(dag_id="dag4", logical_date=fixed_now - timedelta(minutes=10), state="failed")
