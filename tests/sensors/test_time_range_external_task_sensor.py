...
from airflow.sensors.time_range_external_task_sensor import TimeRangeExternalTaskSensor
...
TEST_TASK_ID = 'time_range_sensor_check'
DEV_NULL = '/dev/null'


class TimeRangeExternalTaskSensorTests(unittest.TestCase):
    ...
    def test_time_range_external_task_sensor(self):
        self.test_time_sensor()
        t = TimeRangeExternalTaskSensor(
            task_id='test_time_range_external_task_sensor_check',
    ...
    def test_templated_sensor(self):
        ...
        with dag:
            sensor = TimeRangeExternalTaskSensor(
    ...
    def test_time_range_external_task_sensor_delta_execution_dates(self):
        bash_command_code = """
...
"""
        ...
        task_without_failure = TimeRangeExternalTaskSensor(
            task_id='task_without_failure',
            external_dag_id=dag_external_id,
            external_task_id='task_external_without_failure',
            execution_date_tolerance_before=lambda dt: [dt + timedelta(seconds=i)
                                          for i in range(2)],
            ...
            poke_interval=1,
            dag=dag)
        task_with_failure = TimeRangeExternalTaskSensor(
    ...
    def test_time_range_external_external_task_sensor_delta(self):
        self.test_time_sensor()
        t = TimeRangeExternalTaskSensor(
            task_id='test_time_range_external_task_sensor_check_delta',
            external_dag_id=TEST_DAG_ID,
            external_task_id=TEST_TASK_ID,
            execution_date_tolerance_before=timedelta(0),
            execution_date_tolerance_after=timedelta(0)
            allowed_states=['success'],
            dag=self.dag
        )
        t.run(
    ...
    def test_time_range_external_task_sensor(self):
        ...
        t = TimeRangeExternalTaskSensor(
            ...
            external_dag_id=TEST_DAG_ID,
            external_task_id=TEST_TASK_ID,
            execution_date_tolerance_before=lambda dt: dt + timedelta(0),
            allowed_states=['success'],
            dag=self.dag
        )
        t.run(
        ...
        t2 = TimeRangeExternalTaskSensor(
            task_id='test_time_range_external_task_sensor_check_delta',
            external_dag_id=TEST_DAG_ID,
            external_task_id=TEST_TASK_ID,
            execution_date_tolerance_before=lambda dt: dt + timedelta(days=1),
            allowed_states=['success'],
            timeout=1,
            poke_interval=1,
            dag=self.dag
    ...
    def test_time_range_external_task_sensor_error_delta_and_fn(self):
        self.test_time_sensor()
        ...
        with self.assertRaises(ValueError):
            TimeRangeExternalTaskSensor(
                ...
                execution_date_tolerance_before=lambda dt: dt,
                allowed_states=['success'],
                dag=self.dag
            )
