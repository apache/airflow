...
from airflow.settings import _ENABLE_AIP_44, TracebackSession, is_scarf_analytics_enabled
...
def test_scarf_analytics_disabled(env_var, conf_setting, is_enabled):
    conf_patch = conf_vars({("scarf_analytics", "enabled"): conf_setting})

    if env_var is not None:
        with conf_patch, patch.dict(os.environ, {"SCARF_ANALYTICS": env_var}):
            assert is_scarf_analytics_enabled() == is_enabled
    else:
        with conf_patch:
            assert is_scarf_analytics_enabled() == is_enabled
