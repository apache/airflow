    ...
    def run_dag(self, dag_id: str, dag_folder: str = DEFAULT_DAG_FOLDER) -> None:
        ...
        if dag is None:
            raise AirflowException(
                f"The Dag {dag_id} could not be found. It's either an import problem,wrong dag_id or DAG is not in provided dag_folder.The content of the {dag_folder} folder is {os.listdir(dag_folder)}"
            )
