...
def test_upstream_in_mapped_group_triggers_only_relevant(dag_maker, session):
    ...
    # After running the first t1, the remaining t1 must be run with t2.
    tis["tg.t1", 0].run()
    tis = _one_scheduling_decision_iteration()
...
def test_mapped_tasks_in_mapped_task_group_waits_for_upstreams_to_complete(dag_maker, session):
    ...
    ti2 = dr.get_task_instance(task_id="tg1.t2", map_index=1)
    ti2.state = TaskInstanceState.FAILED
    session.merge(ti2)
    session.commit()
    dr.task_instance_scheduling_decisions()
    ti3_1 = dr.get_task_instance(task_id="tg1.t3", map_index=1)
    assert ti3_1 is not None
