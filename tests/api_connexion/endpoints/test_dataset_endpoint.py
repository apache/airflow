...
class TestGetDatasetEvents(TestDatasetEndpoint):
    def test_should_respond_200(self, session):
        ...
        response = self.client.get("/api/v1/datasets/1/events", environ_overrides={'REMOTE_USER': "test"})
    ...
    def test_should_raises_401_unauthenticated(self, session):
        self._create_dataset(session)
        response = self.client.get("/api/v1/datasets/1/events")
        assert_401(response)
...
class TestGetDatasetEvenetsEndpointPagination(TestDatasetEndpoint):
    @parameterized.expand(
        [
            # Limit test data
            ("/api/v1/datasets/1/events?limit=1&order_by=run_id", ["run1"]),
            (
                "/api/v1/datasets/1/events?limit=3&order_by=run_id",
                [f"run{i}" for i in range(1, 4)],
            ),
            # Offset test data
            (
                "/api/v1/datasets/1/events?offset=1&order_by=run_id",
                [f"run{i}" for i in range(2, 10)],
            ),
            (
                "/api/v1/datasets/1/events?offset=3&order_by=run_id",
                [f"run{i}" for i in range(4, 10)],
            ),
            # Limit and offset test data
            (
                "/api/v1/datasets/1/events?offset=3&limit=3&order_by=run_id",
    ...
    def test_limit_and_offset(self, url, expected_event_runids, session):
        ...
        assert response.status_code == 200
        event_runids = [event["run_id"] for event in response.json["dataset_events"]]
    ...
    def test_should_return_conf_max_if_req_max_above_conf(self, session):
        ...
        response = self.client.get(
            "/api/v1/datasets/1/events?limit=180", environ_overrides={'REMOTE_USER': "test"}
        )
