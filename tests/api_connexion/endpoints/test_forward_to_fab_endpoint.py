...
class TestFABRoleForwarding(TestFABforwarding):
    ...
    def test_post_user_forwards_to_fab(self):
        payload = {
            "username": "TEST_USER1",
            "email": "testemail@test.com",
            "roles": [{"name": "Test"}],
        }
        resp = self.client.post("api/v1/users", json=payload, environ_overrides={"REMOTE_USER": "test"})
        assert resp.status_code == 200

    def test_patch_user_forwards_to_fab(self):
        users = self._create_users(1)
        session = self.app.appbuilder.get_session
        session.add_all(users)
        session.commit()
        ...
        resp = self.client.patch(
            "api/v1/users/TEST_USER1", json=payload, environ_overrides={"REMOTE_USER": "test"}
        )
        assert resp.status_code == 200
