...
class DagBagTest(unittest.TestCase):
    ...
    def test_zip_skip_log(self):
        ...
        with patch('airflow.utils.dag_processing.log') as log_mock:
            log_mock.info = Mock()
    ...
    def validate_dags(self, expected_parent_dag, actual_found_dags, actual_dagbag,
        ...
        expected_id = expected_parent_dag.dag_id
        expected_subdag_ids = [subdag.dag_id for subdag in expected_parent_dag.subdags]
        ...
        if should_be_found:
            ...
            assert actual_dag.dag_id == expected_id
            actual_subdag_ids = [subdag.dag_id for subdag in actual_dag.subdags]
            assert set(actual_subdag_ids) == set(expected_subdag_ids)
        else:
            assert len(actual_found_dags) == 0
