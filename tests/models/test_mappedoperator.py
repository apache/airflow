...
from tests.test_utils.mock_operators import MockOperator, NestedFields
...
def test_mapped_render_nested_template_fields(dag_maker, session):
    with dag_maker(session=session):
        MockOperator.partial(
            task_id="t", arg3=NestedFields(field_1="{{ ti.task_id }}", field_2="value_2")
    ...
    assert ti.task.arg3.field_1 == "t"
    assert ti.task.arg3.field_2 == "value_2"
