...
@pytest.mark.xfail(reason="This test might fail due to serialization issues")
...
def test_short_circuit_decorator(dag_maker, session, condition, expected):
    with dag_maker(serialized=True, session=session):
        ...
        short_circuit = short_circuit.override(task_id="short_circuit")()
...
@pytest.mark.xfail(
    reason="This test might fail due to serialization issues + "
    "when setting 'ignore_downstream_trigger_rules' to True, task_5 is still marked "
    "as success instead of skipped"
)
@pytest.mark.parametrize("condition, expected", [(True, State.SKIPPED), (False, State.SUCCESS)])
def test_short_circuit_decorator__ignore_downstream_trigger_rules(dag_maker, session, condition, expected):
    with dag_maker(
        ...
        short_circuit_respect_trigger_rules = short_circuit.override(
            task_id="short_circuit_respect_trigger_rules", ignore_downstream_trigger_rules=condition
        )()
