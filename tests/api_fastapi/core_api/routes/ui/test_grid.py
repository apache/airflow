...
class TestGetGridDataEndpoint:
    ...
    def test_should_response_200_order_by(self, test_client, order_by, expected):
        response = test_client.get(f"/ui/grid/{DAG_ID}?order_by={order_by}")
        assert response.status_code == 200
    ...
    def test_should_response_200_include_upstream_downstream(
        ...
        response = test_client.get(
            f"/ui/grid/{DAG_ID}?root={SUB_TASK_ID}&include_upstream={include_upstream}&include_downstream={include_downstream}"
        )
        assert response.status_code == 200
        assert response.json() == expected
    ...
    def test_should_response_200_limit(self, test_client, limit, expected):
        response = test_client.get(f"/ui/grid/{DAG_ID}?limit={limit}")
        assert response.status_code == 200
        assert response.json() == expected
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("2024-10-30T00:00:00", "2024-10-30T00:00:00", {"dag_runs": []}),
        ],
    )
    def test_should_response_200_logical_date(
        self, test_client, logical_date_gte, logical_date_lte, expected
    ):
        response = test_client.get(
            f"/ui/grid/{DAG_ID}?logical_date_gte={logical_date_gte}&logical_date_lte={logical_date_lte}"
        )
        assert response.status_code == 200
        assert response.json() == expected
