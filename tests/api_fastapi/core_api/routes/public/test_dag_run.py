...
class TestGetDagRun:
    ...
    def test_get_dag_run(self, test_client, dag_id, run_id, state, run_type, triggered_by, dag_run_note):
        response = test_client.get(f"/v2/dags/{dag_id}/dagRuns/{run_id}")
    ...
    def test_get_dag_run_not_found(self, test_client):
        response = test_client.get(f"/v2/dags/{DAG1_ID}/dagRuns/invalid")
        assert response.status_code == 404
        body = response.json()
        assert body["detail"] == "The DagRun with dag_id: `test_dag1` and run_id: `invalid` was not found"

    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get(f"/v2/dags/{DAG1_ID}/dagRuns/invalid")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get(f"/v2/dags/{DAG1_ID}/dagRuns/invalid")
...
class TestGetDagRuns:
    ...
    def test_get_dag_runs(self, test_client, session, dag_id, total_entries):
        response = test_client.get(f"/v2/dags/{dag_id}/dagRuns")
        assert response.status_code == 200
        body = response.json()
    ...
    def test_get_dag_runs_not_found(self, test_client):
        response = test_client.get("/v2/dags/invalid/dagRuns")
        assert response.status_code == 404
        body = response.json()
        assert body["detail"] == "The DAG with dag_id: `invalid` was not found"

    def test_invalid_order_by_raises_400(self, test_client):
        response = test_client.get("/v2/dags/test_dag1/dagRuns?order_by=invalid")
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get("/v2/dags/test_dag1/dagRuns?order_by=invalid")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get("/v2/dags/test_dag1/dagRuns?order_by=invalid")
        assert response.status_code == 403
    ...
    def test_return_correct_results_with_order_by(self, test_client, order_by, expected_order):
        ...
        response = test_client.get("/v2/dags/test_dag1/dagRuns", params={"order_by": order_by})
        assert response.status_code == 200
        body = response.json()
        ...
        response = test_client.get("/v2/dags/test_dag1/dagRuns", params={"order_by": f"-{order_by}"})
        assert response.status_code == 200
        body = response.json()
        assert body["total_entries"] == 2
    ...
    def test_limit_and_offset(self, test_client, query_params, expected_dag_id_order):
        response = test_client.get("/v2/dags/test_dag1/dagRuns", params=query_params)
        assert response.status_code == 200
        body = response.json()
        assert body["total_entries"] == 2
    ...
    def test_filters(self, test_client, dag_id, query_params, expected_dag_id_list):
        response = test_client.get(f"/v2/dags/{dag_id}/dagRuns", params=query_params)
        assert response.status_code == 200
        body = response.json()
    ...
    def test_bad_filters(self, test_client):
        ...
        response = test_client.get(f"/v2/dags/{DAG1_ID}/dagRuns", params=query_params)
        assert response.status_code == 422
        body = response.json()
        assert body["detail"] == expected_detail

    def test_invalid_state(self, test_client):
        response = test_client.get(f"/v2/dags/{DAG1_ID}/dagRuns", params={"state": ["invalid"]})
        assert response.status_code == 422
        assert (
...
class TestListDagRunsBatch:
    ...
    def test_list_dag_runs_return_200(self, test_client, session):
        response = test_client.post("/v2/dags/~/dagRuns/list", json={})
        assert response.status_code == 200
        body = response.json()
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.post("/v2/dags/~/dagRuns/list", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.post("/v2/dags/~/dagRuns/list", json={})
        assert response.status_code == 403

    def test_list_dag_runs_with_invalid_dag_id(self, test_client):
        response = test_client.post("/v2/dags/invalid/dagRuns/list", json={})
        assert response.status_code == 422
        body = response.json()
        assert body["detail"] == [
            {
    ...
    def test_list_dag_runs_with_dag_ids_filter(self, test_client, dag_ids, status_code, expected_dag_id_list):
        response = test_client.post("/v2/dags/~/dagRuns/list", json={"dag_ids": dag_ids})
        assert response.status_code == status_code
        assert set([each["dag_run_id"] for each in response.json()["dag_runs"]]) == set(expected_dag_id_list)

    def test_invalid_order_by_raises_400(self, test_client):
        response = test_client.post("/v2/dags/~/dagRuns/list", json={"order_by": "invalid"})
        assert response.status_code == 400
        body = response.json()
        assert (
            body["detail"]
    ...
    def test_dag_runs_ordering(self, test_client, order_by, expected_order):
        # Test ascending order
        response = test_client.post("/v2/dags/~/dagRuns/list", json={"order_by": order_by})
        assert response.status_code == 200
        body = response.json()
        assert body["total_entries"] == 4
        ...
        # Test descending order
        response = test_client.post("/v2/dags/~/dagRuns/list", json={"order_by": f"-{order_by}"})
        assert response.status_code == 200
        body = response.json()
        assert body["total_entries"] == 4
    ...
    def test_limit_and_offset(self, test_client, post_body, expected_dag_id_order):
        response = test_client.post("/v2/dags/~/dagRuns/list", json=post_body)
        assert response.status_code == 200
        body = response.json()
        assert body["total_entries"] == 4
        assert [each["dag_run_id"] for each in body["dag_runs"]] == expected_dag_id_order
...
class TestPatchDagRun:
    ...
    def test_patch_dag_run(self, test_client, dag_id, run_id, patch_body, response_body, session):
        response = test_client.patch(f"/v2/dags/{dag_id}/dagRuns/{run_id}", json=patch_body)
        assert response.status_code == 200
        body = response.json()
        assert body["dag_id"] == dag_id
        assert body["dag_run_id"] == run_id
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.patch("/v2/dags/dag_1/dagRuns/run_1", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.patch("/v2/dags/dag_1/dagRuns/run_1", json={})
        assert response.status_code == 403
    ...
    def test_patch_dag_run_with_update_mask(
        ...
        response = test_client.patch(
            f"/v2/dags/{DAG1_ID}/dagRuns/{DAG1_RUN1_ID}",
            params=query_params,
            json=patch_body,
        )
    ...
    def test_patch_dag_run_not_found(self, test_client):
        response = test_client.patch(
            f"/v2/dags/{DAG1_ID}/dagRuns/invalid",
    ...
    def test_patch_dag_run_bad_request(self, test_client):
        response = test_client.patch(f"/v2/dags/{DAG1_ID}/dagRuns/{DAG1_RUN1_ID}", json={"state": "running"})
        assert response.status_code == 422
        body = response.json()
    ...
    def test_patch_dag_run_notifies_listeners(self, test_client, state, listener_state):
        ...
        response = test_client.patch(f"/v2/dags/{DAG1_ID}/dagRuns/{DAG1_RUN1_ID}", json={"state": state})
        assert response.status_code == 200
...
class TestDeleteDagRun:
    def test_delete_dag_run(self, test_client, session):
        response = test_client.delete(f"/v2/dags/{DAG1_ID}/dagRuns/{DAG1_RUN1_ID}")
    ...
    def test_delete_dag_run_not_found(self, test_client):
        response = test_client.delete(f"/v2/dags/{DAG1_ID}/dagRuns/invalid")
        assert response.status_code == 404
        body = response.json()
        assert body["detail"] == "The DagRun with dag_id: `test_dag1` and run_id: `invalid` was not found"

    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.delete(f"/v2/dags/{DAG1_ID}/dagRuns/invalid")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.delete(f"/v2/dags/{DAG1_ID}/dagRuns/invalid")
        assert response.status_code == 403
...
class TestGetDagRunAssetTriggerEvents:
    ...
    def test_should_respond_200(self, test_client, dag_maker, session):
        ...
        response = test_client.get(
            "/v2/dags/TEST_DAG_ID/dagRuns/TEST_DAG_RUN_ID/upstreamAssetEvents",
        )
        assert response.status_code == 200
        expected_response = {
            "asset_events": [
    ...
    def test_should_respond_404(self, test_client):
        response = test_client.get(
            "v2/dags/invalid-id/dagRuns/invalid-run-id/upstreamAssetEvents",
        )
        assert response.status_code == 404
        assert (
...
class TestClearDagRun:
    ...
    def test_clear_dag_run(self, test_client, session):
        response = test_client.post(
            f"/v2/dags/{DAG1_ID}/dagRuns/{DAG1_RUN1_ID}/clear",
            json={"dry_run": False},
        )
        assert response.status_code == 200
        body = response.json()
    ...
    def test_clear_dag_run_dry_run(self, test_client, session, body, dag_run_id, expected_state):
        response = test_client.post(f"/v2/dags/{DAG1_ID}/dagRuns/{dag_run_id}/clear", json=body)
        assert response.status_code == 200
        body = response.json()
    ...
    def test_clear_dag_run_not_found(self, test_client):
        response = test_client.post(f"/v2/dags/{DAG1_ID}/dagRuns/invalid/clear", json={"dry_run": False})
        assert response.status_code == 404
        body = response.json()
        assert body["detail"] == "The DagRun with dag_id: `test_dag1` and run_id: `invalid` was not found"

    def test_clear_dag_run_unprocessable_entity(self, test_client):
        response = test_client.post(f"/v2/dags/{DAG1_ID}/dagRuns/{DAG1_RUN1_ID}/clear")
        assert response.status_code == 422
        body = response.json()
...
class TestTriggerDagRun:
    ...
    def test_invalid_data(self, test_client, post_body, expected_detail):
        ...
        response = test_client.post(f"/v2/dags/{DAG1_ID}/dagRuns", json=post_body)
        assert response.status_code == 422
    ...
    def test_dagrun_creation_exception_is_handled(self, mock_create_dagrun, test_client):
        ...
        response = test_client.post(f"/v2/dags/{DAG1_ID}/dagRuns", json={"logical_date": now})
        assert response.status_code == 400
    ...
    def test_should_respond_404_if_a_dag_is_inactive(self, test_client, session):
        ...
        response = test_client.post("/v2/dags/inactive/dagRuns", json={"logical_date": now})
        assert response.status_code == 404
    ...
    def test_should_respond_400_if_a_dag_has_import_errors(self, test_client, session):
        now = timezone.utcnow().isoformat()
        self._dags_for_trigger_tests(session)
        response = test_client.post("/v2/dags/import_errors/dagRuns", json={"logical_date": now})
        assert response.status_code == 400
        assert (
            response.json()["detail"]
    ...
    def test_response_404(self, test_client):
        now = timezone.utcnow().isoformat()
        response = test_client.post("/v2/dags/randoms/dagRuns", json={"logical_date": now})
        assert response.status_code == 404
    ...
    def test_response_409(self, test_client):
        now = timezone.utcnow().isoformat()
        response = test_client.post(
            f"/v2/dags/{DAG1_ID}/dagRuns", json={"dag_run_id": DAG1_RUN1_ID, "logical_date": now}
        )
