...
class TestDeletePool(TestPoolsEndpoint):
    def test_delete_should_respond_204(self, test_client, session):
        ...
        assert len(pools) == 3
        response = test_client.delete(f"/v2/pools/{POOL1_NAME}")
    ...
    def test_delete_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.delete(f"/v2/pools/{POOL1_NAME}")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.delete(f"/v2/pools/{POOL1_NAME}")
        assert response.status_code == 403

    def test_delete_should_respond_400(self, test_client):
        response = test_client.delete("/v2/pools/default_pool")
...
class TestGetPool(TestPoolsEndpoint):
    def test_get_should_respond_200(self, test_client, session):
        self.create_pools()
        response = test_client.get(f"/v2/pools/{POOL1_NAME}")
    ...
    def test_get_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get(f"/v2/pools/{POOL1_NAME}")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get(f"/v2/pools/{POOL1_NAME}")
        assert response.status_code == 403
...
class TestGetPools(TestPoolsEndpoint):
    ...
    def test_should_respond_200(
        ...
        self.create_pools()
        response = test_client.get("/v2/pools", params=query_params)
        assert response.status_code == 200
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get("/v2/pools", params={"pool_name_pattern": "~"})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get("/v2/pools", params={"pool_name_pattern": "~"})
        assert response.status_code == 403
...
class TestPatchPool(TestPoolsEndpoint):
    ...
    def test_should_respond_200(
        ...
        self.create_pools()
        response = test_client.patch(f"/v2/pools/{pool_name}", params=query_params, json=body)
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.patch(f"/v2/pools/{POOL1_NAME}", params={}, json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.patch(f"/v2/pools/{POOL1_NAME}", params={}, json={})
        assert response.status_code == 403
...
class TestPostPool(TestPoolsEndpoint):
    ...
    def test_should_respond_200(self, test_client, session, body, expected_status_code, expected_response):
        ...
        response = test_client.post("/v2/pools", json=body)
        assert response.status_code == expected_status_code
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.post("/v2/pools", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.post("/v2/pools", json={})
        assert response.status_code == 403
...
class TestBulkPools(TestPoolsEndpoint):
    ...
    def test_bulk_pools(self, test_client, actions, expected_results, session):
        self.create_pools()
        response = test_client.patch("/v2/pools", json=actions)
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.patch("/v2/pools", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.patch("/v2/pools", json={})
        assert response.status_code == 403
