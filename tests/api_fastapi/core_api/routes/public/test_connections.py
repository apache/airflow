...
class TestDeleteConnection(TestConnectionEndpoint):
    def test_delete_should_respond_204(self, test_client, session):
        ...
        assert len(conns) == 1
        response = test_client.delete(f"/v2/connections/{TEST_CONN_ID}")
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.delete(f"/v2/connections/{TEST_CONN_ID}")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.delete(f"/v2/connections/{TEST_CONN_ID}")
...
class TestGetConnection(TestConnectionEndpoint):
    def test_get_should_respond_200(self, test_client, session):
        self.create_connection()
        response = test_client.get(f"/v2/connections/{TEST_CONN_ID}")
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get(f"/v2/connections/{TEST_CONN_ID}")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get(f"/v2/connections/{TEST_CONN_ID}")
        assert response.status_code == 403
...
class TestGetConnections(TestConnectionEndpoint):
    ...
    def test_should_respond_200(
        ...
        response = test_client.get("/v2/connections", params=query_params)
        assert response.status_code == 200
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get("/v2/connections", params={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get("/v2/connections", params={})
        assert response.status_code == 403
...
class TestPostConnection(TestConnectionEndpoint):
    ...
    def test_post_should_respond_201(self, test_client, session, body):
        response = test_client.post("/v2/connections", json=body)
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.post("/v2/connections", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.post("/v2/connections", json={})
        assert response.status_code == 403
...
class TestPatchConnection(TestConnectionEndpoint):
    ...
    def test_patch_should_respond_200(self, test_client, body, session):
        ...
        response = test_client.patch(f"/v2/connections/{TEST_CONN_ID}", json=body)
        assert response.status_code == 200
        _check_last_log(session, dag_id=None, event="patch_connection", logical_date=None)

    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.patch(f"/v2/connections/{TEST_CONN_ID}", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.patch(f"/v2/connections/{TEST_CONN_ID}", json={})
        assert response.status_code == 403
    ...
    def test_patch_should_respond_200_with_update_mask(
        ...
        self.create_connection()
        response = test_client.patch(f"/v2/connections/{TEST_CONN_ID}", json=body, params=update_mask)
        assert response.status_code == 200
    ...
    def test_patch_should_respond_404(self, test_client, body):
        response = test_client.patch(f"/v2/connections/{body['connection_id']}", json=body)
        assert response.status_code == 404
        assert response.json() == {
...
class TestConnection(TestConnectionEndpoint):
    ...
    def test_should_respond_200(self, test_client, body):
        response = test_client.post("/v2/connections/test", json=body)
        assert response.status_code == 200
        assert response.json() == {
            "status": True,
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.post(
            "/v2/connections/test", json={"connection_id": TEST_CONN_ID, "conn_type": "sqlite"}
        )
        assert response.status_code == 401
    ...
    def test_connection_env_is_cleaned_after_run(self, test_client, body):
        test_client.post("/v2/connections/test", json=body)
...
class TestCreateDefaultConnections(TestConnectionEndpoint):
    def test_should_respond_204(self, test_client, session):
        response = test_client.post("/v2/connections/defaults")
        assert response.status_code == 204
        assert response.content == b""
        _check_last_log(session, dag_id=None, event="create_default_connections", logical_date=None)

    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.post("/v2/connections/defaults")
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.post("/v2/connections/defaults")
        assert response.status_code == 403
...
class TestBulkConnections(TestConnectionEndpoint):
    ...
    def test_bulk_connections(self, test_client, actions, expected_results, session):
        self.create_connections()
        response = test_client.patch("/v2/connections", json=actions)
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.patch("/v2/connections", json={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.patch("/v2/connections", json={})
        assert response.status_code == 403
