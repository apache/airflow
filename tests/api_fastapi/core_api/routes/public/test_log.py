...
class TestTaskInstancesLog:
    ...
    def test_should_respond_200_json(self, try_number):
        ...
        response = self.client.get(
            f"v2/dags/{self.DAG_ID}/dagRuns/{self.RUN_ID}/taskInstances/{self.TASK_ID}/logs/{try_number}",
            params={"token": token},
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                f"v2/dags/{DAG_ID}/dagRuns/{RUN_ID}/taskInstances/{TASK_ID}/logs/1",
                f"LOG_DIR/dag_id={DAG_ID}/run_id={RUN_ID}/task_id={TASK_ID}/attempt=1.log",
                {},
                1,
            ),
            (
                f"v2/dags/{DAG_ID}/dagRuns/{RUN_ID}/taskInstances/{MAPPED_TASK_ID}/logs/1",
            ...
            # try_number 2
            (
                f"v2/dags/{DAG_ID}/dagRuns/{RUN_ID}/taskInstances/{TASK_ID}/logs/2",
                f"LOG_DIR/dag_id={DAG_ID}/run_id={RUN_ID}/task_id={TASK_ID}/attempt=2.log",
                {},
                2,
            ),
            (
                f"v2/dags/{DAG_ID}/dagRuns/{RUN_ID}/taskInstances/{MAPPED_TASK_ID}/logs/2",
    ...
    def test_get_logs_response_with_ti_equal_to_none(self, try_number):
        ...
        response = self.client.get(
            f"v2/dags/{self.DAG_ID}/dagRuns/{self.RUN_ID}/taskInstances/Invalid-Task-ID/logs/{try_number}",
            params={"token": token},
        )
    ...
    def test_get_logs_with_metadata_as_download_large_file(self, try_number):
        with mock.patch("airflow.utils.log.file_task_handler.FileTaskHandler.read") as read_mock:
            ...
            response = self.client.get(
                f"v2/dags/{self.DAG_ID}/dagRuns/{self.RUN_ID}/"
    ...
    def test_bad_signature_raises(self):
        ...
        response = self.client.get(
            f"v2/dags/{self.DAG_ID}/dagRuns/{self.RUN_ID}/taskInstances/{self.TASK_ID}/logs/1",
            params={"token": token},
            headers={"Accept": "application/json"},
        )
        # assert response.status_code == 400
    ...
    def test_raises_404_for_invalid_dag_run_id(self):
        response = self.client.get(
            f"v2/dags/{self.DAG_ID}/dagRuns/NO_DAG_RUN/"  # invalid run_id
    ...
    def test_should_raise_404_when_missing_map_index_param_for_mapped_task(self):
        ...
        response = self.client.get(
            f"v2/dags/{self.DAG_ID}/dagRuns/{self.RUN_ID}/taskInstances/{self.MAPPED_TASK_ID}/logs/1",
            params={"token": token},
            headers={"Accept": "text/plain"},
        )
        assert response.status_code == 404
