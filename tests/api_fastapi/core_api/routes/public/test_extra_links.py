...
class TestGetExtraLinks:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "/v2/dags/INVALID/dagRuns/TEST_DAG_RUN_ID/taskInstances/TEST_SINGLE_LINK/links",
                404,
                {"detail": "DAG with ID = INVALID not found"},
                id="missing_dag",
            ),
            pytest.param(
                "/v2/dags/TEST_DAG_ID/dagRuns/INVALID/taskInstances/TEST_SINGLE_LINK/links",
                404,
                {"detail": "DAG Run with ID = INVALID not found"},
                id="missing_dag_run",
            ),
            pytest.param(
                "/v2/dags/TEST_DAG_ID/dagRuns/TEST_DAG_RUN_ID/taskInstances/INVALID/links",
                404,
    ...
    def test_should_respond_200(self, dag_maker, test_client):
        ...
        response = test_client.get(
            f"/v2/dags/{self.dag_id}/dagRuns/{self.dag_run_id}/taskInstances/{self.task_single_link}/links",
        )
    ...
    def test_should_respond_200_multiple_links(self, test_client, session):
        ...
        response = test_client.get(
            f"/v2/dags/{self.dag_id}/dagRuns/{self.dag_run_id}/taskInstances/{self.task_multiple_links}/links",
        )
    ...
    def test_should_respond_200_mapped_task_instance(self, test_client):
        ...
        response = test_client.get(
            f"/v2/dags/{self.dag_id}/dagRuns/{self.dag_run_id}/taskInstances/{self.task_mapped}/links",
