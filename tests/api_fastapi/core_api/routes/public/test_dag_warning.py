...
class TestGetDagWarnings:
    ...
    def test_get_dag_warnings(self, test_client, query_params, expected_total_entries, expected_messages):
        response = test_client.get("/v2/dagWarnings", params=query_params)
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get("/v2/dagWarnings", params={})
        assert response.status_code == 401

    def test_should_respond_403(self, unauthorized_test_client):
        response = unauthorized_test_client.get("/v2/dagWarnings", params={})
        assert response.status_code == 403

    def test_get_dag_warnings_bad_request(self, test_client):
        response = test_client.get("/v2/dagWarnings", params={"warning_type": "invalid"})
        response_json = response.json()
