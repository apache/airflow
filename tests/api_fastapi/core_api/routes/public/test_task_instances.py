...
class TestGetTaskInstance(TestTaskInstanceEndpoint):
    def test_should_respond_200(self, test_client, session):
        ...
        response = test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context"
        )
    ...
    def test_should_respond_200_with_versions(self, test_client, run_id, expected_version_number):
        response = test_client.get(
            f"/v2/dags/dag_with_multiple_versions/dagRuns/{run_id}/taskInstances/task1"
        )
...
class TestGetMappedTaskInstance(TestTaskInstanceEndpoint):
    def test_should_respond_200_mapped_task_instance_with_rtif(self, test_client, session):
        ...
        for map_index in (1, 2):
            response = test_client.get(
                "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances"
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context/1",
        )
        assert response.status_code == 401
    ...
    def test_should_respond_404_wrong_map_index(self, test_client, session):
        ...
        response = test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context/10",
        )
        assert response.status_code == 404
...
class TestGetMappedTaskInstances:
    ...
    def test_should_respond_401(self, unauthenticated_test_client):
        response = unauthenticated_test_client.get(
            "/v2/dags/mapped_tis/dagRuns/run_mapped_tis/taskInstances/task_2/listMapped",
        )
        assert response.status_code == 401
    ...
    def test_should_raise_404_not_found_for_nonexistent_task(
        ...
        response = test_client.get(
            "/v2/dags/mapped_tis/dagRuns/run_mapped_tis/taskInstances/nonexistent_task/listMapped",
        )
        assert response.status_code == 404
...
class TestGetTaskInstances(TestTaskInstanceEndpoint):
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ...
                False,
                "/v2/dags/example_python_operator/dagRuns/~/taskInstances",
    ...
    def test_not_found(self, test_client):
        response = test_client.get("/v2/dags/invalid/dagRuns/~/taskInstances")
        assert response.status_code == 404
        assert response.json() == {"detail": "DAG with dag_id: `invalid` was not found"}

        response = test_client.get("/v2/dags/~/dagRuns/invalid/taskInstances")
        assert response.status_code == 404
        assert response.json() == {"detail": "DagRun with run_id: `invalid` was not found"}

    def test_bad_state(self, test_client):
        response = test_client.get("/v2/dags/~/dagRuns/~/taskInstances", params={"state": "invalid"})
    ...
    def test_return_TI_only_from_readable_dags(self, test_client, session):
        ...
        response = test_client.get("/v2/dags/~/dagRuns/~/taskInstances")
        assert response.status_code == 200
    ...
    def test_should_respond_200_for_order_by(self, order_by_field, base_date, test_client, session):
        ...
        response_asc = test_client.get(
            "/v2/dags/~/dagRuns/~/taskInstances", params={"order_by": order_by_field}
        )
        ...
        response_desc = test_client.get(
            "/v2/dags/~/dagRuns/~/taskInstances", params={"order_by": f"-{order_by_field}"}
        )
    ...
    def test_should_respond_200_for_pagination(self, test_client, session):
        ...
        response_batch1 = test_client.get(
            "/v2/dags/~/dagRuns/~/taskInstances", params={"limit": 5, "offset": 0, "dag_ids": [dag_id]}
        )
        ...
        response_batch2 = test_client.get(
            "/v2/dags/~/dagRuns/~/taskInstances", params={"limit": 5, "offset": 5, "dag_ids": [dag_id]}
        )
...
class TestGetTaskInstancesBatch(TestTaskInstanceEndpoint):
    ...
    def test_should_respond_200(
        ...
        response = test_client.post(
            "/v2/dags/~/dagRuns/~/taskInstances/list",
            json=payload,
        )
        body = response.json()
    ...
    def test_should_respond_422_for_non_wildcard_path_parameters(self, test_client):
        response = test_client.post(
            "/v2/dags/non_wildcard/dagRuns/~/taskInstances/list",
        )
        assert response.status_code == 422
        assert "Input should be '~'" in str(response.json()["detail"])

        response = test_client.post(
            "/v2/dags/~/dagRuns/non_wildcard/taskInstances/list",
        )
        assert response.status_code == 422
        assert "Input should be '~'" in str(response.json()["detail"])
...
class TestGetTaskInstanceTry(TestTaskInstanceEndpoint):
    def test_should_respond_200(self, test_client, session):
        ...
        response = test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context/tries/1"
        )
        assert response.status_code == 200
        assert response.json() == {
            "dag_id": "example_python_operator",
    ...
    def test_should_respond_200_with_different_try_numbers(self, test_client, try_number, session):
        self.create_task_instances(session, task_instances=[{"state": State.SUCCESS}], with_ti_history=True)
        response = test_client.get(
            f"/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context/tries/{try_number}",
        )
    ...
    def test_raises_404_for_nonexistent_task_instance(self, test_client, session):
        self.create_task_instances(session)
        response = test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/nonexistent_task/tries/0"
        )
        assert response.status_code == 404
    ...
    def test_should_respond_200_with_versions(self, test_client, run_id, expected_version_number):
        response = test_client.get(
            f"/v2/dags/dag_with_multiple_versions/dagRuns/{run_id}/taskInstances/task1/tries/0"
        )
        assert response.status_code == 200
        assert response.json() == {
            "task_id": "task1",
...
class TestPostClearTaskInstances(TestTaskInstanceEndpoint):
    ...
    def test_should_respond_200(
        ...
        response = test_client.post(
            f"/v2/dags/{request_dag}/clearTaskInstances",
            json=payload,
        )
        assert response.status_code == 200
        assert response.json()["total_entries"] == expected_ti
    ...
    def test_dag_run_with_future_or_past_flag_returns_400(self, test_client, session, flag):
        ...
        self.dagbag.sync_to_db("dags-folder", None)
        response = test_client.post(f"/v2/dags/{dag_id}/clearTaskInstances", json=payload)
    ...
    def test_clear_taskinstance_is_called_with_queued_dr_state(self, mock_clearti, test_client, session):
        ...
        self.dagbag.sync_to_db("dags-folder", None)
        response = test_client.post(
            f"/v2/dags/{dag_id}/clearTaskInstances",
            json=payload,
        )
        assert response.status_code == 200
    ...
    def test_should_raise_400_for_naive_and_bad_datetime(self, test_client, session, payload, expected):
        ...
        self.dagbag.sync_to_db("dags-folder", None)
        response = test_client.post(
            "/v2/dags/example_python_operator/clearTaskInstances",
            json=payload,
        )
        assert response.status_code == 422
    ...
    def test_raises_404_for_non_existent_dag(self, test_client):
        response = test_client.post(
            "/v2/dags/non-existent-dag/clearTaskInstances",
            json={
                "dry_run": False,
                "reset_dag_runs": True,
                "only_failed": False,
...
class TestGetTaskInstanceTries(TestTaskInstanceEndpoint):
    def test_should_respond_200(self, test_client, session):
        ...
        response = test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context/tries"
        )
        assert response.status_code == 200
    ...
    def test_raises_404_for_nonexistent_task_instance(self, test_client, session):
        self.create_task_instances(session)
        response = test_client.get(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/non_existent_task/tries"
        )
        assert response.status_code == 404
    ...
    def test_should_respond_200_with_versions(self, test_client, run_id, expected_version_number):
        response = test_client.get(
            f"/v2/dags/dag_with_multiple_versions/dagRuns/{run_id}/taskInstances/task1/tries"
        )
        assert response.status_code == 200
...
class TestPatchTaskInstance(TestTaskInstanceEndpoint):
    ENDPOINT_URL = "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context"
    NEW_STATE = "failed"
    DAG_ID = "example_python_operator"
    TASK_ID = "print_the_context"
    RUN_ID = "TEST_DAG_RUN_ID"
    ...
    def test_should_raise_404_for_non_existent_dag(self, test_client):
        response = test_client.patch(
            "/v2/dags/non-existent-dag/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context",
            json={
                "new_state": self.NEW_STATE,
            },
        )
    ...
    def test_should_raise_404_for_non_existent_task_in_dag(self, test_client):
        response = test_client.patch(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/non_existent_task",
            json={
                "new_state": self.NEW_STATE,
            },
        )
...
class TestPatchTaskInstanceDryRun(TestTaskInstanceEndpoint):
    ...
    def test_should_raise_404_for_non_existent_dag(self, test_client):
        response = test_client.patch(
            "/v2/dags/non-existent-dag/dagRuns/TEST_DAG_RUN_ID/taskInstances/print_the_context/dry_run",
            json={
                "new_state": self.NEW_STATE,
            },
        )
    ...
    def test_should_raise_404_for_non_existent_task_in_dag(self, test_client):
        response = test_client.patch(
            "/v2/dags/example_python_operator/dagRuns/TEST_DAG_RUN_ID/taskInstances/non_existent_task/dry_run",
            json={
                "new_state": self.NEW_STATE,
            },
        )
