...
@provide_session
def _create_dag_tags(session=None):
    session.add(DagTag(dag_id=DAG1_ID, name="tag_1"))
    session.add(DagTag(dag_id=DAG2_ID, name="tag_2"))
...
@pytest.mark.parametrize(
    "query_params, dag_id, expected_status_code, dag_display_name, start_date, tags",
    [
        ({}, "fake_dag_id", 404, "fake_dag", datetime(2023, 12, 31, tzinfo=timezone.utc), []),
        ({}, DAG2_ID, 200, DAG2_ID, DAG2_START_DATE, [{"dag_id": "test_dag2", "name": "tag_2"}]),
    ],
)
def test_dag_details(
    test_client, query_params, dag_id, expected_status_code, dag_display_name, start_date, tags
):
...
@pytest.mark.parametrize(
    "query_params, dag_id, expected_status_code, dag_display_name, tags",
    [
        ({}, "fake_dag_id", 404, "fake_dag", []),
        ({}, DAG2_ID, 200, DAG2_ID, [{"dag_id": "test_dag2", "name": "tag_2"}]),
    ],
)
def test_get_dag(test_client, query_params, dag_id, expected_status_code, dag_display_name, tags):
