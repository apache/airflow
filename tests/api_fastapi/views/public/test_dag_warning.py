...
DAG_WARNING_TYPE = DagWarningType.NONEXISTENT_POOL
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"warning_type": "random"}, 0, []),
        ({"limit": 1, "order_by": "message"}, 1, [DAG1_MESSAGE]),
        ({"limit": 1, "offset": 1, "order_by": "message"}, 1, [DAG2_MESSAGE]),
        ({"limit": 1, "offset": 2, "order_by": "dag_id"}, 1, [DAG3_MESSAGE]),
        ({"limit": 1, "offset": 2, "order_by": "-dag_id"}, 1, [DAG1_MESSAGE]),
        ({"limit": 1, "order_by": "timestamp"}, 1, [DAG1_MESSAGE]),
        ({"limit": 1, "order_by": "-timestamp"}, 1, [DAG3_MESSAGE]),
...
def test_get_dag_warnings(test_client, query_params, expected_total_entries, expected_messages):
    ...
    assert [dag_warning["message"] for dag_warning in response_json["import_errors"]] == expected_messages
