...
class TestIstio(unittest.TestCase):
    ...
    def _mock_pod(self, image=None, args=[]):
        # 'image' is not in the function declaration
        # to dodge the pre-commit hook called
        # python2-fastcheck, which incorrectly handles
        # colon in a default string parameter.
        if not image:
