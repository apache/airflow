...
@provide_session
def sync_dags_to_db(dags: Collection[DAG], session: Session = NEW_SESSION) -> Sequence[SerializedDAG]:
    """
    ...
    def _write_dag(dag: DAG) -> SerializedDAG:
        lazy_deserialized_dag = LazyDeserializedDAG.from_dag(dag)
        SerializedDagModel.write_dag(lazy_deserialized_dag, bundle_name, session=session)
        return SerializedDAG.from_dict(lazy_deserialized_dag.data)
