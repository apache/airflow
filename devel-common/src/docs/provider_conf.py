...
if Version(astroid.__version__).major >= 4:
    ...
    def _generate_provider_api_docs(app):
        ...
        # Remove top-level rst files generated by sphinx-apidoc that are not needed
        # This includes intermediate namespace packages (e.g., airflow.providers.common.rst)
        files_to_remove = ["modules.rst", "airflow.rst", "airflow.providers.rst"]

        # For multi-level packages (e.g., common.sql), also remove intermediate rst files
        provider_parts = PACKAGE_ID.split(".")
        for i in range(len(provider_parts)):
            partial_path = ".".join(provider_parts[: i + 1])
            intermediate_rst = f"airflow.providers.{partial_path}.rst"
            files_to_remove.append(intermediate_rst)

        for rst_file in files_to_remove:
            rst_path = output_dir / rst_file
            if rst_path.exists():
                rst_path.unlink()

        # Create compatibility structure for provider index pages
        # sphinx-apidoc generates airflow.providers.X.rst, but we need airflow/providers/X/index.rst
        provider_path_parts = PACKAGE_ID.split(".")
        compat_dir = output_dir / "airflow" / "providers" / Path(*provider_path_parts)
        compat_dir.mkdir(parents=True, exist_ok=True)

        # Create index.rst that provides a landing page for the provider
        package_name_with_dots = f"airflow.providers.{PACKAGE_ID}"
        # Calculate relative path for toctree entries
        levels_up = len(provider_path_parts) + 2  # +2 for airflow/providers
        path_prefix = "../" * levels_up

        # Find all subpackage rst files generated by apidoc
        subpackages = []
        provider_rst_pattern = f"airflow.providers.{PACKAGE_ID}."
        for rst_file in output_dir.glob(f"{provider_rst_pattern}*.rst"):
            # Only include direct subpackages (not nested modules)
            relative_name = rst_file.stem[len(provider_rst_pattern) :]
            if "." not in relative_name:  # Direct subpackage
                subpackages.append(rst_file.stem)

        subpackages.sort()
        toctree_entries = "\n   ".join(f"{path_prefix}{sp}" for sp in subpackages)

        index_content = f"""{package_name_with_dots} package
{"=" * (len(package_name_with_dots) + 8)}

.. automodule:: {package_name_with_dots}
   :members:
   :show-inheritance:
   :undoc-members:
   :noindex:
...
.. toctree::
   ...
   {toctree_entries}
"""
        (compat_dir / "index.rst").write_text(index_content)

        # Create a stub for system tests to satisfy toctree
        if SYSTEM_TESTS_DIR and SYSTEM_TESTS_DIR.exists():
            tests_dir = output_dir / "tests" / "system" / Path(*provider_path_parts)
            tests_dir.mkdir(parents=True, exist_ok=True)
            tests_index_content = f"""System Tests
============

.. note::
   System tests for {PACKAGE_NAME} are located in the source tree under
   ``{SYSTEM_TESTS_DIR.relative_to(AIRFLOW_REPO_ROOT_PATH)}``.

   Due to technical limitations with Astroid 4, system test API documentation
   is not automatically generated. Please refer to the source code directly.
"""
            (tests_dir / "index.rst").write_text(tests_index_content)
...
# Under Astroid 4, sphinx-apidoc generates modules.rst which we don't use
if Version(astroid.__version__).major >= 4:
    if "toc.not_readable" not in suppress_warnings:
