...
class TestHttpRouteWeb:
    ...
    def test_should_check_that_both_httproute_and_healthcheckpolicy_are_created(self):
        docs = render_chart(
            values={
                "httpRoute": {
                    "web": {"enabled": True, "gateway": {"name": "test-gateway", "namespace": "test"}}
                }
            },
    ...
    def test_should_set_httproute_gateway_name(self):
        docs = render_chart(
            values={"httpRoute": {"web": {"enabled": True, "gateway": {"name": "foo", "namespace": "bar"}}}},
            show_only=["templates/webserver/webserver-httproute.yaml"],
        )
    ...
    def test_should_set_healthcheckpolicy_service_name(self):
        ...
        assert jmespath.search("spec.targetRef.name", docs[1]).endswith("-webserver")
