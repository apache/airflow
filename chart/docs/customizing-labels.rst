...
Global pod annotations can be set using ``airflowPodAnnotations``. These are applied to all Airflow component pods (scheduler, API server, triggerer, dag processor, workers, and webserver):
