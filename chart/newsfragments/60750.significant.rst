Automatic ``AIRFLOW__KUBERNETES_ENVIRONMENT_VARIABLES__`` prefix addition removed from ``container_extra_envs`` and ``custom_airflow_environment``

The automatic prefix addition for Kubernetes Executor environment variables has been removed from both the ``container_extra_envs`` and ``custom_airflow_environment`` helper functions.
