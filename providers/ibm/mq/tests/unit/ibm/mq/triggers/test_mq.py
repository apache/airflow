...
class TestMQTrigger:
    ...
    @pytest.mark.asyncio
    async def test_trigger_serialization_with_open_options(self):
        trigger = AwaitMessageTrigger(
            mq_conn_id="mq_default",
            queue_name="QUEUE1",
            poll_interval=2,
        ...
        assert kwargs["open_options"] == 8192

    @pytest.mark.asyncio
    @patch("airflow.providers.ibm.mq.triggers.mq.IBMMQHook")
    async def test_trigger_run_yields_event(self, mock_hook_class):
        """run() delegates to consume() and yields the result as a TriggerEvent."""
        mock_hook = mock_hook_class.return_value
        mock_hook.aconsume = AsyncMock(return_value="test message")
        ...
        mock_hook_class.assert_called_once_with("mq_default", open_options=8192)
        mock_hook.aconsume.assert_called_once_with(queue_name="QUEUE1", poll_interval=0.1)
