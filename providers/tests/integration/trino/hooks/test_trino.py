...
class TestTrinoHookIntegration:
    @mock.patch.dict(
        "os.environ", AIRFLOW_CONN_TRINO_DEFAULT="trino://airflow@trino:8080/"
    )
    ...
    def test_openlineage_methods(self):
        op = TrinoOperator(
            task_id="trino_test", sql="SELECT name FROM tpch.sf1.customer LIMIT 3"
        )
        op.execute({})
        ...
        assert (
            lineage.job_facets["sql"].query
            == "SELECT name FROM tpch.sf1.customer LIMIT 3"
        )
