...
class TestMsSqlToHiveTransfer:
    def setup_method(self):
        self.kwargs = dict(
            sql="sql", hive_table="table", task_id="test_mssql_to_hive", dag=None
        )
    ...
    def test_execute(self, mock_hive_hook, mock_mssql_hook, mock_tmp_file, mock_csv):
        ...
        mock_mssql_hook_get_conn = (
            mock_mssql_hook.return_value.get_conn.return_value.__enter__
        )
        mock_mssql_hook_cursor = (
            mock_mssql_hook_get_conn.return_value.cursor.return_value.__enter__
        )
        mock_mssql_hook_cursor.return_value.description = [
            ("anything", "some-other-thing")
        ]
    ...
    def test_execute_empty_description_field(
        self, mock_hive_hook, mock_mssql_hook, mock_tmp_file, mock_csv
    ):
        type(mock_tmp_file).name = PropertyMock(return_value="tmp_file")
        mock_tmp_file.return_value.__enter__ = Mock(return_value=mock_tmp_file)
        mock_mssql_hook_get_conn = (
        ...
        field_dict = {}
        for col_count, field in enumerate(
            mock_mssql_hook_cursor.return_value.description, start=1
        ):
