...
def test_no_connection_during_initialisation(
    hook_conn, docker_api_client_patcher, hook_kwargs
):
...
def test_init_fails_when_no_base_url_given(hook_kwargs):
    ...
    with pytest.raises(
        AirflowException, match=r"URL to the Docker server not provided\."
    ):
        DockerHook(**hook_kwargs)
...
@pytest.mark.parametrize(
    "tls_config",
    [pytest.param(True, id="bool"), pytest.param(TLSConfig(), id="TLSConfig-object")],
)
def test_init_warn_on_non_https_host_with_enabled_tls(
    base_url, tls_config, hook_kwargs, caplog
):
...
def test_create_api_client(conn_id, hook_conn, docker_api_client_patcher, caplog):
    ...
    hook = DockerHook(
        docker_conn_id=conn_id,
        base_url=TEST_TLS_BASE_URL,
        version=TEST_VERSION,
        tls=True,
        timeout=42,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"client_cert": "foo-bar", "client_key": "spam-egg"}, id="missing-ca-cert"
        ),
        pytest.param(
            {"ca_cert": "foo-bar", "client_key": "spam-egg"}, id="missing-client-cert"
        ),
        pytest.param(
            {"ca_cert": "foo-bar", "client_cert": "spam-egg"}, id="missing-client-key"
        ),
    ],
)
...
def test_construct_tls_config(assert_hostname, ssl_version):
    ...
    if DOCKER_PY_7_PLUS and (assert_hostname is not None or ssl_version is not None):
        ctx = pytest.warns(
            UserWarning, match=r"removed in `docker\.TLSConfig` constructor arguments"
        )
        no_warns = False
    else:
    ...
    with mock.patch.object(
        TLSConfig, "__init__", return_value=None, spec=TLSConfig
    ) as mock_tls_config:
        with ctx:
            if no_warns:
