...
create_transform_params = {
    ...
    "TransformInput": {
        "DataSource": {"S3DataSource": {"S3DataType": "S3Prefix", "S3Uri": data_url}}
    },
    "TransformOutput": {
...
DESCRIBE_TRAINING_FAILED_RETURN.update(
...
class TestSageMakerHook:
    ...
    def test_check_s3_url(
        self, mock_check_prefix, mock_check_bucket, mock_check_key, mock_client
    ):
    ...
    def test_create_tuning_job(self, mock_client, mock_check_tuning_config):
        ...
        mock_session.create_hyper_parameter_tuning_job.assert_called_once_with(
            **create_tuning_params
        )
        assert response == test_arn_return
    ...
    def test_create_transform_job_fs(self, mock_client):
        ...
        hook = SageMakerHook(aws_conn_id="sagemaker_test_conn_id")
        response = hook.create_transform_job(
            create_transform_params_fs, wait_for_completion=False
        )
        mock_session.create_transform_job.assert_called_once_with(
            **create_transform_params_fs
        )
    ...
    def test_create_endpoint_config(self, mock_client):
        ...
        mock_session.create_endpoint_config.assert_called_once_with(
            **create_endpoint_config_params
        )
        assert response == test_arn_return
    ...
    def test_describe_training_job(self, mock_client):
        ...
        mock_session.describe_training_job.assert_called_once_with(
            TrainingJobName=job_name
        )
    ...
    def test_describe_transform_job(self, mock_client):
        ...
        mock_session.describe_transform_job.assert_called_once_with(
            TransformJobName=job_name
        )
        assert response == "InProgress"
    ...
    def test_describe_endpoint_config(self, mock_client):
        ...
        mock_session.describe_endpoint_config.assert_called_once_with(
            EndpointConfigName=config_name
        )
    ...
    @mock.patch.object(AwsLogsHook, "conn")
    @mock.patch.object(SageMakerHook, "get_conn")
    def test_describe_training_job_with_complete_states(
        self, mock_client, mock_log_client, log_state
    ):
        mock_session = mock.Mock()
        mock_log_session = mock.Mock()
        attrs = {"describe_training_job.return_value": DESCRIBE_TRAINING_COMPLETED_RETURN}
    ...
    def test_training_with_logs(
        self, _, mock_describe, mock_client, mock_log_client, mock_check_training
    ):
        mock_check_training.return_value = True
    ...
    @mock.patch.object(SageMakerHook, "get_conn")
    @mock.patch("time.sleep", return_value=None)
    def test_count_processing_jobs_by_name_retries_on_throttle_exception(
        self, _, mock_conn
    ):
        ...
        mock_conn().list_processing_jobs.side_effect = [
            throttle_exception,
            successful_result,
        ]
        hook = SageMakerHook(aws_conn_id="sagemaker_test_conn_id")
    ...
    @mock.patch.object(SageMakerHook, "get_conn")
    def test_count_processing_jobs_by_name_job_not_exists_should_return_falsy(
        self, mock_conn
    ):
    ...
    def test_start_pipeline_returns_arn(self, mock_conn):
        mock_conn().start_pipeline_execution.return_value = {
            "PipelineExecutionArn": "hellotest"
        }
    ...
    def test_stop_pipeline_returns_status(self, mock_conn):
        mock_conn().describe_pipeline_execution.return_value = {
        ...
        mock_conn().stop_pipeline_execution.assert_called_once_with(
            PipelineExecutionArn="test"
        )
    ...
    def test_stop_pipeline_raises_when_already_stopped_if_specified(self, mock_conn):
        error = ClientError(
            error_response={
                "Error": {
                    "Message": "Only pipelines with 'Executing' status can be stopped",
                    "Code": "0",
                }
            },
            operation_name="empty",
    ...
    def test_create_auto_ml_parameter_structure(self, conn_mock):
        ...
        assert conn_mock().create_auto_ml_job.call_args.kwargs == {
            "AutoMLJobConfig": {
                "CompletionCriteria": {"MaxAutoMLJobRuntimeInSeconds": 30}
            },
            "AutoMLJobName": "a",
            "InputDataConfig": [
                {
                    ...
                    "DataSource": {
                        "S3DataSource": {"S3DataType": "S3Prefix", "S3Uri": "b"}
                    },
