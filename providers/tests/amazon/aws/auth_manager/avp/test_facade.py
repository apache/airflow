...
class TestAwsAuthManagerAmazonVerifiedPermissionsFacade:
    ...
    def test_is_authorized_unsuccessful(self, facade):
        ...
        with pytest.raises(
            AirflowException,
            match="Error occurred while making an authorization decision.",
        ):
            facade.is_authorized(
                method="GET", entity_type=AvpEntities.VARIABLE, user=test_user
            )
    ...
    def test_batch_is_authorized_unsuccessful(self, facade):
        avp_response = {
            "results": [{}, {"errors": []}, {"errors": [{"errorDescription": "Error"}]}]
        }
        mock_batch_is_authorized = Mock(return_value=avp_response)
        facade.avp_client.batch_is_authorized = mock_batch_is_authorized
        ...
        with pytest.raises(
            AirflowException,
            match="Error occurred while making a batch authorization decision.",
        ):
    ...
    def test_get_batch_is_authorized_single_result_unsuccessful(self, facade):
        with pytest.raises(
            AirflowException, match="Could not find the authorization result."
        ):
    ...
    def test_is_policy_store_schema_up_to_date_when_schema_up_to_date(
        self, facade, providers_src_folder
    ):
    ...
    def test_is_policy_store_schema_up_to_date_when_schema_is_modified(
        self, facade, providers_src_folder
    ):
        schema_path = providers_src_folder.joinpath(
            "airflow", "providers", "amazon", "aws", "auth_manager", "avp", "schema.json"
