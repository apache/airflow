...
def _create_fake_imap(
    mock_imaplib, with_mail=False, attachment_name="test1.csv", use_ssl=True
):
    if use_ssl:
...
class TestImapHook:
    ...
    def test_connect_and_disconnect_imap_ssl_context_none(
        self, create_default_context, mock_imaplib
    ):
        mock_conn = _create_fake_imap(mock_imaplib)
        ...
        mock_imaplib.IMAP4_SSL.assert_called_once_with(
            "imap_server_address", 1993, ssl_context=None
        )
        mock_conn.login.assert_called_once_with("imap_user", "imap_password")
        assert mock_conn.logout.call_count == 1
    ...
    @patch(imaplib_string)
    @patch("ssl.create_default_context")
    def test_connect_and_disconnect_imap_ssl_context_default(
        self, create_default_context, mock_imaplib
    ):
        mock_conn = _create_fake_imap(mock_imaplib)
    ...
    def test_connect_and_disconnect_imap_ssl_context_override(
        ...
        with conf_vars(
            {("email", "ssl_context"): "none", ("imap", "ssl_context"): "default"}
        ):
            with ImapHook():
                pass
    ...
    def test_has_mail_attachment_with_regex_not_found(self, mock_imaplib):
        ...
        with ImapHook() as imap_hook:
            has_attachment_in_inbox = imap_hook.has_mail_attachment(
                name=r"test_(\d+).csv", check_regex=True
            )
    ...
    @patch(open_string, new_callable=mock_open)
    @patch(imaplib_string)
    def test_download_mail_attachments_with_regex_not_found(
        self, mock_imaplib, mock_open_method
    ):
        _create_fake_imap(mock_imaplib, with_mail=True)
    ...
    @patch(open_string, new_callable=mock_open)
    @patch(imaplib_string)
    def test_download_mail_attachments_with_latest_only(
        self, mock_imaplib, mock_open_method
    ):
        _create_fake_imap(mock_imaplib, with_mail=True)
    ...
    @patch(open_string, new_callable=mock_open)
    @patch(imaplib_string)
    def test_download_mail_attachments_with_escaping_chars(
        self, mock_imaplib, mock_open_method
    ):
    ...
    def test_download_mail_attachments_with_symlink(
        self, mock_imaplib, mock_open_method, mock_is_symlink
    ):
