...
class TestLocalKubernetesExecutor:
    ...
    def test_queued_tasks(self):
        ...
        local_kubernetes_executor = LocalKubernetesExecutor(
            local_executor_mock, k8s_executor_mock
        )
    ...
    def test_running(self):
        ...
        assert local_kubernetes_executor.running == local_running_tasks.union(
            k8s_running_tasks
        )
    ...
    def test_slots_available(self):
        ...
        k8s_executor_mock = mock.MagicMock()
        local_kubernetes_executor = LocalKubernetesExecutor(
            local_executor, k8s_executor_mock
        )

        # Should be equal to Local Executor default parallelism.
        assert local_kubernetes_executor.slots_available == conf.getint(
            "core", "PARALLELISM"
        )
    ...
    def test_kubernetes_executor_knows_its_queue(self):
        ...
        assert k8s_executor_mock.kubernetes_queue == conf.get(
            "local_kubernetes_executor", "kubernetes_queue"
        )
    ...
    def test_log_is_fetched_from_k8s_executor_only_for_k8s_queue(self):
        ...
        simple_task_instance.queue = conf.get(
            "local_kubernetes_executor", "kubernetes_queue"
        )
        ...
        messages, logs = local_k8s_exec.get_task_log(
            ti=simple_task_instance, try_number=3
        )
