...
class TestGoogleDriveHook:
    ...
    def test_get_conn(self, mock_discovery_build, mock_authorize):
        ...
        mock_discovery_build.assert_called_once_with(
            "drive", "v3", cache_discovery=False, http="AUTHORIZE"
        )
    ...
    def test_ensure_folders_exists_when_no_folder_exists(self, mock_get_conn):
        ...
        result_value = self.gdrive_hook._ensure_folders_exists(
            path="AAA/BBB/CCC/DDD", folder_id="root"
        )
    ...
    def test_exists_when_file_exists(self, mock_get_conn, mock_method):
        ...
        result_value = self.gdrive_hook.exists(
    ...
    def test_get_file_id_when_multiple_files_exists(self, mock_get_conn):
        ...
        mock_get_conn.return_value.files.return_value.list.return_value.execute.side_effect = [
            {
                "files": [
                    {"id": "ID_1", "mimeType": "text/plain"},
                    {"id": "ID_2", "mimeType": "text/plain"},
                ]
            }
        ]
    ...
    @mock.patch(
        "airflow.providers.google.suite.hooks.drive.GoogleDriveHook._resolve_file_path"
    )
    def test_upload_file_to_root_directory(
        self,
        mock_resolve_file_path,
        mock_ensure_folders_exists,
        mock_get_conn,
    ...
    def test_upload_file_to_subdirectory(
        ...
        mock_ensure_folders_exists.assert_called_once_with(
            path="AA/BB/CC", folder_id="root"
        )
        mock_get_conn.assert_has_calls(
            [
                mock.call()
    ...
    def test_upload_file_into_folder_id(
        ...
        mock_resolve_file_path.return_value = (
            "Shared_Folder_A/Folder_B"  # path for FOLDER_ID
        )

        return_value = self.gdrive_hook.upload_file(
            "/tmp/file.csv", "/file.csv", folder_id=folder_id
        )
