...
class TestCloudTasksHook:
    ...
    @mock.patch(
        "airflow.providers.google.cloud.hooks.tasks.CloudTasksHook.get_credentials"
    )
    ...
    @mock.patch(
        "airflow.providers.google.cloud.hooks.tasks.CloudTasksHook.get_conn",
        return_value=mock_patch_return_object(
            "list_queues", [Queue(name=FULL_QUEUE_PATH)]
        ),
    )
    ...
    @mock.patch(
        "airflow.providers.google.cloud.hooks.tasks.CloudTasksHook.get_conn",
        return_value=mock_patch_return_object(
            "resume_queue", Queue(name=FULL_QUEUE_PATH)
        ),
    )
    ...
    def test_create_task(self, get_conn):
        ...
        get_conn.return_value.create_task.assert_called_once_with(
            request=dict(
                parent=FULL_QUEUE_PATH, task=Task(name=FULL_TASK_PATH), response_view=None
            ),
            retry=DEFAULT,
            timeout=None,
            metadata=(),
