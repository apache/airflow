...
from airflow.providers.google.cloud.hooks.vision import (
    ERR_DIFF_NAMES,
...
PRODUCTSET_NAME_TEST = (
    f"projects/{PROJECT_ID_TEST}/locations/{LOC_ID_TEST}/productSets/{PRODUCTSET_ID_TEST}"
)
PRODUCT_ID_TEST = "p-id"
PRODUCT_ID_TEST_2 = "p-id-2"
PRODUCT_NAME_TEST = (
    f"projects/{PROJECT_ID_TEST}/locations/{LOC_ID_TEST}/products/{PRODUCT_ID_TEST}"
)
PRODUCT_NAME = (
    f"projects/{PROJECT_ID_TEST}/locations/{LOC_ID_TEST}/products/{PRODUCT_ID_TEST}"
)
...
DETECT_TEST_ADDITIONAL_PROPERTIES = {
...
class TestGcpVisionHook:
    ...
    @mock.patch(
        "airflow.providers.google.cloud.hooks.vision.CloudVisionHook.get_credentials"
    )
    ...
    def test_get_productset(self, get_conn):
        ...
        get_product_set_method.assert_called_once_with(
            name=name, retry=DEFAULT, timeout=None, metadata=()
        )
    ...
    @pytest.mark.parametrize(
        "location, product_set_id", LOCATION_PRODUCTSET_ID_TEST_PARAMS
    )
    @mock.patch("airflow.providers.google.cloud.hooks.vision.CloudVisionHook.get_conn")
    def test_update_productset_no_explicit_name_and_missing_params_for_constructed_name(
        ...
        err = ctx.value
        assert err
        assert ERR_UNABLE_TO_CREATE.format(
    ...
    def test_delete_productset(self, get_conn):
        ...
        delete_product_set_method.assert_called_once_with(
            name=name, retry=DEFAULT, timeout=None, metadata=()
        )
    ...
    def test_remove_product_from_product_set(self, get_conn):
        # Given
        remove_product_from_product_set_method = (
            get_conn.return_value.remove_product_from_product_set
        )
    ...
    @mock.patch(
        "airflow.providers.google.cloud.hooks.vision.CloudVisionHook.annotator_client"
    )
    ...
    def test_create_product_autogenerated_id(self, get_conn):
        ...
        response_product = Product(
            name=ProductSearchClient.product_path(
                PROJECT_ID_TEST, LOC_ID_TEST, autogenerated_id
            )
        )
        create_product_method = get_conn.return_value.create_product
    ...
    def test_update_product_no_explicit_name_and_missing_params_for_constructed_name(
        ...
        err = ctx.value
        assert err
        assert ERR_UNABLE_TO_CREATE.format(label="Product", id_label="product_id") in str(
            err
        )
    ...
    def test_delete_product(self, get_conn):
        ...
        # Then
        assert response is None
        delete_product_method.assert_called_once_with(
            name=name, retry=DEFAULT, timeout=None, metadata=()
        )
    ...
    def test_detect_text_with_additional_properties(self, annotator_client_mock):
        ...
        self.hook.text_detection(
            ...
            additional_properties={"prop1": "test1", "prop2": "test2"},
        )
