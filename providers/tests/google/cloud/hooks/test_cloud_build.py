...
class TestCloudBuildHook:
    ...
    @mock.patch(
        "airflow.providers.google.cloud.hooks.cloud_build.CloudBuildHook.get_credentials"
    )
    ...
    def test_cancel_build(self, get_conn):
        ...
        get_conn.return_value.cancel_build.assert_called_once_with(
            request={"project_id": PROJECT_ID, "id": BUILD_ID},
            retry=DEFAULT,
            timeout=None,
            metadata=(),
        )
    ...
    def test_create_build_without_waiting_for_result(
        self, get_conn, mock_get_id_from_operation
    ):
        get_conn.return_value.run_build_trigger.return_value = mock.MagicMock()
        mock_get_id_from_operation.return_value = BUILD_ID
    ...
    def test_run_build_trigger_with_wait(
        self, get_conn, wait_time, mock_get_id_from_operation
    ):
        get_conn.return_value.run_build_trigger.return_value = mock.MagicMock()
        mock_get_id_from_operation.return_value = BUILD_ID
    ...
    def test_update_build_trigger(self, get_conn):
        self.hook.update_build_trigger(
            trigger_id=TRIGGER_ID, trigger=BUILD_TRIGGER, project_id=PROJECT_ID
        )
...
class TestAsyncHook:
    ...
    async def test_async_cloud_build_service_client_creation_should_execute_successfully(
        ...
        mocked_get_build.assert_called_once_with(
            request=request, retry=DEFAULT, timeout=None, metadata=()
        )
