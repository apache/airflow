...
TEST_AWS_ACCESS_KEY = {
    ...
    SECRET_ACCESS_KEY: TEST_AWS_ACCESS_SECRET,
}
...
SOURCE_AWS_ROLE_ARN = {
    AWS_S3_DATA_SOURCE: {
        ...
        PATH: SOURCE_PATH,
        AWS_ROLE_ARN: AWS_ROLE_ARN_INPUT,
    }
}
...
VALID_TRANSFER_JOB_AWS_RAW[TRANSFER_SPEC][AWS_S3_DATA_SOURCE][AWS_ACCESS_KEY] = (
    TEST_AWS_ACCESS_KEY
)
VALID_TRANSFER_JOB_AWS_WITH_ROLE_ARN_RAW = deepcopy(VALID_TRANSFER_JOB_RAW)
VALID_TRANSFER_JOB_AWS_WITH_ROLE_ARN_RAW[TRANSFER_SPEC].update(
    deepcopy(SOURCE_AWS_ROLE_ARN)
)
VALID_TRANSFER_JOB_AWS_WITH_ROLE_ARN_RAW[TRANSFER_SPEC][AWS_S3_DATA_SOURCE][
    AWS_ROLE_ARN
] = AWS_ROLE_ARN_INPUT
...
class TestTransferJobPreprocessor:
    ...
    @pytest.mark.skipif(
        boto3 is None, reason="Skipping test because boto3 is not available"
    )
    @mock.patch(
        "airflow.providers.google.cloud.operators.cloud_storage_transfer_service.AwsBaseHook"
    )
...
class TestTransferJobValidator:
    def test_should_raise_exception_when_encounters_aws_credentials(self):
        body = {
            "transferSpec": {"awsS3DataSource": {"awsAccessKey": TEST_AWS_ACCESS_KEY}}
        }
    ...
    @pytest.mark.parametrize(
        "body",
        [VALID_TRANSFER_JOB_GCS, VALID_TRANSFER_JOB_AWS, VALID_TRANSFER_JOB_AWS_ROLE_ARN],
    )
...
class TestGcpStorageTransferJobCreateOperator:
    ...
    def test_job_create_aws_with_role_arn(self, aws_hook, mock_hook):
        mock_hook.return_value.create_transfer_job.return_value = (
            VALID_TRANSFER_JOB_AWS_ROLE_ARN
        )
    ...
    def test_templates(
        self, _, create_task_instance_of_operator, body, excepted, session
    ):
...
class TestGcpStorageTransferJobUpdateOperator:
    ...
    def test_job_update(self, mock_hook):
        ...
        body = {
            "transferJob": {"description": "example-name"},
...
class TestGcpStorageTransferJobDeleteOperator:
    ...
    def test_job_delete_with_templates(
    ...
    def test_job_delete_should_throw_ex_when_name_none(self):
        with pytest.raises(
            AirflowException, match="The required parameter 'job_name' is empty or None"
        ):
...
class TestGpcStorageTransferOperationsGetOperator:
    ...
    def test_operation_get_with_templates(
        self, _, create_task_instance_of_operator, session
    ):
    ...
    def test_operation_get_should_throw_ex_when_operation_name_none(self):
        with pytest.raises(
            AirflowException,
            match="The required parameter 'operation_name' is empty or None",
        ):
            CloudDataTransferServiceGetOperationOperator(
                operation_name="", task_id=TASK_ID
            )
...
class TestGcpStorageTransferOperationsPauseOperator:
    ...
    def test_operation_pause_with_templates(
        self, _, create_task_instance_of_operator, session
    ):
    ...
    def test_operation_pause_should_throw_ex_when_name_none(self):
        with pytest.raises(
            ...
            CloudDataTransferServicePauseOperationOperator(
                operation_name="", task_id="task-id"
            )
...
class TestGcpStorageTransferOperationsResumeOperator:
    ...
    def test_operation_resume_with_templates(
        self, _, create_task_instance_of_operator, session
    ):
...
class TestGcpStorageTransferOperationsCancelOperator:
    ...
    def test_operation_cancel_with_templates(
        self, _, create_task_instance_of_operator, session
    ):
...
class TestS3ToGoogleCloudStorageTransferOperator:
    ...
    def test_execute_should_throw_ex_when_delete_job_without_wait(
        ...
        with pytest.raises(
            AirflowException,
            match="If 'delete_job_after_completion' is True, then 'wait' must also be True.",
        ):
            CloudDataTransferServiceS3ToGCSOperator(
                task_id=TASK_ID,
                s3_bucket=AWS_BUCKET_NAME,
