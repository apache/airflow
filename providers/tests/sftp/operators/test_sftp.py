...
LOCAL_DATASET = [
    Dataset(
        namespace=f"file://{socket.gethostbyname(socket.gethostname())}:22",
        name=LOCAL_FILEPATH,
    )
]
...
class TestSFTPOperator:
    ...
    def test_pickle_file_transfer_put(self, dag_maker):
        ...
        with dag_maker(
            dag_id="unit_tests_sftp_op_pickle_file_transfer_put", start_date=DEFAULT_DATE
        ):
        ...
        with pytest.warns(
            ...
            match="Parameter `ssh_hook` is deprecated..*",
        ):
            tis["put_test_task"].run()
            tis["check_file_task"].run()

        pulled = tis["check_file_task"].xcom_pull(
            task_ids="check_file_task", key="return_value"
        )
        assert pulled.strip() == test_local_file_content

    @conf_vars({("core", "enable_xcom_pickling"): "True"})
    def test_file_transfer_no_intermediate_dir_error_put(
    ...
    def test_file_transfer_with_intermediate_dir_put(self, dag_maker):
        ...
        with dag_maker(
            dag_id="unit_tests_sftp_op_file_transfer_with_intermediate_dir_put"
        ):
            SFTPOperator(  # Put test file to remote.
                task_id="test_sftp",
                ssh_hook=self.hook,
        ...
        pulled = tis["test_check_file"].xcom_pull(
    ...
    @conf_vars({("core", "enable_xcom_pickling"): "True"})
    def test_file_transfer_no_intermediate_dir_error_get(
        self, dag_maker, create_remote_file_and_cleanup
    ):
        with dag_maker(
            dag_id="unit_tests_sftp_op_file_transfer_no_intermediate_dir_error_get"
        ):
    ...
    @conf_vars({("core", "enable_xcom_pickling"): "True"})
    def test_file_transfer_with_intermediate_dir_error_get(
        self, dag_maker, create_remote_file_and_cleanup
    ):
        with dag_maker(
            dag_id="unit_tests_sftp_op_file_transfer_with_intermediate_dir_error_get"
        ):
            SFTPOperator(  # Get remote file to local.
                task_id="test_sftp",
                ssh_hook=self.hook,
    ...
    def test_arg_checking(self):
        ...
        with pytest.raises(
            AirflowException, match="Cannot operate without sftp_hook or ssh_conn_id."
        ):
            task_0.execute(None)
    ...
    @mock.patch(
        "airflow.providers.ssh.hooks.ssh.SSHHook.get_conn", spec=paramiko.SSHClient
    )
