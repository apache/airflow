...
class TestSmtpHook:
    ...
    def test_connect_and_disconnect_via_nonssl(self, mock_smtplib):
        ...
        mock_smtplib.SMTP.assert_called_once_with(
            host="smtp_server_address", port=587, timeout=30
        )
        mock_conn.login.assert_called_once_with("smtp_user", "smtp_password")
        assert mock_conn.close.call_count == 1
    ...
    def test_get_email_address_single_email(self, mock_smtplib):
        with SmtpHook() as smtp_hook:
            assert smtp_hook._get_email_address_list("test1@example.com") == [
                "test1@example.com"
            ]
    ...
    def test_get_email_address_comma_sep_string(self, mock_smtplib):
        with SmtpHook() as smtp_hook:
            assert (
                smtp_hook._get_email_address_list("test1@example.com, test2@example.com")
                == TEST_EMAILS
            )
    ...
    def test_get_email_address_list(self, mock_smtplib):
        with SmtpHook() as smtp_hook:
            assert (
                smtp_hook._get_email_address_list(
    ...
    @patch("smtplib.SMTP")
    @patch("ssl.create_default_context")
    def test_send_mime_ssl_none_email_context(
        ...
        mock_smtp_ssl.assert_called_once_with(
            host="smtp_server_address", port=465, timeout=30, context=None
        )

    @patch("smtplib.SMTP_SSL")
    @patch("smtplib.SMTP")
    @patch("ssl.create_default_context")
    def test_send_mime_ssl_extra_context(
        self, create_default_context, mock_smtp, mock_smtp_ssl
    ):
        mock_smtp_ssl.return_value = Mock()
        conn = Connection(
        ...
        db.merge_conn(conn)
        with conf_vars(
            {("smtp", "smtp_ssl"): "True", ("smtp_provider", "ssl_context"): "default"}
        ):
    ...
    def test_send_mime_ssl_none_smtp_provider_context(
        ...
        mock_smtp_ssl.return_value = Mock()
        with conf_vars(
            {("smtp", "smtp_ssl"): "True", ("smtp_provider", "ssl_context"): "none"}
        ):
            with SmtpHook() as smtp_hook:
                smtp_hook.send_email_smtp(
                    to="to", subject="subject", html_content="content", from_email="from"
    ...
    def test_send_mime_dryrun(self, mock_smtp, mock_smtp_ssl):
        with SmtpHook() as smtp_hook:
            smtp_hook.send_email_smtp(
                to="to", subject="subject", html_content="content", dryrun=True
            )
    ...
    def test_send_mime_partial_failure(self, mock_smtp_ssl, mime_message_mock):
        ...
        final_mock.sendmail.assert_called_once_with(
            from_addr="from", to_addrs=["to"], msg="msg"
        )
