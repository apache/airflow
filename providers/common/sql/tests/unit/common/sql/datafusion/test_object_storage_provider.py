...
class TestObjectStorageProvider:
    ...
    def test_gcs_provider_success_with_key_path(self, mock_gcs):
        provider = GCSObjectStorageProvider()
        connection_config = ConnectionConfig(
            conn_id="google_cloud_default",
            credentials={"key_path": "/path/to/key.json"},
        )

        store = provider.create_object_store("gs://demo-data/path", connection_config)

        mock_gcs.assert_called_once_with(bucket_name="demo-data", service_account_path="/path/to/key.json")
