...
def _setup_mock_hook(mock_hook_cls, output: str):
    """Configure BaseAIHook mock to return the given SQL output from run_agent."""
    mock_hook = mock_hook_cls.get_agent_hook.return_value
    mock_hook.create_agent.return_value = MagicMock()
    mock_hook.run_agent.return_value = _make_run_result(output)
    return mock_hook
...
class TestLLMSQLQueryOperator:
    ...
    def test_execute_with_schema_context(self, mock_hook_cls):
        ...
        _setup_mock_hook(mock_hook_cls, "SELECT id, name FROM users WHERE active = true")
    ...
    def test_execute_passes_agent_params(self, mock_hook_cls):
        """agent_params are forwarded in the AgentRunRequest."""
        _setup_mock_hook(mock_hook_cls, "SELECT 1")
        ...
        request = mock_hook_cls.get_agent_hook.return_value.create_agent.call_args[0][0]
    ...
    def test_system_prompt_appended_to_sql_instructions(self, mock_hook_cls):
        ...
        request = mock_hook_cls.get_agent_hook.return_value.create_agent.call_args[0][0]
        assert "Always use LEFT JOINs." in request.instructions
...
class TestLLMSQLQueryOperatorSchemaIntrospection:
    ...
    def test_introspect_schemas_via_db_hook(self, mock_hook_cls):
        ...
        _setup_mock_hook(mock_hook_cls, "SELECT id FROM users")
        ...
        request = mock_hook_cls.get_agent_hook.return_value.create_agent.call_args[0][0]
        assert "users" in request.instructions
    ...
    def test_execute_with_datasource_config_and_db_tables(self, mock_engine_cls, mock_hook_cls):
        ...
        _setup_mock_hook(mock_hook_cls, "SELECT u.id, e.event FROM users u JOIN events e ON u.id = e.user_id")
        ...
        request = mock_hook_cls.get_agent_hook.return_value.create_agent.call_args[0][0]
        assert "users" in request.instructions
        assert "events" in request.instructions
...
class TestLLMSQLQueryOperatorApproval:
    ...
    def test_execute_with_approval_defers(self, mock_hook_cls, mock_upsert, mock_trigger_cls):
        ...
        _setup_mock_hook(mock_hook_cls, "SELECT id FROM users WHERE active")
