...
- :class:`~airflow.providers.common.ai.toolsets.sandbox.SandboxToolset` — give
  ...
  Airflow worker. See :doc:`sandbox`.
...
- ``allowed_tables``: Restrict the agent to a fixed set of tables. ``None``
  (default) exposes all tables in ``schema``; an empty list raises ``ValueError``
...
agent a shell and a filesystem inside a disposable sandbox, provisioned off the
Airflow worker by a :class:`~airflow.providers.common.ai.sandbox.SandboxBackend`
and destroyed when the agent run ends. It exposes four tools, ``run_command``,
``read_file``, ``write_file`` and ``list_directory``, and ships with a hosted
backend on Modal for production and a local microVM backend for development.

It has its own page: :doc:`sandbox`, covering when to reach for it against code
mode, ``KubernetesPodOperator`` and the ``KubernetesExecutor``, how to configure
egress, packages and credentials, how a result gets out, what it costs, and the
limitations to read before designing a Dag around it.
