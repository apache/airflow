...
class OpenSandboxBackend(SandboxBackend):
    ...
    or not the server runs the egress sidecar that enforces it, so after creating
    ...
    def _get_network_policy(spec: SandboxSpec | None) -> NetworkPolicy | None:
        ...
        if not spec.block_network:
            ...
            # No policy means intentionally open egress. Sending an explicit allow policy
