...
class LLMSQLQueryOperator(BaseOperator):
    ...
    for LLM access and optionally a
    ...
    def __init__(
        ...
        *,
        prompt: str,
        llm_conn_id: str = "pydantic_ai_default",
    ...
    def execute(self, context: Context) -> str:
        ...
        agent = self.llm_hook.create_agent(output_type=str, instructions=system_prompt)
    ...
    def _introspect_schemas(self) -> str:
        ...
        if not parts and self.table_names:
            self.log.warning(
                "None of the requested tables (%s) returned schema information.", self.table_names
            )
        return "\n\n".join(parts)
