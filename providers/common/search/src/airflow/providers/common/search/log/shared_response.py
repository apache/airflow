...
def resolve_nested(hit: dict[Any, Any], parent_class=None) -> type[Hit]:
    """
    Resolve nested hits from a search backend by iteratively navigating the ``_nested`` field.

    The result is used to fetch the appropriate document class to handle the hit.

    This method can be used with nested fields which are structured
...
class SearchResponse(AttributeDict):
    """
    Manage and access the response from a search backend (Elasticsearch, OpenSearch, ...).
    ...
    The response parameter stores the dictionary returned by the search client's search method.
    """
    ...
    def __iter__(self) -> Iterator[Hit]:
        """Provide an iterator over the hits in the search response."""
        return iter(self.hits)

    def __getitem__(self, key):
        """Retrieve a specific hit or a slice of hits from the search response."""
        if isinstance(key, (slice, int)):
            return self.hits[key]
        return super().__getitem__(key)

    def __bool__(self):
        """Evaluate the presence of hits in the search response."""
        return bool(self.hits)

    @property
    def hits(self) -> list[Hit]:
        """
        This property provides access to the hits (i.e., the results) of the search response.
        ...
        of the associated `Search` instance (i.e. an instance from the task handler class),
