...
from airflow.providers.common.search.log.json_formatter import SearchJSONFormatter


class TestSearchJSONFormatter:
    ...
    def test_format_log_record(self, json_formatter, log_record):
        json_formatter.json_fields = self.JSON_FIELDS
        data = json.loads(json_formatter.format(log_record))
    ...
    def test_formattime_in_iso8601_format(self, json_formatter, log_record):
        json_formatter.json_fields = ["asctime"]
        iso8601_format = json_formatter.formatTime(log_record)
        try:
            pendulum.parse(iso8601_format, strict=True)
        except ValueError:
            raise Exception("Time is not in ISO8601 format")

    def test_extra_fields(self, json_formatter, log_record):
        json_formatter.json_fields = self.JSON_FIELDS
        json_formatter.extras = self.EXTRA_FIELDS
        data = json.loads(json_formatter.format(log_record))
