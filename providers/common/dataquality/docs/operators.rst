...
Use :class:`~airflow.providers.common.dataquality.operators.dq_check.DQCheckOperator` to run a
...
- ``ruleset`` -- a :class:`~airflow.providers.common.dataquality.rules.RuleSet`, its dict form, or a path to a
...
- ``asset`` -- an :class:`~airflow.sdk.Asset` decorated with
  :func:`~airflow.providers.common.dataquality.assets.asset_quality`. Supplies defaults for ``ruleset``,
...
Results are persisted to the backend configured under ``[common.dataquality] results_path`` (see
...
:func:`~airflow.providers.common.dataquality.assets.asset_quality`, then pass the asset instead of ``table``/
``ruleset``/``conn_id``:
...
:func:`~airflow.providers.common.dataquality.assets.require_quality` (see :doc:`assets`) able to gate a
