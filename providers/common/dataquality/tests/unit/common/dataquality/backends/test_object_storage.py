...
from airflow.providers.common.dataquality.rules import Severity
...
class TestGetBackendFromConfig:
    @conf_vars({("common.dataquality", "results_path"): None})
    ...
    def test_results_path_builds_object_storage_backend(self, tmp_path):
        with conf_vars({("common.dataquality", "results_path"): f"file://{tmp_path}"}):
