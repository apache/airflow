...
| `id` | no | Explicit stable identity for this rule's history, used directly instead of the derived hash. Only set this when told to, e.g. to avoid a collision between a renamed rule and an unrelated rule that reuses its old name. |
