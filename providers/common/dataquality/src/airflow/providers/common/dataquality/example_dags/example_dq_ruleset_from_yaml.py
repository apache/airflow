...
A path string is accepted anywhere a :class:`~airflow.providers.common.dataquality.rules.RuleSet` is --
``DQCheckOperator(ruleset=...)``, ``@task.dq_check(ruleset=...)``, and
:func:`~airflow.providers.common.dataquality.assets.asset_quality` all resolve it via
:meth:`~airflow.providers.common.dataquality.rules.RuleSet.from_file` at Dag-parse time. This keeps rules
