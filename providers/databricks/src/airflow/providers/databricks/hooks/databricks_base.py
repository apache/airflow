...
class BaseDatabricksHook(BaseHook):
    ...
    def __init__(
        ...
        # Cache for async
        self._async_databricks_conn = None
    ...
    async def get_a_databricks_conn(self) -> Connection:
        ...
        return self._async_databricks_conn

    async def a_host(self) -> str | None:
        if self._async_host is None:
            conn = await self.get_a_databricks_conn()
    ...
    async def _a_endpoint_url(self, endpoint):
        ...
        ahost = await self.a_host()
        return f"{schema}://{ahost}{port}/{endpoint}"
                ...
                with attempt:
                    async with self._session.post(
                        ...
                        auth=aiohttp.BasicAuth(
                            self._get_connection_attr("login"), (await self.get_a_databricks_conn()).password
                        ),
                        data="grant_type=client_credentials&scope=all-apis",
                        headers={
                            **self.user_agent_header,
    ...
    async def _a_get_aad_token(self, resource: str) -> str:
        ...
        try:
            ...
            async for attempt in self._a_get_retry_object():
                with attempt:
                    if (await self.get_a_databricks_conn()).extra_dejson.get("use_azure_managed_identity", False):
                        client_id = (await self.get_a_databricks_conn()).extra_dejson.get(
                            "azure_managed_identity_client_id", None
                        )
                        # Managed identity authenticates against the link-local IMDS endpoint
                        # (169.254.169.254), which must be reached directly and is unsupported behind a
                        ...
                        async with AsyncClientSecretCredential(
                            client_id=self._get_connection_attr("login"),
                            client_secret=(await self.get_a_databricks_conn()).password,
                            tenant_id=(await self.get_a_databricks_conn()).extra_dejson["azure_tenant_id"],
                            **self._get_azure_credential_kwargs(),
                        ) as credential:
                            token = await credential.get_token(f"{resource}/.default")
                    jsn = {
    ...
    async def _a_get_aad_headers(self) -> dict:
        ...
        """
        headers = {}
        if "azure_resource_id" in (await self.get_a_databricks_conn()).extra_dejson:
            mgmt_token = await self._a_get_aad_token(AZURE_MANAGEMENT_ENDPOINT)
            headers["X-Databricks-Azure-Workspace-Resource-Id"] = (await self.get_a_databricks_conn()).extra_dejson[
                "azure_resource_id"
            ]
            headers["X-Databricks-Azure-SP-Management-Token"] = mgmt_token
        return headers
    ...
    async def _a_get_k8s_jwt_token(self) -> str:
        ...
        if "k8s_projected_volume_token_path" in (await self.get_a_databricks_conn()).extra_dejson:
            self.log.info("Using Kubernetes projected volume token")
    ...
    async def _a_get_k8s_projected_volume_token(self) -> str:
        ...
        projected_token_path: str = (await self.get_a_databricks_conn()).extra_dejson["k8s_projected_volume_token_path"]
    ...
    async def _a_get_k8s_token_request_api(self) -> str:
        ...
        audience = (await self.get_a_databricks_conn()).extra_dejson.get("audience", DEFAULT_K8S_AUDIENCE)
        expiration_seconds = (await self.get_a_databricks_conn()).extra_dejson.get("expiration_seconds", 3600)
        token_path = (await self.get_a_databricks_conn()).extra_dejson.get(
            "k8s_token_path", DEFAULT_K8S_SERVICE_ACCOUNT_TOKEN_PATH
        )
        namespace_path = (await self.get_a_databricks_conn()).extra_dejson.get(
            "k8s_namespace_path", DEFAULT_K8S_NAMESPACE_PATH
        )
    ...
    async def _a_get_federation_subject_token(self) -> tuple[str, str | None]:
        ...
        provider = (await self.get_a_databricks_conn()).extra_dejson.get("federated_token_provider")
        if provider:
            ...
            return subject_token, (await self.get_a_databricks_conn()).extra_dejson.get("client_id")
    ...
    async def _a_is_aws_federation(self) -> bool:
        """Async version of _is_aws_federation."""
        conn = await self.get_a_databricks_conn()
    ...
    async def _a_get_token(self, raise_error: bool = False) -> str | None:
        if "token" in (await self.get_a_databricks_conn()).extra_dejson:
            self.log.info(
                "Using token auth. For security reasons, please set token in Password field instead of extra"
            )
            return (await self.get_a_databricks_conn()).extra_dejson["token"]
        if not (await self.get_a_databricks_conn()).login and (await self.get_a_databricks_conn()).password:
            self.log.debug("Using token auth.")
            return (await self.get_a_databricks_conn()).password
        if "azure_tenant_id" in (await self.get_a_databricks_conn()).extra_dejson:
            if (await self.get_a_databricks_conn()).login == "" or (await self.get_a_databricks_conn()).password == "":
                raise AirflowException("Azure SPN credentials aren't provided")
            self.log.debug("Using AAD Token for SPN.")
        ...
        if (await self.get_a_databricks_conn()).extra_dejson.get("use_azure_managed_identity", False):
            ...
            return await self._a_get_aad_token(DEFAULT_DATABRICKS_SCOPE)
        if (await self.get_a_databricks_conn()).extra_dejson.get(DEFAULT_AZURE_CREDENTIAL_SETTING_KEY, False):
        ...
        if (await self.get_a_databricks_conn()).extra_dejson.get("service_principal_oauth", False):
            if (await self.get_a_databricks_conn()).login == "" or (await self.get_a_databricks_conn()).password == "":
                raise AirflowException("Service Principal credentials aren't provided")
            self.log.debug("Using Service Principal Token.")
            return await self._a_get_sp_token(self._get_oidc_token_service_url())
        if (await self.get_a_databricks_conn()).extra_dejson.get("federated_token_provider"):
            self.log.debug("Using OIDC token federation with a supplied token provider.")
            return await self._a_get_federated_databricks_token(self._get_oidc_token_service_url())
        if await self._a_is_aws_federation():
            self.log.debug("Using AWS IAM OIDC token federation.")
            return await self._a_get_federated_databricks_token(self._get_oidc_token_service_url())
        if (await self.get_a_databricks_conn()).login == "federated_k8s" or (await self.get_a_databricks_conn()).extra_dejson.get(
            "federated_k8s", False
        ):
            self.log.debug("Using Kubernetes OIDC token federation.")
            return await self._a_get_federated_databricks_token(self._get_oidc_token_service_url())
    ...
    async def _a_do_api_call(self, endpoint_info: tuple[str, str], json: dict[str, Any] | None = None):
        ...
        if token:
            ...
            self.log.info("Using basic auth.")
            login = (await self.get_a_databricks_conn()).login
            if not login:
                raise ValueError("`login` must be present in Connection")
            auth = aiohttp.BasicAuth(login, (await self.get_a_databricks_conn()).password)
