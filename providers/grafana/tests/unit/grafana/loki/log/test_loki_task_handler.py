...
class StubRuntimeTI:
    ...
    def log_relative_path(self, try_number=1):
        return f"test_dag/test_task/test_run/{try_number}.log"


@pytest.fixture
def mock_runtime_ti():
    return StubRuntimeTI()
...
def test_read_generates_optimized_logql(tmp_log_folder, mock_runtime_ti):
    ...
    with requests_mock.Mocker() as m:
        ...
        assert (
            query_param
            == '{job="airflow_tasks",dag_id="test_dag"} | json | task_id="test_task" | run_id="test_run" | try_number="1" | map_index="-1"'
        )
