...
class KubernetesPodTrigger(BaseTrigger):
    ...
    async def _wait_for_container_completion(self) -> TriggerEvent:
        ...
        if self.logging_interval is not None:
            # if required to read logs then read logs first time after 10 seconds and then switch to logging interval
            # to inform the user that something is happening in the pod
            time_get_more_logs = time_begin + datetime.timedelta(seconds=10)
        while True:
            pod = await self._get_pod()
            ...
            if time_get_more_logs and datetime.datetime.now(tz=datetime.timezone.utc) > time_get_more_logs:
                if self.get_logs and self.logging_interval:
                    ...
                    time_get_more_logs = datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(
