...
def inject_openlineage_properties_into_dataproc_job_config(
    job: dict, context: Context, inject_parent_job_info: bool
) -> dict:
    """
    Inject OpenLineage properties into Spark job configuration.

    It's not removing any configuration or modifying the job in any other way.
    This function add desired OpenLineage properties to Dataproc job configuration.
    ...
    Note:
        Any modification to job will be skipped if:
            ...
            - Any OpenLineage properties with parent job information are already present in the Spark job configuration.
    ...
    Returns:
        The modified job configuration with OpenLineage properties injected, if applicable.
    """
    ...
    if not _is_openlineage_provider_accessible():
        log.warning(
            ...
            "properties injection. Spark job configuration will remain unchanged."
        )
        return job
