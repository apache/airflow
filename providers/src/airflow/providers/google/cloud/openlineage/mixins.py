...
class _BigQueryInsertJobOperatorOpenLineageMixin:
    ...
    def _deduplicate_outputs(self, outputs: Sequence[OutputDataset | None]) -> list[OutputDataset]:
        final_outputs = {}
        for single_output in outputs:
            ...
            if (
                ...
                single_output.facets["columnLineage"] = self._merge_column_lineage_facets(
                    [
    ...
    def _get_inputs_and_outputs_for_copy_job(
        ...
        inputs = tuple(self._get_input_dataset(input_table) for input_table in input_tables)
    ...
    def _get_column_level_lineage_facet_for_query_job(
        self, properties: dict, output: OutputDataset, inputs: Sequence[InputDataset]
    ...
    @staticmethod
    def _merge_column_lineage_facets(facets: list[ColumnLineageDatasetFacet]) -> ColumnLineageDatasetFacet:
        """
        Merge multiple column lineage facets into a single facet.

        Note:
            Transformation information will be lost if present.

        Args:
            facets: A list of column lineage facets to be merged.

        Returns:
            A single merged column lineage facet.
        """
        merged_fields: dict[str, set[tuple[str, str, str]]] = defaultdict(set)

        for facet in facets:
            for field_name, field in facet.fields.items():
                merged_fields[field_name].update(
                    (input_field.namespace, input_field.name, input_field.field)
                    for input_field in field.inputFields
                )
        ...
        return ColumnLineageDatasetFacet(
            fields={
                field_name: Fields(
                    inputFields=[
                        InputField(namespace, name, column)
                        for namespace, name, column in sorted(input_fields)
                    ],
                    transformationType="",
                    transformationDescription="",
                )
                for field_name, input_fields in merged_fields.items()
            }
        )
