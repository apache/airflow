...
class NameDeterminer:
    ...
    def __init__(
        self, label: str, id_label: str, get_path: Callable[[str, str, str], str]
    ) -> None:
        self.label = label
...
class CloudVisionHook(GoogleBaseHook):
    ...
    def create_product(
        ...
        if not product_id:
            ...
            self.log.info(
                "Extracted autogenerated Product ID from the response: %s", product_id
            )
    ...
    def remove_product_from_product_set(
        ...
        self.log.info(
            "Remove Product[name=%s] from Product Set[name=%s]",
            product_name,
            product_set_name,
        )
    ...
    def batch_annotate_images(
        ...
        response = client.batch_annotate_images(
            requests=requests, retry=retry, timeout=timeout
        )
    ...
    def _get_autogenerated_id(response) -> str:
        try:
            ...
            raise AirflowException(
                f"Unable to get name from response... [{response}]\n{e}"
            )
        if "/" not in name:
