    ...
    def _process_output(
        ...
        if last_description is None:
            raise AirflowException(
                "There is missing description present for the output file. ."
            )
    ...
    def _create_sql_query(self) -> str:
        ...
        if self._encryption is not None or self._credential is not None:
            ...
            if self._encryption is not None:
                maybe_encryption = self._generate_options(
                    "ENCRYPTION", escaper, self._encryption, False
                )
            maybe_credential = ""
            if self._credential is not None:
                maybe_credential = self._generate_options(
                    "CREDENTIAL", escaper, self._credential, False
                )
        ...
        format_options = (
            self._generate_options("FORMAT_OPTIONS", escaper, self._format_options) + "\n"
        )
        copy_options = (
            self._generate_options("COPY_OPTIONS", escaper, self._copy_options) + "\n"
        )
        storage_cred = ""
        ...
        if self._validate is not None:
            if isinstance(self._validate, bool):
                ...
                raise AirflowException(
                    f"Incorrect data type for validate parameter: {type(self._validate)}"
                )
