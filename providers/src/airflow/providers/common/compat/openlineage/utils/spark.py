        ...
        def inject_parent_job_information_into_spark_properties(properties: dict, context) -> dict:
            ...
            if any(str(key).startswith("spark.openlineage.parent") for key in properties):
                log.info(
                    ...
                    "in the Spark job configuration. Skipping the automatic injection of OpenLineage "
            ...
            log.info("Successfully injected OpenLineage parent job information into Spark job configuration.")
            return properties
