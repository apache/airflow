...
Extra (optional)
    ...
    * ``host_key`` - The base64 encoded ssh-rsa public key of the host or "ssh-<key type> <key data>" (as you would find in the ``known_hosts`` file). Specifying this allows making the connection if and only if the public key of the endpoint matches this value. Supported key types are ``rsa``, ``ecdsa``, and ``ed25519``. DSA/DSS (``ssh-dss``) host keys are not supported because `paramiko 4.0 removed DSS support <https://www.paramiko.org/changelog.html>`__; generate a new host key and update this field.
