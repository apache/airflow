...
def _validate_job_dir(
    ...
    :param base_dir: The base directory job_dir is expected to be under. Defaults to the
        standard POSIX/Windows base directory, but callers configured with a custom
        ``remote_base_dir`` must pass it here so job_dir is validated against the base
        directory that was actually used to build it.
    ...
    expected_prefix = base_dir + ("\\" if remote_os == "windows" else "/")
...
def build_posix_cleanup_command(job_dir: str, base_dir: str | None = None) -> str:
    ...
    :param base_dir: The configured base directory job_dir is expected to be under
        (see :func:`_validate_job_dir`). Defaults to the standard base directory.
    :return: Shell command to remove the directory
    :raises ValueError: If job_dir is not under the expected base directory
    """
    _validate_job_dir(job_dir, "posix", base_dir=base_dir)
...
def build_windows_cleanup_command(job_dir: str, base_dir: str | None = None) -> str:
    ...
    :raises ValueError: If job_dir is not under the expected base directory
    """
    _validate_job_dir(job_dir, "windows", base_dir=base_dir)
