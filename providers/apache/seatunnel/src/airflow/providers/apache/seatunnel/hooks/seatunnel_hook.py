...
class SeaTunnelHook(BaseHook):
    ...
    conn_type = "apache_seatunnel"
    hook_name = "Apache SeaTunnel"

    def __init__(self, seatunnel_conn_id: str = default_conn_name, engine: str = "zeta") -> None:
        super().__init__()
        ...
        seatunnel_home = self.extra_dejson.get("seatunnel_home")
        if not seatunnel_home:
            raise AirflowException("SeaTunnel home directory must be specified in the connection.")
        # Ensure seatunnel_home is a string for type checker
        self.seatunnel_home: str = str(seatunnel_home)

    @staticmethod
    def get_ui_field_behaviour() -> dict:
        """Return custom field behavior for connection form."""
        return {
    ...
    def run_job(self, config_file: str, engine: str | None = None) -> str:
        """
        Run a SeaTunnel job with the given configuration file.

        :param config_file: Path to the SeaTunnel configuration file.
        :param engine: Engine to use (flink, spark, or zeta).
                        Overrides the instance's engine.
        ...
        if engine not in ["flink", "spark", "zeta"]:
            raise AirflowException(
                f"Unsupported engine: {engine}. Must be one of 'flink', 'spark', or 'zeta'."
            )
