...
class TestSparkSubmitOperatorK8sTracking:
    ...
    def test_get_hook_passes_kubernetes_driver_container_name(self):
        operator = self._make_operator(kubernetes_driver_container_name="custom-driver")

        hook = operator._get_hook()

        assert hook.kubernetes_driver_container_name == "custom-driver"

    def test_kubernetes_driver_container_name_is_templatable(self):
        assert "kubernetes_driver_container_name" in SparkSubmitOperator.template_fields
    ...
    def test_k8s_submit_job_returns_encoded_external_id(self):
        operator = self._make_operator(track_driver_via_k8s_api=True)
        hook = self._make_k8s_hook()
        hook._kubernetes_driver_pod_name = "spark-abc-driver"
    ...
    def test_k8s_submit_job_raises_when_pod_name_missing(self):
        operator = self._make_operator(track_driver_via_k8s_api=True)
        hook = self._make_k8s_hook()
        hook._kubernetes_driver_pod_name = None
        hook._connection = {"namespace": "mynamespace"}
        operator._hook = hook
    ...
    def test_k8s_poll_until_complete_sets_pod_name_and_calls_poll_api(self):
        ...
        assert hook._kubernetes_driver_pod_name == "spark-abc-driver"
        hook._poll_k8s_driver_via_api.assert_called_once()
