        ...
        "config": {
            "kafka_listener": {
                ...
                "options": {
                    ...
                    "config_dict": {
                        "description": "JSON confluent-kafka client configuration for the listener's\nproducer. Each key overrides the same key from the connection's\nconfig. When it provides ``bootstrap.servers``, it allows running\nthe listener without a Kafka Airflow connection. Callback options\n(e.g. ``error_cb``, ``oauth_cb``) may be given as dotted-path\nstrings and are resolved to callables before the producer is built.\n",
                        "version_added": "1.14.1",
                        "type": "string",
                        "example": '{"bootstrap.servers": "broker:29092"}',
                        "default": "",
                    },
                    "topic": {
