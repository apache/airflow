...
@lru_cache(maxsize=1)
def _get_topic_dagrun():
    return conf.get(CONFIG_SECTION, TOPIC_CONFIG_MAP[EventProducerKafkaTopic.DAG_RUN], fallback=DEFAULT_TOPIC)

@lru_cache(maxsize=1)
def _get_topic_task_instance():
    return conf.get(CONFIG_SECTION, TOPIC_CONFIG_MAP[EventProducerKafkaTopic.TASK_INSTANCE], fallback=DEFAULT_TOPIC)
...
def _get_topic(topic_type: EventProducerKafkaTopic) -> str:
    ...
    if topic is not None:
        ...
        return topic
    return _get_topic_map[topic_type]()
...
_get_topic_map = {
    EventProducerKafkaTopic.DAG_RUN: _get_topic_dagrun,
    EventProducerKafkaTopic.TASK_INSTANCE: _get_topic_task_instance,
}
