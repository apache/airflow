...
To enable event publishing you need to
  ...
  * configure a broker address — either through an Airflow Kafka connection
    (``kafka_config_id``) or directly with ``config_dict``
...
.. code-block:: ini
    ...
    config_dict = {"bootstrap.servers": "broker1:9092,broker2:9092"}
    topic = airflow.events

Alternatively, point the listener at an Airflow Kafka connection and keep
the client config on the connection's extra:
...
``config_dict`` is a JSON object accepting the full confluent-kafka client
...
before the producer is built. When both ``kafka_config_id`` and
``config_dict`` are set, each ``config_dict`` key overrides the same key
from the connection's config.

.. code-block:: ini

    [kafka_listener]
    config_dict = {"bootstrap.servers": "broker:9092", "security.protocol": "SASL_SSL",
        "sasl.mechanisms": "OAUTHBEARER", "oauth_cb": "my_company.auth.oauth_cb"}
...
.. code-block:: ini
    ...
    AIRFLOW__KAFKA_LISTENER__CONFIG_DICT={"bootstrap.servers": "broker1:9092,broker2:9092"}
