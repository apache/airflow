...
_CONFIG_DICT_JSON = '{"bootstrap.servers": "broker:29092"}'
...
def _clear_cached_values():
    ...
    listener._get_config_dict.cache_clear()
...
@conf_vars(
    {
        ("kafka_listener", "dag_run_events_enabled"): "True",
        ("kafka_listener", "config_dict"): _CONFIG_DICT_JSON,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("   ", None, id="whitespaces_only"),
        pytest.param(
            _CONFIG_DICT_JSON,
...
def test_get_config_dict(raw_value, expected):
    with conf_vars({("kafka_listener", "config_dict"): raw_value}):
        assert listener._get_config_dict() == expected


def test_get_config_dict_not_a_json_object():
    with conf_vars({("kafka_listener", "config_dict"): '["broker:29092"]'}):
        with pytest.raises(ValueError, match="must be a JSON object"):
            listener._get_config_dict()
...
class TestGetProducer:
    ...
    def test_invalid_config_dict_warns_and_returns_none(self, monkeypatch):
        log_warning_mock = MagicMock()
        monkeypatch.setattr(listener.log, "warning", log_warning_mock)

        with conf_vars({("kafka_listener", "config_dict"): "{not json"}):
            with patch(_PRODUCER_CLS) as producer_cls_mock:
                assert listener._get_producer() is None

        producer_cls_mock.assert_not_called()
        log_warning_mock.assert_called_once()
        assert "failed to initialize producer" in log_warning_mock.call_args.args[0]
    ...
    def test_hook_built_from_listener_config(self):
        """The listener section options are translated into KafkaProducerHook params."""
        hook_mock = MagicMock()
        kafka_config_id = "kafka_default"

        with conf_vars({("kafka_listener", "kafka_config_id"): kafka_config_id}):
        ...
        hook_cls_mock.assert_called_once_with(
            ...
            config_dict=json.loads(_CONFIG_DICT_JSON),
        )
    ...
    def test_hook_defaults_when_options_unset(self):
        ...
        # Override the class fixture's `config_dict` back to empty.
        with conf_vars({("kafka_listener", "config_dict"): ""}):
            with patch(_PRODUCER_HOOK_CLS, return_value=hook_mock) as hook_cls_mock:
                listener._get_producer()

        # Unset options must not be passed to the hook at all (e.g. as "" or None),
        # so the hook's own defaults (the "kafka_default" connection) apply.
        hook_cls_mock.assert_called_once_with()
