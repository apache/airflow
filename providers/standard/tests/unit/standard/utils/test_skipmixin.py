...
from airflow.providers.standard.utils.skipmixin import XCOM_SKIPMIXIN_KEY, SkipMixin
...
class TestSkipMixin:
    ...
    def test_skip__only_mapped_operators_passed(self):
        ti = Mock(map_index=2, dag_id="test_dag", task_id="test_task")
        ...
        # Create a SkipMixin instance with a task_id
        skip_mixin = SkipMixin()
        skip_mixin.task_id = "test_task"
        skip_mixin.log = MagicMock()
        ...
        with patch("airflow.providers.standard.utils.skipmixin.getattr") as mock_getattr:
            ...
            with patch("airflow.providers.standard.utils.skipmixin.hasattr", return_value=True):
                # Test that DownstreamTasksSkipped is raised
                with pytest.raises(DownstreamTasksSkipped) as exc_info:
                    skip_mixin.skip(
                        ti=ti,
                        tasks=[task_mock],
                ...
                # Verify XCom was set correctly
                xcom_push.assert_called_once_with(key=XCOM_SKIPMIXIN_KEY, value={"skipped": ["mapped_task"]})
                # Verify the exception contains the correct tasks
                assert exc_info.value.tasks == ["mapped_task"]

    @pytest.mark.skipif(not AIRFLOW_V_3_0_PLUS, reason="Airflow 2 had a different implementation")
    def test_skip__only_none_mapped_operators_passed(self):
        ti = Mock(map_index=-1, dag_id="test_dag", task_id="test_task")
