...
class TestSkipMixin:
    ...
    # FIXME: This test needs better adjustments for testing Mapped Operators in Airflow 3
    ...
    def test_mapped_tasks_skip_all_except(self, dag_maker):
        ...
        assert (
            get_state(branch_b_ti_0) == State.NONE if AIRFLOW_V_3_0_PLUS else State.SKIPPED
        )  # FIXME: This should be always State.SKIPPED
        assert (
            get_state(branch_a_ti_1) == State.NONE if AIRFLOW_V_3_0_PLUS else State.SKIPPED
        )  # FIXME: This should be always State.SKIPPED
