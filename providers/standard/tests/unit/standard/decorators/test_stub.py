...
from airflow.providers.standard.decorators.stub import _infer_value_schema, _StubOperator, stub
...
class TestMappedStubArgBindingParams:
    """The serializer hook captures ordered per-parameter metadata for mapped stubs."""

    def get_hook_fields(self, operator):
        return _StubOperator.get_mapped_serialized_fields(operator)
    ...
    def test_mapped_stub_inside_mapped_task_group_unconstructible(self):
        """The SDK bans expansion inside an expanded group outright, so no hook guard is needed."""
