...
class DbtCloudJobRunDetailsException(AirflowException):
    ...
    def __str__(self):
        ...
        job_run_msg = f" Job run details: {json.dumps({k: self.run_details[k] for k in run_data_keys if k in self.run_details})}"
        ...
        try:
            ...
            for s in self.run_details["run_steps"]:
                run_steps_msg += json.dumps({k: s[k] for k in run_steps_keys if k in s})
