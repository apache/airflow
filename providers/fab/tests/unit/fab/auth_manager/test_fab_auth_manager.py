...
class TestFabAuthManager:
    ...
    def test_get_fastapi_middlewares_disabled(self, flask_app, auth_manager_with_appbuilder):
        ...
        previous = flask_app.config.get("AUTH_ROLE_PUBLIC")
        flask_app.config["AUTH_ROLE_PUBLIC"] = None
        middleware = [auth_manager_with_appbuilder.get_jwt_refresh_middleware()] if AIRFLOW_V_3_4_PLUS else []

        try:
            assert auth_manager_with_appbuilder.get_fastapi_middlewares() == middleware
        finally:
            flask_app.config["AUTH_ROLE_PUBLIC"] = previous
    ...
    def test_get_fastapi_middlewares_enabled(self, flask_app, auth_manager_with_appbuilder):
        ...
        base_middleware = (
            [auth_manager_with_appbuilder.get_jwt_refresh_middleware()] if AIRFLOW_V_3_4_PLUS else []
        )
        try:
