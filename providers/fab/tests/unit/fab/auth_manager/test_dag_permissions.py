...
class TestDagPermissions:
    ...
    def test_cleanup_dag_permissions_with_session(self):
        ...
        with patch("airflow.providers.fab.auth_manager.models") as mock_models:
            ...
            # Should call session.query for Resource
    ...
    def test_cleanup_dag_permissions_with_resources_and_permissions(self):
        ...
        with patch("airflow.providers.fab.auth_manager.models") as mock_models:
            ...
            mock_session.query.return_value.filter.return_value.all.return_value = [mock_dag_resource]
            ...
            # Should perform deletion operations
            assert mock_session.query.call_count >= 3  # Resource, Permission, assoc queries
        ...
        with patch("airflow.providers.fab.auth_manager.models") as mock_models:
            ...
            def mock_query_side_effect(model):
                ...
                if model == mock_resource:
                    ...
                    mock_query.filter.return_value.all.return_value = []  # No permissions
                    mock_query.filter.return_value.delete.return_value = None
                return mock_query
            ...
            # Should still delete resources even without permissions
            assert mock_session.query.call_count >= 2  # Resource queries
