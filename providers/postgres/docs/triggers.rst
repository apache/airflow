...
.. exampleinclude:: /../providers/postgres/tests/system/postgres/example_postgres_cdc_asset_watcher.py
    :language: python
...
=================== ============================================================================
``conn_id``         Airflow connection ID for the PostgreSQL database
``table``           Name of the table to monitor
``cdc_column``      Column used to track changes (e.g., ``updated_at``)
``polling_interval``  Polling frequency in seconds (default: 10.0)
``state_key``       Key used to retrieve the last known CDC value via ``Variable.get()``
=================== ============================================================================
