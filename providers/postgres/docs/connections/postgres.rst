...
Extra (optional)
    ...
    * ``cursor`` - Specifies the cursor type to use when querying the database. You can choose one of the following:
      ...
      - ``dictcursor``: Returns query results as Python dictionaries using ``psycopg.extras.DictCursor``.
      - ``realdictcursor``: Similar to ``DictCursor``, but uses ``psycopg.extras.RealDictCursor`` for slightly better performance.
      - ``namedtuplecursor``: Returns query results as named tuples using ``psycopg.extras.NamedTupleCursor``.

      For more information, refer to the psycopg documentation on `connection classes <https://www.psycopg.org/psycopg3/docs/api/connections.html>`_.
