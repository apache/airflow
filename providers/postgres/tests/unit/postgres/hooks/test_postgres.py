...
class TestPostgresHookConnPPG2:
    ...
    def test_get_conn_non_default_id(self, mocker):
        mock_connect = mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
    ...
    def test_get_conn_with_invalid_cursor(self, mocker):
        mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
        self.connection.extra = '{"cursor": "mycursor"}'
        with pytest.raises(ValueError):
            self.db_hook.get_conn()

    def test_get_conn_from_connection(self, mocker):
        mock_connect = mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
        conn = Connection(login="login-conn", password="password-conn", host="host", schema="database")
        hook = PostgresHook(connection=conn)
        hook.get_conn()
    ...
    def test_get_conn_from_connection_with_database(self, mocker):
        mock_connect = mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
        conn = Connection(login="login-conn", password="password-conn", host="host", schema="database")
    ...
    def test_get_conn_from_connection_with_options(self, mocker):
        mock_connect = mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
        conn = Connection(login="login-conn", password="password-conn", host="host", schema="database")
    ...
    def test_get_conn_rds_iam_postgres(self, mocker, aws_conn_id, port):
        mock_connect = mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
    ...
    def test_get_conn_extra(self, mocker):
        mock_connect = mocker.patch("airflow.providers.postgres.hooks.postgres.psycopg2.connect")
...
class TestPostgresHookConnPPG3:
    ...
    def test_get_uri(self, mocker):
        ...
        self.db_hook.get_conn()
        assert mock_connect.call_count == 1
        assert self.db_hook.get_uri() == "postgresql+psycopg://login:password@host:5432/database"
    ...
    def test_get_conn_with_invalid_cursor(self, mocker):
        mocker.patch("psycopg.connection.Connection.connect")
        self.connection.extra = '{"cursor": "mycursor"}'
        with pytest.raises(ValueError):
            self.db_hook.get_conn()
    ...
    def test_get_uri_from_connection_without_database_override(self, mocker):
        ...
        assert self.db_hook.get_uri() == "postgresql+psycopg://login:password@host:1/database"
    ...
    def test_get_uri_from_connection_with_database_override(self, mocker):
        ...
        assert hook.get_uri() == "postgresql+psycopg://login:password@host:1/database-override"
...
class TestPostgresHookPPG3:
    ...
    def test_get_all_db_log_messages(self, mocker):
        raise pytest.xfail(
            reason="Notice handling is callback-based in psycopg3 and cannot be tested this way."
        )
