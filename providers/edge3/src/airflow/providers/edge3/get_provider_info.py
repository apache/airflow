        ...
        "config": {
            "edge": {
                ...
                "options": {
                    ...
                    "jwt_audience": {
                        "description": "Expected ``aud`` claim of edge worker tokens when ``trusted_jwks_url`` is\nset. Leave empty to skip audience verification, which is required for\nprovider tokens that do not carry an ``aud`` claim. Ignored when\n``trusted_jwks_url`` is empty.\n",
                        "version_added": "4.4.0",
                        "type": "string",
                        "example": "api",
                        "default": None,
