        ...
        "config": {
            "edge": {
                ...
                "options": {
                    ...
                    "trusted_jwks_url": {
                        "description": "When set, edge worker tokens are verified against this JSON Web Key Set\n(JWKS) URL of a trusted OpenID Connect provider, using asymmetric\nsignatures, instead of the shared ``[api_auth] jwt_secret``. This lets\nedge workers authenticate with tokens minted by an external identity\nprovider.\n\nWhen empty (the default) the shared-secret (symmetric) verification is\nused, so existing deployments are unaffected.\n\nSecurity note: enabling this disables the per-request ``method`` claim\ncheck that the shared-secret path uses to scope a token to a single\nendpoint (externally issued tokens do not carry it). A valid signature\nthen only proves who signed the token, not that the identity may act as\nan edge worker. If the issuer serves other workloads, configure\n``jwt_issuer`` and a ``jwt_verifier`` to restrict which identities are\naccepted; otherwise any validly signed token from the issuer is accepted.\n",
                        "version_added": "4.4.0",
                        "type": "string",
                    ...
                    "jwt_issuer": {
                        "description": "Expected ``iss`` claim of edge worker tokens when ``trusted_jwks_url`` is\nset. Leaving it empty skips issuer verification, so a token from any issuer\nwhose key is in the JWKS is accepted; in that case ``jwt_verifier`` must be\nconfigured to decide which identities may act as edge workers (requests are\nrejected otherwise). Set it to bind tokens to your provider. Ignored when\n``trusted_jwks_url`` is empty.\n",
                        "version_added": "4.4.0",
                        "type": "string",
                    ...
                    "jwt_audience": {
                        "description": "Expected ``aud`` claim of edge worker tokens when ``trusted_jwks_url`` is\nset. When set, the token must carry a matching ``aud``. When empty, only\ntokens that carry no ``aud`` claim are accepted (a token that does carry\nan ``aud`` is rejected). Ignored when ``trusted_jwks_url`` is empty.\n",
                        "version_added": "4.4.0",
                        "type": "string",
                        "example": "api",
                        "default": None,
                    ...
                    "jwt_algorithm": {
                        "description": "Comma-separated list of JWS signing algorithms accepted for edge worker\ntokens when ``trusted_jwks_url`` is set. Set it to match the algorithm the\ntrusted provider signs with. Ignored when ``trusted_jwks_url`` is empty.\n",
                        "version_added": "4.4.0",
                        "type": "string",
                        "example": "RS256,RS512",
                        "default": "RS256",
                    ...
                    "jwt_leeway": {
                        "description": "Number of seconds of leeway allowed when validating the expiry and issued-at\nclaims of edge worker tokens issued by a trusted OIDC provider, to account for\nclock skew between the identity provider and the API server. Ignored when\n``trusted_jwks_url`` is empty.\n",
                        "version_added": "4.4.0",
                        "type": "integer",
                        "example": None,
                        "default": "30",
                    ...
                    "jwt_verifier": {
                        "description": 'Import path of a callable that authorizes edge worker tokens issued by a\ntrusted OIDC provider, run after signature, issuer and audience checks pass.\nIt receives the validated claims (``dict``) and a request context object\ncarrying the request ``method``, and returns a mapping with an\n``authorized`` boolean (e.g. ``{"authorized": True}``); a falsy result or a\nraised exception rejects the request. Use it to restrict which identities\n(for example by ``sub``, ``client_id`` or ``scope``) may act as edge workers.\nRequired when ``trusted_jwks_url`` is set but ``jwt_issuer`` is empty, since\nissuer verification is then skipped. When empty (and ``jwt_issuer`` is set)\nany token that passes signature, issuer and audience verification is\nauthorized. Ignored when ``trusted_jwks_url`` is empty.\n',
                        "version_added": "4.4.0",
                        "type": "string",
