...
# under the License.
"""
This Dag will use Papermill to run the notebook "hello_world" on a kernel started through a
Jupyter server or kernel gateway (e.g. JupyterHub, Jupyter Kernel Gateway, Enterprise Gateway)
reached over HTTP(S) with token authentication. The ``jupyter_kernel_default`` connection
must have an ``http(s)://`` URL as host and the auth token as password.
"""
...
DAG_ID = "example_papermill_operator_gateway"


# [START howto_operator_papermill_gateway_kernel]
@task
...
# [END howto_operator_papermill_gateway_kernel]
