...
class RedisHook(BaseHook):
    ...
    def _build_startup_nodes(self) -> list[ClusterNode]:
        ...
        if isinstance(self.startup_nodes, str):
            entries = self.startup_nodes.split(",")
        elif isinstance(self.startup_nodes, list) and all(
            isinstance(entry, str) for entry in self.startup_nodes
        ):
            entries = self.startup_nodes
        else:
            raise ValueError(
                "`startup_nodes` must be a comma-separated string or a list of strings, got "
