...
def get_backend_from_config() -> ObjectStorageResultsBackend | None:
    """Build the results backend configured under ``[dataquality]``, or ``None`` when not configured."""
    from airflow.providers.common.compat.sdk import conf

    results_path = conf.get("dataquality", "results_path", fallback=None)
    if not results_path:
        return None
    conn_id = conf.get("dataquality", "results_conn_id", fallback=None)
