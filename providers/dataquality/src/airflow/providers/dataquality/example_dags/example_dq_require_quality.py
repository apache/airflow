...
- ``example_dq_require_quality_producer`` checks a table and produces the asset.
  :class:`~airflow.providers.dataquality.operators.dq_check.DQCheckOperator` attaches its summary
  (including the quality ``score``) to the asset event automatically because the asset carries
  quality config attached with :func:`~airflow.providers.dataquality.assets.asset_quality`.
- ``example_dq_require_quality_consumer`` is scheduled by that asset. Its first task, built by
  :func:`~airflow.providers.dataquality.assets.require_quality`, reads the score off the triggering
...
from airflow.providers.dataquality.assets import asset_quality, require_quality
