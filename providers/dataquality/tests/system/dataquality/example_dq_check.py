...
infrastructure required). Results are persisted through the ``[dataquality] results_path`` config
...
# shares the one store configured under [dataquality] results_path. setdefault() so a real deployment's
