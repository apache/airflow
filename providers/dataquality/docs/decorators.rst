...
``@task.dq_check`` wraps :class:`~airflow.providers.dataquality.operators.dq_check.DQCheckOperator` in
...
Return a :class:`~airflow.providers.dataquality.rules.RuleSet`, its dict form, or a YAML path to use a
...
.. exampleinclude:: /../src/airflow/providers/dataquality/example_dags/example_dq_check_decorator_dynamic.py
    :language: python
