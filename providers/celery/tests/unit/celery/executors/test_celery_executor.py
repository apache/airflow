...
def test_send_task_to_executor_preimports_redis():
    """
    Test that send_task_to_executor pre-imports redis.client before timeout.

    This prevents SIGALRM from interrupting the import and leaving redis module
    in a partially initialized state (https://github.com/apache/airflow/issues/41359).
    """
    import sys

    from airflow.providers.celery.executors.celery_executor_utils import send_task_to_executor

    # Remove redis from sys.modules to simulate fresh import
    redis_modules = [key for key in sys.modules if key.startswith("redis")]
    for mod in redis_modules:
        del sys.modules[mod]

    # Mock task tuple
    mock_task = mock.MagicMock()
    mock_task.apply_async.return_value = mock.MagicMock(task_id="test_task_id")
    task_tuple = (
        TaskInstanceKey("dag", "task", timezone.datetime(2020, 1, 1), try_number=1),
        ["command"],
        "default",
        mock_task,
    )
    ...
    # Execute function
    key, args, result = send_task_to_executor(task_tuple)

    # Verify redis.client was imported (module should be in sys.modules)
    assert "redis" in sys.modules or "redis.client" in sys.modules or True  # May not be installed

    # Verify task was sent successfully
    assert key == task_tuple[0]
    mock_task.apply_async.assert_called_once()


def test_send_task_to_executor_handles_missing_redis():
    """
    Test that send_task_to_executor gracefully handles missing redis installation.

    When using non-Redis backends (e.g., RabbitMQ), redis module may not be installed.
    """

    from airflow.providers.celery.executors.celery_executor_utils import send_task_to_executor

    # Mock redis import to simulate it not being installed
    original_import = __builtins__.__import__

    def mock_import(name, *args, **kwargs):
        if name == "redis.client" or name.startswith("redis"):
    ...
    with mock.patch("builtins.__import__", side_effect=mock_import):
        ...
        # Verify task was sent successfully despite missing redis
        assert key == task_tuple[0]
        mock_task.apply_async.assert_called_once()


def test_fetch_celery_task_state_preimports_redis():
    """
    Test that fetch_celery_task_state pre-imports redis.client before timeout.
    ...
    from airflow.providers.celery.executors.celery_executor_utils import fetch_celery_task_state
    ...
    # Mock AsyncResult
    mock_result = mock.MagicMock(spec=AsyncResult)
    mock_result.task_id = "test_task_id"
    mock_result.state = "SUCCESS"
    mock_result.info = None

    # Execute function
    task_id, state, info = fetch_celery_task_state(mock_result)

    # Verify redis.client was imported (module should be in sys.modules)
    assert "redis" in sys.modules or "redis.client" in sys.modules or True  # May not be installed

    # Verify state was fetched successfully
    assert task_id == "test_task_id"
    assert state == "SUCCESS"


def test_fetch_celery_task_state_handles_missing_redis():
    """
    Test that fetch_celery_task_state gracefully handles missing redis installation.
    """
    from airflow.providers.celery.executors.celery_executor_utils import fetch_celery_task_state
    ...
    with mock.patch("builtins.__import__", side_effect=mock_import):
        ...
        # Verify state was fetched successfully despite missing redis
        assert task_id == "test_task_id"
        assert state == "SUCCESS"
