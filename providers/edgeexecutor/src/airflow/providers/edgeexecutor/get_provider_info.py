...
def get_provider_info():
    return {
        "package-name": "apache-airflow-providers-edgeexecutor",
        "name": "Edge Executor",
        "description": "Handle edge workers on remote sites via HTTP(s) connection and orchestrates work over distributed sites\n",
        "plugins": [
            {
                "name": "edge_executor",
                "plugin-class": "airflow.providers.edgeexecutor.plugins.edge_executor_plugin.EdgeExecutorPlugin",
            }
        ],
        "executors": ["airflow.providers.edgeexecutor.executors.EdgeExecutor"],
        "config": {
            "edge": {
