...
def sanitize_uri(uri: SplitResult) -> SplitResult:
    if not uri.netloc:
        raise ValueError("URI format presto:// must contain a host")
    if uri.port is None:
    ...
    if len(uri.path.split("/")) != 4:  # Leading slash, catalog, schema, and table names.
        raise ValueError("URI format presto:// must contain a catalog, schema, and table")
    return uri
...
def create_asset(
    *, host: str, catalog: str, schema: str, table: str, port: int = 8080, extra: dict | None = None
    ...
    return Asset(uri=f"presto://{host}:{port}/{catalog}/{schema}/{table}", extra=extra)
...
def convert_asset_to_openlineage(asset: Asset, lineage_context) -> OpenLineageDataset:
    ...
    return OpenLineageDataset(namespace=f"presto://{parsed.netloc}", name=f"{catalog}.{schema}.{table}")
