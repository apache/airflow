...
def _init_argparse() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m airflow_google_provider_resource_cleanup",
...
def main():
    ...
    if hours >= 1:
        ...
        parts.append(f"{hours_count} hour{'s' if hours_count != 1 else ''}")
    if minutes >= 1:
        minutes_count = int(minutes)
        parts.append(f"{minutes_count} minute{'s' if minutes_count != 1 else ''}")
