...
async def test_handle_delete_skips_resources_without_asset_type(mock_args, handle_delete_dependencies):
    handle_delete_dependencies["load_json_fn"].return_value = [
        {"name": "missing-asset-type"},
        {"name": "none-asset-type", "assetType": None},
        {"name": "compute-disk", "assetType": "compute.googleapis.com/Disk"},
    ]
    ...
    assert calls_by_asset_type["compute"] == [
        {"name": "compute-disk", "assetType": "compute.googleapis.com/Disk"}
    ]


def test_get_delete_handlers_registers_implemented_handlers():
...
def test_asset_type_patterns_are_unique():
    patterns = list(constants.ASSET_TYPES.values())
...
def test_check_min_age():
    ...
    assert check_min_age({"name": "old", "createTime": "2026-05-16T00:00:00Z"}, 3, now=now)
