...
class TestGCSDagBundle:
    ...
    def test_supports_versioning(self):
        ...
        with pytest.raises(AirflowException, match="Refreshing a specific version is not supported"):
            bundle.refresh()
        with pytest.raises(AirflowException, match="GCS url with version is not supported"):
    ...
    def test_gcs_bucket_and_prefix_validated(self, mock_gcs_hook_property):
        ...
        with pytest.raises(AirflowException, match="GCS bucket 'non-existing-bucket' does not exist."):
            bundle.initialize()
        ...
        with pytest.raises(
            AirflowException, match=f"GCS prefix 'gs://{GCS_BUCKET_NAME}/non-existing-prefix' does not exist."
        ):
            bundle.initialize()
