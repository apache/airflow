...
@pytest.fixture
def clean_cloud_logging_handlers():
    yield
    for handler_ref in reversed(logging._handlerList[:]):
        handler = handler_ref()
        if isinstance(handler, CloudLoggingTaskHandler):
            logging._removeHandlerRef(handler_ref)
            del handler


class TestCloudLoggingRemoteLogIOFromConfig:
    @conf_vars(
        {
    ...
    def test_from_config(self):
        subject = CloudLoggingRemoteLogIO.from_config()
    ...
    def test_from_config_rejects_non_dict_remote_task_handler_kwargs(self):
        with pytest.raises(ValueError, match="remote_task_handler_kwargs"):
            CloudLoggingRemoteLogIO.from_config()
    ...
    def test_from_config_rejects_remote_base_without_log_name(self, remote_base_log_folder):
        with conf_vars({("logging", "remote_base_log_folder"): remote_base_log_folder}):
            with pytest.raises(ValueError, match="Cloud Logging log name"):
                CloudLoggingRemoteLogIO.from_config()
    ...
    def test_provider_registers_stackdriver_scheme(self):
        ...
        assert info.classpath == (
            "airflow.providers.google.cloud.log.cloud_logging_task_handler.CloudLoggingRemoteLogIO"
        )


class TestCloudLoggingRemoteLogIO:
    ...
    @mock.patch("airflow.providers.google.cloud.log.cloud_logging_task_handler.LoggingServiceV2Client")
    def test_read_logs(self, mock_client, mock_get_creds_and_project_id):
        ...
        assert "Cloud Logging" in messages[0]
        assert logs == ["MSG1\nMSG2"]
    ...
    @mock.patch("airflow.providers.google.cloud.log.cloud_logging_task_handler.gcp_logging.Client")
    ...
    @mock.patch(
        "airflow.providers.google.cloud.log.cloud_logging_task_handler.CloudLoggingRemoteLogIO.transport",
...
@pytest.mark.usefixtures("clean_cloud_logging_handlers")
@mock.patch("airflow.providers.google.cloud.log.cloud_logging_task_handler.get_credentials_and_project_id")
@mock.patch("airflow.providers.google.cloud.log.cloud_logging_task_handler.gcp_logging.Client")
def test_should_pass_message_to_client(mock_client, mock_get_creds_and_project_id):
    mock_get_creds_and_project_id.return_value = ("creds", "project_id")

    transport_type = mock.MagicMock()
    cloud_logging_task_handler = CloudLoggingTaskHandler(transport=transport_type, labels={"key": "value"})
    logger = logging.getLogger("logger")
    logger.setLevel(logging.INFO)
    logger.addHandler(cloud_logging_task_handler)

    logger.info("test-message")
    cloud_logging_task_handler.flush()
    ...
    try:
        ...
        with context_manager:
            with conf_vars(
                ...
                # Core still instantiates the deprecated classes for stackdriver://
                # (StackdriverRemoteLogIO on main, StackdriverTaskHandler on released cores).
                ...
                if task_log is not None:
                    ...
                    assert isinstance(task_log, CloudLoggingRemoteLogIO)
...
@pytest.mark.db_test
class TestCloudLoggingHandlerTask:
    DAG_ID = "dag_for_testing_cloud_logging_task_handler"
    TASK_ID = "task_for_testing_cloud_logging_task_handler"

    @pytest.fixture(autouse=True)
    def task_instance(self, create_task_instance, clean_cloud_logging_handlers):
    ...
    def _setup_handler(self, **handler_kwargs):
        ...
        cloud_logging_task_handler = CloudLoggingTaskHandler(**handler_kwargs)
        self.logger = logging.getLogger("logger")
        self.logger.addHandler(cloud_logging_task_handler)
        return cloud_logging_task_handler
    ...
    def test_should_set_labels(self, mock_client, mock_get_creds_and_project_id):
        ...
        cloud_logging_task_handler = self._setup_handler()
        cloud_logging_task_handler.set_context(self.ti)
    ...
    def test_should_read_logs_for_all_try(self, mock_client, mock_get_creds_and_project_id):
        ...
        cloud_logging_task_handler = self._setup_handler()
        logs, metadata = cloud_logging_task_handler.read(self.ti)
    ...
    def test_should_read_logs_for_single_try(self, mock_client, mock_get_creds_and_project_id):
        ...
        logs, metadata = cloud_logging_task_handler.read(self.ti, 3)
        date_label = "logical_date" if AIRFLOW_V_3_0_PLUS else "execution_date"
        filter_str = (
            'resource.type="global"\n'
            'logName="projects/project_id/logs/airflow"\n'
    ...
    def test_should_read_logs_with_pagination(self, mock_client, mock_get_creds_and_project_id):
        ...
        logs, metadata1 = cloud_logging_task_handler.read(self.ti, 3)
        date_label = "logical_date" if AIRFLOW_V_3_0_PLUS else "execution_date"
        filter_str = (
            'resource.type="global"\n'
            'logName="projects/project_id/logs/airflow"\n'
        ...
        logs, metadata2 = cloud_logging_task_handler.read(self.ti, 3, metadata1[0])
    ...
    def test_should_read_logs_with_download(self, mock_client, mock_get_creds_and_project_id):
        ...
        cloud_logging_task_handler = self._setup_handler()
        logs, metadata1 = cloud_logging_task_handler.read(self.ti, 3, {"download_logs": True})
    ...
    def test_should_read_logs_with_custom_resources(self, mock_client, mock_get_creds_and_project_id):
        ...
        cloud_logging_task_handler = self._setup_handler(resource=resource)
    ...
    def test_should_use_credentials(self, mock_client, mock_get_creds_and_project_id):
        ...
        cloud_logging_task_handler = CloudLoggingTaskHandler(gcp_key_path="KEY_PATH")
        client = cloud_logging_task_handler.io._client
    ...
    def test_should_return_valid_external_url(self, mock_client, mock_get_creds_and_project_id, monkeypatch):
        ...
        cloud_logging_task_handler = CloudLoggingTaskHandler(gcp_key_path="KEY_PATH")
        url = cloud_logging_task_handler.get_external_log_url(self.ti, self.ti.try_number)
...
class TestCloudLoggingTaskHandlerExceptionHandling:
    ...
    def test_read_falls_back_when_cloud_logging_unavailable(
        ...
        handler = CloudLoggingTaskHandler()
        ti = mock.MagicMock()
        ti.task_id = "t"
        ti.dag_id = "d"
        ti.try_number = 1
    ...
    def test_close_swallows_transport_flush_errors(self, capsys):
        ...
        # ``CloudLoggingRemoteLogIO``; its value lives in a slot, not ``__dict__``, so
