...
# under the License.
"""Handler that integrates with Google Cloud Logging."""
...
# Routed to the same ``airflow.providers.google.cloud.log.cloud_logging_task_handler`` namespace
...
class CloudLoggingRemoteLogIO(LoggingMixin):
    """Remote log IO that streams logs to and reads from Google Cloud Logging."""
    ...
    @classmethod
    def from_config(cls) -> CloudLoggingRemoteLogIO:
        ...
        if not log_name:
            raise ValueError(
                "Cannot derive a Cloud Logging log name from "
    ...
    def transport(self) -> Transport:
        """Object responsible for sending data to Cloud Logging."""
    ...
    def read(self, relative_path: str, ti: RuntimeTI) -> LogResponse:
        """Read logs from Cloud Logging using task instance labels."""
        ...
        return [f"Reading remote log from Cloud Logging for {relative_path}"], [messages] if messages else []
...
class CloudLoggingTaskHandler(logging.Handler):
    ...
    route Python standard logging messages directly to the Cloud Logging
    API.
    ...
    def __init__(
        ...
        super().__init__()
        self.io = CloudLoggingRemoteLogIO(
    ...
    def close(self) -> None:
        ...
        try:
            ...
            print(
                f"CloudLoggingTaskHandler.close: transport flush failed: {type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
