...
class GCSDagBundle(BaseDagBundle):
    ...
    def _initialize(self):
        with self.lock():
            ...
            if not self.gcs_dags_dir.is_dir():
                raise AirflowException(f"Local DAGs path: {self.gcs_dags_dir} is not a directory.")

            try:
                self.gcs_hook.get_bucket(bucket_name=self.bucket_name)
            except NotFound:
                raise AirflowException(f"GCS bucket '{self.bucket_name}' does not exist.")
    ...
    def view_url_template(self) -> str | None:
        """Return a URL for viewing the DAGs in GCS. Currently, versioning is not supported."""
        if self.version:
            raise AirflowException("GCS url with version is not supported")
