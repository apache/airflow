...
class GitDagBundle(BaseDagBundle):
    ...
    :param refresh_on_initialize: Whether ``initialize()`` fetches the latest commits from the remote
        ...
        not present on disk, and explicit ``refresh()`` calls still fetch.

        Only applies when the bundle has no pinned version - version-pinned initialization already
        reuses the on-disk repo and is unaffected. So this is only worth setting where workers
        resolve code from ``tracking_ref`` rather than from a recorded bundle version, i.e. when
        ``[dag_processor] disable_bundle_versioning`` (or the Dag-level ``disable_bundle_versioning``)
        is set.
    """
