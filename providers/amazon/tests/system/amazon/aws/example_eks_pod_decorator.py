...
DAG_ID = "example_eks_pod_decorator"
...
with DAG(
    ...
    # The launch template enforces IMDSv2 and is required for internal compliance when running
    # these system tests on AWS infrastructure. It is not required for the decorator to work.
    ...
    # [START howto_decorator_eks_pod]
    # Credentials come from the AWS connection; the worker needs no aws CLI or kubeconfig.
    @task.eks_pod(
        pod_name="run-pod",
        cluster_name=cluster_name,
    ...
    def run_pod():
        print("Hello from an EKS pod")

    start_pod = run_pod()
    # [END howto_decorator_eks_pod]
