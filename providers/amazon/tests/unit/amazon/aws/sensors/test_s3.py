...
class TestS3KeySensor:
    ...
    @mock.patch("airflow.providers.amazon.aws.sensors.s3.S3Hook.yield_file_metadata")
    def test_poke_wildcard(self, mock_yield_file_metadata):
        op = S3KeySensor(task_id="s3_key_sensor", bucket_key="s3://test_bucket/file*", wildcard_match=True)

        mock_yield_file_metadata.return_value = []
        assert op.poke(None) is False
        mock_yield_file_metadata.assert_called_once_with("file", "test_bucket")

        mock_yield_file_metadata.return_value = [{"Key": "dummyFile", "Size": 0}]
        assert op.poke(None) is False

        mock_yield_file_metadata.return_value = [{"Key": "file1", "Size": 0}]
        assert op.poke(None) is True

    @mock.patch("airflow.providers.amazon.aws.sensors.s3.S3Hook.yield_file_metadata")
    def test_poke_wildcard_multiple_files(self, mock_yield_file_metadata):
        op = S3KeySensor(
            task_id="s3_key_sensor",
        ...
        mock_yield_file_metadata.side_effect = [[{"Key": "file1", "Size": 0}], []]
        assert op.poke(None) is False

        mock_yield_file_metadata.side_effect = [[{"Key": "file1", "Size": 0}], [{"Key": "file2", "Size": 0}]]
        assert op.poke(None) is False

        mock_yield_file_metadata.side_effect = [
            [{"Key": "file1", "Size": 0}],
            [{"Key": "test.zip", "Size": 0}],
        ]
        assert op.poke(None) is True

        mock_yield_file_metadata.assert_any_call("file", "test_bucket")
        mock_yield_file_metadata.assert_any_call("", "test_bucket")
    ...
    @mock.patch("airflow.providers.amazon.aws.sensors.s3.S3Hook.yield_file_metadata")
    def test_poke_with_use_regex(self, mock_yield_file_metadata, key, pattern, expected):
        op = S3KeySensor(
        ...
        mock_yield_file_metadata.return_value = [{"Key": key, "Size": 0}]
