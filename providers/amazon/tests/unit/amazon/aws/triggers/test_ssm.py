...
class TestSsmRunCommandTrigger:
    EXPECTED_WAITER_NAME = "run_command_instance_complete"
    COMMAND_ID = "123e4567-e89b-12d3-a456-426614174000"

    def test_serialization(self):
        trigger = SsmRunCommandTrigger(command_id=self.COMMAND_ID)
        classpath, kwargs = trigger.serialize()

        assert classpath == BASE_TRIGGER_CLASSPATH + "SsmRunCommandTrigger"
        assert kwargs.get("command_id") == self.COMMAND_ID
    ...
    async def test_run_success(self, mock_get_waiter, mock_async_conn):
        ...
        mock_client.list_command_invocations.return_value = {
            "CommandInvocations": [
                {"CommandId": self.COMMAND_ID, "InstanceId": "i-1234567890abcdef0"},
                {"CommandId": self.COMMAND_ID, "InstanceId": "i-1234567890abcdef1"},
            ]
        }
        mock_get_waiter().wait = mock.AsyncMock()

        trigger = SsmRunCommandTrigger(command_id=self.COMMAND_ID)
        generator = trigger.run()
        response = await generator.asend(None)

        assert response == TriggerEvent({"status": "success", "command_id": self.COMMAND_ID})
        assert_expected_waiter_type(mock_get_waiter, self.EXPECTED_WAITER_NAME)
        assert mock_get_waiter().wait.call_count == 2
        mock_client.list_command_invocations.assert_called_once_with(CommandId=self.COMMAND_ID)
    ...
    async def test_run_fails(self, mock_get_waiter, mock_async_conn):
        ...
        mock_get_waiter().wait.side_effect = WaiterError(
            "name", "terminal failure", {"CommandInvocations": [{"CommandId": "i-1234567890abcdef0"}]}
        )
