...
class S3RemoteLogIO(LoggingMixin):  # noqa: D101
    ...
    def read(self, relative_path: str) -> tuple[list[str], list[str] | None]:
        logs = []
        messages = []
...
if TYPE_CHECKING:
    ...
    # Ensure type compatibility. We don't make it a subclass so that this provider is still usable on Airflow
    # 2
    _: type[RemoteLogIO] = S3RemoteLogIO
