...
class CloudRemoteLogIO(LoggingMixin):  # noqa: D101
    ...
    def read(self, relative_path: str, ti: TaskInstance | None = None) -> tuple[list[str], list[str] | None]:
        logs = []
        messages = [{"log_group": self.log_group, "log_stream": relative_path}]
        try:
...
if TYPE_CHECKING:
    ...
    # 2
    _: type[RemoteLogIO] = CloudRemoteLogIO
...
class CloudwatchTaskHandler(FileTaskHandler, LoggingMixin):
    ...
    def _read_remote_logs(self, task_instance, try_number, metadata=None) -> tuple[list[str], list[str]]:
