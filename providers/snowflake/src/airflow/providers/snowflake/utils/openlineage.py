...
def _check_openlineage_modules_are_importable():
    ...
    This function checks whether the necessary OpenLineage modules are available for use.
    Since the Snowflake provider does not directly require OpenLineage, users must ensure
    that the relevant dependencies are installed to enable some OpenLineage features.

    If the required modules are missing, an `AirflowOptionalProviderFeatureException` is raised,
    ...
    try:
        from openlineage.client.event_v2 import Job, Run, RunEvent, RunState  # noqa: F401
        from openlineage.client.facet_v2 import job_type_job, parent_run  # noqa: F401
...
def emit_openlineage_events_for_snowflake_queries(
    ...
    Raises:
        AirflowOptionalProviderFeatureException: If OpenLineage modules are missing.
    """
    _check_openlineage_modules_are_importable()  # This must be done at the very beginning !
    ...
    for query_id in query_ids:
        ...
        log.debug(
            "Metadata for query ID `%s`: `%s`", query_id, query_metadata if query_metadata else "not found"
        )
        ...
        event_batch = _create_snowflake_event_pair(
            ...
            job_name=f"snowflake_query:{query_id}",
