...
from airflow.providers.snowflake.utils.openlineage import (
    _check_openlineage_modules_are_importable,
...
def test_check_openlineage_modules_are_importable_on_previous_version_openlineage_provider():
    """When using OpenLineage, the snowflake events emission depends on openlineage provider >= 1.7"""
...
def test_check_openlineage_modules_are_importable_on_previous_version_openlineage_client():
    """When using OpenLineage, the snowflake events depends on openlineage client >= 1.15.0"""
    original_import = __import__

    def custom_import(name, *args, **kwargs):
        if name == "openlineage.client.facet_v2":
            raise ModuleNotFoundError("No module named 'openlineage.client.facet_v2")
        else:
            return original_import(name, *args, **kwargs)
    ...
    with mock.patch("builtins.__import__", side_effect=custom_import):
        with pytest.raises(AirflowOptionalProviderFeatureException) as exc:
            _check_openlineage_modules_are_importable()
    assert str(exc.value.args[0]) == "No module named 'openlineage.client.facet_v2"
    assert str(exc.value.args[1]) == "Please install `openlineage-python>=1.15.0`"
...
def test_emit_openlineage_events_with_old_openlineage_provider():
    original_import = __import__
    ...
    with (
        ...
        mock.patch("builtins.__import__", side_effect=custom_import),
    ):
        with pytest.raises(AirflowOptionalProviderFeatureException) as exc:
            emit_openlineage_events_for_snowflake_queries(
                query_ids=query_ids,
...
def test_emit_openlineage_events_with_old_openlineage_client():
    original_import = __import__
