...
class AzureBatchHook(BaseHook):
    ...
    def get_conn(self) -> BatchClient:
        ...
        if not batch_account_url:
            raise ValueError("Batch Account URL parameter is missing.")

        credential: AzureNamedKeyCredential | TokenCredential
    ...
    def configure_pool(
        ...
        **kwargs,
        ...
        if use_latest_image_and_sku:
            ...
            return batch_models.BatchPoolCreateOptions(
                id=pool_id,
                vm_size=vm_size,
    ...
    def create_pool(self, pool: BatchPoolCreateOptions) -> None:
        ...
        except (ResourceExistsError, HttpResponseError) as err:
            if not self._is_error_code(err, "PoolExists"):
                raise
            self.log.info("Pool %s already exists", pool.id)

    @staticmethod
    def _is_error_code(err: HttpResponseError, code: str) -> bool:
        """Return True if the Batch error response carries the given code."""
        error = getattr(err, "model", None)
    ...
    def configure_job(
        ...
        **kwargs,
        ...
        """
        return batch_models.BatchJobCreateOptions(
            id=job_id,
    ...
    def create_job(self, job: BatchJobCreateOptions) -> None:
        ...
        try:
            ...
            if not self._is_error_code(err, "JobExists"):
                raise
    ...
    def add_single_task_to_job(self, job_id: str, task: BatchTaskCreateOptions) -> None:
        ...
        try:
            ...
            if not self._is_error_code(err, "TaskExists"):
                raise
    ...
    def test_connection(self):
        ...
        try:
            ...
            # test the connection.
            next(iter(self.get_conn().list_jobs()), None)
