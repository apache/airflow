...
class TestAzureBatchHook:
    ...
    @mock.patch(f"{MODULE}._get_default_azure_credential")
    def test_fallback_to_azure_identity_credential_adapter_when_name_and_key_is_not_provided(
        self, mock_get_default_azure_credential, create_mock_connections
    ):
        create_mock_connections(
            Connection(
        ...
        hook.get_conn()
        mock_get_default_azure_credential.assert_called_once_with(
