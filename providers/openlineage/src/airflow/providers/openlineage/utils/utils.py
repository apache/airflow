...
def _extract_ol_info_from_asset_event(asset_event: AssetEvent) -> dict[str, str] | None:
    ...
    if asset_event.extra:
        ...
        if isinstance(ol_info_from_extra, dict):
            job_name = ol_info_from_extra.get("job_name")
            job_namespace = ol_info_from_extra.get("job_namespace")
            run_id = ol_info_from_extra.get("run_id")
            ...
            if job_name and job_namespace:
                ...
                if run_id:
                    if not is_valid_uuid(str(run_id)):
                        log.warning(
                            "Invalid run_id in AssetEvent.extra; ignoring value. event_id=%s, run_id=%s",
                            asset_event.id,
                            run_id,
                        )
                    else:
