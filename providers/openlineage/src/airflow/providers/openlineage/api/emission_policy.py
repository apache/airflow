...
from airflow.providers.openlineage.utils.emission_policy import (
    ...
    INCLUDE_SOURCE_CODE,
    OL_EMISSION_POLICY_DAG_PARAM,
    OL_EMISSION_POLICY_TASK_PARAM,
    _merge_param,
)
...
def extend_global_openlineage_emission_policy(
    ...
    *,
    ...
    if isinstance(obj, DAG):
        ...
        if dag_flags:
            _merge_param(obj, OL_EMISSION_POLICY_DAG_PARAM, dag_flags)

        task_flags = {k: v for k, v in provided.items() if k in _TASK_FLAG_KEYS}
        if task_flags:
            for task in obj.task_dict.values():
                _merge_param(task, OL_EMISSION_POLICY_TASK_PARAM, task_flags)
        ...
        task_flags = {k: v for k, v in provided.items() if k in _TASK_FLAG_KEYS}
        if task_flags:
            _merge_param(obj, OL_EMISSION_POLICY_TASK_PARAM, task_flags)
