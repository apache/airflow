...
Dag-level ``include_full_task_info=True`` adds extra fields in airflow.task.
"""
...
DAG_ID = "openlineage_policy_full_task_info_dag"
...
with DAG(
    ...
    run = BashOperator(task_id="run", bash_command="echo full_info_test")
    check_events = OpenLineageTestOperator(
        task_id="check_events", file_path=get_expected_event_file_path(DAG_ID)
    )
    run >> check_events

extend_global_openlineage_emission_policy(dag, include_full_task_info=True)
