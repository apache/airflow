...
DAG_ID = "openlineage_policy_conf_source_code_dag"
...
with DAG(
    ...
    t_conf_suppressed = BashOperator(task_id="t_conf_suppressed", bash_command="echo conf_test")
    t_auth_override = extend_global_openlineage_emission_policy(
        BashOperator(task_id="t_authoring_override", bash_command="echo auth_override"),
        include_source_code=True,
    )
    check_events = OpenLineageTestOperator(
        task_id="check_events", file_path=get_expected_event_file_path(DAG_ID)
    )
    t_conf_suppressed >> t_auth_override >> check_events
