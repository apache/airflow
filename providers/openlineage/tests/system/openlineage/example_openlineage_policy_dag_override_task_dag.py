...
A dag-level flag (``include_source_code=False``) is overridden per-task.
"""
...
DAG_ID = "openlineage_policy_dag_override_task_dag"
...
with DAG(
    ...
    t_default = BashOperator(task_id="t_default", bash_command="echo dag_default")
    t_override = BashOperator(task_id="t_override", bash_command="echo task_override")
    check_events = OpenLineageTestOperator(
        task_id="check_events", file_path=get_expected_event_file_path(DAG_ID)
    )
    t_default >> t_override >> check_events

extend_global_openlineage_emission_policy(dag, include_source_code=False)
extend_global_openlineage_emission_policy(t_override, include_source_code=True)
