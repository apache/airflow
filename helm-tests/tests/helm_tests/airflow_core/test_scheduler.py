...
class TestSchedulerServiceAccount:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("LocalExecutor", None),  # Non-pod-launching executor - no automountServiceAccountToken set
            ("CeleryExecutor", None),  # Pod-launching executor with default (true) - no explicit setting
            ("CeleryKubernetesExecutor", None),  # Pod-launching executor with default (true) - no explicit setting
            ("KubernetesExecutor", None),  # Pod-launching executor with default (true) - no explicit setting
            ("LocalKubernetesExecutor", None),  # Pod-launching executor with default (true) - no explicit setting
            ("CeleryExecutor,KubernetesExecutor", None),  # Pod-launching executor with default (true) - no explicit setting
        ],
    )
...
class TestSchedulerServiceAccountTokenVolume:
    ...
    def test_pod_level_automount_service_account_token_disabled(self, executor, automount_service_account, is_pod_launching):
    ...
    def test_service_account_token_volume_custom_configuration(self, executor, mount_path, expiration_seconds):
    ...
    def test_service_account_token_volume_mounted_in_scheduler_container(self, executor, container_index, container_name):
        ...
        volume_mounts = jmespath.search(f"spec.template.spec.containers[{container_index}].volumeMounts", docs[0]) or []
    ...
    def test_service_account_token_volume_NOT_mounted_in_init_containers(self, executor, has_init_containers):
        ...
        if has_init_containers and init_containers:
            ...
            assert sa_token_mount is None, "Init container should not have service account token volume mount for security"
    ...
    def test_service_account_token_volume_NOT_mounted_in_log_groomer_sidecar(self):
        ...
        log_groomer_container = next((c for c in containers if c.get("name") == "scheduler-log-groomer"), None)
        ...
        if log_groomer_container:
            ...
            assert sa_token_mount is None, "Log groomer sidecar should not have service account token volume mount for security"
    ...
    def test_service_account_token_volume_mount_only_in_scheduler_container(self, executor, is_pod_launching):
        ...
        scheduler_sa_token_mount = next((vm for vm in scheduler_volume_mounts if vm.get("name") == "kube-api-access"), None)
        assert scheduler_sa_token_mount is not None, "Scheduler container should have service account token volume mount"
        ...
        for init_container in init_containers:
            ...
            init_sa_token_mount = next((vm for vm in init_volume_mounts if vm.get("name") == "kube-api-access"), None)
            assert init_sa_token_mount is None, f"Init container '{init_container.get('name')}' should not have service account token volume mount"
        ...
        for sidecar_container in sidecar_containers:
            ...
            sidecar_sa_token_mount = next((vm for vm in sidecar_volume_mounts if vm.get("name") == "kube-api-access"), None)
            assert sidecar_sa_token_mount is None, f"Sidecar container '{sidecar_container.get('name')}' should not have service account token volume mount"
    ...
    def test_service_account_token_volume_mount_per_container_security_policy(self, executor, container_name, should_have_mount):
        ...
        if should_have_mount:
            assert sa_token_mount is not None, f"Container '{container_name}' should have service account token volume mount"
            assert sa_token_mount["mountPath"] == "/var/run/secrets/kubernetes.io/serviceaccount"
            assert sa_token_mount["readOnly"] is True
        else:
            assert sa_token_mount is None, f"Container '{container_name}' should NOT have service account token volume mount for security"
    ...
    def test_service_account_token_volume_mount_security_validation_all_executors(self):
        ...
        for executor in executors:
            ...
            is_pod_launching = executor in ["CeleryExecutor", "CeleryKubernetesExecutor", "KubernetesExecutor", "LocalKubernetesExecutor", "CeleryExecutor,KubernetesExecutor"]
            
            if not is_pod_launching:
                # Non-pod-launching executors should not have service account token volume
                volumes = jmespath.search("spec.template.spec.volumes", docs[0]) or []
                volume_names = [v.get("name") for v in volumes]
                assert "kube-api-access" not in volume_names, f"Non-pod-launching executor '{executor}' should not have service account token volume"
                continue
            
            # Pod-launching executors should have volume but only mount it in scheduler container
            volumes = jmespath.search("spec.template.spec.volumes", docs[0]) or []
            volume_names = [v.get("name") for v in volumes]
            assert "kube-api-access" in volume_names, f"Pod-launching executor '{executor}' should have service account token volume"
            ...
            for container_type, container in all_containers:
                ...
                if container_name == "scheduler":
                    assert sa_token_mount is not None, f"Scheduler container should have service account token mount for executor '{executor}'"
                else:
                    assert sa_token_mount is None, f"Container '{container_name}' ({container_type}) should NOT have service account token mount for executor '{executor}' (security policy)"
