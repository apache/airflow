...
class TestDBCleanupDeployment:
    """Tests DB cleanup deployments."""

    def test_should_have_a_schedule_with_defaults(self):
        doc = render_chart(
            values={
                "dbCleanup": {"enabled": True},
            },
            show_only=["templates/db-cleanup/db-cleanup-cronjob.yaml"],
        )[0]
...
class TestDBCleanup:
    ...
    def test_scheduler_name(self):
        docs = render_chart(
            values={"dbCleanup": {"enabled": True}, "schedulerName": "airflow-scheduler"},
            show_only=["templates/db-cleanup/db-cleanup-cronjob.yaml"],
        )
    ...
    def test_default_command_and_args(self):
        ...
        assert jmespath.search("spec.jobTemplate.spec.template.spec.containers[0].args", docs[0]) == [
            "bash",
            "-c",
            "exec \\\nairflow db clean --yes --clean-before-timestamp $( date --utc --date='now - 2592000 seconds' +'%Y-%m-%d %H:%M:%S%:z' )",
        ]

    @pytest.mark.parametrize(
        ("clean_before", "skip_archive", "verbose", "batch_size", "tables", "command_args"),
        [
            (86400, False, False, None, None, ""),
            (86401, True, False, None, None, " --skip-archive"),
            (86402, False, True, None, None, " --verbose"),
            (86403, False, False, 200, None, " --batch-size 200"),
            (86400, False, False, None, ["xcom"], " --tables xcom"),
            (
                172800,
                True,
                True,
    ...
    def test_db_cleanupcommand_options(
        self, clean_before, skip_archive, verbose, batch_size, tables, command_args
    ):
        docs = render_chart(
            values={
                "dbCleanup": {
                    "enabled": True,
                    "cleanBeforeSeconds": clean_before,
        ...
        assert jmespath.search("spec.jobTemplate.spec.template.spec.containers[0].args", docs[0]) == [
            "bash",
            "-c",
            f"exec \\\nairflow db clean --yes --clean-before-timestamp $( date --utc --date='now - {clean_before} seconds' +'%Y-%m-%d %H:%M:%S%:z' ){command_args}",
        ]
    ...
    def test_command_and_args_overrides(self, command, args):
        docs = render_chart(
            values={"dbCleanup": {"enabled": True, "command": command, "args": args}},
            show_only=["templates/db-cleanup/db-cleanup-cronjob.yaml"],
        )
    ...
    def test_should_add_extra_volumes(self):
        ...
        assert jmespath.search("spec.jobTemplate.spec.template.spec.volumes[-1]", docs[0]) == {
    ...
    def test_should_add_extra_volume_mounts(self):
        ...
        assert jmespath.search(
            "spec.jobTemplate.spec.template.spec.containers[0].volumeMounts[-1]", docs[0]
        ) == {
            "name": "foobar-airflow",
            "mountPath": "foo/bar",
        }
...
class TestDBCleanupServiceAccount:
    """Tests DB cleanup of service accounts."""
    ...
    def test_should_add_component_specific_labels(self):
        docs = render_chart(
            ...
            show_only=["templates/db-cleanup/db-cleanup-serviceaccount.yaml"],
        )
    ...
    def test_overridden_automount_service_account_token(self):
        docs = render_chart(
            values={
                "dbCleanup": {"enabled": True, "serviceAccount": {"automountServiceAccountToken": False}},
            },
            show_only=["templates/db-cleanup/db-cleanup-serviceaccount.yaml"],
        )
