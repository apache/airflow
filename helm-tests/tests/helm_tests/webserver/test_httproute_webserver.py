...
# The Webserver only exists on Airflow 2 (`airflowVersion` < 3.0.0); the API server replaces it on 3.
AIRFLOW_2 = "2.11.0"
...
class TestHTTPRouteWebserver:
    ...
    def test_should_pass_validation_with_minimal_config(self):
        ...
        assert jmespath.search("metadata.name", docs[0]) == "release-name-webserver-httproute"
        ...
        assert jmespath.search("spec.rules", docs[0]) == [
            {
                ...
                "backendRefs": [{"name": "my-release-webserver", "port": 8080}],
            },
        ]
    ...
    def test_custom_path_and_path_type_should_apply(self):
        ...
        assert jmespath.search("spec.rules[0].matches[0].path.value", docs[0]) == "/web"
            ...
            values={
                ...
                "webserver": {
                    ...
                    "httpRoute": {
                        ...
                        "rules": [
                            {
                                "matches": [{"path": {"type": "PathPrefix", "value": "/custom-web"}}],
    ...
    def test_httproute_not_created_when_unset(self):
        docs = render_chart(
            values={"airflowVersion": AIRFLOW_2, "webserver": {"enabled": True, "httpRoute": {}}},
            show_only=SHOW_ONLY,
            api_versions=GATEWAY_API_VERSIONS,
        )
        assert docs == []
    ...
    def test_httproute_not_created_when_disabled(self):
        docs = render_chart(
            values={
                "airflowVersion": AIRFLOW_2,
                "webserver": {"enabled": True, "httpRoute": {"enabled": False}},
            },
            show_only=SHOW_ONLY,
            api_versions=GATEWAY_API_VERSIONS,
        )
    ...
    def test_should_fail_when_parent_refs_empty(self):
        with pytest.raises(HelmFailedError) as exc_info:
            render_chart(
                values={
                    "airflowVersion": AIRFLOW_2,
                    "webserver": {"enabled": True, "httpRoute": {"enabled": True}},
                },
                show_only=SHOW_ONLY,
                api_versions=GATEWAY_API_VERSIONS,
            )
    ...
    def test_should_fail_schema_when_parent_refs_explicitly_empty(self):
        with pytest.raises(HelmFailedError) as exc_info:
            render_chart(
                values={
                    "airflowVersion": AIRFLOW_2,
                    "webserver": {"enabled": True, "httpRoute": {"enabled": True, "parentRefs": []}},
                },
                show_only=SHOW_ONLY,
                api_versions=GATEWAY_API_VERSIONS,
            )
