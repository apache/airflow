...
The `file_task_handler` logger is more flexible. The default format can be changed, `{dag_id}/{task_id}/{execution_date}/{try_number}.log` by supplying Jinja templating in the `FILENAME_TEMPLATE` configuration variable. See the `file_task_handler` for more information.
...
In order the increase the robustness of the scheduler, DAGS are now processed in their own process. Therefore each
...
How often the scheduler should relist the contents of the DAG directory. If while developing your
dags they are not being picked up, have a look at this number and decrease it when necessary.
...
convenience variables to the config. In case you run a sceure Hadoop setup it might be
