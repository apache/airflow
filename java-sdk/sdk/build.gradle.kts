...
abstract class GenerateDagDslTask : DefaultTask() {
    ...
    fun generate() {
        ...
        outDir.resolve("org/apache/airflow/sdk").apply { mkdirs() }.resolve("Builder.kt").writeText(
            ...
            |   *
            |   * This is not [Task] under another name. Python declares the task and
            |   * Java supplies only its body, so the handler names the pair it binds
            ...
            |   * ```
            |   *
            |   * Register every handler a class holds with [Bundle.register]; there
            |   * is no [Dag] annotation on this surface, because the Dag is the
            |   * Python file's.
            |   *
