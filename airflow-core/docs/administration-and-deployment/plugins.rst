...
server URL configuration required. Because it calls the public Core API, responses still
redact sensitive connection/variable values and ``config`` access obeys ``[api] expose_config``;
it is not a way to read secret values. See :doc:`/installation/upgrading_to_airflow3` for the
full list of available sub-clients and the trust/redaction caveats.
