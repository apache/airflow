...
Airflow does not automatically purge task state rows on a schedule. Cleanup (also known as "garbage collection") is the responsibility of the user (you) and must be triggered explicitly via the CLI. This page explains what gets cleaned up, how to run it, and how to integrate it into a recurring maintenance workflow.


What gets cleaned up
--------------------

The cleanup command operates only on **task state** rows in the ``MetastoreStateBackend``. Asset state rows are never touched by this command. Asset state rows are removed only by the orphan sweep when an asset is deactivated (see :ref:`state-store`).
...
By default (``state_cleanup_batch_size = 0``) all eligible rows are deleted in a single statement. On deployments with large ``task_state`` tables, set a batch size to reduce lock duration per transaction::
...
Because Airflow does not run cleanup automatically, you need to schedule it. The recommended approaches are:
...
You can also drive cleanup from a Dag so it is visible in the UI and benefits from Airflow's retry and alerting:
...
How often to run cleanup depends on your write volume and the value of ``default_retention_days``. A daily run is sufficient for most deployments. For high-throughput pipelines that write task state on every task execution, consider running cleanup more frequently to keep the ``task_state`` table small.
