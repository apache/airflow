    ...
    public class EtlPipeline {
      ...
      static class Wiring implements EtlPipelineDeps {
        void depends() {
          ...
          rows.then(audit()); // ordering-only edge: extract >> audit
        }
      }
    }
...
``then`` and ``Flow.of`` work here exactly as they do on the interface surface; inside the wiring
class ``Flow`` is inherited by simple name, so it needs no import and never collides with
