...
.. list-table::
   ...
   * - ``bundles_root``
     ...
     - One or more directories searched, in order, for a ``bundle.mjs`` with embedded metadata. Accepts a
       string, a path, or a list of strings/paths. When omitted, the bundle is located through a Dag bundle
       instead (see the note below). Explicitly setting this option to ``null`` or an empty list is invalid.
   * - ``dag_bundle_name``
