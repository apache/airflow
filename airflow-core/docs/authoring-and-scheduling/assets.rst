...
.. code-block:: python
    ...
    from airflow.sdk import DAG, TruncateToHourlyMapper, PartitionedAssetTimetable
...
* Temporal mappers such as ``TruncateToHourlyMapper``, ``TruncateToDailyMapper``, and
  ``TruncateToYearlyMapper`` normalize time keys to a chosen grain. For input key
  ``2026-03-10T09:37:51``, the default outputs are:

  * ``TruncateToHourlyMapper`` -> ``2026-03-10T09``
  * ``TruncateToDailyMapper`` -> ``2026-03-10``
...
* ``ProductMapper`` maps composite keys segment-by-segment.
  ...
  ``ProductMapper(IdentityMapper(), TruncateToDailyMapper())`` produces
  ``us|2026-03-10``.
...
.. code-block:: python
    ...
    with DAG(
        ...
        schedule=PartitionedAssetTimetable(
            ...
            default_partition_mapper=ProductMapper(IdentityMapper(), TruncateToDailyMapper()),
        ),
    ):
        ...
...
.. code-block:: python
    ...
    from airflow.sdk import Asset, DAG, TruncateToDailyMapper, IdentityMapper, PartitionedAssetTimetable
    ...
    with DAG(
        ...
        schedule=PartitionedAssetTimetable(
            ...
            default_partition_mapper=TruncateToDailyMapper(),
