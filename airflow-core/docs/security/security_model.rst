...
**Pass configuration via environment variables**
   ...
   the flag, and a child started through ``exec`` re-applies it as its first step because ``execve``
   resets it. This marks both processes as non-dumpable, which prevents same-UID sibling
