...
contains config option value. The following is a sample JSON file.
  ...
  .. code-block:: json
    ...
    {
        "value_specified_in_database_sql_alchemy_conn_secret": "sql_alchemy_conn_value",
        "value_specified_in_core_fernet_key_secret": "fernet_key_value"
    }

The YAML file structure is similar to that of JSON, with key containing config option ``_secret`` version value
and the value containing config option value. The following is a sample YAML file.
...
and the value should be the config option value. The following is a sample file.
