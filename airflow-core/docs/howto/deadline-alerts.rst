...
``DeadlineReference.AVERAGE_RUNTIME``
    ...
    The deadline is set to the current time plus the calculated average runtime (scaled by the
    optional ``multiplier``) plus the interval. If insufficient historical data exists, no
    deadline is created.
