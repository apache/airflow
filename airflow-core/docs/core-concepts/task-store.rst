...
Data persisted via task store is accessed through the task context via ``context["task_store"]`` and exposes the synchronous methods ``get``, ``set``, ``delete``, and ``clear``, plus the async counterparts ``aget`` and ``aset`` for use inside ``async`` tasks.
...
.. code-block:: python
    ...
    value = await task_store.aget("job_id", default="123456789")
    await task_store.aset("job_id", job_id, retention=NEVER_EXPIRE)
