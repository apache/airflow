...
See :ref:`Cross-team asset event filtering with producer_teams <asset_access_control>` in the Assets
...
For example, if a consumer's schedule reference has ``producer_teams=["team_x"]`` and the producer's
...
Teamless consumers (Dags with no team association) always pass through the ``consumer_teams`` filter,
regardless of its value. This ensures that global/shared Dags continue to receive events from all producers.

See :ref:`Cross-team asset event filtering with consumer_teams <asset_consumer_teams>` in the Assets
documentation for usage details and validation rules.
...
.. list-table::
   ...
   * - (no team, DAG)
     ...
     - (any)
     - ✅ Allowed
     - Global producer, consumer in list
   * - (no team, DAG)
     - Team B
     - (any)
     - ``["team_c"]``
...
- **Teamless consumer**: Accepts events from any source (DAG or API), regardless of team or ``consumer_teams``.
...
The REST API also accepts an optional ``allow_consumer_teams`` parameter in the request body. When provided,
it restricts which consumer teams can receive the event, following the same rules as the task-level
``consumer_teams``. When omitted or empty, no consumer-team filtering is applied. When Multi-Team mode
is disabled, the parameter is accepted but ignored.
