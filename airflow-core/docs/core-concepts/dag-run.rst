...
To observe those skips, set ``on_skipped_intervals_callback`` on the Dag (see :doc:`Callbacks </administration-and-deployment/logging-monitoring/callbacks>`) or implement the ``on_dag_skipped_intervals`` listener hook (see :doc:`Listeners </administration-and-deployment/listeners>`).
