...
* ``is_authorized_all_dags``: Return whether the user is authorized to perform a given action on every Dag of the deployment, including the Dags of teams the user is not a member of. Override this when your Dag decisions do not depend on the Dag ID: Airflow then skips authorizing the Dags one at a time to build the list of Dags the user has access to, and stops narrowing its Dag queries to that list. On a large deployment that saves an authorization decision per Dag on every request, and a list it would otherwise repeat in every statement. If not overridden, it returns ``False``.
