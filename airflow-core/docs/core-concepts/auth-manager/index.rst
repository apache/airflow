...
* ``is_authorized_all_dags``: Return whether the user is authorized to perform a given action on every Dag of the deployment, including the Dags of teams the user is not a member of. Override this when your Dag decisions do not depend on the Dag ID: Airflow then stops narrowing its Dag queries to the list of Dags the user has access to, which on a large deployment is a list it would otherwise repeat in every statement. If not overridden, it returns ``False`` and the Dags are authorized one at a time.
