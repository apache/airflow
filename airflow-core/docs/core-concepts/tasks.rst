...
For advanced cases, subclass :class:`~airflow.sdk.definitions.retry_policy.RetryPolicy`
...
task that also needs to resume from where it left off, pair a retry policy
with the task state store described in
