...
@pytest.mark.db_test
@pytest.mark.need_serialized_dag(True)
def test_nested_group_inner_retries_before_outer_evaluates(dag_maker, session):
    """Inner group retries independently; outer group must NOT retry when inner succeeds on retry.

    Scenario:
        outer (retries=1)
          ├── outer_ok   — always succeeds
          └── inner (retries=1, retry_delay=0)
                └── flaky — fails once, then succeeds

    Expected flow:
        1. All tasks run: outer_ok=SUCCESS, flaky=FAILED
        2. Inner group triggers retry (clears flaky)
        3. flaky runs again → SUCCESS
        4. All tasks succeeded → outer group does NOT retry → DagRun SUCCESS
    """
    call_count = {"n": 0}
    ...
    def flaky():
        call_count["n"] += 1
        if call_count["n"] <= 1:
            raise AirflowException("first attempt fails")

    with dag_maker(
        dag_id="test_nested_ordering_integration",
        schedule=None,
        start_date=timezone.datetime(2024, 1, 1),
    ):
        with TaskGroup(
            ...
            PythonOperator(task_id="outer_ok", python_callable=lambda: None)
            with TaskGroup(
                group_id="inner",
                retries=1,
                retry_delay=timedelta(seconds=0),
    ...
    # DagRun should succeed — inner group retried and succeeded, outer never needed to retry.
    assert dr.state == DagRunState.SUCCESS

    # Inner group should have retried once.
    ...
    # Outer group should NOT have a TGI record (never retried).
    ...
    # The flaky task should have run twice total.
