...
class TestWorkloadTokenScopeEnforcement:
    """Verify that workload tokens can read but not write variables."""
    ...
    def test_workload_token_can_read_variable(self, workload_client, session):
        """GET /variables/{key} allows workload tokens (read-only access for callbacks)."""
        Variable.set(key="readable_var", value="hello", session=session)
        session.commit()

        response = workload_client.get("/execution/variables/readable_var")
        assert response.status_code == 200
    ...
    def test_workload_token_rejected_on_put_variable(self, workload_client):
        ...
        response = workload_client.put(
            "/execution/variables/test_key",
            json={"value": "{}"},
        )
        assert response.status_code == 403
        assert "Token type 'workload' not allowed" in response.json()["detail"]

    def test_workload_token_rejected_on_delete_variable(self, workload_client):
        """DELETE /variables/{key} rejects workload tokens — write requires execution scope."""
        response = workload_client.delete("/execution/variables/test_key")
        assert response.status_code == 403
        assert "Token type 'workload' not allowed" in response.json()["detail"]
