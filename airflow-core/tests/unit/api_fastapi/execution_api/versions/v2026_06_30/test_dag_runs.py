...
def test_get_dag_run_returns_no_consumed_asset_events(old_ver_client):
    """Bare-DagRun routes serialize no consumed asset events, so they need no partition_key converter."""
    response = old_ver_client.get("/execution/dag-runs/test_dag_run_consumed_event/run1")

    assert response.status_code == 200
    # The Dag run really has a consumed event carrying a partition_key, but this route does not
    # eager-load the relationship and ``safe_extract_from_orm`` defaults it away. Assert the empty
    # list rather than the absence of the key: an ``all(...)`` over these events would hold
