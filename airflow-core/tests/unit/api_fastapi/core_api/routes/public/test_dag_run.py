...
class TestGetRecentDagRunConfigurations:
    """Test the get_recent_dag_run_configurations endpoint."""

    def test_should_respond_200_with_recent_configurations(self, test_client, dag_maker):
        """Test getting recent configurations."""
        # Create DAG using dag_maker
        with dag_maker(DAG1_ID, schedule=None, start_date=timezone.utcnow(), serialized=True):
        ...
        # Create test DAG runs with different configurations
        for i, config in enumerate(test_configs):
            dag_run = dag_maker.create_dagrun(
                ...
                state=DagRunState.SUCCESS,
                conf=config,
                logical_date=timezone.utcnow() - timedelta(hours=i),
                start_date=timezone.utcnow() - timedelta(hours=i),
                triggered_by=DagRunTriggeredByType.UI,
                triggering_user_name="test",
            )

        response = test_client.get(f"/api/v2/dags/{DAG1_ID}/dagRuns/recent-configurations")
        ...
        # Check that configurations are ordered by start_date desc
        configs = data["configurations"]
        assert configs[0]["run_id"] == "manual_0"  # Most recent
        assert configs[1]["run_id"] == "manual_1"
        assert configs[2]["run_id"] == "manual_2"  # Oldest
        
        # Check configuration content
        assert configs[0]["conf"] == {"param1": "value1", "param2": "value2"}
        assert configs[1]["conf"] == {"param1": "value3", "param2": "value4"}
        assert configs[2]["conf"] == {"param1": "value5", "param2": "value6"}

    def test_should_respect_limit_parameter(self, test_client):
        """Test that limit parameter is respected."""
        response = test_client.get(
            f"/api/v2/dags/{DAG1_ID}/dagRuns/recent-configurations",
            params={"limit": 2}
        )
    ...
    def test_should_respond_404_for_nonexistent_dag(self, test_client):
        """Test 404 response for non-existent DAG."""
        response = test_client.get("/api/v2/dags/nonexistent/dagRuns/recent-configurations")
        assert response.status_code == 404

    def test_should_respond_200_with_empty_configurations(self, test_client, dag_maker):
        """Test response when no configurations exist."""
        # Create a DAG using dag_maker
        dag_id = "empty_dag"
        with dag_maker(dag_id, schedule=None, start_date=timezone.utcnow(), serialized=True):
            EmptyOperator(task_id="task_1")
        
        # Sync DAG to database
        dag_maker.sync_dagbag_to_db()
        
        # Create a manual run without conf
        dag_maker.create_dagrun(
            run_id="manual_no_conf",
            run_type=DagRunType.MANUAL,
            state=DagRunState.SUCCESS,
            conf=None,
        ...
        response = test_client.get(f"/api/v2/dags/{dag_id}/dagRuns/recent-configurations")
        
        assert response.status_code == 200
        data = response.json()
        assert data["configurations"] == []

    def test_should_only_return_manual_runs_with_conf(self, test_client, dag_maker):
        """Test that only manual runs with conf are returned."""
        # Create a DAG using dag_maker
        with dag_maker(DAG1_ID, schedule=None, start_date=timezone.utcnow(), serialized=True):
            EmptyOperator(task_id="task_1")
        ...
        # Create different types of DAG runs
        # Manual run with conf - should be included
        dag_maker.create_dagrun(
            run_id="manual_with_conf",
            run_type=DagRunType.MANUAL,
            state=DagRunState.SUCCESS,
            conf={"param": "value"},
            logical_date=timezone.utcnow(),
        ...
        # Manual run without conf - should be excluded
        dag_maker.create_dagrun(
            run_id="manual_no_conf",
            run_type=DagRunType.MANUAL,
            state=DagRunState.SUCCESS,
        ...
        # Scheduled run with conf - should be excluded
        dag_maker.create_dagrun(
            run_id="scheduled_with_conf",
            ...
            triggering_user_name="scheduler",
        )
        ...
        assert response.status_code == 200
        data = response.json()
        # Should only return the manual run with conf
        assert len(data["configurations"]) == 1
        assert data["configurations"][0]["run_id"] == "manual_with_conf"
        assert data["configurations"][0]["conf"] == {"param": "value"}

    def test_should_respect_max_limit_from_config(self, test_client, dag_maker):
        """Test that the configured maximum limit is respected."""
        # Create a DAG using dag_maker
        with dag_maker(DAG1_ID, schedule=None, start_date=timezone.utcnow(), serialized=True):
            EmptyOperator(task_id="task_1")
        ...
        # Create 5 test configurations
        for i in range(5):
            dag_maker.create_dagrun(
                run_id=f"manual_{i}",
                run_type=DagRunType.MANUAL,
                state=DagRunState.SUCCESS,
                conf={"param": f"value{i}"},
                logical_date=timezone.utcnow() - timedelta(hours=i),
                start_date=timezone.utcnow() - timedelta(hours=i),
                triggered_by=DagRunTriggeredByType.UI,
                triggering_user_name="test",
        ...
        with mock.patch("airflow.configuration.conf.getint") as mock_getint:
            mock_getint.return_value = 2
            
            response = test_client.get(
                f"/api/v2/dags/{DAG1_ID}/dagRuns/recent-configurations",
                params={"limit": 10}  # Request more than configured max
            )
            
            assert response.status_code == 200
            data = response.json()
            # Should only return 2 configurations due to config limit
            assert len(data["configurations"]) == 2
            assert data["configurations"][0]["run_id"] == "manual_0"
            assert data["configurations"][1]["run_id"] == "manual_1"
