...
class TestGetAssetLineage(TestAssets):
    ...
    def test_should_respond_200_with_lineage(self, test_client, testing_dag_bundle, session):
        ...
        edges = data["edges"]
        # Convert list of dicts to list of tuples for easier checking
        edges_tuples = [(e["source_id"], e["target_id"]) for e in edges]
        assert ("task:upstream_dag:producer_task", f"asset:{asset1.id}") in edges_tuples
        assert (f"asset:{asset1.id}", "task:downstream_dag:consumer_task") in edges_tuples
