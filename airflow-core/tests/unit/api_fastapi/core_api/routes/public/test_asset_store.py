...
def _create_asset_store_row(session, asset_id: int, key: str, value: str) -> None:
...
class TestListAssetState(TestAssetStateEndpoint):
    ...
    def test_returns_all_keys(self, test_client):
        _create_asset_store_row(self._session, self.asset.id, "watermark", "2026-05-01")
        _create_asset_store_row(self._session, self.asset.id, "file_count", "42")
    ...
    def test_pagination_limit(self, test_client):
        for k in ("watermark", "file_count", "last_run"):
            _create_asset_store_row(self._session, self.asset.id, k, "v")
        self._session.commit()
...
class TestGetAssetState(TestAssetStateEndpoint):
    ...
    def test_key_with_slash_is_supported(self, test_client):
        ...
        _create_asset_store_row(self._session, self.asset.id, "partition/date", "2026-05-01")
        self._session.commit()
...
class TestSetAssetState(TestAssetStateEndpoint):
    ...
    def test_worker_write_core_api_read_roundtrip(self, test_client, value):
        ...
        _create_asset_store_row(self._session, self.asset.id, "k", value)
        self._session.commit()
        assert test_client.get(f"{self._base_url}/k").json()["value"] == value
...
class TestDeleteAssetState(TestAssetStateEndpoint):
    ...
    def test_only_deletes_target_key(self, test_client):
        _create_asset_store_row(self._session, self.asset.id, "watermark", "a")
        _create_asset_store_row(self._session, self.asset.id, "file_count", "b")
        self._session.commit()
    ...
    def test_key_with_slash_is_supported(self, test_client):
        _create_asset_store_row(self._session, self.asset.id, "partition/date", "v")
        self._session.commit()
...
class TestClearAssetState(TestAssetStateEndpoint):
    ...
    def test_clear_does_not_affect_other_assets(self, test_client):
        ...
        self._session.flush()
        _create_asset_store_row(self._session, self.asset.id, "watermark", "mine")
        _create_asset_store_row(self._session, other_asset.id, "watermark", "theirs")
        self._session.commit()
...
class TestRoutesNeverCallCustomBackend(TestAssetStateEndpoint):
    ...
    def test_route_never_calls_get_state_backend(self, test_client, method, path_suffix, kwargs):
        _create_asset_store_row(self._session, self.asset.id, "watermark", "v1")
        self._session.commit()
