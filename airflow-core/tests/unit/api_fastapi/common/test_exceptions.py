...
a = {
    "orig_error": "UNIQUE constraint failed: dag_run.dag_id, dag_run.run_id",
    "reason": "Unique constraint violation",
    "statement": "INSERT INTO dag_run (dag_id, queued_at, logical_date, "
    "start_date, end_date, state, run_id, creating_job_id, run_type, "
    "triggered_by, conf, data_interval_start, data_interval_end, "
    "run_after, last_scheduling_decision, log_template_id, "
    "updated_at, clear_number, backfill_id, bundle_version, "
    "scheduled_by_job_id, context_carrier, created_dag_version_id) "
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT "
