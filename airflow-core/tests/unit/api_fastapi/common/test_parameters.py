...
class TestSearchParam:
    ...
    def test_to_orm_split_or_false_treats_pipe_as_literal(self):
        """With ``split_or=False``, the pipe character is passed through literally."""
        param = _SearchParam(DagModel.dag_id, split_or=False).set_value("2026-01-01|us")
        statement = select(DagModel)
        statement = param.to_orm(statement)
    ...
    def test_to_orm_split_or_false_tilde_alias_still_works(self):
        """``split_or=False`` must not interfere with the ``~`` → ``%`` alias."""
        param = _SearchParam(DagModel.dag_id, split_or=False)
...
class TestPrefixSearchParam:
    ...
    def test_to_orm_split_or_false_treats_pipe_as_literal(self):
        """With ``split_or=False``, the pipe character is part of the prefix and not an OR delimiter."""
        param = _PrefixSearchParam(DagModel.dag_id, split_or=False).set_value("2026-01-01|us")
        statement = select(DagModel)
        statement = param.to_orm(statement)
    ...
    def test_to_orm_split_or_false_tilde_alias_still_works(self):
        """``split_or=False`` must not interfere with the ``~`` → empty alias."""
        param = _PrefixSearchParam(DagModel.dag_id, split_or=False)
        param.set_value(param.transform_aliases("~"))
        statement = select(DagModel)
        statement = param.to_orm(statement)
