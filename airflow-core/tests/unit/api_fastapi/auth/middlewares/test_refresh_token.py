...
class TestJWTRefreshMiddleware:
    ...
    async def test_dispatch_with_refreshed_user(
        self, mock_conf, mock_get_auth_manager, middleware, mock_request, mock_user
    ):
    ...
    @pytest.mark.asyncio
    @patch("airflow.api_fastapi.auth.middlewares.refresh_token.get_auth_manager")
    async def test_refresh_user_expired_token(self, mock_get_auth_manager, middleware, mock_request):
        mock_auth_manager = MagicMock()
        mock_get_auth_manager.return_value = mock_auth_manager
        mock_auth_manager.get_user_from_token = AsyncMock(side_effect=ExpiredSignatureError())
    ...
    @pytest.mark.asyncio
    @patch("airflow.api_fastapi.auth.middlewares.refresh_token.get_auth_manager")
    async def test_refresh_user_invalid_token(self, mock_get_auth_manager, middleware, mock_request):
        mock_auth_manager = MagicMock()
        mock_get_auth_manager.return_value = mock_auth_manager
        mock_auth_manager.get_user_from_token = AsyncMock(side_effect=InvalidTokenError())
