...
class TestResolveMpStartMethod:
    ...
    def test_core_value_used(self):
        assert process_utils.resolve_mp_start_method("scheduler") == "forkserver"
        assert process_utils.resolve_mp_start_method("triggerer") == "forkserver"

    @conf_vars({("core", "mp_start_method"): "forkserver", ("scheduler", "mp_start_method"): "fork"})
...
class TestSetComponentMpStartMethod:
    ...
    def test_forkserver_sets_preload(self, mock_mp):
        mock_mp.get_all_start_methods.return_value = ALL_MP_METHODS
        process_utils.set_component_mp_start_method("triggerer")
        mock_mp.set_start_method.assert_called_once_with("forkserver", force=True)
