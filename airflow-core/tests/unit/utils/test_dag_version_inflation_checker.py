...
class TestDAGTaskDetector:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('dag(dag_id="my_dag")', {"dag": ("airflow.decorators", "dag")}, True),
            ("D(x=1)", {"D": ("airflow", "DAG")}, True),
            # A *DAG/*Dag suffix is a class-naming convention, so it stands on its own.
            ('MyCustomDAG(dag_id="my_dag")', {}, True),
            ('TeamDag(dag_id="my_dag")', {}, True),
            ('sdk.DAG(dag_id="my_dag")', {}, True),
            ('airflow.sdk.DAG(dag_id="my_dag")', {}, True),
            # Lowercase is not a naming convention, so on an attribute it needs an airflow owner.
            ('sdk.dag(dag_id="x")', {"sdk": ("airflow", "sdk")}, True),
            ('company.dag(dag_id="x")', {"company": ("company", "tools")}, False),
            ('company.dag(dag_id="x")', {}, False),
            # A bare lowercase name proves nothing at all, so the suffix stays case-sensitive.
            ("dag(x=1)", {}, False),
            ("dag(x=1)", {"dag": ("company.lib", "dag")}, False),
            ("create_dag()", {}, False),
            ("get_dag()", {}, False),
            ("my_function()", {}, False),
            # An import binds a bare name, so an attribute segment is never an alias.
            ("config.D(x=1)", {"D": ("airflow", "DAG")}, False),
        ],
    )
    def test_is_dag_constructor(self, code, from_imports, expected):
        """Whether a call is treated as constructing a Dag, given the file's imports."""
...
class TestIntegrationScenarios:
    ...
    def test_nested_dag_named_with_does_not_exit_outer_dag_context(self):
        """A nested with-statement whose context manager merely looks like a Dag must not exit the Dag context."""
        code = dedent(
            """
            from airflow import DAG
