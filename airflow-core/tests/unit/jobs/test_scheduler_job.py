...
def test_partitioned_dag_run_segment_rollup_holds_until_all_segments_arrive(
    ...
    ``ConstantMapper`` collapses each region key onto a single ``all_regions``
    partition, so all three events accumulate into one APDR; the
    ``SegmentWindow`` holds the downstream run until ``us``, ``eu``, and ``apac``
    are all present.
    """
    asset_1 = Asset(name="asset-1")
    with dag_maker(
