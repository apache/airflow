...
class TestSchedulerJob:
    ...
    def test_queued_dagruns_stops_creating_when_max_active_is_reached(self, dag_maker):
        ...
        for dr in drs:
            ...
            session.merge(dr)
        session.commit()
        assert session.scalar(select(func.count()).select_from(DagRun).where(DagRun.state == State.RUNNING)) == 10
        for _ in range(20):
            self.job_runner._create_dag_runs([orm_dag], session)
        assert session.scalar(select(func.count()).select_from(DagRun)) == 10
        assert session.scalar(select(func.count()).select_from(DagRun).where(DagRun.state == State.RUNNING)) == 10
        assert session.scalar(select(func.count()).select_from(DagRun).where(DagRun.state == State.QUEUED)) == 0
    ...
    def test_scheduler_task_start_date_catchup_true(self, testing_dag_bundle):
        ...
        session = settings.Session()
        tiq = select(TaskInstance).where(TaskInstance.dag_id == dag_id)
        ti1s = session.scalars(tiq.where(TaskInstance.task_id == "dummy1")).all()
        ti2s = session.scalars(tiq.where(TaskInstance.task_id == "dummy2")).all()
    ...
    def test_scheduler_verify_priority_and_slots(self, dag_maker, mock_executor):
        ...
        ti0 = session.scalars(
            select(TaskInstance).where(
                TaskInstance.task_id == "test_scheduler_verify_priority_and_slots_t0"
            )
        ).first()
        ...
        ti1 = session.scalars(
            select(TaskInstance).where(
                TaskInstance.task_id == "test_scheduler_verify_priority_and_slots_t1"
            )
        ).first()
        assert ti1.state == State.QUEUED
        ...
        ti2 = session.scalars(
            select(TaskInstance).where(
                TaskInstance.task_id == "test_scheduler_verify_priority_and_slots_t2"
            )
        ).first()
        assert ti2.state == State.QUEUED
    ...
    def test_verify_integrity_not_called_for_versioned_bundles(self, dag_maker, session):
        ...
        session.merge(dr)
        session.commit()
        drs = session.scalars(select(DagRun).options(joinedload(DagRun.task_instances))).all()
        dr = drs[0]
    ...
    def test_create_dag_runs_assets(self, session, dag_maker):
        ...
        assert (
            session.scalars(select(AssetDagRunQueue).where(AssetDagRunQueue.target_dag_id == dag2.dag_id)).one()
            is not None
        )
        ...
        assert (
            session.scalars(select(AssetDagRunQueue).where(AssetDagRunQueue.target_dag_id == dag3.dag_id)).one_or_none()
            is None
        )
    ...
    def test_more_runs_are_not_created_when_max_active_runs_is_reached(self, dag_maker, caplog):
        ...
        assert (
            session.scalar(
                select(func.count()).select_from(DagRun).where(DagRun.state.in_([DagRunState.RUNNING, DagRunState.QUEUED]))
            )
            == 0
        )
    ...
    def test_max_active_runs_in_a_dag_doesnt_prevent_backfill_from_running_catchup_true(self, dag_maker):
        ...
        total_running_count = (
            session.scalar(select(func.count(DagRun.id)).where(DagRun.state == State.RUNNING))
        )
    ...
    def test_dag_file_processor_process_task_instances(self, state, start_date, end_date, dag_maker):
        ...
        with create_session() as session:
            ...
            self.job_runner._schedule_dag_run(dr, session)
            assert session.scalar(select(func.count()).select_from(TaskInstance).where(TaskInstance.state == State.SCHEDULED)) == 1
    ...
    def test_dag_file_processor_process_task_instances_depends_on_past(
        ...
        with create_session() as session:
            ...
            self.job_runner._schedule_dag_run(dr, session)
            assert session.scalar(select(func.count()).select_from(TaskInstance).where(TaskInstance.state == State.SCHEDULED)) == 2
    ...
    def test_catchup_works_correctly(self, dag_maker, testing_dag_bundle):
        ...
        assert (
            session.scalar(
                ...
                .where(DagRun.logical_date != DEFAULT_DATE)  # exclude the first run
            )
...
def test_schedule_dag_run_with_upstream_skip(dag_maker, session):
    ...
    def test_start_queued_dagruns_uses_latest_max_active_runs_from_dag_model(self, dag_maker, session):
        ...
        dag_run_1 = (
            session.scalars(select(DagRun).where(DagRun.dag_id == dag.dag_id).order_by(DagRun.logical_date)).first()
        )
        ...
        running_count = session.scalar(
            select(func.count()).where(DagRun.dag_id == dag.dag_id, DagRun.state == DagRunState.RUNNING)
        )
        queued_count = session.scalar(
            select(func.count()).where(DagRun.dag_id == dag.dag_id, DagRun.state == DagRunState.QUEUED)
        )
