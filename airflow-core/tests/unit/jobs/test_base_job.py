...
class TestJob:
    ...
    @pytest.mark.parametrize(
        ("job_type", "config_section", "threshold_value"),
        [
            ("DagProcessorJob", "dag_processor", 120),
            ("SchedulerJob", "scheduler", 180),
            ("TriggererJob", "triggerer", 90),
        ],
    )
    def test_health_check_threshold(self, job_type, config_section, threshold_value):
        config_key = f"{config_section}_health_check_threshold"
