...
class TestCalculatedDeadlineDatabaseCalls:
    ...
    def test_average_runtime_multiplier_validation(self):
        with pytest.raises(ValueError, match="multiplier must be a finite positive number"):
        ...
        with pytest.raises(ValueError, match="multiplier must be a finite positive number"):
            DeadlineReference.AVERAGE_RUNTIME(max_runs=10, multiplier=float("nan"))

        with pytest.raises(ValueError, match="multiplier must be a finite positive number"):
            DeadlineReference.AVERAGE_RUNTIME(max_runs=10, multiplier=float("inf"))
