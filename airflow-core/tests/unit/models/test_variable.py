...
class TestVariable:
    ...
    def test_get_variable_with_team(self, testing_team, session):
        ...
        result = Variable.get(key="key", team_id=testing_team.id)
        assert result == "value"
    ...
    def test_variable_update_with_team(self, testing_team, session):
        Variable.set(key="test_key", value="value1", team_id=testing_team.id, session=session)
        Variable.update(key="test_key", value="value2", team_id=testing_team.id, session=session)
        assert Variable.get("test_key", team_id=testing_team.id) == "value2"
    ...
    def test_variable_delete_with_team(self, testing_team, session):
        ...
        # No-op if the variable doesn't exist
        Variable.delete(key=key, team_id=testing_team.id, session=session)
        with pytest.raises(KeyError):
            Variable.get(key)
