...
class TestDag:
    ...
    def test_create_dagrun_logs_when_dag_at_max_active_runs(self, testing_dag_bundle, caplog):
        dag = DAG(
            dag_id="test_create_dagrun_logs_when_dag_at_max_active_runs", schedule=None, max_active_runs=1
        )
        scheduler_dag = sync_dag_to_db(dag)
        ...
        with caplog.at_level("INFO", logger="airflow.serialization.definitions.dag"):
            scheduler_dag.create_dagrun(
                run_id="queued_run",
                logical_date=DEFAULT_DATE + timedelta(hours=1),
                data_interval=(DEFAULT_DATE + timedelta(hours=1), DEFAULT_DATE + timedelta(hours=1)),
        ...
        assert {
            ...
            "dag_id": "test_create_dagrun_logs_when_dag_at_max_active_runs",
            "run_id": "queued_run",
            "active_runs": 1,
            "max_active_runs": 1,
            "log_level": "info",
    ...
    def test_create_dagrun_does_not_log_when_dag_below_max_active_runs(self, testing_dag_bundle, caplog):
        dag = DAG(
            dag_id="test_create_dagrun_does_not_log_when_dag_below_max_active_runs",
            schedule=None,
            max_active_runs=1,
        )
        scheduler_dag = sync_dag_to_db(dag)
    ...
    def test_create_dagrun_does_not_log_for_non_queued_state(self, testing_dag_bundle, caplog):
        dag = DAG(
            dag_id="test_create_dagrun_does_not_log_for_non_queued_state", schedule=None, max_active_runs=1
        )
        scheduler_dag = sync_dag_to_db(dag)
        scheduler_dag.create_dagrun(
            run_id="running_run",
    ...
    def test_create_dagrun_does_not_log_for_backfill_run(self, testing_dag_bundle, caplog, session):
        dag = DAG(dag_id="test_create_dagrun_does_not_log_for_backfill_run", schedule=None, max_active_runs=1)
        scheduler_dag = sync_dag_to_db(dag)
        scheduler_dag.create_dagrun(
            run_id="running_run",
            logical_date=DEFAULT_DATE,
