...
from airflow.models.dag import DagModel, DagOwnerAttributes, DagTag, get_asset_triggered_next_run_info
...
class TestDag:
    ...
    def test_bulk_write_to_db_max_active_runs(self, testing_dag_bundle, state, catchup, expected_next_dagrun):
        ...
        SerializedDAG.bulk_write_to_db("testing", None, [scheduler_dag])
    ...
    def test_bulk_write_to_db_assets(self, testing_dag_bundle):
        ...
        session = settings.Session()
        scheduler_dag1 = SerializedDAG.deserialize_dag(SerializedDAG.serialize_dag(dag1))
        scheduler_dag2 = SerializedDAG.deserialize_dag(SerializedDAG.serialize_dag(dag2))
        scheduler_dag1.clear()
        SerializedDAG.bulk_write_to_db("testing", None, [scheduler_dag1, scheduler_dag2], session=session)
        session.commit()
...
def test_get_next_data_interval(
    ...
    assert create_scheduler_dag(dag).get_next_data_interval(dag_model) == expected_data_interval
