...
class TestDag:
    ...
    @staticmethod
    def _make_awaiting_input_dag(dag_id, mock_object, *, defaults=None, params=None):
        ...
        class AskOperator(BaseOperator):
            ...
            def execute_complete(self, context, event):
                if "error" in event:
                    # The no-input fallback resumes with the sweep's timeout error event so
                    # operator error handling runs; mirror HITLOperator and raise.
                    raise RuntimeError(event["error"])
                mock_object(event["chosen_options"], event["params_input"])
    ...
    def test_dag_test_hitl_task_stays_parked_until_external_response(self, testing_dag_bundle, monkeypatch):
        """
        ...
        """
        mock_object = mock.MagicMock()
        dag = self._make_awaiting_input_dag("test_dag_test_hitl_external", mock_object)

        parked_states_seen = []
        wait_loops = 0

        def respond_instead_of_waiting(seconds):
            nonlocal wait_loops
            wait_loops += 1
            # Fail fast instead of looping forever if the resume is never picked up.
            assert wait_loops < 20, "dag.test() kept waiting after the external response"
        ...
        monkeypatch.setattr(time, "sleep", respond_instead_of_waiting)
        ...
        mock_object.assert_called_once_with(["Approve"], {})
