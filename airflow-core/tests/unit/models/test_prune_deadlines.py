...
class TestPruneDeadlines:
    ...
    def test_handle_miss_then_prune_does_not_delete_missed(self, dagrun, session):
        ...
        # Reach the post-miss state directly: the deadline is marked ``missed`` and its callback
        # is queued (owned by the scheduler/triggerer). We set this up explicitly rather than via
        # ``handle_miss`` so the test pins the prune ``~missed`` guard in isolation, independent of
        # how the missed state is produced.
