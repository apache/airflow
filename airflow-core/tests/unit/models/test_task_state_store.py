...
class TestTaskStateStoreModel:
    ...
    def test_cascade_delete_on_dag_run(self, session: Session, dag_run: DagRun):
        """Deleting a DagRun cascades to its task_state_store rows — no orphans remain."""
        session.add(
            TaskStateStoreModel(
                dag_run_id=dag_run.id,
                task_id=TASK_ID,
        ...
        remaining = session.scalars(
            select(TaskStateStoreModel).where(TaskStateStoreModel.dag_run_id == dag_run.id)
        ).all()
        assert remaining == []
    ...
    def test_different_dag_runs_are_isolated(self, session: Session, dag_run: DagRun):
        ...
        result = session.scalar(
            select(TaskStateStoreModel).where(
                TaskStateStoreModel.dag_run_id == run2.id,
                TaskStateStoreModel.key == "watermark",
            )
        )
