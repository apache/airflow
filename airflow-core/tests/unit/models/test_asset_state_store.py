...
class TestAssetStateStoreModel:
    def test_insert_and_read(self, session: Session, asset: AssetModel):
        ...
        result = session.scalar(
            select(AssetStateStoreModel).where(
                ...
                AssetStateStoreModel.key == "last_processed_at",
            )
        )
    ...
    def test_duplicate_pk_raises(self, session: Session, asset: AssetModel):
        ...
        session.add(AssetStateStoreModel(asset_id=asset.id, key="watermark", value="first"))
        session.flush()

        session.add(AssetStateStoreModel(asset_id=asset.id, key="watermark", value="duplicate"))
    ...
    def test_multiple_keys_per_asset(self, session: Session, asset: AssetModel):
        ...
        for key in ("watermark", "last_file_count", "last_error"):
            session.add(AssetStateStoreModel(asset_id=asset.id, key=key, value=f"val_{key}"))
        session.flush()
    ...
    def test_cascade_delete_on_asset(self, session: Session, asset: AssetModel):
        """Deleting an asset cascades to its asset_state_store rows — no orphans remain."""
        session.add(
    ...
    def test_different_assets_are_isolated(self, session: Session, asset: AssetModel):
        ...
        result = session.scalar(
            select(AssetStateStoreModel).where(
                AssetStateStoreModel.asset_id == asset2.id,
