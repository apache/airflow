...
from airflow.sdk import DAG, BaseOperator, Metadata, task, task_group
...
class TestTaskInstance:
    ...
    def test_outlet_asset_extra(self, dag_maker, session):
        ...
        dag_maker.run_ti("write1", dr)
        dag_maker.run_ti("write2", dr)
    ...
    def test_outlet_asset_alias_asset_inactive(self, dag_maker, session):
        ...
        dr = dag_maker.create_dagrun()
        dag_maker.run_ti("producer_without_inactive", dr)
        dag_maker.run_ti("producer_with_inactive", dr)
    ...
    def test_inlet_unresolved_asset_alias(self, dag_maker, session, mock_supervisor_comms):
        ...
        # Should be done.
        assert not ti.dag_run.task_instance_scheduling_decisions(session=session).schedulable_tis
    ...
    def test_skipped_task_call_on_skipped_callback(self, dag_maker):
        ...
        ti = dag_maker.run_ti(task.task_id)
...
class TestTaskInstanceRecordTaskMapXComPush:
    ...
    def test_not_recorded_if_leaf(self, dag_maker, xcom_value):
        ...
        dag_maker.run_ti("push_something")
