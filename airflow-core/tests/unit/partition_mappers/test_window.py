...
class TestForwardWindow:
    def test_default_direction_is_backward(self):
        assert WeekWindow().direction is WindowDirection.BACKWARD

    @pytest.mark.parametrize(
        ("window", "anchor", "expected"),
        [
            pytest.param(
                HourWindow(direction=WindowDirection.FORWARD),
                datetime(2024, 3, 4, 0),
            ...
            pytest.param(
                MonthWindow(direction=WindowDirection.FORWARD),
                datetime(2024, 3, 1),
                ...
                id="month_forward_trailing",
            ),
            pytest.param(
                QuarterWindow(direction=WindowDirection.FORWARD),
                datetime(2024, 1, 1),
                [datetime(2023, 11, 1), datetime(2023, 12, 1), datetime(2024, 1, 1)],
                id="quarter_forward_trailing",
            ),
            pytest.param(
                YearWindow(direction=WindowDirection.FORWARD),
                datetime(2024, 1, 1),
                [datetime(2023, m, 1) for m in range(2, 13)] + [datetime(2024, 1, 1)],
                id="year_forward_trailing",
            ),
        ],
    )
    def test_forward_yields_trailing_period_ending_at_anchor(self, window, anchor, expected):
        result = list(window.to_upstream(anchor))
        assert result == expected
        assert result[-1] == anchor

    def test_month_forward_trailing_not_calendar_month(self):
        """Month forward yields the trailing period (prev_month_start, anchor], not a calendar month."""
        anchor = datetime(2024, 3, 1)
        result = list(MonthWindow(direction=WindowDirection.FORWARD).to_upstream(anchor))
    ...
    @pytest.mark.parametrize(
        "window",
        [
            pytest.param(MonthWindow(direction=WindowDirection.FORWARD), id="month"),
            pytest.param(QuarterWindow(direction=WindowDirection.FORWARD), id="quarter"),
            pytest.param(YearWindow(direction=WindowDirection.FORWARD), id="year"),
        ],
    )
    def test_forward_on_non_day_one_raises(self, window):
        """_require_day_one fires before the forward shift — non-day-1 + FORWARD direction still raises."""
        with pytest.raises(ValueError, match="expects a period start on day 1"):
    ...
    @pytest.mark.parametrize(
        "window",
        [
            pytest.param(HourWindow(direction=WindowDirection.FORWARD), id="hour"),
            pytest.param(DayWindow(direction=WindowDirection.FORWARD), id="day"),
            pytest.param(WeekWindow(direction=WindowDirection.FORWARD), id="week"),
            pytest.param(MonthWindow(direction=WindowDirection.FORWARD), id="month"),
            pytest.param(QuarterWindow(direction=WindowDirection.FORWARD), id="quarter"),
            pytest.param(YearWindow(direction=WindowDirection.FORWARD), id="year"),
        ],
    ...
    def test_serialize_roundtrip_with_forward(self, window):
        """WindowDirection.FORWARD survives serialize → deserialize; behaviour is identical."""
        restored = decode_window(encode_window(window))
        assert restored.direction is WindowDirection.FORWARD
    ...
    def test_serialize_roundtrip_backward(self, window_cls):
        window = window_cls(direction=WindowDirection.BACKWARD)
        restored = decode_window(encode_window(window))
        assert restored.direction is WindowDirection.BACKWARD
    ...
    def test_forward_non_day_one_does_not_raise(self, window_cls, anchor, expected_count):
        window = window_cls(direction=WindowDirection.FORWARD)
        result = list(window.to_upstream(anchor))
