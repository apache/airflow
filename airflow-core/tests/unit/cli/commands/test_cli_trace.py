...
class TestCliDagsTriggerSpan:
    @staticmethod
    def _api_client(run_id):
        api_client = mock.MagicMock()
        api_client.dags.trigger.return_value.model_dump.return_value = {"dag_run_id": run_id}
        return api_client

    def test_emits_span(self, captured_spans, parser, monkeypatch):
        monkeypatch.setenv("TRACEPARENT", EXAMPLE_TRACEPARENT)
        api_client = self._api_client("trace_run")
        cli_args = parser.parse_args(["dags", "trigger", "example_bash_operator", "--run-id", "trace_run"])
        dag_command.dag_trigger(cli_args, api_client=api_client)
        ...
        api_client.dags.trigger.assert_called_once()

    def test_no_span_without_traceparent(self, captured_spans, parser, monkeypatch):
        monkeypatch.delenv("TRACEPARENT", raising=False)
        api_client = self._api_client("trace_run_3")
        cli_args = parser.parse_args(["dags", "trigger", "example_bash_operator", "--run-id", "trace_run_3"])
        dag_command.dag_trigger(cli_args, api_client=api_client)

        # No inbound trace context -> no stray CLI span (would surface as a second trace);
        # the command still runs.
        assert len(captured_spans.get_finished_spans()) == 0
        api_client.dags.trigger.assert_called_once()

    def test_honours_traceparent(self, captured_spans, parser, monkeypatch):
        api_client = self._api_client("trace_run_2")
        monkeypatch.setenv("TRACEPARENT", EXAMPLE_TRACEPARENT)
...
class TestCliDagsTestSpan:
    ...
    def test_emits_span(self, mock_get_dag, captured_spans, parser, monkeypatch):
        monkeypatch.setenv("TRACEPARENT", EXAMPLE_TRACEPARENT)
...
class TestCliBackfillCreateSpan:
    ...
    def test_emits_span_on_dry_run(self, mock_dry_run, captured_spans, parser, monkeypatch):
        monkeypatch.setenv("TRACEPARENT", EXAMPLE_TRACEPARENT)
    ...
    def test_emits_span_on_non_dry_run(self, mock_create, captured_spans, parser, monkeypatch):
        monkeypatch.setenv("TRACEPARENT", EXAMPLE_TRACEPARENT)
