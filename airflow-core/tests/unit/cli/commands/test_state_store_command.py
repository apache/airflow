...
from airflow._shared.state import BaseStoreBackend
from airflow.cli.commands.state_store_command import clean_state_store
from airflow.state.metastore import MetastoreBackend

pytestmark = pytest.mark.db_test


class TestStateStoreCleanCommand:
    def test_clean_calls_backend(self):
        ...
        with (
            ...
            patch.object(backend, "cleanup", autospec=True),
        ):
    ...
    def test_custom_backend_is_skipped(self, capsys):
        ...
        args = Namespace(dry_run=False, verbose=False)
        custom_backend = MagicMock(spec=BaseStoreBackend)
        with mock.patch(
            "airflow.cli.commands.state_store_command.get_state_backend",
            ...
            custom_backend.cleanup.assert_not_called()
