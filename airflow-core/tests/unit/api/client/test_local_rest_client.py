...
class TestLocalRESTClient:
    ...
    def test_close_releases_in_process_loop(self):
        client = get_local_rest_client(process_type="test")
        # Force transport creation, which starts the background event loop + thread.
        client.pools.list()
        api = client._in_process_api
        assert api._finalizer is not None
        assert api._finalizer.alive

        client.close()

        # The finalizer (which stops the loop/thread and unwinds the lifespan) has run.
        assert not api._finalizer.alive
        # Idempotent: closing again is a no-op and does not raise.
        client.close()

    def test_raises_when_orm_access_blocked(self, monkeypatch):
        # In task/worker contexts ORM access is blocked; the client must refuse rather than
        # escalate to privileged Core API access.
        monkeypatch.setattr(LocalRESTClient, "_is_orm_access_blocked", lambda _: True)

        client = LocalRESTClient()

        with pytest.raises(RuntimeError, match="metadata database access is "):
            _ = client._http
...
class TestPoolsClientViaLocalREST:
    ...
    def test_identifier_with_reserved_url_chars_is_encoded(self, client):
        # Reserved URL characters (?, #) in a path segment must be percent-encoded, otherwise the
        # request would be misrouted (e.g. "?" starting a query string) and the round-trip would fail.
        name = "weird?name#1"
        client.pools.create(name=name, slots=2)
        result = client.pools.get(name)
        assert result.pool == name
...
class TestTaskInstancesClientViaLocalREST:
    ...
    def test_update_task_instance_note(self, client, dag_maker):
        with dag_maker("update_ti_dag", schedule=None):
            EmptyOperator(task_id="my_task")
        dr = dag_maker.create_dagrun(state=DagRunState.SUCCESS)
        # The PATCH route returns a collection of affected task instances.
        result = client.task_instances.update("update_ti_dag", dr.run_id, "my_task", note="a note")
        assert result.total_entries == 1
        assert result.task_instances[0].task_id == "my_task"
        assert result.task_instances[0].note == "a note"
...
class TestAssetsClientViaLocalREST:
    ...
    def test_materialize_asset(self, client, dag_maker, session):
        # materialize_asset() calls get_auth_manager().is_authorized_dag(...) directly (not via the
        # requires_access_* helpers), so this also guards that the SystemUser bypass reaches such
        # direct auth-manager checks and returns a typed DAGRunResponse rather than 403/500.
        from airflow.sdk import Asset

        with dag_maker("producer_dag", schedule=None):
            EmptyOperator(task_id="produce", outlets=[Asset(name="mat_asset", uri="s3://mat/1")])
        client.dags.unpause("producer_dag")

        asset_model = session.scalars(select(AssetModel).filter_by(name="mat_asset")).one()
        result = client.assets.materialize(asset_model.id)
        assert result.dag_id == "producer_dag"
