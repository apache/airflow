...
def test_trigger_dag_populates_partition_date_for_cron_partition_timetable(dag_maker, session):
    ...
    assert dag_run.partition_date is not None
    assert dag_run.partition_date.year == 2025
    assert dag_run.partition_date.month == 6
    assert dag_run.partition_date.day == 1
    assert dag_run.partition_date.tzinfo is not None
