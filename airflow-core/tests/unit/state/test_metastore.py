...
@pytest.fixture
def backend() -> MetastoreStateStoreBackend:
...
class TestMetastoreStateStoreBackendTaskScope:
    def test_get_returns_none_for_missing_key(
        self, session: Session, backend: MetastoreStateStoreBackend, dag_run: DagRun
    ):
    ...
    def test_delete_removes_key(self, session: Session, backend: MetastoreStateStoreBackend, dag_run: DagRun):
        scope = TaskScope(dag_id=DAG_ID, run_id=RUN_ID, task_id=TASK_ID)
        backend.set(scope, "job_id", "app_1234", session=session)
    ...
    def test_set_raises_for_missing_dag_run(self, session: Session, backend: MetastoreStateStoreBackend):
    ...
    def test_cleanup_batches_deletes(
        self, session: Session, backend: MetastoreStateStoreBackend, dag_run: DagRun
    ):
...
class TestMetastoreStateStoreBackendAssetScope:
    def test_get_returns_none_for_missing_key(
        self, session: Session, backend: MetastoreStateStoreBackend, asset: AssetModel
    ):
    ...
    def test_set_asset_state_store_writes_writer(
        self, backend: MetastoreStateStoreBackend, asset_committed: AssetModel, create_task_instance
    ):
...
class TestMetastoreStateStoreBackendAsync:
    async def test_aset_and_aget_task_roundtrip(
        self, backend: MetastoreStateStoreBackend, dag_run_committed: DagRun
    ):
        scope = TaskScope(dag_id=DAG_ID, run_id=RUN_ID, task_id=TASK_ID)
    ...
    async def test_adelete_task_removes_key(
        self, backend: MetastoreStateStoreBackend, dag_run_committed: DagRun
    ):
        scope = TaskScope(dag_id=DAG_ID, run_id=RUN_ID, task_id=TASK_ID)
        await backend.aset(scope, "job_id", "app_async")
    ...
    async def test_aset_and_aget_asset_roundtrip(
        self, backend: MetastoreStateStoreBackend, asset_committed: AssetModel
    ):
        scope = AssetScope(asset_id=asset_committed.id)
    ...
    async def test_aset_task_raises_for_missing_dag_run(self, backend: MetastoreStateStoreBackend):
        scope = TaskScope(dag_id="nonexistent_dag", run_id="nonexistent_run", task_id=TASK_ID)
        with pytest.raises(ValueError, match="No DagRun found"):
            await backend.aset(scope, "job_id", "app_async")
...
class TestResolveStoreBackend:
    @conf_vars({("state_store", "backend"): "airflow.state.metastore.MetastoreStateStoreBackend"})
    def test_resolve_returns_configured_backend(self):
        ...
        assert resolve_state_backend() is MetastoreStateStoreBackend
