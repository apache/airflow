...
class TestOtelTrace:
    ...
    @pytest.mark.parametrize(
        ["host", "port", "should_use_env_vars"],
        [
            pytest.param("-", "0", True, id="no_host_no_port"),
            pytest.param("otel_collector1", "0", True, id="no_port"),
            pytest.param("-", "4318", True, id="no_host"),
            pytest.param("otel_collector1", "4318", False, id="host_and_port"),
        ],
    )
    def test_config_priorities(self, host, port, should_use_env_vars):
        env_endpoint = "http://localhost:4318"

        with env_vars(
            {
                "AIRFLOW__TRACES__OTEL_HOST": host,
                "AIRFLOW__TRACES__OTEL_PORT": port,
                "OTEL_EXPORTER_OTLP_ENDPOINT": env_endpoint,
            }
        ):
            tracer = otel_tracer.get_otel_tracer(Trace)

            if should_use_env_vars:
                assert tracer.span_exporter._endpoint == env_endpoint
            else:
                assert tracer.span_exporter._endpoint == f"http://{host}:{port}/v1/traces"
