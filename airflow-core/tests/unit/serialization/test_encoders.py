...
class TestEncodeDeadlineAlert:
    """Round-trip tests for encode_deadline_alert / ensure_serialized_deadline_alert.

    These were only covered indirectly (via dagrun-creation and migration tests). The
    ``interval`` round-trip — both a fixed ``timedelta`` and a dynamic ``VariableInterval`` —
    is the serialization primitive that the UI interval-shape handling and the 0117 migration
    both depend on, so it warrants a focused, direct round-trip test.
    """

    # An importable (non-async) callable for SyncCallback; identity doesn't matter for serde.
    _CB = "airflow.models.deadline.Deadline.prune_deadlines"

    def _alert(self, interval, *, name="alert", kwargs=None):
        ...
        return DeadlineAlert(
            ...
            interval=interval,
            callback=SyncCallback(self._CB, kwargs=kwargs),
            name=name,
        )

    def test_timedelta_interval_round_trips(self):
        from airflow.serialization.encoders import encode_deadline_alert, ensure_serialized_deadline_alert

        alert = self._alert(datetime.timedelta(hours=2), name="td", kwargs={"config": {"k": "v"}})

        encoded = encode_deadline_alert(alert)
        # The interval is the Airflow-serialized timedelta dict, not a bare number.
        assert encoded["interval"]["__classname__"] == "datetime.timedelta"
        assert encoded["interval"]["__data__"] == 7200.0

        restored = ensure_serialized_deadline_alert(alert)
        assert restored.interval == datetime.timedelta(hours=2)
        assert restored.name == "td"

    def test_nested_callback_kwargs_round_trip_intact(self):
        """A callback's deeply-nested kwargs (dicts within lists within dicts) must survive the
        encode→decode round-trip exactly. Complements Bug #20 (which fixed *hashing* of nested
        kwargs); this verifies serialization *fidelity* of the nested structure through the JSON
        ``callback_def`` column."""
        from airflow.serialization.encoders import ensure_serialized_deadline_alert

        nested = {"config": {"retries": 3, "tags": [1, 2, {"deep": True}]}, "flat": "x"}
        alert = self._alert(datetime.timedelta(minutes=5), name="nested", kwargs=nested)

        restored = ensure_serialized_deadline_alert(alert)
        # The decoded callback's kwargs must equal the original nested structure exactly.
        assert restored.callback.kwargs == nested

    @pytest.mark.parametrize(
        "key",
        [
            pytest.param("deadline_seconds", id="normal"),
            pytest.param("", id="empty_key"),
            pytest.param("with spaces & special/chars.日本語", id="special_chars"),
            pytest.param("a" * 250, id="long_key"),
        ],
    )
    def test_variable_interval_round_trips(self, key):
        """A VariableInterval's ``key`` must survive encode→decode intact for any string content —
        empty, special characters / unicode, and long keys included (it's a user-provided Variable
        name and goes through the JSON ``interval`` column)."""
        from airflow.sdk.definitions.deadline import VariableInterval
        from airflow.serialization.encoders import encode_deadline_alert, ensure_serialized_deadline_alert

        alert = self._alert(VariableInterval(key), name="vi")

        encoded = encode_deadline_alert(alert)
        assert encoded["interval"]["__classname__"].endswith("VariableInterval")
        assert encoded["interval"]["__data__"] == {"key": key}

        restored = ensure_serialized_deadline_alert(alert)
        assert isinstance(restored.interval, VariableInterval)
        assert restored.interval.key == key

    def test_legacy_numeric_interval_decodes_to_timedelta(self):
        """A blob predating the JSON-interval change stored ``interval`` as a bare number
        (total_seconds). decode must still accept it for backward compatibility."""
        ...
        alert = self._alert(datetime.timedelta(minutes=5), name="legacy")
        encoded = encode_deadline_alert(alert)
        # Simulate the legacy on-disk shape: a bare float instead of the serialized dict.
        encoded["interval"] = 300.0

        restored = decode_deadline_alert(encoded)
        assert restored.interval == datetime.timedelta(seconds=300)
