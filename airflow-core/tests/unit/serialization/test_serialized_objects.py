...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"input_format": "%Y-%m-%dT%H:%M:%S%z", "output_format": "%Y-%m-%dT%H"},
        ),
        (
            DailyMapper,
            [],
            "airflow.partition_mappers.temporal.DailyMapper",
            {"input_format": "%Y-%m-%dT%H:%M:%S%z", "output_format": "%Y-%m-%d"},
        ),
        (
            WeeklyMapper,
            [],
            "airflow.partition_mappers.temporal.WeeklyMapper",
            {"input_format": "%Y-%m-%dT%H:%M:%S%z", "output_format": "%Y-%m-%d (W%V)"},
        ),
        (
            MonthlyMapper,
            [],
            "airflow.partition_mappers.temporal.MonthlyMapper",
            {"input_format": "%Y-%m-%dT%H:%M:%S%z", "output_format": "%Y-%m"},
        ),
        (
            QuarterlyMapper,
            [],
            "airflow.partition_mappers.temporal.QuarterlyMapper",
            {"input_format": "%Y-%m-%dT%H:%M:%S%z", "output_format": "%Y-Q{quarter}"},
        ),
        (
            YearlyMapper,
            [],
            "airflow.partition_mappers.temporal.YearlyMapper",
            {"input_format": "%Y-%m-%dT%H:%M:%S%z", "output_format": "%Y"},
        ),
    ],
)
