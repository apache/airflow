...
On the CLI, ``airflowctl dags clear`` discards task state the same way, since it clears every task
instance in the matched Dag run(s) through the same endpoint, but it has no ``--keep-task-state``
equivalent yet to opt back in. ``airflow dags clear`` and ``airflow tasks clear`` (airflow-core,
both deprecated in favor of their ``airflowctl`` equivalents) are unaffected: they call
``clear_task_instances()`` directly and never touch task state.
