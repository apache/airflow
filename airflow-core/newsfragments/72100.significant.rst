...
On the CLI, ``airflowctl tasks clear --keep-task-state`` is the equivalent flag. ``airflow dags
clear`` and ``airflow tasks clear`` (airflow-core) are unaffected: the former clears Dag runs and
never touches task state, and the latter is deprecated in favor of ``airflowctl tasks clear``.
