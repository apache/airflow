...
Each migrated command now emits a ``RemovedInAirflow4Warning`` pointing at its ``airflowctl``
equivalent (for example ``airflow dags trigger`` → ``airflowctl dags trigger``). The ``airflow``
commands continue to work for now but will become thin aliases for their ``airflowctl``
counterparts in a future Airflow release; switch to ``airflowctl`` when convenient.
