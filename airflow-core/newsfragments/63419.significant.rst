...
The ``partition_key`` is also exposed on ``dag_run.partition_key`` inside task context, and can be supplied when triggering a DAG run manually via the REST API
