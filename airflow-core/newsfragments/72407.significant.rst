...
Manual runs, backfills and asset materializations can also start the drain
themselves, so they execute on a paused Dag without resuming its schedule.
Choose **Run without resuming the schedule** in the UI, or set ``drain_dag``
in the REST API request; the Dag pauses again once its runs finish.
