...
and dispatched by ``trigger_workloads`` alongside other workload types. A deprecated
``supports_connection_test`` compat attribute remains; ``queued_connection_tests`` and
...
Legacy ``supports_callbacks`` / ``supports_connection_test`` usage on out-of-tree executors is
still honored, with a deprecation warning in each case:

- Class attributes: ``BaseExecutor.__init_subclass__`` folds ``= True`` / ``= False``
  declarations into ``supported_workload_types`` (adding or removing the type while preserving
  everything else inherited from the parent class) and drops the plain bool so the compat
  attribute keeps working on the subclass.
- Class-level reads (``SomeExecutor.supports_callbacks``) and instance reads still return a
  ``bool`` derived from ``supported_workload_types``.
- Instance assignment (``self.supports_callbacks = True`` in ``__init__``) still works and toggles
  the corresponding entry in that instance's ``supported_workload_types``.

Subclasses that override the no-longer-invoked ``trigger_tasks``, ``trigger_connection_tests``
or ``order_queued_tasks_by_priority`` methods also get a warning at class-definition time.
