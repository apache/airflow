...
class DagFileProcessorManager(LoggingMixin):
    ...
    def _refresh_dag_bundles(self, known_files: dict[str, set[DagFileInfo]]):
        ...
        if self._bundles_manager.check_config_path_changes():
            ...
            self._bundles_manager.sync_bundles_to_db(deactivate_missing=not self.bundle_names_to_parse)
            dag_bundles = list(self._bundles_manager.get_all_dag_bundles())
            if self.bundle_names_to_parse:
                dag_bundles = [bundle for bundle in dag_bundles if bundle.name in self.bundle_names_to_parse]
            self._dag_bundles = dag_bundles
            ...
            for bundle_name in removed_bundles:
                ...
                self._bundle_name_to_team_name.pop(bundle_name, None)
