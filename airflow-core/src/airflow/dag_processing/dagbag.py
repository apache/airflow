    ...
    def _load_modules_from_file(self, filepath, safe_mode):
        ...
        def parse(mod_name, filepath):
            try:
                ...
                if hasattr(new_module, "__airflow_dag_parsing_errors"):
                    relative_filepath = self._get_relative_fileloc(filepath)
                    if new_module.__airflow_dag_parsing_errors:
                        self.import_errors[relative_filepath] = new_module.__airflow_dag_parsing_errors
        ...
        with zipfile.ZipFile(filepath) as current_zip_file:
            for zip_info in current_zip_file.infolist():
                ...
                try:
                    ...
                    # Collect individual DAG parsing errors from safe_dag context manager
                    if hasattr(current_module, "__airflow_dag_parsing_errors"):
                        fileloc = os.path.join(filepath, zip_info.filename)
                        relative_fileloc = self._get_relative_fileloc(fileloc)
                        if current_module.__airflow_dag_parsing_errors:
                            self.import_errors[relative_fileloc] = current_module.__airflow_dag_parsing_errors
