...
class DagBag(LoggingMixin):
    ...
    :param include_examples: whether to include the examples that ship
        ...
        .. deprecated::
            Passing this explicitly is deprecated. Example DAGs are now loaded via the
            DAG bundle system (see ``[core] LOAD_EXAMPLES``) rather than by this class
            scanning ``airflow.example_dags`` directly. This parameter is kept only for
            backwards compatibility with callers that construct ``DagBag`` directly
            (e.g. custom scripts, DAG validation tests) and will be removed in a future
            release.
    ...
    def __init__(
        ...
        if is_arg_set(include_examples):
            warnings.warn(
                "Passing `include_examples` to DagBag is deprecated and will be removed in a "
                "future release. Example DAGs are now loaded via the DAG bundle system; see the "
                "`[core] LOAD_EXAMPLES` configuration option.",
                DeprecationWarning,
                stacklevel=2,
            )
...
class BundleDagBag(DagBag):
    ...
    def __init__(self, *args, bundle_path: Path | None = None, **kwargs):
        ...
        # (example DAGs are loaded via their own dedicated bundle, see DagBundlesManager).
