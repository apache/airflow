...
class SerializedDAG:
    ...
    def validate_partition_key(self, partition_key: str | None) -> None:
        ...
        A ``None`` value is always accepted. A non-``None`` value must be non-empty,
        at most ``ID_LEN`` characters long, and may only be supplied when the Dag's
        timetable sets ``partitioned=True`` or ``partitioned_at_runtime=True``.
        ...
        :raises InvalidPartitionKeyError: When ``partition_key`` is an empty string or
            exceeds ``ID_LEN`` characters.
        """
