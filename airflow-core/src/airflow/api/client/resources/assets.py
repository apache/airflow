...
class AssetsClient:
    ...
    def get(self, asset_id: int) -> AssetResponse:
        """Get an asset by ID."""
        resp = self._http.get(f"/api/v2/assets/{quote_segment(asset_id)}")
    ...
    def materialize(self, asset_id: int) -> DAGRunResponse:
        """Trigger materialization of an asset (creates a Dag run that produces it)."""
        resp = self._http.post(f"/api/v2/assets/{quote_segment(asset_id)}/materialize")
        resp.raise_for_status()
    ...
    def get_queued_events(self, asset_id: int) -> QueuedEventCollectionResponse:
        ...
        resp = self._http.get(f"/api/v2/assets/{quote_segment(asset_id)}/queuedEvents")
        resp.raise_for_status()
    ...
    def delete_queued_events(self, asset_id: int) -> None:
        ...
        resp = self._http.delete(f"/api/v2/assets/{quote_segment(asset_id)}/queuedEvents")
        resp.raise_for_status()
