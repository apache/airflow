...
class DagRunsClient:
    ...
    def get(self, dag_id: str, dag_run_id: str) -> DAGRunResponse:
        """Get a Dag run by ID."""
        resp = self._http.get(f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns/{quote_segment(dag_run_id)}")
    ...
    def list(self, dag_id: str, *, limit: int = 100, offset: int = 0) -> DAGRunCollectionResponse:
        ...
        resp = self._http.get(
            f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns",
    ...
    def trigger(
        ...
        *,
        ...
        if note is not None:
            body["note"] = note
        resp = self._http.post(f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns", json=body)
        resp.raise_for_status()
        return DAGRunResponse.model_validate(resp.json())
    ...
    def update(
        ...
        *,
        ...
        resp = self._http.patch(
            f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns/{quote_segment(dag_run_id)}", json=body
        )
        resp.raise_for_status()
        return DAGRunResponse.model_validate(resp.json())
    ...
    def delete(self, dag_id: str, dag_run_id: str) -> None:
        """Delete a Dag run."""
        resp = self._http.delete(f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns/{quote_segment(dag_run_id)}")
        resp.raise_for_status()
    ...
    def clear(
        ...
        *,
        ...
        resp = self._http.post(
            f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns/{quote_segment(dag_run_id)}/clear", json=body
        )
        resp.raise_for_status()
        data = resp.json()
        if dry_run:
