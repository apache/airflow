...
class TaskInstancesClient:
    ...
    def get(
        ...
        *,
        ...
        base = f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns/{quote_segment(dag_run_id)}/taskInstances/{quote_segment(task_id)}"
        if map_index is not None:
            base = f"{base}/{quote_segment(map_index)}"
    ...
    def list(
        ...
        *,
        ...
        """
        resp = self._http.get(
            f"/api/v2/dags/{quote_segment(dag_id)}/dagRuns/{quote_segment(dag_run_id)}/taskInstances",
    ...
    def update(
        ...
        *,
        ...
        The Core API returns the affected task instances (state changes may span related task
        instances), so this returns a :class:`TaskInstanceCollectionResponse`.
        """
        body: dict[str, Any] = {}
    ...
    def clear(
        ...
        *,
        ...
        resp = self._http.post(f"/api/v2/dags/{quote_segment(dag_id)}/clearTaskInstances", json=body)
        resp.raise_for_status()
        data = resp.json()
        return TaskInstanceCollectionResponse.model_construct(
            total_entries=data["total_entries"],
