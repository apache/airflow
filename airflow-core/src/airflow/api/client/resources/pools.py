...
class PoolsClient:
    ...
    def get(self, name: str) -> PoolResponse:
        """Get a pool by name."""
        resp = self._http.get(f"/api/v2/pools/{quote_segment(name)}")
    ...
    def update(
        ...
        *,
        ...
        resp = self._http.patch(f"/api/v2/pools/{quote_segment(name)}", json=body)
        resp.raise_for_status()
        return _pool_from_json(resp.json())

    def delete(self, name: str) -> None:
        """Delete a pool by name."""
        resp = self._http.delete(f"/api/v2/pools/{quote_segment(name)}")
        resp.raise_for_status()
