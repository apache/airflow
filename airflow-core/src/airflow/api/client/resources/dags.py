...
class DagsClient:
    ...
    def get(self, dag_id: str) -> DAGResponse:
        """Get a Dag by ID."""
        resp = self._http.get(f"/api/v2/dags/{quote_segment(dag_id)}")
    ...
    def get_details(self, dag_id: str) -> DAGDetailsResponse:
        ...
        resp = self._http.get(f"/api/v2/dags/{quote_segment(dag_id)}/details")
        resp.raise_for_status()
    ...
    def pause(self, dag_id: str) -> DAGResponse:
        """Pause a Dag."""
        resp = self._http.patch(f"/api/v2/dags/{quote_segment(dag_id)}", json={"is_paused": True})
        resp.raise_for_status()
        return DAGResponse.model_validate(resp.json())

    def unpause(self, dag_id: str) -> DAGResponse:
        """Unpause a Dag."""
        resp = self._http.patch(f"/api/v2/dags/{quote_segment(dag_id)}", json={"is_paused": False})
        resp.raise_for_status()
        return DAGResponse.model_validate(resp.json())

    def delete(self, dag_id: str) -> None:
        """Delete a Dag."""
        resp = self._http.delete(f"/api/v2/dags/{quote_segment(dag_id)}")
        resp.raise_for_status()
