...
def upgrade():
    ...
    # Drop all rows for now. This will only affect users who have a pending Dag Processor callback during
    # migration
...
def downgrade():
    ...
    Add/drop/modify columns, create foreign key for trigger and rename the table name.
    """
    op.execute("DELETE FROM callback")  # See comment in upgrade()
