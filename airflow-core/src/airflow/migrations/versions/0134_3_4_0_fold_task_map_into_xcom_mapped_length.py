...
In Airflow 3 every task_map row is written by the same execution API call that
writes the pushing task's ``return_value`` XCom row, at the same coordinates, so
the length lives on that row instead. ``task_map.keys`` is dropped rather than
migrated: nothing has ever read it, so downgrade restores every map as the list
variant.
...
def upgrade():
    ...
    with disable_sqlite_fkeys(op):
        ...
        # A correlated subquery rather than UPDATE ... FROM: MySQL has no such form and SQLite
        # only gained it in 3.33. The EXISTS keeps this a no-op for rows with no task_map row.
        op.execute(
            xcom.update()
            .where(
...
def downgrade():
    ...
    with disable_sqlite_fkeys(op):
        ...
        # Pinned to the same key as the backfill: task_map's PK has no key column, so two
        # keys at one set of coordinates would collide on it.
        op.execute(
            task_map.insert().from_select(
