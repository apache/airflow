...
def downgrade():
    ...
    if op.get_bind().dialect.name == "mysql":
        # MySQL/MariaDB error 1093: can't specify target table in a FROM clause of a subquery
        # that modifies it. Wrapping in a derived table works around this.
        op.execute(
            "DELETE FROM dataset WHERE id IN ("
            "    SELECT id FROM ("
            "        SELECT d.id FROM dataset d"
            "        JOIN (SELECT uri, MIN(id) AS min_id FROM dataset GROUP BY uri) m"
            "        ON d.uri = m.uri"
            "        WHERE d.id > m.min_id"
            "    ) AS sub"
            ")"
        ...
        op.execute(
            """
            with unique_dataset as (select min(id) as min_id, uri as uri from dataset group by uri),
