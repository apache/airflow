...
"""
Track dag_version on AssetPartitionDagRun and index partitioned_asset_key_log.asset_partition_dag_run_id.

The new ``dag_version_id`` UUID column lets the scheduler discard provisional
...
def upgrade():
    """Add ``dag_version_id`` to ``asset_partition_dag_run`` and index ``partitioned_asset_key_log.asset_partition_dag_run_id``."""
    with disable_sqlite_fkeys(op):
        with op.batch_alter_table("asset_partition_dag_run", schema=None) as batch_op:
            batch_op.add_column(sa.Column("dag_version_id", sa.Uuid(), nullable=True))
...
def downgrade():
    """Drop the ``partitioned_asset_key_log.asset_partition_dag_run_id`` index and the APDR ``dag_version_id`` column."""
    with disable_sqlite_fkeys(op):
        with op.batch_alter_table("partitioned_asset_key_log", schema=None) as batch_op:
