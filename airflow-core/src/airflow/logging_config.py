...
def load_logging_config() -> dict[str, Any]:
    ...
    global REMOTE_TASK_LOG
    logging_class_path = conf.get("logging", "logging_config_class", fallback=None)

    user_defined = logging_class_path is not None

    logging_class_path = (
        logging_class_path or "airflow.config_templates.airflow_local_settings.DEFAULT_LOGGING_CONFIG"
    )
    try:
        ...
        raise ImportError(
            f"Unable to load {'custom ' if user_defined else ''} logging config from {logging_class_path} due to {err}"
        )
    else:
