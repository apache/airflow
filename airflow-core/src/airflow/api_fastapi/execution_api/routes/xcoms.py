...
def set_xcom(
    ...
    if mapped_length is not None:
        # Only the return value can expand a downstream, and that is the only key the scheduler
        # reads a length from, so storing one under any other key would be write-only.
