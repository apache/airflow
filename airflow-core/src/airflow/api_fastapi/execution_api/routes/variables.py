...
from airflow.api_fastapi.execution_api.deps import JWTBearerDep, get_team_id_dep
...
def get_variable(
    variable_key: str, team_id: Annotated[str | None, Depends(get_team_id_dep)]
) -> VariableResponse:
    ...
    try:
        variable_value = Variable.get(variable_key, team_id=team_id)
    except KeyError:
        raise HTTPException(
...
def put_variable(
    variable_key: str, body: VariablePostBody, team_id: Annotated[str | None, Depends(get_team_id_dep)]
):
    """Set an Airflow Variable."""
    if not variable_key:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Not Found")

    Variable.set(key=variable_key, value=body.value, description=body.description, team_id=team_id)
...
def delete_variable(variable_key: str, team_id: Annotated[str | None, Depends(get_team_id_dep)]):
    """Delete an Airflow Variable."""
    if not variable_key:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Not Found")

    Variable.delete(key=variable_key, team_id=team_id)
