...
# A named alias (like TaskArgBinding below) so every schema keeps one shared, titled
# ArgValueSchema definition; a free-form JSON object rather than a typed model because
# the fragment is whatever pydantic generates from the stub annotation at parse time
# (``anyOf``, ``items``, ``enum``, ``$defs``, ...) and a typed model would silently strip
# any keyword it does not know when the spec is re-serialized along the delivery path.
...
Generated by pydantic from the stub function's parameter annotation and carried verbatim;
consumers validate the keywords they understand and ignore the rest, per JSON-schema
semantics, so newer producers never break an older core or runtime.
"""
