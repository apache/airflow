...
def requires_access_dag_run_clear_bulk() -> Callable[[BulkDAGRunClearBody, BaseUser, str], None]:
    def inner(
        ...
        # Same authorization pattern as ``requires_access_dag_run_bulk`` (see #67095):
        # collect the distinct concrete dag_ids across the per-entity body, resolve
        # each to its team, then batch-check PUT access for every (dag_id, run_id).
        # Wildcard / unresolvable entries are surfaced as 400s by the route, not 403s.
        resolved_dag_ids: set[str] = set()
            ...
            requests.append(
                {
                    "method": "PUT",
                    "access_entity": DagAccessEntity.RUN,
                    "details": DagDetails(id=entity_dag_id, team_name=dag_id_to_team.get(entity_dag_id)),
                }
            )
