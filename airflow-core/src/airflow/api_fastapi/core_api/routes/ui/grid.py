...
def _fetch_dag_runs(
    ...
    """
    Fetch DagRuns with version information.
    ...
    Args:
        ...
        offset: Query offset
        limit: Query limit
        run_after: Date range filter

    Returns:
        List of DagRun objects with version information loaded
    """
    base_query = select(DagRun).options(joinedload(DagRun.created_dag_version)).where(DagRun.dag_id == dag_id)

    # Fall back to DAG timetable when no order_by is provided
    if order_by.value == order_by.get_primary_key_string():
        latest_serdag = _get_latest_serdag(dag_id, session)
        if latest_serdag and latest_serdag.dag:
    ...
    return list(session.scalars(dag_runs_select_filter))
...
def get_grid_runs(
    ...
    try:
        dag_runs = _fetch_dag_runs(dag_id, session, order_by, offset, limit, run_after)
