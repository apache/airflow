...
def get_dag_bundle_files(
    ...
    # The duration is aggregated over live rows only. A Dag removed from a file keeps its row,
    # frozen at an older parse, so an unrestricted ``MAX`` would pair that stale duration with the
