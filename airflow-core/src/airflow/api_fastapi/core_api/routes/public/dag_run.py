...
# Sibling router for endpoints that need the parent ``/dags/{dag_id}`` prefix
# without the ``/dagRuns`` segment, mirroring how ``task_instances_router``
# exposes ``/clearTaskInstances`` next to its ``/dagRuns/.../taskInstances`` routes.
...
def bulk_clear_dag_runs(
    ...
    # Resolve and dedup the (dag_id, dag_run_id) targets up-front. The set
    # mirrors ``BulkDagRunService.handle_bulk_delete``: duplicate entries in
    # the payload collapse to one operation instead of failing or repeating.
    ...
    if body.dry_run:
        ...
        for run_dag_id, run_id in resolved_targets:
            # ``get_dag_run_and_dag_for_clear`` raises 404 on a missing run;
            # the bulk endpoint fails fast to match the single-run contract.
            get_dag_run_and_dag_for_clear(
