...
def get_dag_run_and_dag_for_clear(
    *,
    ...
    """
    Look up the Dag Run + its Dag for a clear operation.

    Raises ``HTTPException(404)`` if the run or the Dag are missing.
    Used by both the single ``clear_dag_run`` route and the bulk variant.
    """
...
def dry_run_clear_dag_run(
    *,
    ...
    """
    Return the task instances (or ``NewTaskResponse`` placeholders) a clear would affect.

    Mirrors the existing single-run ``clear_dag_run`` dry-run logic — ``only_new``
    returns placeholder rows for tasks added in the latest Dag version that have
    no ``TaskInstance`` row yet; otherwise the existing TI rows are returned,
    optionally filtered to failed states.
    """
    if only_new:
        # A task is "new" when it exists in the latest Dag version but has no TI row
        # in this run yet. This is more reliable than dag.clear(only_new=True,
        # dry_run=True), which returns nothing when ``created_dag_version_id`` is
        # ``None`` (e.g. LocalDagBundle).
    ...
    # Query TIs directly with eager loading so all serialization-required
    # relationships (dag_run, dag_model, dag_version, rendered_task_instance_fields)
    # are populated. ``dag.clear(dry_run=True)`` returns raw ORM objects without joins.
...
def perform_clear_dag_run(
    *,
    ...
    """
    Clear a single Dag Run and (optionally) apply a note in the same transaction.

    ``note=None`` leaves the run's existing note untouched; any string value
    (including ``""``) replaces it. Returns the post-clear Dag Run, re-fetched
    from the session so the caller serializes the new state.
    """
