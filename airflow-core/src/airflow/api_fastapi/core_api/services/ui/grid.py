...
def _get_nodes(
    ...
    if isinstance(task_node, TaskGroup):
        if node_id:
            # only not node_id in root group
            yield {
                node_id: {
                    "type": "group",
                    "parent_id": parent_id,
        ...
        for child in get_task_group_children_getter()(task_node):
            yield from _get_nodes(task_node=child, parent_node=task_node)
        return
    if isinstance(task_node, BaseOperator):
        yield {
            node_id: {
...
def get_task_group_map_new(dag: DAG) -> dict[str, dict[str, Any]]:
    task_nodes = {}

    for node in _get_nodes(task_node=dag.task_group, parent_node=None):
        if node:
            task_nodes.update(node)
