...
def fill_task_instance_summaries(
    ...
    for (task_id, run_id), tis in grouped_task_instances.items():
        ...
        # Get dag_version information from the first TaskInstance
        first_ti = tis[0]
        dag_version_id = str(first_ti.dag_version_id) if first_ti.dag_version_id else None
        dag_version_number = None
        if first_ti.dag_version:
            dag_version_number = first_ti.dag_version.version_number
...
def _merge_node_dicts(current, new) -> None:
    ...
    for node in new:
        if node["id"] in current_ids:
            ...
            if "children" in current_node and "children" in node:
                # Ensure children are not None before recursive call
                current_children = current_node["children"]
                new_children = node["children"]
                if current_children is not None and new_children is not None:
                    _merge_node_dicts(current_children, new_children)
        else:
            current.append(node)
...
def _get_aggs_for_node(detail):
    ...
    # Get dag_version information from the first detail entry
    dag_version_id = None
    dag_version_number = None
    if detail:
        first_detail = detail[0]
        dag_version_id = first_detail.get("dag_version_id")
        dag_version_number = first_detail.get("dag_version_number")
