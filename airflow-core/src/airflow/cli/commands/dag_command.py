...
def dag_report(args) -> None:
    ...
    for bundle in all_bundles:
        if bundle.name not in bundles_to_reserialize:
            continue
        bundle.initialize()
        dagbag = manager.get_dagbag(dag_folder=bundle.path, include_examples=False)
...
def dag_reserialize(args, session: Session = NEW_SESSION) -> None:
    ...
    for bundle in all_bundles:
        if bundle.name not in bundles_to_reserialize:
            continue
        bundle.initialize()
        dag_bag = manager.get_dagbag(dag_folder=bundle.path, bundle_path=bundle.path, include_examples=False)
