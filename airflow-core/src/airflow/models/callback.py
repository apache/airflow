...
class Callback(Base):
    ...
    def __init__(self, priority_weight: int = 1, state: str | None = None, data: dict | None = None):
    ...
    @staticmethod
    def create_from_sdk_def(callback_def):
...
class TriggererCallback(Callback):
    ...
    def queue(self):
        # TODO: queue the trigger (in the next PR)
        super().queue()

    def handle_event(self, event: TriggerEvent, session: Session):
        # TODO: modify fields based on the event (in the next PR)
        pass
...
class DagProcessorCallback(Callback):
    ...
    def __init__(self, priority_weight: int, callback: CallbackRequest):
        super().__init__(
            ...
            data={"req_type": callback.__class__.__name__, "req_data": callback.to_json()},
        )
        self.state = None
