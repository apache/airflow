...
def _stop_remaining_tasks_in_group(
    *,
    ...
    session: Session,
    group_tis: Sequence[TaskInstance],
) -> None:
    """
...
def _handle_task_group_retry_fast_fail(*, task_instance: TaskInstance, session: Session) -> None:
    ...
    for group in iter_retryable_task_groups(getattr(task, "task_group", None)):
        if not group.retry_fast_fail:
            continue
        task_ids = [t.task_id for t in group.iter_tasks()]
        if not task_ids:
            continue
        tis = session.scalars(
        ...
        if should_retry:
            _stop_remaining_tasks_in_group(
                task_instance=task_instance, task_group=group, session=session, group_tis=tis
            )
...
def clear_task_instances(
    ...
    clear_task_group_instances: bool = False,
) -> None:
    """
