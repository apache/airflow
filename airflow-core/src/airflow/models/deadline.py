...
import uuid6
from sqlalchemy import Boolean, ForeignKey, Index, Integer, and_, func, select, text
...
class Deadline(Base):
    ...
    callback_id: Mapped[str] = mapped_column(
        UUIDType(binary=False), ForeignKey("callback.id"), nullable=False
    )
    ...
    def __init__(
        ...
        self.callback = Callback.create_from_sdk_def(
            callback_def=callback, prefix="deadline_alerts", dag_id=dag_id
        )
