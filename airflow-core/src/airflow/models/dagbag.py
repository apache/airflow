...
async def capture_with_reraise_async():
    """
    Async-friendly capture_warnings context manager.
    Delegates the blocking part to a thread.
    """
    # Run the context manager itself in a thread
    # It will yield the captured_warnings list
    yield await asyncio.to_thread(_capture_with_reraise_inner)


def _capture_with_reraise_inner():
    with warnings.capture_with_reraise() as captured:
...
async def sync_to_async_with_timeout(func, *args, timeout: float | None = None, **kwargs):
    """
    Run a synchronous function in a thread with an optional timeout.

    Args:
        func: Synchronous callable to run.
        *args, **kwargs: Arguments to pass to the function.
        timeout: Maximum number of seconds to wait. If None or lower or equal to zero, waits indefinitely.
...
class AsyncDagBag(DagBag):
    ...
    async def process_file(self, filepath, only_if_updated=True, safe_mode=True):
        from airflow.sdk.definitions._internal.contextmanager import DagContext

        if filepath is None or not await asyncio.to_thread(os.path.isfile, filepath):
            return []
        ...
        async with capture_with_reraise_async() as captured_warnings:
            if filepath.endswith(".py") or not await asyncio.to_thread(zipfile.is_zipfile, filepath):
                mods = await self._load_modules_from_file(filepath, safe_mode)
            else:
                mods = await self._load_modules_from_zip(filepath, safe_mode)
        ...
        found_dags = await self._process_modules(filepath, mods, file_last_changed_on_disk)
    ...
    async def _load_modules_from_file(self, filepath, safe_mode):
        ...
        def parse():
            """The actual module parsing logic (runs in thread)."""
            try:
                loader = importlib.machinery.SourceFileLoader(mod_name, filepath)
                spec = importlib.util.spec_from_loader(mod_name, loader)
                new_module = importlib.util.module_from_spec(spec)
        ...
        try:
            return await sync_to_async_with_timeout(parse, timeout=dagbag_import_timeout)
        except asyncio.TimeoutError:
            # Async timeout with asyncio.wait_for (no signals needed)
            timeout_msg = (
                f"DagBag import timeout for {filepath} after {dagbag_import_timeout}s.\n"
                "Please take a look at these docs to improve your DAG import time:\n"
                f"* {get_docs_url('best-practices.html#top-level-python-code')}\n"
            ...
            self.import_errors[filepath] = f"DagBag import timeout after {dagbag_import_timeout}s"
            return []
    ...
    async def _load_modules_from_zip(self, filepath, safe_mode):
        ...
        def iter_zip():
            """Helper to iterate over the zip synchronously."""
            with zipfile.ZipFile(filepath) as zf:
                for zip_info in zf.infolist():
                    yield zip_info, zf

        async for zip_info, current_zip_file in async_iter(iter_zip()):
            zip_path = Path(zip_info.filename)
            if zip_path.suffix not in [".py", ".pyc"] or len(zip_path.parts) > 1:
                continue
            ...
            def import_module():
                try:
                    ...
                    DagContext.autoregistered_dags.clear()
                    fileloc = str(Path(filepath) / zip_info.filename)
                    self.log.exception("Failed to import: %s", fileloc)
                    if self.dagbag_import_error_tracebacks:
            ...
            current_module = await asyncio.to_thread(import_module)
    ...
    @staticmethod
    def _top_level_dags(mods):
        from airflow.models.dag import DAG
        from airflow.sdk import DAG as SDKDAG
        from airflow.sdk.definitions._internal.contextmanager import DagContext

        # yield DAGs from modules
        for mod in mods:
            for obj in mod.__dict__.values():
                if isinstance(obj, (DAG, SDKDAG)):
                    yield obj, mod

        # yield DAGs from autowired context
        for dag, mod in DagContext.autoregistered_dags:
            yield dag, mod

    async def _process_modules(self, filepath, mods, file_last_changed_on_disk):
        from airflow.sdk.definitions._internal.contextmanager import DagContext
        ...
        for dag, mod in self._top_level_dags(mods):
            dag.fileloc = mod.__file__
            if self.bundle_path:
            ...
            try:
                # CPU-bound: validate and bag in a worker thread if async
                await asyncio.to_thread(dag.validate)
                await asyncio.to_thread(self.bag_dag, dag)
            except AirflowClusterPolicySkipDag:
                pass
            except Exception as e:
                self.log.exception("Failed to bag_dag: %s", dag.fileloc)
    ...
    async def collect_dags(
        ...
        dag_folder = correct_maybe_zipped(str(dag_folder))
        files_to_parse = await asyncio.to_thread(list_py_file_paths, dag_folder, safe_mode)

        if include_examples:
            from airflow import example_dags
            example_dag_folder = next(iter(example_dags.__path__))
            example_files = await asyncio.to_thread(list_py_file_paths, example_dag_folder, safe_mode)
            files_to_parse.extend(example_files)

        async def process(filepath):
            try:
                file_parse_start_dttm = timezone.utcnow()
                found_dags = await self.process_file(filepath, only_if_updated, safe_mode)
                file_parse_end_dttm = timezone.utcnow()
                return FileLoadStat(
                    file=filepath.replace(settings.DAGS_FOLDER, ""),
                    duration=file_parse_end_dttm - file_parse_start_dttm,
        ...
        stats = await asyncio.gather(*(process(fp) for fp in files_to_parse))
        self.dagbag_stats = sorted(stats, key=lambda x: x.duration, reverse=True)
