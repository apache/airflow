...
def collect_task_group_tis(
    ...
    for task in task_group.iter_tasks():
        ...
        if not task_tis:
            log.debug(
                "Task %r not found in TI map for TaskGroup %r — skipping group retry evaluation.",
                task.task_id,
                task_group.group_id,
            )
            return None
...
def should_task_group_retry(
    *,
    ...
    Built-in conditions (``"any_failed"``, ``"all_failed"``) are evaluated
    inline.  Custom callable conditions are resolved via ``import_string``,
    so they must live in an installed package (e.g., an Airflow plugin) —
    DAG-file-local callables cannot be imported by the scheduler.

    .. todo::
        Add a plugin registration system (similar to
        ``priority_weight_strategy``) so that custom retry conditions can
        be discovered via entry points instead of raw ``import_string``.
    """
    ...
    # All string paths are handled above (builtin names + import_string).
    # If we reach here with a non-callable, it's an unsupported type.
    log.warning(
...
class TaskGroupInstance(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            name="task_group_instance_dag_run_fkey",
            ondelete="CASCADE",
        ),
        Index("idx_task_group_instance_dag_run", "dag_id", "run_id"),
    )
