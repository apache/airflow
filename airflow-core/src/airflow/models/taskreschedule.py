...
class TaskReschedule(Base):
    ...
    task_instance = relationship(
        "TaskInstance", primaryjoin="TaskReschedule.try_id == foreign(TaskInstance.id)", uselist=False
    )
    ...
    def stmt_for_task_instance(
        ...
        *,
        ...
        :meta private:
        """
        return select(cls).where(cls.try_id == ti.id).order_by(desc(cls.id) if descending else asc(cls.id))
