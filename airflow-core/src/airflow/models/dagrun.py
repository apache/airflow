...
_POSTGRES_UNNEST_INSERT_CACHE: dict[frozenset[str], tuple[Any, tuple[str, ...]]] = {}


def _build_postgres_unnest_insert(keys: frozenset[str]) -> tuple[Any, tuple[str, ...]]:
    """
    Build the cached ``INSERT ... SELECT * FROM unnest(...)`` statement for TaskInstance.

    The statement uses one array bind per column so the wire payload scales with
    columns rather than rows, which is materially faster than the multi-row
    ``VALUES`` form generated by SQLAlchemy's ORM ``bulk_insert_mappings`` for
    large DagRuns (mapped task expansion, big DAGs).

    :param keys: the set of dict keys present in the input rows. Keys that do not
        map to a TaskInstance column are ignored — mirroring the behaviour of
        ``bulk_insert_mappings``.
    :return: a tuple of ``(prepared statement, ordered attribute names)``.
    """
    specs: list[tuple[str, str, Any]] = []  # (attr_name, sql_col_name, sa_type)
    for attr_name in sorted(keys):
        attr = TI.__mapper__.attrs.get(attr_name)
        if attr is None:
            continue
        ...
        if not cols:
            continue
        col = cols[0]
        specs.append((attr_name, col.name, col.type))
    if not specs:
        raise RuntimeError("No TaskInstance columns matched the provided insert mapping")

    sql_cols = ", ".join(spec[1] for spec in specs)
    # No explicit ``::<type>[]`` cast in the text: the typed ``bindparam`` below
    # has SQLAlchemy inject one for us when compiled against the postgres dialect.
    sql_unnest = ", ".join(f":{spec[0]}" for spec in specs)
    stmt = text(f"INSERT INTO task_instance ({sql_cols}) SELECT * FROM unnest({sql_unnest})").bindparams(
        *(bindparam(name, type_=postgresql.ARRAY(sa_type)) for name, _, sa_type in specs)
    )
    return stmt, tuple(name for name, _, _ in specs)


def _bulk_insert_task_instance_dicts_postgres(task_dicts: list[dict[str, Any]], session: Session) -> None:
    """
    PostgreSQL fast path for bulk-inserting TaskInstance rows.

    Equivalent to ``session.bulk_insert_mappings(TI.__mapper__, task_dicts)``
    but emits ``INSERT ... SELECT * FROM unnest(:col1::t1[], :col2::t2[], ...)``
    so the driver sends one typed array per column instead of one bind tuple per
    row. Required Python-side column defaults (``id``, ``updated_at``) are filled
    by the caller in :py:meth:`TaskInstance.insert_mapping`.
    """
    if not task_dicts:
        return
    keys = frozenset(task_dicts[0])
    entry = _POSTGRES_UNNEST_INSERT_CACHE.get(keys)
    if entry is None:
        entry = _build_postgres_unnest_insert(keys)
        _POSTGRES_UNNEST_INSERT_CACHE[keys] = entry
    stmt, attr_names = entry
    params = {name: [d.get(name) for d in task_dicts] for name in attr_names}
        ...
        try:
            if hook_is_noop:
                if get_dialect_name(session) == "postgresql":
                    _bulk_insert_task_instance_dicts_postgres(list(tasks), session)
                else:
