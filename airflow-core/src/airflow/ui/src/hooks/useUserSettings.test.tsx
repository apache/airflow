...
describe("useDefaultDependencyView", () => {
  it("defaults to 'tasks' when nothing is stored", () => {
    const { result } = renderHook(() => useDefaultDependencyView());

    expect(result.current[0]).toBe("tasks");
  });

  it("reads an existing stored dependency view", () => {
  ...
  it("persists a new dependency view to localStorage", () => {
    const { result } = renderHook(() => useDefaultDependencyView());
    ...
    expect(JSON.parse(localStorage.getItem(DEFAULT_DEPENDENCY_VIEW_KEY) ?? '""')).toBe("immediate");
  });
});
