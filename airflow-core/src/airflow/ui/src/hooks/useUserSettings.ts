...
/** Dependency scope shown in Dag graphs when a graph has no per-graph choice. */
export type DependencyView = "all" | "immediate" | "tasks";
...
/** Fallback dependency scope used when a graph has no per-graph choice. */
export const useDefaultDependencyView = () =>
  useLocalStorage<DependencyView>(DEFAULT_DEPENDENCY_VIEW_KEY, "tasks");
