...
export const DetailsLayout = ({ children, error, isLoading, tabs }: Props) => {
  ...
  // Dual refs for cross-component hover synchronization
  // Each component binds its own overlay, but receives the other's ref for remote control
  const gridHoverRef = useRef<HTMLDivElement>(null);
  const ganttHoverRef = useRef<HTMLDivElement>(null);
