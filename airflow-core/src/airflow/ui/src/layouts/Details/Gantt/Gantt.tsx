    ...
    <Box height={`${fixedHeight}px`} minW="500px" mt={36} w="100%">
      <Bar
        ...
        options={{
          ...
          scales: {
            x: {
              max: formatDate(runs.find((run) => run.dag_run_id === runId)?.end_date, selectedTimezone),
              min: formatDate(runs.find((run) => run.dag_run_id === runId)?.start_date, selectedTimezone),
              position: "top",
              stacked: true,
              ticks: {
                align: "start",
