...
const secondsBetween = (start?: string, end?: string): number | undefined => {
...
const TaskInstanceTooltip = ({ children, positioning, taskInstance, ...rest }: Props) => {
  ...
  // Pick the right timestamps depending on the shape
  ...
  // Compute then format via renderDuration()
  const seconds = secondsBetween(startedIso, endedIso);
  const formattedDuration = seconds === undefined ? undefined : renderDuration(seconds);
    ...
    <Tooltip
      ...
      content={
        <Box>
          ...
          {formattedDuration === undefined ? undefined : (
            <Text>
              {translate("duration")}: {formattedDuration}
            </Text>
          )}
