...
import {
  useTaskStateStoreServiceClearTaskStateStore,
...
export const ClearAllTaskStateStoreButton = ({ dagId, mapIndex, runId, taskId }: Props) => {
  ...
  const { isPending, mutate } = useTaskStateStoreServiceClearTaskStateStore(
    useStoreMutation({
      invalidationKeys: [
        useTaskStateStoreServiceListTaskStateStoreKey,
        useTaskStateStoreServiceGetTaskStateStoreKey,
  ...
  return (
    <>
      <Button colorPalette="danger" onClick={onOpen} variant="outline">
        <FiTrash2 /> {translate("taskStateStore.clearAll.title")}
      </Button>
      ...
      <DeleteDialog
        ...
        warningText={translate("taskStateStore.clearAll.warning")}
      />
    </>
  );
};
