...
export const Settings = () => {
  const { t: translate } = useTranslation(["common", "components", "dag", "dags"]);

  useDocumentTitle(translate("settings.title"));

  const [graphDirection, setGraphDirection] = useDefaultGraphDirection();
  const [dependencyView, setDependencyView] = useDefaultDependencyView();
  ...
  const dependencyOptions: Array<SelectOption<DependencyView>> = [
    ...
    <Box maxW="720px">
      ...
      <Stack gap={8}>
        <Section title={translate("settings.graph.title")}>
          ...
          />
          <SelectSetting
            helper={translate("settings.graph.defaultDependencyView.helper")}
            label={translate("settings.graph.defaultDependencyView.label")}
            onChange={setDependencyView}
            options={dependencyOptions}
            testId="default-dependency-view"
