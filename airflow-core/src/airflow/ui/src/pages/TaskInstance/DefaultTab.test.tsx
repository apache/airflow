...
vi.mock("src/queries/useConfig", () => ({ useConfig: vi.fn() }));
...
describe("DefaultTab", () => {
  ...
  it("renders logs on the index route by default", () => {
    vi.mocked(useConfig).mockReturnValue("logs");
  ...
  it("redirects to the configured tab", () => {
    vi.mocked(useConfig).mockReturnValue("details");
    renderAt("/dags/d1/runs/r1/tasks/t1");
  ...
  it("falls back to logs for an unknown tab value", () => {
    vi.mocked(useConfig).mockReturnValue("not-a-tab");
    renderAt("/dags/d1/runs/r1/tasks/t1");
