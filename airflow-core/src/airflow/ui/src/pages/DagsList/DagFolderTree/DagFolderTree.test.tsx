...
describe("DagFolderTree (single bundle)", () => {
  ...
  it("truncates long names so the selected row highlight is not cut off", () => {
    ...
    // The row spans the panel and the label truncates, so the highlight covers the whole row.
  ...
  it("expands down to the selected folder once folders finish loading", () => {
    // Folders are fetched asynchronously, so the first render has nothing to expand. Landing on a
    // URL that already points at a nested folder must still open the tree once they arrive.
    const props = {
      onSelectFolder: vi.fn(),
      selectedBundle: undefined,
      selectedFolder: "team_a/etl",
    };
    const { rerender } = render(<DagFolderTree folders={[]} {...props} />, { wrapper: BaseWrapper });
...
describe("DagFolderTree (multiple bundles)", () => {
  ...
  it("expands down to the selected folder once folders finish loading", () => {
    ...
    expect(node("ml")).toHaveAttribute("aria-expanded", "true");
    // ``etl`` exists in both bundles; the one under ``ml`` is the one revealed and selected.
    const selected = nodes().filter((item) => item.getAttribute("aria-selected") === "true");
