...
describe("Dag Filters", () => {
  it("Filter by selected last run state", async () => {
    ...
    screen.getByTestId("hub-filter-trigger").click();
    const trigger = within(await screen.findByTestId("hub-last-run-state-filter")).getByRole("combobox");

    await waitFor(() => trigger.click());
    await waitFor(() => screen.getByTestId("hub-last-run-state-filter-success").click());
    await waitFor(() => expect(screen.getByText("tutorial_taskflow_api_success")).toBeInTheDocument());

    await waitFor(() => trigger.click());
    await waitFor(() => screen.getByTestId("hub-last-run-state-filter-failed").click());
