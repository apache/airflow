...
describe("gridQueryKeys", () => {
  it("includes the Dag structure key so structure refreshes on invalidation instead of polling", () => {
    const keys = gridQueryKeys("dag_1");

    expect(keys.some((key) => key[0] === useGridServiceGetDagStructureKey)).toBe(true);
  });
