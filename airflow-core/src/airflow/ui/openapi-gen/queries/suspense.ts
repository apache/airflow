...
* @throws ApiError
*/
export const useGraphServiceGetGroupIdsSuspense = <TData = Common.GraphServiceGetGroupIdsDefaultResponse, TError = unknown, TQueryKey extends Array<unknown> = unknown[]>({ dagId, runId }: {
  dagId: string;
  runId?: string;
}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UseGraphServiceGetGroupIdsKeyFn({ dagId, runId }, queryKey), queryFn: () => GraphService.getGroupIds({ dagId, runId }) as TData, ...options });
/**
* Get Calendar
