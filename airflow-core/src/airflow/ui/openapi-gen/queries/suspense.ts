...
* @throws ApiError
*/
export const useAssetServiceGetAssetLineageSuspense = <TData = Common.AssetServiceGetAssetLineageDefaultResponse, TError = unknown, TQueryKey extends Array<unknown> = unknown[]>({ assetId, depth }: {
  assetId: number;
  depth?: number;
}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UseAssetServiceGetAssetLineageKeyFn({ assetId, depth }, queryKey), queryFn: () => AssetService.getAssetLineage({ assetId, depth }) as TData, ...options });
/**
