...
* @throws ApiError
*/
export const useAssetServiceGetAssetLineageSuspense = <TData = Common.AssetServiceGetAssetLineageDefaultResponse, TError = unknown, TQueryKey extends Array<unknown> = unknown[]>({ assetId }: {
  assetId: number;
}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UseAssetServiceGetAssetLineageKeyFn({ assetId }, queryKey), queryFn: () => AssetService.getAssetLineage({ assetId }) as TData, ...options });
/**
