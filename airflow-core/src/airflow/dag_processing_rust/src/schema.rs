...
diesel::table! {
    asset_watcher (asset_id, trigger_id) {
        #[max_length = 1500]
        name -> Varchar,
        asset_id -> Int4,
        trigger_id -> Int4,
...
diesel::table! {
    dag (dag_id) {
        ...
        last_parse_duration -> Nullable<Float8>,
...
diesel::table! {
    dag_bundle (name) {
        ...
        #[max_length = 200]
        signed_url_template -> Nullable<Varchar>,
        template_params -> Nullable<Json>,
    }
}

diesel::table! {
    dag_bundle_team (dag_bundle_name, team_id) {
        #[max_length = 250]
        dag_bundle_name -> Varchar,
        team_id -> Uuid,
    }
}
...
diesel::table! {
    deadline (id) {
        ...
        callback -> Json,
        #[max_length = 20]
        callback_state -> Nullable<Varchar>,
        trigger_id -> Nullable<Int4>,
    }
}
...
diesel::table! {
    hitl_detail (ti_id) {
        ...
        multiple -> Nullable<Bool>,
        params -> Json,
        assignees -> Nullable<Json>,
        created_at -> Timestamptz,
        responded_at -> Nullable<Timestamptz>,
        responded_by -> Nullable<Json>,
        chosen_options -> Nullable<Json>,
        params_input -> Json,
    }
}
...
diesel::joinable!(asset_watcher -> asset (asset_id));
diesel::joinable!(asset_watcher -> trigger (trigger_id));
diesel::joinable!(backfill_dag_run -> backfill (backfill_id));
diesel::joinable!(backfill_dag_run -> dag_run (dag_run_id));
diesel::joinable!(callback -> trigger (trigger_id));
diesel::joinable!(connection -> team (team_id));
diesel::joinable!(dag -> dag_bundle (bundle_name));
diesel::joinable!(dag_bundle_team -> dag_bundle (dag_bundle_name));
diesel::joinable!(dag_bundle_team -> team (team_id));
...
diesel::joinable!(deadline -> trigger (trigger_id));
diesel::joinable!(hitl_detail -> task_instance (ti_id));
diesel::joinable!(hitl_detail_history -> task_instance_history (ti_history_id));
diesel::joinable!(serialized_dag -> dag_version (dag_version_id));
diesel::joinable!(slot_pool -> team (team_id));
...
diesel::joinable!(variable -> team (team_id));
