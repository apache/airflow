...
class TaskGroupRetryDep(BaseTIDep):
    ...
    IGNORABLE = True
    # This is NOT a task-specific dep (like depends_on_past or trigger_rule) — it is a
    # group-level scheduling constraint.  Setting IS_TASK_DEP = False means
    # ``ignore_task_deps`` will *not* bypass it.  It can be bypassed by
    # ``ignore_all_deps`` (via IGNORABLE = True) or ``ignore_task_group_retry_delay``.
    IS_TASK_DEP = False
    ...
    def _get_dep_statuses(self, ti, session, dep_context=None):
        ...
        # Use pre-fetched pending retries from DepContext when available (batch query
        # done once per scheduling loop in DagRun._get_ready_tis) to avoid issuing
        # one DB query per task.
        prefetched = getattr(dep_context, "pending_task_group_retries", None) if dep_context else None
        if prefetched is not None:
            for gid in group_ids:
                tgi = prefetched.get(gid)
                if tgi is not None:
                    yield self._failing_status(
                        reason=(
                            f"Waiting for TaskGroup retry delay "
                            f"({tgi.task_group_id}) until {tgi.next_retry_at}."
                        )
                    )
                    return
        ...
        # Fallback: query the DB directly (e.g. when called outside the scheduler loop).
        now = timezone.utcnow()
