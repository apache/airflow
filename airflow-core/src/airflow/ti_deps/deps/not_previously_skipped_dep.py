    ...
    def _get_dep_statuses(self, ti, dep_context, *, session):
        ...
        for parent in upstream:
            if parent.inherits_from_skipmixin:
                ...
                # `full=False`: the payload is a plain mapping of lists of task ids, so
                # the scheduler only needs primitive values here and reads XCom without
                ...
                if not isinstance(prev_result, dict):
                    # Not the shape SkipMixin writes. Treat it as absent -- and do not
                    # fail the dep, because a task may write anything under this key.
                    log.warning(
