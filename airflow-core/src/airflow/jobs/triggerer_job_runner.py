...
class messages:
    ...
    class TriggerStateChanges(BaseModel):
        ...
        events: Annotated[
            list[tuple[int, TriggerEventEntry]] | None,
...
class TriggerRunnerSupervisor(WatchedSubprocess):
    ...
    events: KeyedHeadQueue[int, TriggerEventEntry] = attrs.field(factory=deque, init=False)
        ...
        if self.events:
            with create_session() as session:
                while self.events:
                    ...
                    is_last_event = entry.trigger_id not in self.events
                    remaining_events = len(self.events.get(entry.trigger_id, []))
                    log.info(
                    ...
                    if self.on_trigger_event(
                        trigger_id=entry.trigger_id, event=entry.event, is_last_event=is_last_event, session=session
                    ):
                        ...
                        log.info("Event %s handled for trigger %s", entry.event.payload, entry.trigger_id)
                        # Emit stat event
                        stats.incr("triggers.succeeded")
                    else:
                        self.events.append((entry.trigger_id, entry.event))

    def on_trigger_event(self, trigger_id: int, event: TriggerEvent, is_last_event: bool, session: Session) -> bool:
        """Record that a trigger fired an event."""
        return Trigger.submit_event(
        ...
        if self.failed_triggers:
            ...
            with create_session() as session:
                while self.failed_triggers:
                    trigger_id, trigger, exc = self.failed_triggers.popleft()

                    # Tell the model to fail this trigger's deps
                    if trigger_id not in self.events and self.on_trigger_failure(
                        trigger_id=trigger_id, trigger=trigger, exc=exc, session=session
                    ):
                        log.warning("Trigger %s has failed: %s", trigger_id, exc)
                        # Emit stat event
                        ...
                        self.failed_triggers.append((trigger_id, trigger, exc))
                session.flush()

    def on_trigger_failure(self, trigger_id: int, trigger: Trigger, exc: list[str] | None, session: Session) -> None:
    ...
    def update_triggers(self, requested_trigger_ids: set[int]):
        ...
        cancel_trigger_ids = self.running_triggers - new_trigger_ids
        if new_trigger_ids:
...
class TriggerRunner:
    ...
    # Outbound queue of events
    events: PartitionedQueue[int, TriggerEventEntry]
    ...
    def process_trigger_events(self, finished_ids: list[int]) -> messages.TriggerStateChanges:
        ...
        events_to_send: list[tuple[int, TriggerEvent]] = []
        ...
        while self.failed_triggers:
            ...
            failures_to_send.append((trigger_id, trigger, tb))
    ...
    def sanitize_trigger_events(self, msg: messages.TriggerStateChanges) -> messages.TriggerStateChanges:
        ...
        events_to_send: list[tuple[int, TriggerEventEntry]] = []

        if msg.events:
            for trigger_id, entry in msg.events:
                try:
        ...
        with _make_trigger_span(ti=trigger.task_instance, trigger_id=trigger_id, name=name) as span:
            try:
                ...
                async for event in event_stream:
                    ...
                    await self.events.put((trigger_id, TriggerEventEntry(trigger_id=trigger_id, event=event, persist_seq=seq)))
