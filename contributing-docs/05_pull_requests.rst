...
* When an agent drafts the PR description, start it with a short human-authored summary that you type
  yourself (why the change is needed, key decisions, caveats), and put the agent-written text below
  it under a collapsed ``<details>`` tag containing the AI Summary. Never let the agent write the
  Human Summary. PRs written without Gen-AI tools keep using the regular PR template unchanged.
