...
Extra (optional)
    ...
    * ``conn_timeout`` - An optional timeout (in seconds) for the TCP connect. Default is ``10``. The parameter to :class:`~airflow.providers.ssh.hooks.ssh.SSHHook` conn_timeout takes precedence.
