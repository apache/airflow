...
.. code-block:: bash
    ...
    celery --app airflow.executors.celery_executor_utils.app inspect ping -d celery@${HOSTNAME}

To check if the all workers in the cluster running is working correctly, run:

.. code-block:: bash

    celery --app airflow.executors.celery_executor_utils.app inspect ping
