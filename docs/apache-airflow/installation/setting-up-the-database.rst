...
Usually, you need to run ``airflow db migrate`` in order to create the database schema that Airflow can use.

Similarly, upgrading Airflow usually requires an extra step of upgrading the database. This is done
with ``airflow db migrate`` CLI command. You should make sure that Airflow components are
not running while the migration is being executed.
