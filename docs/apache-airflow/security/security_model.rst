...
must be assessed and addressed by the Deployment Manager. However, there are exception to this. The `/health` endpoint responsible for probing for getting health check updates should be publicly accessible. This is because other systems would want to retrieve that information. Another exception is the `/login` endpoint as the users are expected to be unathenticated to use this endpoint.
