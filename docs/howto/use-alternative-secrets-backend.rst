...
To enable GCP Secrets Manager to retrieve connection, specify :py:class:`~airflow.providers.google.secrets.secrets_manager.GcpSecretsManagerSecretsBackend`
as the ``backend`` in  ``[secrets]`` section of ``airflow.cfg``.
