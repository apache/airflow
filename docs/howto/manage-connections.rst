...
(e.g. `google-compute-platform` if `conn_type` is `google-compute-platform`)
...
Scopes (comma separated)
    ...
    it using URI syntax, with the following requirements:
      * scheme part must be equals ``google-cloud-platform`` (Note: Look for the
        minus character)
      * authority (username, password, host, port), path is ignored
      * query parameters contains contains information specific to this type of
