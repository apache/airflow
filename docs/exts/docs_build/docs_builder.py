...
class AirflowDocsBuilder:
    ...
    def __init__(self, package_name: str):
        ...
        if self.package_name.startswith("apache-airflow-providers-"):
            ...
            self.provider_path = (AIRFLOW_ROOT_PATH / "providers").joinpath(*self.package_id.split("."))
    ...
    def _doctree_dir(self) -> str:
        return f"{AIRFLOW_DOCS_PATH}/_doctrees/docs/{self.package_name}"

    @property
    def _inventory_cache_dir(self) -> str:
        return f"{AIRFLOW_DOCS_PATH}/_inventory_cache"
    ...
    def _build_dir(self) -> str:
        if self.is_versioned:
            version = "stable"
            return f"{AIRFLOW_DOCS_PATH}/_build/docs/{self.package_name}/{version}"
        else:
            return f"{AIRFLOW_DOCS_PATH}/_build/docs/{self.package_name}"
    ...
    def _src_dir(self) -> str:
        return f"{AIRFLOW_DOCS_PATH}/{self.package_name}"
    ...
    def check_spelling(
        ...
        :param skip_deletion: whether to skip deletion of the generated files
        ...
        if self.is_provider:
            self.prepare_provider_dir()
        if self.is_airflow:
            self.prepare_airflow_dir()
        build_cmd = [
            "sphinx-build",
    ...
    def build_sphinx_docs(self, skip_deletion: bool, verbose: bool) -> list[DocBuildError]:
        ...
        if self.is_provider or self.is_airflow:
            if skip_deletion:
                console.print(
                    f"[bright_blue]{self.package_name:60}:[/] Leaving generated files in {self._src_dir}."
                )
