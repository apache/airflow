...
API if ``check_fn`` is provided) to check whether it is present or not.
Please keep in mind that it makes one API call per key, especially when used to check a large volume of keys.
