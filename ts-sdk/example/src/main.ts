...
//
// Both Dags in `dags/` are declared in Python with `@task.stub` tasks routed to
// the Node coordinator, so this side registers task handlers and nothing else:
...
// One register call lists everything this bundle provides. Note that
// `build_message` appears under both Dags: two different handlers, told apart
// by the dag_id each is bound to and never by the task_id alone.
