...
// A cycle by module graph, not by evaluation: the coordinator reads a bundle,
// and a bundle starts the coordinator. Both directions reference only hoisted
// function declarations from inside function bodies, so neither module touches
// the other's bindings while it is still evaluating.
import { serveBundle } from "../coordinator/runtime.js";
...
export class Bundle {
  ...
  register(...items: Registerable[]): void {
    ...
    const incomingTasks = new Set<string>();
    for (const item of items) {
      ...
      if (candidate instanceof Dag) {
        ...
        this.#checkTaskHandler(candidate, incomingDags, incomingTasks);
  ...
  #checkTaskHandler(
    ...
    if (entry?.kind === "handlers" ? entry.handlers.has(handler.taskId) : incomingTasks.has(pair)) {
      throw new Error(
        `A handler for Dag "${handler.dagId}" task "${handler.taskId}" is already registered`,
      );
    }
    incomingTasks.add(pair);
  }
  ...
  async serve(): Promise<void> {
    ...
    assertOwnBundle(this, "bundle.serve()");
  ...
  getTaskHandler(dagId: string, taskId: string): TaskFunction | undefined {
    ...
    return entry.kind === "dag"
      ? getDagTaskRecords(entry.dag).get(taskId)?.handler
      : entry.handlers.get(taskId);
  }
}

/** Internal: reject a `this` that is not a Bundle built by this copy, naming
 *  the cause. Shared so the two entry points into the runtime agree. */
export function assertOwnBundle(value: unknown, accessor: string): asserts value is Bundle {
