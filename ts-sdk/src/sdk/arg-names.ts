...
// Stating a binding explicitly, for a name the Python side never used.
...
// Read back off the handler at the dispatch site. A global symbol, as the
// brands are: two resolved copies of the package must agree on the key, or a
// handler wrapped by one and dispatched by the other would lose its renames.
...
/**
 * Bind an argument under a name the Python side never used.
 *
 ...
 * ```
 *
 * Folding already absorbs ordinary spelling differences, so this is for a name
 * that genuinely differs: a clearer word than the Dag chose, or a TypeScript
 * reserved word like `enum`. It should be rare in a real Dag.
 *
 * The map's keys are checked against the handler's own parameter type, so
 * `{ labl: "run_label" }` is a compile error. Its values are Python names,
 * which `tsc` cannot see and does not check.
 */
export function withArgNames<TArgs, TReturn>(
  // NoInfer, so `TArgs` comes from the handler alone: inferring it from the map
  // too would make every key correct by construction and the check vacuous.
  ...
  // A wrapper rather than a property on the author's own function: one handler
  // can be registered for two tasks that rename differently, and mutating it
  // would let the second registration silently change the first.
...
/**
 * Internal: the renames a handler was wrapped with, or `undefined`.
 *
 * Not re-exported from the package root: an author states renames, and reading
 * them back is the runtime's business.
 */
