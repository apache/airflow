...
export interface BoundArgs {
  /**
   * The object the handler is called with. A Proxy: reading a name folds it on
   * demand and matches it against the folded wire names, so `regionCode` and
   * `region_code` both reach Python's `region_code`.
   */
