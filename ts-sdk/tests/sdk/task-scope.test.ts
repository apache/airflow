...
describe("the task scope", () => {
  ...
  it("still resolves in a floating promise, which is why one must be awaited", async () => {
    // By the time an unawaited promise resolves, the runtime has already
    // reported the task's terminal state. Pinned because the fix is to await
    // the work, not to expect a throw here.
