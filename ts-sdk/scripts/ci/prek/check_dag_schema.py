...
if __name__ == "__main__":
    ...
    run_command(["pnpm", "exec", "prettier", "--write", "src/generated/dag-schema-fields.ts"], cwd=directory)
    ...
    if diff.stdout.strip():
        message = (
            f"{GENERATED} is out of date with the vendored Dag serialization schema "
            "(ts-sdk/schema/dag-schema.json).\n"
            "Regenerate it with `pnpm run generate:dag-schema` in ts-sdk/ and commit the result."
        )
        if console:
