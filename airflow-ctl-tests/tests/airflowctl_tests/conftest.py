...
def test_commands(login_command, date_param):
    ...
    return [
        ...
        "connections get --connection-id=test_con",
        "connections get --connection-id=test_con -o json",
        "connections update --connection-id=test_con --conn-type=postgres",
        "connections test --connection-id=test_con",
        "connections import tests/airflowctl_tests/fixtures/test_connections.json",
        "connections delete --connection-id=test_con",
        "connections delete --connection-id=test_import_conn",
        # DAGs commands
        "dags list",
        ...
        "pools list",
        "pools get --name=test_pool",
        "pools get --name=test_pool -o yaml",
        "pools update --name=test_pool --slots=10",
        "pools import tests/airflowctl_tests/fixtures/test_pools.json",
        "pools export --file=tests/airflowctl_tests/fixtures/pools_export.json --output=json",
        "pools delete --name=test_pool",
        "pools delete --name=test_import_pool",
        # Providers commands
        "providers list",
        # Variables commands
        "variables create --key=test_key --value=test_value",
        "variables list",
        "variables get --key=test_key",
        "variables get --key=test_key -o table",
        "variables update --key=test_key --value=updated_value",
        "variables import tests/airflowctl_tests/fixtures/test_variables.json",
        "variables export --file=tests/airflowctl_tests/fixtures/variables_export.json",
        "variables delete --key=test_key",
        "variables delete --key=test_import_var",
        "variables delete --key=test_import_var_with_desc",
        # Version command
        "version --remote",
    ]
