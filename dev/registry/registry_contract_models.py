...
def _validated_dump(model_type: type[BaseModel], payload: dict[str, Any]) -> dict[str, Any]:
...
def validate_providers_catalog(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ProvidersCatalogContract, payload)


def validate_modules_catalog(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ModulesCatalogContract, payload)


def validate_provider_modules(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ProviderModulesContract, payload)


def validate_provider_parameters(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ProviderParametersContract, payload)


def validate_provider_connections(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ProviderConnectionsContract, payload)


def validate_provider_version_metadata(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ProviderVersionMetadataContract, payload)


def validate_provider_versions(payload: dict[str, Any]) -> dict[str, Any]:
    return _validated_dump(ProviderVersionsContract, payload)
...
PAYLOAD_CONTRACTS: tuple[PayloadContract, ...] = (
    PayloadContract("ProvidersCatalogPayload", "providers-catalog.schema.json", ProvidersCatalogContract),
    PayloadContract("ModulesCatalogPayload", "modules-catalog.schema.json", ModulesCatalogContract),
    PayloadContract("ProviderModulesPayload", "provider-modules.schema.json", ProviderModulesContract),
    PayloadContract(
        "ProviderParametersPayload", "provider-parameters.schema.json", ProviderParametersContract
    ),
    PayloadContract(
        "ProviderConnectionsPayload", "provider-connections.schema.json", ProviderConnectionsContract
    ),
    PayloadContract(
        "ProviderVersionMetadataPayload",
        "provider-version-metadata.schema.json",
        ProviderVersionMetadataContract,
    ),
    PayloadContract("ProviderVersionsPayload", "provider-versions.schema.json", ProviderVersionsContract),
)
...
def build_schema_documents() -> dict[str, dict[str, Any]]:
    ...
    for contract in PAYLOAD_CONTRACTS:
        schema = contract.model_type.model_json_schema()
        schema.setdefault("$schema", "https://json-schema.org/draft/2020-12/schema")
        documents[contract.schema_filename] = schema
    return documents
...
def build_openapi_document() -> dict[str, Any]:
    ...
    root_models: dict[str, type[BaseModel]] = {
        contract.component_name: contract.model_type for contract in PAYLOAD_CONTRACTS
    }
    component_schemas = _collect_openapi_component_schemas(root_models)
