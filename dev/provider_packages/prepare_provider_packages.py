...
DEPENDENCIES_JSON_FILE_PATH = AIRFLOW_SOURCES_ROOT_PATH / "airflow" / "provider_dependencies.json"
    ...
    if additional_extras:
        for entry in additional_extras:
            ...
            if name in extras_dict:
                # remove non-versioned extras if versioned ones are coming
                existing_extras = set(extras_dict[name])
                for new_extra in extras:
                    for extra in existing_extras:
                        # remove extra if exists
                        if new_extra.startswith(extra):
                            extras_dict[name].remove(extra)
                            break
                    extras_dict[name].append(new_extra)
            else:
                extras_dict[name] = extras
    return extras_dict
