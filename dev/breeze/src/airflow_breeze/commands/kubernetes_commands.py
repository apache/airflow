...
option_parallelism_cluster = click.option(
    ...
    type=click.IntRange(1, max(1, int(mp.cpu_count())) // 2 if not generating_command_images() else 4),
    default=max(1, int(mp.cpu_count()) // 2 if not generating_command_images() else 2),
    envvar="PARALLELISM",
    show_default=True,
)
