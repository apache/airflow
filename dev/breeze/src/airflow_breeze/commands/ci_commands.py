...
def _determine_milestone_version(
    ...
    # Priority 1: Check for backport labels
    ...
    # Priority 2: Check if it's a bug fix (title or labels)
    if _has_bug_fix_indicators(title, labels):
        ...
        get_console().print(
            "[info]Bug fix merged to main - cannot determine milestone version automatically[/]"
        )
        return None, "bug fix but merged to main (no version branch)"

    return None, "no matching criteria"
