...
class ShellParams:
    ...
    def get_backend_compose_files(self, backend: str) -> list[Path]:
        ...
        if self.project_name == "prefligit":
            # do not forward ports for prefligit runs - to not clash with running containers from
            # breeze
            return [backend_docker_compose_file]
    ...
    def compose_file(self) -> str:
        ...
        if self.mount_sources in USE_AIRFLOW_MOUNT_SOURCES and self.use_airflow_version is None:
            ...
            sys.exit(1)
        if self.forward_ports and not self.project_name == "prefligit":
            compose_file_list.append(DOCKER_COMPOSE_DIR / "base-ports.yml")
        if self.debug_components and not self.project_name == "prefligit":
