...
def _update_build_files(provider_id: str, version_suffix: str, target_provider_root_sources_path: Path):
    ...
    get_console().print()
    get_console().print(f"Generating build files for {provider_id}")
    get_console().print()
...
def generate_build_files(provider_id: str, version_suffix: str, target_provider_root_sources_path: Path):
    with ci_group(f"Generate setup files for '{provider_id}'"):
        _update_build_files(provider_id, version_suffix, target_provider_root_sources_path)
        get_console().print(f"[success]Generated regular package setup files for {provider_id}[/]\n")
...
def cleanup_build_remnants(target_provider_root_sources_path: Path):
    if get_verbose():
        get_console().print(f"Cleaning remnants in {target_provider_root_sources_path}")
...
def build_provider_package(
    ...
    get_console().print(
        f"[info]Building provider package: {provider_id} in format {package_format} in "
    ...
    get_console().print(
        f"\n[success]Prepared provider package {provider_id} in " f"format {package_format}[/]"
    )
