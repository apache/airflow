...
def check_uv_version(quiet: bool = False):
    ...
    ``pyproject.toml``. Matches the ``check_docker_version`` pattern: reports the
    detected version on success and exits with install/upgrade guidance on failure.
    """
...
"""
    ...
    if compare_version(uv_version, required_uv_version):
        if not quiet:
