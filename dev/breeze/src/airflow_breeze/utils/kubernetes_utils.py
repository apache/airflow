...
def create_virtualenv(force_venv_setup: bool) -> RunCommandResult:
    ...
    get_console().print(f"[info]Reinstalling uv=={UV_VERSION} in {K8S_ENV_PATH}")
...
def _attempt_to_connect(port_number: int, output: Output | None, wait_seconds: int = 0) -> bool:
    ...
    for attempt in itertools.count(1):
        ...
        try:
            response = requests.get(f"http://localhost:{port_number}/v2/monitor/health")
        except ConnectionError:
            get_console(output=output).print(
                f"The api server is not yet ready at http://localhost:{port_number}/v2/monitor/health "
            )
            ...
            if response.status_code == 200:
                get_console(output=output).print(
                    ...
                    f"http://localhost:{port_number}/v2/monitor/health and it is healthy."
                )
                return True
            else:
                get_console(output=output).print(
