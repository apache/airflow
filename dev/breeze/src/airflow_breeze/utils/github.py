...
def get_repo_and_branch_from_pr(
    pr_number: str, github_repository: str = "apache/airflow", github_token: str | None = None
) -> tuple[str, str, str]:
    """
    Gets the owner, repo, and branch from a PR number using GitHub API.

    :param pr_number: PR number (without #)
    :param github_repository: GitHub repository in format owner/repo (default: apache/airflow)
    :param github_token: GitHub token for API authentication (optional, but recommended to avoid rate limits)
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 403:
                get_console().print(
                    "[warning]GitHub API rate limit may have been exceeded. "
        ...
        get_console().print(
            f"[info]Resolved PR #{pr_number} to {owner}/{repo}:{branch}[/]"
        )
        return owner, repo, branch

    except requests.Timeout:
        get_console().print(f"[error]Request to GitHub API timed out while fetching PR #{pr_number}")
        sys.exit(1)
    except Exception as e:
        get_console().print(f"[error]Error fetching PR #{pr_number}: {e}")
        sys.exit(1)
