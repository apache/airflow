...
class SelectiveChecks:
    ...
    @cached_property
    def skip_prefligits(self) -> str:
        prefligits_to_skip = set()
        prefligits_to_skip.add("identity")
        ...
        # The prefligit will semi-randomly skip such list of files into several groups and we want
        ...
        if self.full_tests_needed:
            ...
            # run all the prefligits just to be sure everything is ok when some structural changes occurred
            return ",".join(sorted(prefligits_to_skip))
        if not (
            ...
            prefligits_to_skip.add("ts-compile-lint-ui")
            prefligits_to_skip.add("ts-compile-lint-simple-auth-manager-ui")
        if not self._matching_files(FileGroupForCi.ALL_PYTHON_FILES, CI_FILE_GROUP_MATCHES):
            prefligits_to_skip.add("flynt")
        if not self._matching_files(
            FileGroupForCi.HELM_FILES,
            CI_FILE_GROUP_MATCHES,
        ):
            prefligits_to_skip.add("lint-helm-chart")
        if not (
            ...
            prefligits_to_skip.add("check-provider-yaml-valid")
        return ",".join(sorted(prefligits_to_skip))
