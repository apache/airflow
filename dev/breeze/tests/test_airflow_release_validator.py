...
class TestCompareArchives:
    ...
    def test_expected_diffs_ignored_in_wheel(self, validator: AirflowReleaseValidator, tmp_path: Path):
        """Should ignore expected differences like hatch_build.py and RECORD."""
        whl1 = tmp_path / "test1.whl"
        whl2 = tmp_path / "test2.whl"

        with zipfile.ZipFile(whl1, "w") as z:
            z.writestr("module/main.py", "content")
            z.writestr("module/hatch_build.py", "patched version")
            z.writestr("module/RECORD", "record1")

        with zipfile.ZipFile(whl2, "w") as z:
            z.writestr("module/main.py", "content")
            z.writestr("module/hatch_build.py", "original version")
            z.writestr("module/RECORD", "record2")

        matches, details = validator._compare_archives(whl1, whl2)

        assert matches is True  # Expected diffs are ignored
        assert details == []
...
class TestPatchHatchBuild:
    """Tests for _patch_hatch_build_for_env_var method."""

    def test_patches_hatch_build_file(self, validator: AirflowReleaseValidator, tmp_path: Path):
        """Should patch hatch_build.py to support AIRFLOW_GIT_VERSION env var."""
        worktree = tmp_path / "worktree"
        hatch_build_path = worktree / "airflow-core" / "hatch_build.py"
        hatch_build_path.parent.mkdir(parents=True)

        original_content = '''class SomeBuildHook:
    def get_git_version(self) -> str:
        """
        Return a version to identify the state of the underlying git repo.

        The git version will be the latest commit hash.
        """
        # Original implementation
        return "some-version"
'''
        hatch_build_path.write_text(original_content)

        validator._patch_hatch_build_for_env_var(worktree)

        patched_content = hatch_build_path.read_text()
        assert "AIRFLOW_GIT_VERSION" in patched_content
        assert "os.environ.get" in patched_content

    def test_skips_if_already_patched(self, validator: AirflowReleaseValidator, tmp_path: Path):
        """Should skip patching if AIRFLOW_GIT_VERSION already present."""
        worktree = tmp_path / "worktree"
        hatch_build_path = worktree / "airflow-core" / "hatch_build.py"
        hatch_build_path.parent.mkdir(parents=True)

        already_patched = """class SomeBuildHook:
    def get_git_version(self) -> str:
        if env_version := os.environ.get("AIRFLOW_GIT_VERSION"):
            return env_version
        return "some-version"
"""
        hatch_build_path.write_text(already_patched)

        validator._patch_hatch_build_for_env_var(worktree)

        # File should not be modified
        assert hatch_build_path.read_text() == already_patched

    def test_handles_missing_file(self, validator: AirflowReleaseValidator, tmp_path: Path):
        """Should handle missing hatch_build.py gracefully."""
        worktree = tmp_path / "worktree"
        worktree.mkdir()
        # hatch_build.py does not exist
