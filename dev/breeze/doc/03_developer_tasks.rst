...
You can run static checks via prefligit.
...
Prefligits run by default on staged changes that you have locally changed. It will run it on all the
...
re-run latest prefligits on your changes, but it can take a long time (few minutes) to wait for the result.
...
.. code-block:: bash
     ...
     prefligit run mypy-airflow --file airflow/utils/code_utils.py --file airflow/utils/timeout.py
...
to specific commits you already have in your branch. You can use ``prefligit`` to run the checks
...
.. code-block:: bash
     ...
     prefligit run mypy-airflow --last-commit

The above will run mypy check for all files in the last commit in your branch.

.. code-block:: bash

     prefligit identity --verbose --from-ref HEAD^^^^ --to-ref HEAD
...
This command requires the ``prefligit`` tool, which should be installed by following `this guide <../../../contributing-docs/03_contributors_quick_start.rst#configuring-prefligit>`__.
