...
It means that your prefligit hook is installed with (already End-Of-Life) Python 3.8 and you should reinstall
it and clean prefligit cache.

This can be done with ``uv tool`` to install ``prefligit``)

.. code-block:: bash

    uv tool uninstall prefligit
    uv tool install prefligit --python 3.10 --force
    prefligit clean
    prefligit install
...
.. code-block:: bash
    ...
    pipx uninstall prefligit
    pipx install prefligit --python $(which python3.10) --force
    prefligit clean
    prefligit install

If you installed ``prefligit`` differently, you should remove and reinstall
...
try to run ``pipx list`` to view which packages has bad interpreter (it can be more than just breeze, for example  prefligit)
...
.. code-block:: bash
    ...
    [INFO] Locking prefligit directory
