...
- [Selective CI Checks](#selective-ci-checks)
  ...
  - [Skipping prefligits (Static checks)](#skipping-prefligits-static-checks)
...
* if `CI Image building` is disabled, only basic prefligits are enabled - no 'image-depending` prefligits
  are enabled.
...
## Skipping prefligits (Static checks)

Our CI always run prefligit checks with `--all-files` flag. This is in order to avoid cases where
different check results are run when only subset of files is used. This has an effect that the prefligit
tests take a long time to run when all of them are run. Selective checks allow to save a lot of time
for those tests in regular PRs of contributors by smart detection of which prefligits should be skipped
...
* The provider specific checks are skipped when builds are running in v2_* branches (we do not build
  ...
  * lint-helm-chart
  * mypy-providers
* If "full tests" mode is detected, no more prefligits are skipped - we run all of them
...
| skip-prefligits                                         | Which prefligits should be skipped during the static-checks run                                         | flynt,identity                           |      |
...
This is because provider.yaml change will automatically trigger (via `update-provider-dependencies` prefligit)
