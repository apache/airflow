...
* if `CI Image building` is disabled, only basic prek hooks are enabled - no 'image-depending` prek
  are enabled.
...
for those tests in regular PRs of contributors by smart detection of which preks should be skipped
...
* The provider specific checks are skipped when builds are running in v2_* branches (we do not build
  ...
  * lint-helm-chart
  * mypy-providers
* If "full tests" mode is detected, no more preks are skipped - we run all of them
