...
def setup_test_data(session, num_dag_runs=10000, tasks_per_dag_run=10):
    ...
    for i in range(num_dag_runs):
        ...
        end_time = run_date + timedelta(minutes=30)
        dag_run = DagRun(
        ...
        dag_run.end_date = end_time
        dag_run.data_interval_start = run_date
        dag_run.data_interval_end = end_time
...
def benchmark_old_query(session, start_date, end_date, dag_ids):
    ...
    return elapsed, (dag_run_types, dag_run_states, task_instance_states)
...
def benchmark_new_query(session, start_date, end_date, dag_ids):
    ...
    elapsed = time.time() - start
    return elapsed, (dag_run_types_dict, dag_run_states_dict, task_instance_states)
...
def main():
    ...
    try:
        # Setup test data
        setup_test_data(session, num_dag_runs=10000)
        ...
        # Warm up
        print("\nWarming up...")
        benchmark_old_query(session, start_date, end_date, dag_ids)
        benchmark_new_query(session, start_date, end_date, dag_ids)
        ...
        for i in range(iterations):
            ...
            # Old query
            old_time, old_results = benchmark_old_query(session, start_date, end_date, dag_ids)
            old_times.append(old_time)
            print(f"  Old query (func.coalesce):      {old_time:.4f}s")

            # New query
            new_time, new_results = benchmark_new_query(session, start_date, end_date, dag_ids)
