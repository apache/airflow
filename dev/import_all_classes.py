...
def import_all_classes(
    ...
    provider_prefixes = [mk_prefix(provider_id) for provider_id in provider_ids] if provider_ids else [prefix]

    def onerror(_):
        nonlocal tracebacks
        exception_str = traceback.format_exc()
        if any([provider_prefix in exception_str for provider_prefix in provider_prefixes]):
